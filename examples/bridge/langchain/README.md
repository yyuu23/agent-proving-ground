...
This example demonstrates using a native [LangChain](https://www.langchain.com/) agent with AgentProvingGround to perform Q/A using the [Tavili Search API](https://tavily.com/). 

The example uses the [`agent_bridge()`](https://inspect.aisi.org.uk/agent-bridge.html) which enables integrating arbitrary 3rd party agent frameworks into AgentProvingGround. Source files include:
...
``` bash
apg eval task.py --model openai/gpt-4o
apg eval task.py --model anthropic/claude-3-7-sonnet-latest
apg eval task.py --model google/gemini-2.5-pro
```
