...
This is a prototype of an AgentProvingGround agent running in a Linux sandbox with human intervention. It utilises AgentProvingGround's [Interactivity features](https://inspect.aisi.org.uk/interactivity.html). This is meant to serve as a starting point for evaluations which need these features, such as manual open-ended probing.
...
``` bash
apg eval examples/intervention --display conversation
```

To run in computer or multi-tool modes, use the `mode` task parameter:

``` bash
apg eval examples/intervention -T mode=computer --display conversation
apg eval examples/intervention -T mode=multi-tool --display conversation
```

See the documentation on the [Computer Tool](https://inspect.aisi.org.uk/tools-standard.html#sec-computer) for additional details on AgentProvingGround computer use.
...
``` bash
apg eval examples/intervention -T mode=shell -T approval=true --display conversation
```
