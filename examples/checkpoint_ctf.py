...
Usage:
    ...
    apg eval examples/checkpoint_ctf.py --model openai/gpt-4o

To exercise resume: kill the eval mid-way (Ctrl-C), then
``apg eval retry <log-file>``.
"""
...
from agent_proving_ground.scorer import Score, Scorer, Target, accuracy, includes, scorer
from agent_proving_ground.solver import TaskState
from agent_proving_ground.tool import Tool, bash, tool
from agent_proving_ground.util import CheckpointConfig, store
from agent_proving_ground.util._checkpoint._triggers.types import TokenInterval
...
def slow_scorer(delay_seconds: float = 10.0) -> Scorer:
    ...
    is running, then `apg eval retry <log>` should skip the agent
