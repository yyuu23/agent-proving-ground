"""Trackio Tracking hook for AgentProvingGround.
...
first-class Trace primitive that maps cleanly onto an AgentProvingGround EvalSample.
...
from agent_proving_ground.hooks import (
    Hooks,
    RunEnd,
    RunStart,
    SampleEnd,
...
def _content_to_text(content: Any) -> str:
    """Best-effort coercion of an AgentProvingGround ChatMessage.content to a plain string."""
    if content is None:
        return ""
...
@hooks(
    ...
    description="Log each AgentProvingGround sample to Trackio as a trackio.Trace.",
)
class TrackioHooks(Hooks):
    """Logs each AgentProvingGround sample as a ``trackio.Trace``.
    ...
    ``asyncio.to_thread`` to keep AgentProvingGround's async event loop unblocked.
    """
    ...
    async def on_run_start(self, data: RunStart) -> None:
        if self._initialized:
            return
        project = os.getenv("TRACKIO_PROJECT") or "agent-proving-ground"
