...
def chunked_corpus(tmp_path_factory: pytest.TempPathFactory) -> "ChunkedCorpus":
    ...
    from agent_proving_ground.log._recorders.chunked.format import DEFAULT_CHUNK_SIZE
...
def fast_retry_waits(request):
    ...
    import agent_proving_ground.model._providers.util.chatapi as chatapi
    import agent_proving_ground.model._retry as model_retry
...
def isolate_active_model():
    ...
    `APG_EVAL_MODEL`. Restore the contextvar after each test.
    """
    from agent_proving_ground.model._model import active_model_context_var
...
def registrations_at_session_start() -> dict[str, object]:
    ...
    """
    from agent_proving_ground._util.entrypoints import ensure_entry_points
...
def pytest_configure(config):
    ...
    os.environ["APG_EVAL_LOG_MODEL_API"] = "1"
