"""Unit tests for :mod:`agent_proving_ground._util.discovery`."""
...
def test_prepare_discovery_dir_relocks_existing_loose_perms(
    ...
    Defends against an older AgentProvingGround that bound without the chmod, or
...
def test_prepare_picks_up_monkeypatched_process_pid_alive(
    ...
    A test that monkey-patches ``agent_proving_ground._util.process.pid_alive``
    affects the next call — verifies the late-binding behavior of
    ``_resolve_pid_alive``.
    """
    (tmpdir_path / "555.json").write_text(json.dumps({"pid": 555, "socket_path": None}))
    monkeypatch.setattr("agent_proving_ground._util.process.pid_alive", lambda pid: False)
    prepare_discovery_dir(tmpdir_path)
    assert not (tmpdir_path / "555.json").exists()
...
def test_list_uses_default_pid_alive_when_no_fn_supplied(
    ...
    monkeypatch.setattr("agent_proving_ground._util.process.pid_alive", lambda pid: pid == 100)
