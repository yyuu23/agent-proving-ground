...
from agent_proving_ground import (
    Epochs,
    Task,
    TaskSource,
    eval,
...
from agent_proving_ground.approval._policy import ApprovalPolicyConfig, ApproverPolicyConfig
...
def test_eval_sample_records_turn_count_and_token_limit_usage():
    ...
    from agent_proving_ground.model._model_output import ModelOutput, ModelUsage
...
def test_eval_sample_limit_values_reflect_final_retry_attempt() -> None:
    ...
    from agent_proving_ground.model._model_output import ModelOutput, ModelUsage
    from agent_proving_ground.solver import Generate, TaskState, solver
    from agent_proving_ground.util._limit import TokenLimit, token_limit_usage, turn_count
...
def test_eval_sandbox_init_when_first_task_has_no_sandbox():
    ...
    """
    from agent_proving_ground._eval.run import SandboxManager
...
def capture_eval_warnings(caplog):
    ...
    loader_logger = logging.getLogger("agent_proving_ground._eval.loader")
...
async def test_retry_sample_source_tolerates_missing_log_file(tmp_path: Path) -> None:
    ...
    """
    from agent_proving_ground._eval.task.run import eval_log_sample_source
    from agent_proving_ground._util.asyncfiles import AsyncFilesystem
    from agent_proving_ground.dataset import MemoryDataset
    from agent_proving_ground.log import EvalConfig, EvalDataset, EvalLog, EvalSpec
...
def capture_probe_warnings(caplog):
    ...
    run_logger = logging.getLogger("agent_proving_ground._eval.task.run")
...
def _write_prior_eval_log(log_dir: Path) -> tuple[Any, bytes]:
    ...
    from agent_proving_ground._util.file import local_path
...
def test_retry_presence_probe_gives_up_after_max_failures(
    ...
    from agent_proving_ground._eval.task.run import (
    ...
    with caplog.at_level(logging.WARNING, logger="agent_proving_ground._eval.task.run"):
        anyio.run(check)
...
def test_retry_presence_probe_concurrent_failures_respect_cap(
    ...
    from agent_proving_ground._util import async_zip
    from agent_proving_ground._util._async import tg_collect
    from agent_proving_ground._util.asyncfiles import AsyncFilesystem
    from agent_proving_ground.dataset import MemoryDataset
...
def test_retry_presence_probe_not_used_for_json_logs(tmp_path: Path) -> None:
    ...
    from agent_proving_ground._eval.task.run import _never_prior_exists, eval_log_sample_source
    from agent_proving_ground.dataset import MemoryDataset
