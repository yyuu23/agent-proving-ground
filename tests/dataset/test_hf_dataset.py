...
from agent_proving_ground.dataset._sources.hf import (
    _call_with_hf_retry,
...
def test_retry_gives_up_after_max_tries(fast_retry, monkeypatch):
    ...
    monkeypatch.setattr("agent_proving_ground.dataset._sources.hf._hf_max_tries", lambda: 3)
...
def _install_fake_datasets(monkeypatch, load_dataset):
    ...
    monkeypatch.setattr(
        "agent_proving_ground.dataset._sources.hf.verify_required_version",
        lambda *_a, **_k: None,
    )
...
def test_hf_dataset_retry_false_disables_retry(monkeypatch):
    ...
    from agent_proving_ground.dataset import hf_dataset

    with pytest.raises(HfHubHTTPError):
        hf_dataset(
            path="agent-proving-ground-test/nonexistent-dataset",
            split="test",
            cached=False,
            retry=False,
        )
...
def _install_fake_datasets_full(monkeypatch, tmp_path, load_dataset, load_from_disk):
    ...
    monkeypatch.setattr(
        "agent_proving_ground.dataset._sources.hf.inspect_cache_dir",
...
def test_hf_dataset_auto_id_custom_mapping_stable_ids_and_order(tmp_path, monkeypatch):
    ...
    from agent_proving_ground.dataset import Sample, hf_dataset
...
def test_hf_dataset_cache_hit_does_not_invoke_retry(tmp_path, monkeypatch):
    ...
    from agent_proving_ground._util.file import safe_filename
    ...
    monkeypatch.setattr(
        "agent_proving_ground.dataset._sources.hf._call_with_hf_retry", must_not_call_retry
    )
