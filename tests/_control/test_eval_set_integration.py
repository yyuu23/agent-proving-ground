...
from agent_proving_ground._control.discovery import list_discovered_servers
from agent_proving_ground._control.eval_state import get_eval_states
from agent_proving_ground._control.events import decode_cursor, sample_events
from agent_proving_ground._control.messages import sample_messages
...
from agent_proving_ground.log._samples import active_samples
from agent_proving_ground.solver import Generate, Solver, TaskState, generate, solver
...
def test_ctl_ls_reused_tolerant_success_log_reports_errored_samples(
    ...
    # polarity.
    import agent_proving_ground._eval.evalset as evalset_mod
    from agent_proving_ground._control.eval_state import get_eval_states
...
def test_ctl_ls_aggregates_legacy_batch_retries(short_data_dir: Path) -> None:
    ...
    the task selector permanently ambiguous — ``apg ctl samples <task_id>``
    ...
    """
    from agent_proving_ground._cli.ctl import _resolve_target_eval
...
def test_ctl_reused_log_solver_skips_finish_step() -> None:
    ...
    from agent_proving_ground._control.eval_state import clear_all_eval_states, get_eval_state
    from agent_proving_ground._eval.evalset import Log, _register_reused_logs
    from agent_proving_ground._eval.task.log import plan_to_eval_plan
    from agent_proving_ground._eval.task.run import plan_agent_name
    from agent_proving_ground.log._file import EvalLogInfo
    from agent_proving_ground.log._log import EvalConfig, EvalDataset, EvalLog, EvalSpec
    from agent_proving_ground.model import GenerateConfig
    from agent_proving_ground.solver import Plan, system_message
...
def test_keep_alive_park_entered_with_completed_state(
    ...
    monkeypatch.setattr("agent_proving_ground._eval.evalset._keep_alive_park", spy)
...
def test_runtime_keep_parks_eval_set_launched_without_flag(
    ...
    """A runtime `apg ctl keep` parks an eval-set started without the flag.
    ...
    from agent_proving_ground.hooks import _hooks as hooks_mod
    ...
    # locally from agent_proving_ground.hooks._hooks), so latching here lands the request
...
def test_ctl_flush_writes_buffered_samples_and_buffer_config(
    ...
    """
    from agent_proving_ground._control.buffer import flush_task_samples, task_buffer_config
...
def test_ctl_samples_shows_score_for_single_scorer(short_data_dir: Path) -> None:
    ...
    from agent_proving_ground.scorer import Score, Target, accuracy, scorer
...
def test_ctl_eval_finishes_when_samples_early_stopped(short_data_dir: Path) -> None:
    ...
    """
    from agent_proving_ground.util._early_stopping import EarlyStop
...
def test_ctl_eval_reports_http_retries_per_eval(short_data_dir: Path) -> None:
    ...
    """
    from agent_proving_ground._util.retry import http_retries_count, report_http_retry
...
def test_task_retry_detaches_superseded_attempt_live(
    ...
    from agent_proving_ground._control import eval_state as eval_state_mod
...
def test_ctl_limits_reflects_and_retunes_sample_limiter(short_data_dir: Path) -> None:
    ...
    """
    from agent_proving_ground._control.limits import task_limits
...
def test_ctl_sample_cancel_scores_work_so_far(short_data_dir: Path) -> None:
    ...
    """
    from agent_proving_ground._control.cancel import cancel_sample
...
def test_ctl_task_cancel_aborts_run(short_data_dir: Path) -> None:
    ...
    it.
    """
    from agent_proving_ground._control.cancel import cancel_task
