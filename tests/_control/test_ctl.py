"""Unit tests for the `apg ctl` CLI.
...
from agent_proving_ground._cli.ctl import (
    _KNOB_SCOPE,
    _KNOB_SINCE,
    _SHORT_ID_LEN,
    _ConfigResult,
...
from agent_proving_ground._control.discovery import DiscoveredControlServer
...
def test_activity_cell_renders_tool_and_multi_tool() -> None:
    ...
    from agent_proving_ground._cli.ctl import _format_activity
...
def test_sample_detail_includes_activity(
    ...
    from agent_proving_ground._cli.ctl import _print_sample_detail
...
def test_event_summary_renders_pending_model_and_tool() -> None:
    ...
    from agent_proving_ground._cli.ctl import _event_summary
...
def test_sorted_samples_orders_running_queued_terminal_pending() -> None:
    ...
    could tie with.
    """
    from agent_proving_ground._control.state import _sorted_samples
...
def test_errors_table_lists_retried_and_errored(
    ...
    from agent_proving_ground._cli.ctl import _print_errors_table
...
def test_print_events_table_and_footer(
    ...
    from agent_proving_ground._cli.ctl import _print_events
...
def test_keep_alive_footer_off_hints_keep_command(
    ...
    assert "apg ctl process keep" in out
...
def test_errored_samples_footer_points_at_triage_command(
    ...
    assert "apg ctl sample errors" in out
...
def test_footer_reports_paused_tasks(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "apg ctl task resume" in out
    assert "apg ctl model resume" not in out
    assert "apg ctl process resume" not in out
...
def test_footer_reports_model_paused_sources(
    ...
    assert "apg ctl model resume" in out
    assert "apg ctl task resume" not in out
    assert "apg ctl process resume" not in out
...
def test_format_paused_renders_source_lists() -> None:
    from agent_proving_ground._cli.ctl import _format_paused
...
def test_resolve_target_server_defaults_to_sole_process(
    ...
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl.list_discovered_servers", lambda: [_FakeServer(7)]
    )
...
def test_resolve_target_server_matches_pid(
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl.list_discovered_servers", lambda: servers)
...
def test_resolve_target_server_none_running_exits(
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl.list_discovered_servers", lambda: [])
    with pytest.raises(click.exceptions.Exit):
        _resolve_target_server(None)
...
def _stub_httpx(
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl.httpx.Client", _Client)
    monkeypatch.setattr("agent_proving_ground._cli.ctl.httpx.HTTPTransport", _Transport)
    monkeypatch.setattr("agent_proving_ground._cli.ctl.httpx.AsyncClient", _AsyncClient)
    monkeypatch.setattr("agent_proving_ground._cli.ctl.httpx.AsyncHTTPTransport", _Transport)
    return counter
...
async def test_get_with_retry_retries_timeout_then_succeeds(
    ...
    from agent_proving_ground._cli.ctl import _get_with_retry_async
...
async def test_get_with_retry_exhausts_and_exits(
    ...
    from agent_proving_ground._cli.ctl import _REQUEST_ATTEMPTS, _get_with_retry_async
    ...
    assert "apg ctl process anomalies 7" in err
...
def test_config_read_retries_timeout_then_succeeds(
    ...
    from agent_proving_ground._cli.ctl import _exec_limits
...
async def test_get_with_retry_busy_raises_without_terminal_echo(
    ...
    from agent_proving_ground._cli.ctl import _get_with_retry_async, _ServerBusy
...
def test_fetch_summaries_busy_server_skipped_when_degradable(
    ...
    from agent_proving_ground._cli.ctl import _DEGRADED_READ_ATTEMPTS, _fetch_summaries
...
def test_fetch_summaries_exact_id_match_short_circuits_fan_out(
    ...
    from agent_proving_ground._cli.ctl import _fetch_summaries
...
def test_list_discovered_servers_sorts_newest_first(
    ...
    from agent_proving_ground._control import discovery
...
def test_events_poll_with_full_task_id_skips_sibling_servers(
    ...
    """
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl.list_discovered_servers",
...
def test_sample_detail_read_retries_busy_timeout(
    ...
    from agent_proving_ground._cli.ctl import _fetch_sample_detail
...
def test_sample_events_read_retries_busy_timeout(
    ...
    from agent_proving_ground._cli.ctl import _fetch_sample_events
...
async def test_get_with_retry_does_not_retry_connection_error(
    ...
    from agent_proving_ground._cli.ctl import _get_with_retry_async, _ServerUnreachable
...
def test_fetch_summaries_unreachable_server_does_not_cancel_in_flight_siblings(
    ...
    from agent_proving_ground._cli.ctl import _fetch_summaries, _ServerUnreachable
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._get_with_retry_async", fake_get)
...
def test_fetch_summaries_unresponsive_server_exits(
    ...
    from agent_proving_ground._cli.ctl import _REQUEST_ATTEMPTS, _fetch_summaries
...
def test_fetch_summaries_all_busy_narrates_once_per_invocation(
    ...
    assert err.count("apg ctl process anomalies") == 1
    assert "apg ctl process anomalies 7" in err
...
def _patch_surface(
    ...
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._fetch_summaries",
    ...
    if samples_by_eval is not None:
        ...
        monkeypatch.setattr(
            "agent_proving_ground._cli.ctl._fetch_samples_async", fake_fetch_samples
        )
...
def test_task_list_human_footer_flags_errored_samples(
    ...
    assert result.exit_code == 0, result.output
    assert "2 samples errored — see `apg ctl sample errors`" in result.stdout
...
def test_sample_errors_requests_server_side_filter(
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._fetch_samples_async", fake_samples)
    result = cli_runner().invoke(ctl_command, ["sample", "errors", "--json"])
    assert result.exit_code == 0, result.output
...
def test_fetch_samples_sends_filter_param(
    ...
    from agent_proving_ground._cli.ctl import _fetch_samples
...
def test_sample_list_attempt_budget_splits_scoped_from_fan_out(
    ...
    """
    from agent_proving_ground._cli.ctl import _REQUEST_ATTEMPTS
...
def test_sample_list_long_idle_points_at_process_anomalies(
    ...
    assert "`apg ctl process anomalies 7`" in result.output
...
def test_sample_list_idle_pointer_spans_processes(
    ...
    assert result.exit_code == 0, result.output
    assert (
        "`apg ctl process anomalies` shows each running process's" in result.output
    )
...
def test_sample_list_scoped_busy_points_at_process_anomalies(
    ...
    """
    from agent_proving_ground._cli.ctl import _ServerBusy
    ...
    assert "apg ctl process anomalies 7" in result.stderr
...
def test_sample_show_row_lookup_requests_full_listing(
    ...
    calls = _capture_fetch_kwargs(monkeypatch)
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._fetch_sample_detail",
...
def _patch_samples_unreachable_for(
    ...
    from agent_proving_ground._cli.ctl import _ServerUnreachable
...
def test_sample_show_reports_detail_summary_fields(
    ...
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._fetch_sample_detail", lambda *a, **k: detail
    )
    result = cli_runner().invoke(
        ctl_command, ["sample", "show", "aaa111", "s1", "--json"]
    )
...
def test_sample_show_is_a_single_read(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._fetch_samples", fail_fetch)
    detail = {
        "sample_id": "s1",
        "epoch": 1,
        "status": "completed",
...
def test_sample_show_old_server_fallback_unreachable_degrades(
    ...
    assert ("apg ctl process anomalies 7" in result.stderr) == busy
    payload = json.loads(result.stdout)
    assert payload["error"] == {"message": "boom"}
...
def test_tasks_alias_delegates_with_deprecation_note(
    ...
    assert payload["tasks"][0]["task_id"] == "aaa111"
    assert "is now `apg ctl task list`" in result.stderr
...
def test_samples_alias_accepts_content_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "is now `apg ctl sample list`" in result.stderr
...
def test_errors_alias_accepts_content_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.exit_code == 0, result.output
    assert seen["content"] is True
    assert "is now `apg ctl sample errors`" in result.stderr
...
def test_events_alias_accepts_content_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._fetch_sample_events", fake_events)
    result = cli_runner().invoke(
        ctl_command, ["events", "aaa111", "s1", "--content", "--json"]
    )
    assert result.exit_code == 0, result.output
    assert seen["content"] is True
    assert "is now `apg ctl sample events`" in result.stderr
...
def _stub_limits(
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._exec_limits", fake_limits)
...
def test_limits_alias_delegates_to_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "is now `apg ctl config`" in result.stderr
...
def test_config_set_buffer_error_does_not_claim_unapplied_knobs(
    ...
    _patch_surface(monkeypatch, [_full_summary("aaa111", "t1")])
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._exec_limits",
...
def test_config_gates_key_on_pre_version_server(
    ...
    accepts it.
    """
    from agent_proving_ground._control import CONTROL_API_VERSION
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._exec_limits", _no_patch)
...
def test_knob_since_table_is_consistent() -> None:
    ...
    from agent_proving_ground._cli.ctl import _PROVENANCE_SINCE
...
def test_mutation_envelope_help_sketches_actual_keys() -> None:
    ...
    """
    from agent_proving_ground._cli.ctl import _MUTATION_ENVELOPE_HELP, _mutation_envelope
...
def test_config_help_sketches_compose_config_keys() -> None:
    ...
    from agent_proving_ground._cli.ctl import _compose_config, _DirectiveScope, config_command
...
def test_sample_show_json_payload_matches_help_sketch(
    ...
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._fetch_sample_detail", lambda *a, **k: dict(detail)
    )
    result = cli_runner().invoke(
        ctl_command, ["sample", "show", "aaa111", "s1", "--json"]
    )
...
def test_sample_events_json_payload_matches_help_sketch(
    ...
    _patch_surface(monkeypatch, [_full_summary("aaa111", "t1")])
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._fetch_sample_events",
...
def test_sample_messages_json_payload_matches_help_sketch(
    ...
    _patch_surface(monkeypatch, [_full_summary("aaa111", "t1")])
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._fetch_sample_messages",
...
def test_config_retry_overrides_accept_clear_keyword(
    ...
    from agent_proving_ground.model._generate_overrides import MAX_GENERATE_CONFIG_OVERRIDE
...
def test_discovery_api_version_parsed_with_bootstrap_default(
    ...
    """
    import agent_proving_ground._control.discovery as discovery
    import agent_proving_ground._util.process as process
...
def test_process_release_json_envelope(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl.list_discovered_servers", lambda: [_DiscServer(7)]
    )
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._request_json",
...
def test_process_keep_pid_is_positional(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._request_json", record)
...
def test_events_unseeded_defaults_to_recent_tail(
    ...
    from agent_proving_ground._control.events import encode_cursor
...
def test_print_messages_table_and_footer(
    ...
    from agent_proving_ground._cli.ctl import _print_messages
...
def test_messages_unseeded_defaults_to_recent_tail(
    ...
    _patch_surface(monkeypatch, [_full_summary("aaa111", "t1")])
    monkeypatch.setattr("agent_proving_ground._cli.ctl._fetch_sample_messages", fake_messages)
    runner = cli_runner()
...
def test_compose_config_labels_every_knob_with_scope() -> None:
    from agent_proving_ground._cli.ctl import _compose_config, _DirectiveScope
...
def test_log_flush_resolves_sole_active_task(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    *active* task is the default target — the same rule `ctl config` uses —
    ...
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._post_flush", lambda *a, **k: {"flushed": 1}
    )
...
def test_log_flush_json_mutation_envelope(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_surface(monkeypatch, [_full_summary("aaa111", "t1")])
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._post_flush", lambda *a, **k: {"flushed": 2}
    )
    result = cli_runner().invoke(ctl_command, ["task", "log-flush", "--json"])
    assert result.exit_code == 0, result.output
    payload = json.loads(result.stdout)
...
def test_task_cancel_json_mutation_envelope(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl._request_json", spy)
...
def test_task_resume_terse_line_notes_still_held(
    ...
    assert result.stdout == (
        ...
        "(still held by process pause — `apg ctl process resume`)\n"
    )
...
def test_task_resume_notes_model_latch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "apg ctl model resume" in result.output
...
def test_use_terse_resolves_by_tty(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from agent_proving_ground._cli.ctl import _use_terse
...
def test_log_flush_terse_line(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "agent_proving_ground._cli.ctl._post_flush", lambda *a, **k: {"flushed": 0}
    )
...
def test_print_config_process_scope_shows_buffer_placeholder(
    ...
    """
    from agent_proving_ground._cli.ctl import _print_config
...
def test_resolve_scope_siblings_counts_active_only() -> None:
    ...
    from agent_proving_ground._cli.ctl import _resolve_scope
...
def test_keep_alias_accepts_positional_pid(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.exit_code == 0, result.output
    assert posted == ["/tmp/8.sock"]
    assert "is now `apg ctl process keep`" in result.stderr
...
def test_sample_list_caps_reads_in_flight(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from agent_proving_ground._cli.ctl import _MAX_CONCURRENT_READS
...
def test_keep_alive_retries_busy_timeout(
    ...
    from agent_proving_ground._cli.ctl import _request_json
...
def test_json_unexpected_exception_envelope_with_traceback_on_stderr(
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl.list_discovered_servers", boom)
    result = cli_runner().invoke(ctl_command, ["task", "list", "--json"])
    assert result.exit_code == 1
    error = _error_envelope(result)
...
def test_envelope_failures_rejects_runner_without_as_json() -> None:
    ...
    """
    from agent_proving_ground._cli.ctl import _envelope_failures
...
def trace_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl.inspect_trace_dir", lambda: tmp_path)
    return tmp_path
...
def test_process_anomalies_dead_pid_reads_gz(
    ...
    monkeypatch.setattr("agent_proving_ground._cli.ctl.pid_alive", lambda _pid: False)
...
def test_process_anomalies_live_pid_durations_date_to_read(
    ...
    """
    monkeypatch.setattr("agent_proving_ground._cli.ctl.pid_alive", lambda _pid: True)
...
def test_process_anomalies_missing_trace_file_errors(trace_dir: Path) -> None:
    ...
    assert "apg trace anomalies" in result.stderr
...
def test_process_anomalies_widen_skips_corrupt_gz_stream(
    ...
    from agent_proving_ground._util.trace import read_trace_file
