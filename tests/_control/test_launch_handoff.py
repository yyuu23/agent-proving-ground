"""Tests for the synchronous launch handoff (``apg eval[-set|-retry] --json``).

The launch-and-babysit workflow needs ``apg eval`` to say where the
...
- ``apg eval --json`` renders the handoff as a ``launch`` JSON line
...
import agent_proving_ground
from _control.conftest import cli_runner
from agent_proving_ground import Task, task
from agent_proving_ground._cli.detach import _child_args, _handoff_record, _wait_for_record
from agent_proving_ground._cli.eval import (
...
def test_handoff_fires_after_control_bind(
    ...
    logs = agent_proving_ground.eval(handoff_task(), model="mockllm/model", log_dir=log_dir)
...
def fresh_display(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    import agent_proving_ground._display.core.active as active_mod
    import agent_proving_ground.util._display as display_mod
...
class EvalJsonResult(NamedTuple):
    """Parsed output of an ``apg eval --json`` CLI invocation."""
...
def _run_eval_json(
    ...
    """Run ``apg eval --json`` on a trivial task; return parsed stdout lines."""
...
def test_eval_json_overrides_trace(
    ...
    written to stdout — and it also binds to ``APG_EVAL_TRACE``, so a
...
from agent_proving_ground import Task, task
from agent_proving_ground.dataset import Sample
from agent_proving_ground.solver import generate, solver
...
def test_eval_json_redirects_subprocess_stdout_to_stderr(
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "agent_proving_ground._cli.main",
            "eval",
            task_path.name,
            "--model",
            "mockllm/model",
...
def test_debug_diagnostics_go_to_stderr(
    ...
    from agent_proving_ground._cli.common import CommonOptions, process_common_options
...
from agent_proving_ground import Task, task
from agent_proving_ground.dataset import Sample
from agent_proving_ground.solver import solver
...
def _run_eval_set_json(
    ...
    """Run ``apg eval-set --json`` on a trivial task; return parsed stdout lines."""
    task_path = short_data_dir / "handoff_cli_task.py"
    task_path.write_text(task_file)
    log_dir = str(short_data_dir / "logs")
...
def _run_eval_retry_json(short_data_dir: Path, log_files: list[str]) -> EvalJsonResult:
    """Run ``apg eval-retry --json`` on prior logs; return parsed stdout lines."""
    runner = cli_runner()
    result = runner.invoke(
        eval_retry_command,
...
def test_eval_retry_json_emits_launch_then_done(
    ...
    """``apg eval-retry --json`` follows the eval contract: launch then done.
...
def parkless_keep_alive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agent_proving_ground._eval.evalset.wait_for_shutdown_async", no_wait)
...
def test_all_reused_eval_set_park_emits_launch_handoff(
    ...
    """
    log_dir = str(short_data_dir / "logs")
    agent_proving_ground.eval_set([handoff_task()], model="mockllm/model", log_dir=log_dir)
    assert len(handoff_listener) == 1  # the completed first run

    agent_proving_ground.eval_set(
...
def test_detach_child_args_strip_flag_and_force_json() -> None:
    ...
    via APG_EVAL_DETACH — which a project .env re-injects into the
...
def test_eval_detach_hands_off_and_leaves_eval_running(
    ...
    """``apg eval --detach`` end to end: handoff, detached run, exit.
...
def test_eval_detach_via_dotenv_detaches_exactly_once(short_data_dir: Path) -> None:
    """APG_EVAL_DETACH in a project ``.env`` must detach exactly once.
    ...
    task_path = short_data_dir / "handoff_cli_task.py"
    task_path.write_text(TASK_FILE)
    (short_data_dir / ".env").write_text("APG_EVAL_DETACH=true\n")
    xdg_dir = short_data_dir / "xdg"
    env = {**os.environ, "XDG_DATA_HOME": str(xdg_dir)}
    env.pop("APG_EVAL_DETACH", None)
    ...
    assert len(done_records) == 1
    assert done_records[0]["logs"][0]["status"] == "success"
    detach_dir = xdg_dir / "agent_proving_ground" / "detach"
...
def test_eval_detach_sigterm_terminates_child(short_data_dir: Path) -> None:
    ...
    ``timeout 10m apg eval --detach …`` (or a CI cancellation) delivers
...
def _run_eval_human(
    ...
    """Run ``apg eval`` (no ``--json``) with the given display mode."""
    task_path = short_data_dir / "handoff_cli_task.py"
    task_path.write_text(TASK_FILE)
...
def _run_eval_set_human(
    ...
    """Run ``apg eval-set`` (no ``--json``) with the plain display."""
    task_path = short_data_dir / "handoff_cli_task.py"
    task_path.write_text(TASK_FILE)
...
def test_ctl_pointer_absent_from_direct_eval_call(
    ...
    """
    log_dir = str(short_data_dir / "logs")
    agent_proving_ground.eval(handoff_task(), model="mockllm/model", log_dir=log_dir)
    captured = capsys.readouterr()
...
def test_ctl_pointer_printed_at_all_reused_park(
    ...
    ``apg ctl task list``), so the pointer must come from the park.
    """
...
def test_ctl_pointer_gated_off_under_textual_display(
    ...
    import agent_proving_ground._display.core.active as active_mod
    import agent_proving_ground.util._display as display_mod
    from agent_proving_ground._display.textual.display import TextualDisplay
