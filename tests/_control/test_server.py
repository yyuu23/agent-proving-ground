...
def test_stop_reraises_cancellation_without_warning(
    ...
    with caplog.at_level(logging.WARNING, logger="agent_proving_ground._control.server"):
...
def test_error_detail_prefers_server_body() -> None:
    ...
    from agent_proving_ground._cli.ctl import _error_detail
...
def test_start_does_not_publish_discovery_when_bind_fails(
    ...
    socket would strand ``apg ctl`` clients — and under a keep-alive
    ...
    import agent_proving_ground._control.server as server_mod
    from agent_proving_ground._control.discovery import list_discovered_servers
    from agent_proving_ground._control.server import ControlServer, control_server
...
async def test_samples_endpoint_caps_and_histograms(
    ...
    """
    from agent_proving_ground._control import server as server_mod
    from agent_proving_ground._control import state as state_mod
    from agent_proving_ground._control.state import DEFAULT_SAMPLE_LIST_LIMIT
...
def test_control_server_cleans_up_partial_startup_failure(
    ...
    import agent_proving_ground._control.discovery as discovery
    from agent_proving_ground._control import server as server_mod
    from agent_proving_ground._control.server import control_server
...
async def test_samples_endpoint_parses_filter(
    ...
    """
    from agent_proving_ground._control import server as server_mod
    from agent_proving_ground._control.state import SampleListing
...
async def test_404_body_shape_distinguishes_missing_route(
    ...
    """
    from agent_proving_ground._cli.ctl import _handler_404
    from agent_proving_ground._control import server as server_mod
...
def test_resolve_ctl_server_values() -> None:
    ...
    ``APG_EVAL_CTL_SERVER`` env value verbatim. Any other string is
    ...
    user.
    """
    from agent_proving_ground._control.server import resolve_ctl_server
...
def test_eval_set_park_skipped_when_intent_off() -> None:
    ...
    timeout.
    """
    from agent_proving_ground._control.server import request_release, reset_keep_alive
    from agent_proving_ground._eval.evalset import _keep_alive_park
...
def test_peer_uid_reports_own_uid_over_socketpair() -> None:
    ...
    from agent_proving_ground._util.sockets import peer_uid
