...
when `apg ctl samples` is run during a task-level retry teardown: the
...
from agent_proving_ground._control.state import _summary_from_eval_sample_summary
from agent_proving_ground.log import EvalSampleSummary
...
if TYPE_CHECKING:
    ...
    from agent_proving_ground._control.eval_state import LiveEvalData
...
async def test_summaries_from_missing_log_degrade_to_empty(tmp_path) -> None:
    from agent_proving_ground._control.eval_state import EvalState
    from agent_proving_ground._control.state import _sample_summaries_from_log
...
async def test_full_sample_from_missing_log_degrades_to_none(tmp_path) -> None:
    from agent_proving_ground._control.eval_state import clear_all_eval_states, register_eval
    from agent_proving_ground._control.state import _full_sample
...
async def test_fallback_summaries_read_once_and_memoized(monkeypatch) -> None:
    import agent_proving_ground.log._file as log_file
    from agent_proving_ground._control.eval_state import clear_all_eval_states, register_eval
    from agent_proving_ground._control.state import current_sample_summaries

    monkeypatch.setattr("agent_proving_ground.log._samples.active_samples", lambda: [])
    calls = {"n": 0}
...
def test_pending_summary_carries_token_turn_keys() -> None:
    from agent_proving_ground._control.state import _pending_summary
...
async def test_listing_withholds_error_message_unless_content(monkeypatch) -> None:
    ...
    """
    from agent_proving_ground._control.eval_state import clear_all_eval_states, register_eval
    from agent_proving_ground._control.state import current_sample_listing
...
async def test_error_detail_withholds_free_text_unless_content(monkeypatch) -> None:
    ...
    import agent_proving_ground._control.state as state_mod
    from agent_proving_ground.log import EvalError
...
def test_running_summary_reports_token_limit_and_turns(monkeypatch) -> None:
    ...
    from agent_proving_ground._control.state import _sample_summaries_from_active
    ...
    monkeypatch.setattr("agent_proving_ground.log._samples.active_samples", lambda: [s])
...
def _pending_model_event(retries: int | None = None) -> "ModelEvent":
    from agent_proving_ground.event._model import ModelEvent
    from agent_proving_ground.model import GenerateConfig, ModelOutput
...
def test_activity_pending_model() -> None:
    from agent_proving_ground._control.state import _sample_activity
...
def test_activity_retry_wait_when_nothing_pending() -> None:
    from agent_proving_ground._control.state import _sample_activity
    from agent_proving_ground.log._samples import ActiveSampleRetryWait
...
async def test_retry_wait_clear_preserves_concurrent_siblings_record() -> None:
    ...
    from agent_proving_ground._util._async import tg_collect
    from agent_proving_ground.log._samples import (
        _sample_active,
        clear_active_sample_retry_wait,
        report_active_sample_retry_wait,
...
async def test_model_retry_before_sleep_stamps_retry_wait() -> None:
    from agent_proving_ground.log._samples import _sample_active
...
async def test_batch_admin_retry_does_not_stamp_retry_wait() -> None:
    ...
    from agent_proving_ground.model._retry import batch_admin_retry_config
...
def test_task_summary_adds_live_counts_to_the_eval_total(monkeypatch) -> None:
    ...
    from agent_proving_ground._control.state import _build_summary
