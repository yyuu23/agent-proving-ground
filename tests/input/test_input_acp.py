...
:mod:`agent_proving_ground.util._input.acp`; the registry plumbing it consumes is
...
from agent_proving_ground.util._input.acp import (
...
def acp_server_running(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "agent_proving_ground.util._input.acp.acp_server_accepting_clients", lambda: True
    )
...
async def test_entry_returns_none_when_shim_guard_suppresses(
    ...
    monkeypatch.setattr(
        "agent_proving_ground.util._input.acp._request_from_driver_with_fallback", _explode
    )
    result = await acp_handler(_input_request())
    assert result is None
