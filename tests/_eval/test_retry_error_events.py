...
from agent_proving_ground._eval.task.run import _eval_retry_error, _sample_transcript_config
from agent_proving_ground.event import Event, InfoEvent, ModelEvent
...
from agent_proving_ground.log._recorders.streaming import eval_retry_error_from_history
from agent_proving_ground.log._recorders.types import SampleEvent
from agent_proving_ground.log._transcript import _transcript, init_transcript
from agent_proving_ground.model import ChatMessage, ChatMessageUser, GenerateConfig, ModelOutput
...
def test_sample_transcript_config_requires_buffer_for_bounded_mode(
    ...
    monkeypatch.setenv("APG_TRANSCRIPT_BOUNDED", "true")
...
def test_sample_transcript_config_defaults_to_unbounded_with_buffer(
    ...
    monkeypatch.delenv("APG_TRANSCRIPT_BOUNDED", raising=False)
    logger = _TaskLoggerShim(
        SampleBufferDatabase(str(tmp_path / "test.eval"), db_dir=tmp_path)
    )
