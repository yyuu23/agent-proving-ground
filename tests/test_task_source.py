...
from agent_proving_ground import Task, TaskSource, eval, eval_async, task_source
...
from agent_proving_ground.log import EvalLog, EvalSample
...
def test_task_source_load_by_registered_name() -> None:
    # resolve a registered source by name, like apg eval <name>
    logs = eval(tasks="decorated_generations", model="mockllm/model", display="none")
...
def test_task_source_load_from_file(tmp_path: Path) -> None:
    ...
    src.write_text(
        "from agent_proving_ground import Task, TaskSource, task_source\n"
        "from agent_proving_ground.dataset import Sample\n"
...
def test_task_source_name_rejected_outside_eval() -> None:
    ...
    # CLI path, e.g. `apg eval-set file.py@source`) — not a confusing
    # "task not found"
    with pytest.raises(ValueError, match="only supported by `eval"):
        resolve_tasks(
