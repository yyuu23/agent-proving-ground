...
(`_CheckpointerSetup` built by `create_checkpointer` with `APG_CHECKPOINTING=1`,
...
from agent_proving_ground.event._span import SpanBeginEvent
from agent_proving_ground.log import Transcript
from agent_proving_ground.log._transcript import init_transcript, transcript
from agent_proving_ground.util import checkpointer, current_checkpointer
from agent_proving_ground.util._checkpoint import Manual
...
def _patch_sample_active(value: object) -> Iterator[None]:
    with patch("agent_proving_ground.log._samples.sample_active", return_value=value):
        yield
...
def _harness(tmp_path: Path) -> Iterator[_Harness]:
    ...
    with (
        ...
        patch.dict(os.environ, {"APG_CHECKPOINTING": "1"}),
    ):
