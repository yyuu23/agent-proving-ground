...
from agent_proving_ground.event._checkpoint import CheckpointEvent
from agent_proving_ground.event._event import Event
from agent_proving_ground.event._info import InfoEvent
from agent_proving_ground.event._model import ModelEvent
from agent_proving_ground.event._span import SpanBeginEvent, SpanEndEvent
from agent_proving_ground.log import Transcript, expand_events
from agent_proving_ground.log._transcript import init_transcript
from agent_proving_ground.log._transcript_store import TranscriptEventStore
...
from agent_proving_ground.util import current_checkpointer
from agent_proving_ground.util._checkpoint import (
    Manual,
    TimeInterval,
    TokenInterval,
    TurnInterval,
...
from agent_proving_ground.util._checkpoint._layout.schemas import Checkpoint, SnapshotDetails
from agent_proving_ground.util._checkpoint._triggers import CheckpointTriggerKind
from agent_proving_ground.util._checkpoint.checkpointer import Checkpointer, ResumeCheckpoint
from agent_proving_ground.util._checkpoint.checkpointer_impl import (
    CheckpointFailureLimitExceeded,
    _CheckpointerSetup,
    _EnteredCheckpointer,
    _scan_next_checkpoint_id,
)
from agent_proving_ground.util._checkpoint.checkpointer_noop import _NoopCheckpointer
from agent_proving_ground.util._checkpoint.config import ResolvedCheckpointConfig
from agent_proving_ground.util._checkpoint.hydrate import HydrationResult, _HostHydrationResult
from agent_proving_ground.util._checkpoint.report import ResumeReport
from agent_proving_ground.util._restic import ResticBackupSummary
...
def _patch_sample_runtime(events: list[object]) -> Iterator[None]:
    ...
    with (
        patch(
            "agent_proving_ground.util._checkpoint.checkpointer_impl.sample_state",
            return_value=fake_state,
        ),
        patch(
            "agent_proving_ground.util._checkpoint.checkpointer_impl.transcript",
...
async def test_time_interval_fires_when_elapsed_exceeds_threshold(dirs: _Dirs) -> None:
    ...
    with patch("agent_proving_ground.util._checkpoint.checkpointer_impl.time.monotonic", clock):
        cp = _counting(
...
async def test_token_interval_fires_when_usage_crosses_threshold(
    ...
    with patch("agent_proving_ground.model._model.sample_total_tokens", tokens):
        cp = _counting(
...
async def test_fire_reopens_checkpoint_span_after_failure(dirs: _Dirs) -> None:
    from agent_proving_ground.util._span import current_span_id
...
def _checkpoint_resume_events(count: int) -> list[Event]:
    from agent_proving_ground.util._checkpoint.hydrate import _wrap_prior_run
...
def test_synthesize_trailing_checkpoint_event(tmp_path: Path) -> None:
    ...
    from agent_proving_ground.util._checkpoint.hydrate import (
...
def test_validate_resume_state_accepts_wrapped_checkpoint_shape(tmp_path: Path) -> None:
    from agent_proving_ground.util._checkpoint.hydrate import _validate_resume_state
...
# --- tracing (apg trace anomalies / dump --filter checkpoint) ---------


async def test_fire_emits_trace_action(dirs: _Dirs) -> None:
    """A fire is wrapped in a `Checkpoint` trace action (enter/exit).

    The action is what `apg trace anomalies` consumes (a live fire shows
    ...
    ``apg trace ... --filter checkpoint`` (a case-insensitive match on
    ``record.message``) catches the whole subsystem.
    """
    from agent_proving_ground._util.constants import TRACE
    ...
    target = logging.getLogger("agent_proving_ground.util._checkpoint.checkpointer_impl")
    handler = _Collector()
...
async def test_failure_recorded_as_info_event_and_warning(
    ...
    # on the `agent_proving_ground` logger — restore propagation for caplog.
    inspect_logger = logging.getLogger("agent_proving_ground")
...
class _FakeActiveSample:
    ...
    # agent_proving_ground log handler (e.g. via `eval()`).
    task: str = "test_task"
...
def _patch_sample_active(value: object) -> Iterator[None]:
    with patch(
        "agent_proving_ground.log._samples.sample_active",
        return_value=value,
    ):
        yield
...
def _patch_cache_dir(tmp_path: Path) -> Iterator[None]:
    ...
    with patch(
        "agent_proving_ground.util._checkpoint._layout.staging_dir.inspect_cache_dir",
...
def _patch_restic(tmp_path: Path) -> Iterator[None]:
    ...
    with (
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.resolve_restic",
            side_effect=fake_resolve,
        ),
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.init_repo",
            side_effect=fake_init_repo,
        ),
        patch(
            "agent_proving_ground.util._checkpoint.checkpointer_impl.run_backup",
...
async def test_fire_writes_restic_config_and_checkpoint_files(
    ...
    from agent_proving_ground.util._checkpoint.checkpointer_factory import create_checkpointer
    ...
    from agent_proving_ground.util._checkpoint._layout.schemas import Checkpoint
...
def _populated_assistant_internal() -> Iterator[None]:
    ...
    from agent_proving_ground.model._assistant_internal import init_sample_assistant_internal
    from agent_proving_ground.model._providers.anthropic import assistant_internal
...
async def test_write_host_context_includes_assistant_internal(tmp_path: Path) -> None:
    ...
    from agent_proving_ground.util._checkpoint._layout import host_context
...
def test_attempt_reflects_resume_checkpoint() -> None:
    from agent_proving_ground.util._checkpoint.checkpointer import ResumeCheckpoint
...
async def test_setup_aenter_defers_io_setup(tmp_path: Path) -> None:
    ...
    from agent_proving_ground.util._subprocess import ExecResult
    ...
    from agent_proving_ground.util._sandbox.context import sandbox_environments_context_var

    sandbox_token = sandbox_environments_context_var.set({"web": fake_env})

    with (
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.ensure_sample_checkpoints_dir",
            new=AsyncMock(return_value=sample_ckpt_path),
        ) as ensure_ckpt,
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.ensure_context_dir",
            new=AsyncMock(return_value=f"{sample_ckpt_path}/context"),
        ) as ensure_ctx,
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.ensure_restic_config",
        ...
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.sandbox",
            return_value=fake_env,
        ) as get_sandbox,
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.inject_restic",
            new=AsyncMock(),
        ) as inject,
        patch(
            "agent_proving_ground.util._checkpoint.hydrate.init_sandbox_repo",
            new=AsyncMock(),
        ) as init_sandbox,
    ):
        io_mocks = (
...
async def test_checkpointer_setup_resets_transcript_store_after_seed_failure(
    ...
    with (
        patch(
            "agent_proving_ground.util._checkpoint.checkpointer_impl.hydrate",
...
async def test_materialize_pooled_model_event_expands_seed_payload(
    ...
    from agent_proving_ground.event._pool import materialize_pooled_events
    from agent_proving_ground.util._checkpoint._layout import host_context
...
def test_resume_report_defaults_and_coercion() -> None:
    from agent_proving_ground.util._checkpoint.report import (
        ResumeReport,
...
def test_resume_report_exported_from_util() -> None:
    from agent_proving_ground.util import ResumeReport as Exported
    from agent_proving_ground.util._checkpoint.report import ResumeReport
...
async def test_on_checkpoint_mutations_captured_in_snapshot(dirs: _Dirs) -> None:
    ...
    from agent_proving_ground.util._checkpoint._layout.host_context import STORE
...
def test_task_stores_checkpoint_callbacks() -> None:
    from agent_proving_ground import Task, task_with
