...
from agent_proving_ground._eval.loader import resolve_tasks
from agent_proving_ground._eval.run import eval_run
from agent_proving_ground._eval.task import log as task_log_module
from agent_proving_ground._eval.task.log import (
    TaskLogger,
    resolve_package_revision,
    resolve_task_distribution,
)
from agent_proving_ground._util.background import background_task_group, set_background_task_group
from agent_proving_ground._util.git import GitContext
from agent_proving_ground._util.package import DirectUrl, VcsInfo
from agent_proving_ground.dataset import Sample
from agent_proving_ground.log import EvalRevision
...
from agent_proving_ground.log._recorders.buffer.database import SampleBufferDatabase
from agent_proving_ground.log._recorders.eval import EvalRecorder
from agent_proving_ground.log._recorders.recorder import Recorder
...
class TestResolveTaskDistribution:
    ...
    def test_returns_none_when_task_not_in_registry(self):
        with patch("agent_proving_ground._eval.task.log.registry_lookup", return_value=None):
            assert resolve_task_distribution("pkg/some_task") is None

    def test_returns_none_when_no_distribution_for_object(self):
        with (
            patch("agent_proving_ground._eval.task.log.registry_lookup", return_value=object()),
            patch(
                "agent_proving_ground._eval.task.log.get_distribution_for_object",
                return_value=None,
            ),
        ):
            assert resolve_task_distribution("pkg/some_task") is None

    def test_returns_none_for_agent_proving_ground_itself(self):
        with (
            patch("agent_proving_ground._eval.task.log.registry_lookup", return_value=object()),
            patch(
                "agent_proving_ground._eval.task.log.get_distribution_for_object",
                return_value=_fake_dist("agent-proving-ground"),
            ),
        ):
            assert resolve_task_distribution("agent_proving_ground/some_task") is None
...
async def test_eval_run_cleans_initialized_loggers_when_setup_fails(
    ...
    monkeypatch.setattr("agent_proving_ground._eval.run.TaskLogger", FakeTaskLogger)
...
async def test_task_logger_start_stale_flush_timer_rolls_back_failed_start(
    ...
    monkeypatch.setattr(
        "agent_proving_ground._eval.task.log.run_in_background", fail_background_start
    )
...
async def test_flush_samples_drains_quiet_only_state() -> None:
    # `apg ctl task log-flush` regression: with only reused (quiet)
...
class TestResolvePackageRevision:
    ...
    def test_returns_none_when_not_a_vcs_install(self):
        ...
        with patch(
            "agent_proving_ground._eval.task.log.get_distribution_direct_url",
...
def test_package_and_revision_logged_for_git_install():
    ...
    with (
        patch("agent_proving_ground._eval.task.log.resolve_task_distribution", return_value=dist),
        patch(
            "agent_proving_ground._eval.task.log.get_distribution_direct_url",
            return_value=direct_url,
        ),
...
def test_package_revision_preferred_over_cwd_git_context():
    ...
    with (
        ...
        patch(
            "agent_proving_ground._eval.task.log.git_context",
...
def test_revision_none_when_task_not_from_package():
    task = Task()
    with patch(
        "agent_proving_ground._eval.task.log.resolve_task_distribution", return_value=None
    ):
        [log] = eval(task, model="mockllm/model")
...
def test_falls_back_to_cwd_git_context_when_no_package_revision():
    task = Task()
    with (
        patch("agent_proving_ground._eval.task.log.resolve_task_distribution", return_value=None),
        patch(
            "agent_proving_ground._eval.task.log.git_context",
            return_value=GitContext(
                origin="https://github.com/some/cwd-repo",
