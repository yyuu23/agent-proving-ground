...
agent_proving_ground/event/_pool_index.py for the per-event strategy.
...
O(history length). See agent_proving_ground/log/_condense.py walk-cache for the batch
path.
...
from agent_proving_ground.log._condense import condense_events, condense_sample, expand_events
...
from agent_proving_ground.model._chat_message import ChatMessage, ChatMessageBase, ChatMessageUser
...
def hash_counter(monkeypatch: pytest.MonkeyPatch) -> Iterator[HashCounter]:
    ...
    import agent_proving_ground.event._pool as pool
