...
from agent_proving_ground.model._model_output import Logprob, Logprobs
from agent_proving_ground.scorer._target import Target
from agent_proving_ground.scorer._target_perplexity import target_perplexity
...
async def test_auto_tokenizes_target_text() -> None:
    ...
    with patch(
        "agent_proving_ground.scorer._target_perplexity.get_model",
        return_value=mock,
    ):
        scorer = target_perplexity()
        result = await scorer(state, Target(["The answer"]))
