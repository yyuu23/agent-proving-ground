...
from agent_proving_ground import eval as inspect_eval
from agent_proving_ground._display.core.display import TaskCancel
from agent_proving_ground._eval.evalset import eval_set
from agent_proving_ground._eval.task.run import task_run as original_task_run
...
def test_abort_cancel_produces_error_status() -> None:
    ...
    with tempfile.TemporaryDirectory() as log_dir:
        with patch("agent_proving_ground._eval.run.task_run", capturing_task_run):
...
def test_score_resolution_cancel_completes_eval() -> None:
    ...
    """
    from agent_proving_ground._control.cancel import cancel_task as ctl_cancel_task
...
def test_interrupt_in_retry_drain_window_resolves_cancelled() -> None:
    ...
    with tempfile.TemporaryDirectory() as log_dir:
        with (
            patch(
                "agent_proving_ground._eval.task.run.record_sample_cancelled",
                recording_cancelled,
            ),
            patch(
                "agent_proving_ground._eval.task.run.record_sample_errored",
                recording_errored,
            ),
...
def test_external_interrupt_with_pending_resolution_logs_cancelled(
    ...
    """
    from agent_proving_ground._control.eval_state import get_eval_states
    from agent_proving_ground.log import list_eval_logs, read_eval_log
...
def test_errored_attempt_marked_retry_pending() -> None:
    ...
    with tempfile.TemporaryDirectory() as log_dir:
        with patch("agent_proving_ground._eval.run.mark_eval_retry_pending", recording_mark):
            success, logs = eval_set(
                tasks=[
                    Task(
                        dataset=[Sample(input="x", target="y")],
