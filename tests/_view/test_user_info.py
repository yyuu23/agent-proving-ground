...
from agent_proving_ground._view.user_info import user_info
...
def _patch_git(values: dict[str, str | None]) -> AbstractContextManager[object]:
    ...
    return patch("agent_proving_ground._view.user_info._git_config", side_effect=fake)
...
def test_falls_back_to_os_user_when_git_returns_nothing() -> None:
    with _patch_git({"user.name": None, "user.email": None}):
        with patch("agent_proving_ground._view.user_info.getpass.getuser", return_value="osuser"):
            info = user_info()
...
def test_skips_git_when_unavailable() -> None:
    ...
    with patch("agent_proving_ground._view.user_info.shutil.which", return_value=None):
        with patch("agent_proving_ground._view.user_info.getpass.getuser", return_value="osuser"):
            info = user_info()
    assert info.name == "osuser"
    assert info.email is None
