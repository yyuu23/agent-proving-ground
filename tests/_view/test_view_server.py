...
import agent_proving_ground._eval.evalset
import agent_proving_ground._eval.task.resolved
import agent_proving_ground._util.file
import agent_proving_ground.dataset
import agent_proving_ground.log
import agent_proving_ground.log._recorders.buffer.filestore
import agent_proving_ground.model
from agent_proving_ground._util.asyncfiles import AsyncFilesystem
from agent_proving_ground._util.event_loop_monitor import event_loop_monitor
from agent_proving_ground._util.json import to_json_safe
from agent_proving_ground._view import fastapi_server
from agent_proving_ground._view.common import (
    get_direct_url,
    get_log_bytes,
    normalize_uri,
...
from agent_proving_ground._view.fastapi_server import AccessPolicy, FileMappingPolicy
from agent_proving_ground.event import ScoreEvent
from agent_proving_ground.log import list_eval_logs_async
from agent_proving_ground.model._generate_config import GenerateConfig
from agent_proving_ground.scorer import Score
...
def write_eval_log(base_dir: Path, filename: str, status: str = "success") -> str:
    ...
    eval_log = agent_proving_ground.log.EvalLog(
        status=status,  # type: ignore[arg-type]
        eval=agent_proving_ground.log.EvalSpec(
            created="2025-01-01T00:00:00Z",
            task="task",
            task_id="task_id",
            dataset=agent_proving_ground.log.EvalDataset(),
            model="model",
            config=agent_proving_ground.log.EvalConfig(),
        ),
    )
    agent_proving_ground.log.write_eval_log(eval_log, full_path, "eval")
    return full_path


def _create_sample_buffer(log_path: str) -> None:
    """Create a minimal sample buffer on disk for the given log file."""
    from agent_proving_ground.log._recorders.buffer.filestore import (
        Manifest,
    ...
    from agent_proving_ground.log._recorders.buffer.types import EventData, SampleData
    ...
    buf.write_manifest(
        Manifest(
            samples=[
                SampleManifest(
                    summary=agent_proving_ground.log.EvalSampleSummary(
                        id="sample1",
                        epoch=0,
                        input="test input",
                        target="test target",
...
def test_api_app_config(view_client: ViewTestClient) -> None:
    ...
    resp.raise_for_status()
    config = resp.json()
    assert config["inspect_version"] == agent_proving_ground.__version__
...
def test_api_log_edit_requires_frontend_request(view_client: ViewTestClient) -> None:
    ...
    assert resp.status_code == 403
    assert agent_proving_ground.log.read_eval_log(full_path, header_only=True).tags == []
...
def test_api_log_edit_metadata_null_value_persists(
    ...
    # Verify on disk too.
    persisted = agent_proving_ground.log.read_eval_log(full_path, header_only=True)
...
def test_api_pending_samples_preserves_non_finite_scores(
    ...
    full_path = write_eval_log(view_client.log_dir, fname)
    _create_sample_buffer(full_path)
    buffer = agent_proving_ground.log._recorders.buffer.filestore.SampleBufferFilestore(
...
def _patch_flat_filesystem(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from agent_proving_ground._view import common
...
async def test_list_eval_logs_async_uses_fsspec_path_with_fs_options(
    ...
    from agent_proving_ground._util._async import current_async_backend
    from agent_proving_ground._util.file import FileInfo
    from agent_proving_ground.log import _file as log_file
...
def write_fake_eval_log(file_path: str) -> None:
    ...
    agent_proving_ground.log.write_eval_log(eval_log, full_file_path, "eval")
...
def write_fake_eval_log_buffer(
    ...
    samples = [
        agent_proving_ground.log._recorders.buffer.filestore.SampleManifest(
            summary=agent_proving_ground.log.EvalSampleSummary(
                id="id",
                epoch=0,
                input="hello",
    ...
    segments = [
        agent_proving_ground.log._recorders.buffer.filestore.Segment(
            id=i,
            last_event_id=i + 1,
    ...
    manifest = agent_proving_ground.log._recorders.buffer.filestore.Manifest(
        metrics=[],
        samples=samples,
        segments=segments,
    )
    ...
    for i in range(num_segments):
        sample = agent_proving_ground.log._recorders.buffer.SampleData(  # pyright: ignore[reportPrivateImportUsage]
            events=[
                agent_proving_ground.log._recorders.buffer.EventData(  # pyright: ignore[reportPrivateImportUsage]
                    id=1,
                    event_id="event_id",
                    sample_id="sample_id",
                    epoch=0,
...
def test_fastapi_log_delete_forbidden(
    ...
    assert agent_proving_ground._util.file.filesystem("memory://").exists(mock_s3_eval_file)
...
def test_fastapi_eval_set(test_client: TestClient) -> None:
    ...
    fs = agent_proving_ground._util.file.filesystem(eval_set_dir)
    fs.mkdir(eval_set_dir)
    agent_proving_ground._eval.evalset.write_eval_set_info(
        eval_set_id=eval_set_id,
        log_dir=eval_set_dir,
        tasks=[
            agent_proving_ground._eval.task.resolved.ResolvedTask(
                id="task_id",
                task=agent_proving_ground._eval.task.Task(
                    name="task-name",
                    dataset=agent_proving_ground.dataset.MemoryDataset(
                        samples=[
                            agent_proving_ground.dataset.Sample(input="input", target="target")
                        ],
                    ),
                ),
                sandbox=None,
                ...
                task_file="task_file",
                task_args={},
                model=agent_proving_ground.model.get_model("mockllm/model"),
                model_roles={},
                sequence=0,
            )
        ],
        all_logs=[],
        eval_set_args=agent_proving_ground._eval.evalset.EvalSetArgsInTaskIdentifier(
...
def test_fastapi_nan_metric(test_client: TestClient) -> None:
    ...
    eval_log = agent_proving_ground.log.EvalLog(
        ...
        results=agent_proving_ground.log.EvalResults(
            scores=[
                agent_proving_ground.log.EvalScore(
                    name="accuracy",
                    scorer="my_scorer",
                    metrics={
                        "accuracy": agent_proving_ground.log.EvalMetric(
                            name="accuracy", value=float("nan")
                        ),
                        "inf_metric": agent_proving_ground.log.EvalMetric(
...
def test_fastapi_only_dir_access_policy_accepts_listed_file_uri(
    ...
    fs = agent_proving_ground._util.file.filesystem(log_file)
...
def test_fastapi_inspect_json_response_nan() -> None:
    resp = fastapi_server.ApgJsonResponse(
...
def test_flow_uses_map_file(test_client: TestClient) -> None:
    ...
    fs = agent_proving_ground._util.file.filesystem(flow_dir)
    fs.mkdir(flow_dir)
    with cast(
        ContextManager[IO[bytes]],
...
def _make_s3_eval_log(status: str = "success") -> agent_proving_ground.log.EvalLog:
    return agent_proving_ground.log.EvalLog(
        status=status,  # type: ignore[arg-type]
        eval=agent_proving_ground.log.EvalSpec(
            created="2025-01-01T00:00:00Z",
            task="task",
...
def _write_eval_log_to_s3(s3_path: str, status: str = "success") -> None:
    ...
    """
    agent_proving_ground.log.write_eval_log(_make_s3_eval_log(status), s3_path, "eval")
...
def test_api_pending_sample_data_urls_s3_populates_direct_url(
    ...
    agent_proving_ground.log.write_eval_log(eval_log, s3_log, "eval")
...
async def test_stream_log_bytes_local_does_not_restat_known_size(
    ...
    calls = 0
    real_get_log_size = agent_proving_ground._view.common.get_log_size
    ...
    monkeypatch.setattr(agent_proving_ground._view.common, "get_log_size", counting_get_log_size)
