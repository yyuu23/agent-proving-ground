...
def test_anthropic_reasoning_tokens_errors_on_4_7_plus(model_name):
    ...
    Sending `{type: "enabled", budget_tokens}` to these models 400s, so AgentProvingGround
...
def test_ollama_clamps_extended_effort_values(effort, expected):
    from agent_proving_ground.model._providers.ollama import OllamaAPI
...
def test_sambanova_clamps_extended_effort_values(effort, expected):
    from agent_proving_ground.model._providers.sambanova import SambaNovaAPI
...
def test_perplexity_clamps_only_extended_top_end(effort, expected):
    from agent_proving_ground.model._providers.perplexity import PerplexityAPI
...
def test_grok_xhigh_clamped_on_older_sdk(monkeypatch) -> None:
    ...
    import agent_proving_ground.model._providers.grok as grok_module
