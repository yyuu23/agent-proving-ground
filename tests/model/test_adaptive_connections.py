...
async def test_adaptive_with_max_retries_zero_works() -> None:
    ...
    independently of AgentProvingGround's outer retry loop, so the controller still gets
...
async def test_batch_mode_disables_adaptive() -> None:
    ...
    """
    from agent_proving_ground.model._generate_config import BatchConfig
...
async def test_connection_limit_history_captured_in_eval_stats() -> None:
    ...
    from agent_proving_ground._eval.task.log import collect_eval_data
    from agent_proving_ground.log._log import EvalStats
...
def test_parse_adaptive_connections_cli_value_forms() -> None:
    ...
    """
    from agent_proving_ground._cli.eval import _parse_adaptive_connections_cli
...
def test_attempt_timeout_marks_as_retry_in_should_retry() -> None:
    ...
    """
    from agent_proving_ground.model._model import AttemptTimeoutError
    from agent_proving_ground.util._concurrency import _request_had_retry, init_concurrency
...
async def test_cache_hits_do_not_count_as_adaptive_successes(tmp_path) -> None:
    ...
    monkey_env.setenv("APG_CACHE_DIR", str(tmp_path))
    try:
        init_concurrency()
        model = get_model("mockllm/model")
        cfg = GenerateConfig(
...
def test_sample_semaphore_uses_dynamic_for_adaptive() -> None:
    ...
    from agent_proving_ground._eval.task.run import create_sample_semaphore
    from agent_proving_ground.log._log import EvalConfig
    from agent_proving_ground.util._concurrency import DynamicSampleLimiter, init_concurrency
...
def test_sample_semaphore_explicit_max_samples_wins() -> None:
    ...
    from agent_proving_ground._eval.task.run import create_sample_semaphore
    from agent_proving_ground.log._log import EvalConfig
    from agent_proving_ground.util._concurrency import DynamicSampleLimiter, ResizableLimiter
...
async def test_ensure_model_controller_eager_creation() -> None:
    ...
    from agent_proving_ground._eval.task.run import create_sample_semaphore
    from agent_proving_ground.log._log import EvalConfig
    from agent_proving_ground.model._model import ensure_model_controller
    from agent_proving_ground.util._concurrency import DynamicSampleLimiter, init_concurrency
...
async def test_ensure_model_controller_composes_model_config() -> None:
    ...
    """
    from agent_proving_ground.model._model import ensure_model_controller
    from agent_proving_ground.util._concurrency import init_concurrency
...
def test_sample_semaphore_composes_model_config() -> None:
    ...
    Regression (meridianlabs-ai/agent_proving_ground#32): the task_run call site passed
...
def test_sample_semaphore_shared_across_retry_attempts() -> None:
    ...
    from agent_proving_ground._eval.task.run import create_sample_semaphore
    from agent_proving_ground.log._log import EvalConfig
    from agent_proving_ground.util._concurrency import ResizableLimiter, init_concurrency
...
def test_resolve_adaptive_returns_concrete_AdaptiveConcurrency() -> None:
    from agent_proving_ground.util._concurrency import resolve_adaptive
