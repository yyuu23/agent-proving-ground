...
from agent_proving_ground.model._providers.util.hooks import ConverseHooks  # noqa: E402
...
def test_request_created_stashes_request_id_on_context() -> None:
    """request-created handler must put the AgentProvingGround request_id on AWSRequest.context.
...
def test_request_created_increments_attempts() -> None:
    ...
    with patch("agent_proving_ground.model._providers.util.hooks.report_http_retry"):
        hooks.converse_request_created(request=request)
...
def test_throttle_then_retry_classifies_as_rate_limit() -> None:
    ...
    with patch(
        "agent_proving_ground.model._providers.util.hooks.report_http_retry"
    ) as mock_report:
...
def test_retry_after_deadline_decays_across_sdk_backoff() -> None:
    ...
    from agent_proving_ground.model._providers.util.hooks import (
        ConverseHooks,
        RequestInfo,
    )
