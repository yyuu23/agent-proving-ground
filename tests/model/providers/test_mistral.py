...
def test_mistral_tool_schema_formatting(tiktok_tool_with_description_param):
    ...
    """
    from agent_proving_ground.model._providers.mistral import mistral_chat_tools
...
def test_completion_content_chunks_image_url_string():
    ...
    from agent_proving_ground.model._providers.mistral import completion_content_chunks
...
async def test_mistral_chat_forwards_config_extra_headers() -> None:
    ...
    with mock.patch("agent_proving_ground.model._providers.mistral.Mistral", return_value=client):
...
def test_mistral_reasoning_effort_mapping() -> None:
    """Mistral accepts only "high"/"none" — other AgentProvingGround values map to "high"."""
    from agent_proving_ground.model._providers.mistral_conversation import (
...
async def test_mistral_chat_forwards_reasoning_effort() -> None:
    ...
    async def request_effort(config: GenerateConfig) -> object:
        ...
        with mock.patch(
            "agent_proving_ground.model._providers.mistral.Mistral", return_value=client
        ):
            with pytest.raises(RuntimeError, match="stop after capture"):
                await api.generate(
                    input=[ChatMessageUser(content="hi")],
