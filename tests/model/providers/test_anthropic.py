...
def test_anthropic_full_thinking_beta_via_extra_header(header_key: str) -> None:
    ...
    The underscore form is AgentProvingGround's convention; the hyphen form is the literal
...
def _warn_once_messages() -> Any:
    ...
    # init_logger sets propagate=False on the agent_proving_ground logger once any
    # earlier test triggers it.
    from agent_proving_ground._util import logger as _inspect_logger
...
def _computer_tool_info() -> ToolInfo:
    ...
    from agent_proving_ground.tool._tool_params import ToolParam, ToolParams
    from agent_proving_ground.tool._tools._computer._computer import _COMPUTER_TOOL_PARAMETERS
...
async def test_anthropic_container_replayed_after_client_tool_call() -> None:
    ...
    from agent_proving_ground._util.content import ContentToolUse
...
async def test_anthropic_stream_capture_restores_container() -> None:
    ...
    the id (anthropics/anthropic-sdk-python#1776). AgentProvingGround's stream capture
...
async def test_anthropic_container_continuation_live() -> None:
    ...
    from agent_proving_ground.tool import ToolDef, ToolResult
