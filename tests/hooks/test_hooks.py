...
import agent_proving_ground.hooks._startup as hooks_startup_module
...
from agent_proving_ground._util.registry import _registry, registry_info, registry_lookup
...
from agent_proving_ground.solver._solver import Generate, Solver, solver
...
def test_hooks_with_error_passes_exception_to_run_end(mock_hooks: MockHooks) -> None:
    with pytest.raises(RuntimeError, match="test"):
        with patch("agent_proving_ground._eval.eval.eval_init", side_effect=RuntimeError("test")):
            eval(
...
def test_hooks_with_base_exception_passes_exception_to_run_end(
    ...
    with pytest.raises(KeyboardInterrupt):
        with patch("agent_proving_ground._eval.eval.eval_init", side_effect=KeyboardInterrupt()):
            eval(
                Task(dataset=[Sample("sample_1")], solver=_fail_n_times_solver(1)),
                model="mockllm/model",
                retry_on_error=0,
...
def test_api_key_override_falls_back_to_legacy(mock_hooks: MockHooks) -> None:
    ...
    with environ_var("APG_API_KEY_OVERRIDE", "._legacy_hook_override"):
        with patch(
            "agent_proving_ground.hooks._hooks.override_api_key_legacy", legacy_hook_override
        ):
            overridden = override_api_key("TEST_VAR", "test_value")
...
def test_required_hooks_when_all_installed(
    ...
    with environ_var("APG_REQUIRED_HOOKS", "test_hooks"):
        init_hooks()
...
def test_required_hooks_when_one_missing(
    ...
    with environ_var("APG_REQUIRED_HOOKS", "test_hooks,fake"):
...
def test_no_attempt_end_without_attempt_start(mock_hooks: MockHooks) -> None:
    ...
    with patch(
        "agent_proving_ground.hooks._hooks.emit_sample_start",
