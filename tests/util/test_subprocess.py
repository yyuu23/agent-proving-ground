...
import agent_proving_ground.util._subprocess as _subprocess_mod
from agent_proving_ground.util import subprocess
from agent_proving_ground.util._subprocess import _log_stream
from agent_proving_ground.util._subprocess import logger as _subprocess_logger
...
async def test_subprocess_timeout_with_lost_child_watcher(
    ...
    # agent_proving_ground's package loggers have propagate=False once init_logger()
...
async def test_subprocess_redirect_to_logger(monkeypatch) -> None:
    monkeypatch.setenv("APG_SUBPROCESS_REDIRECT_TO_LOGGER", "1")
    messages: list[str] = []
