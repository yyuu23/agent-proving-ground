...
from agent_proving_ground.util._restic.resolver import (
    _DOWNLOAD_TIMEOUT,
    SUPPORTED_PLATFORMS,
    Platform,
    _archive_filename,
...
def _patch_cache_dir(tmp_path: Path) -> Iterator[None]:
    ...
    with patch(
        "agent_proving_ground.util._restic.resolver.inspect_cache_dir",
...
def _patch_sums_file(tmp_path: Path, sums_text: str) -> Iterator[None]:
    ...
    with patch("agent_proving_ground.util._restic.resolver._SHA256SUMS_FILE", sums_file):
        yield
...
def _patch_download(side_effect: Callable[..., Path]) -> Iterator[MagicMock]:
    ...
    with patch(
        "agent_proving_ground.util._restic.resolver.download", side_effect=side_effect
    ) as mock:
        yield mock
...
async def test_cache_hit_short_circuits(tmp_path: Path) -> None:
    with _patch_cache_dir(tmp_path):
        ...
        with patch("agent_proving_ground.util._restic.resolver.download") as download_mock:
...
async def test_unreadable_vendored_sums_raises_runtime_error(tmp_path: Path) -> None:
    ...
    with (
        _patch_cache_dir(tmp_path),
        patch("agent_proving_ground.util._restic.resolver._SHA256SUMS_FILE", missing),
        _patch_download(_download_writes_archive) as download_mock,
    ):
...
def test_vendored_sums_cover_all_supported_platforms() -> None:
    ...
    """
    from agent_proving_ground.util._restic.resolver import _SHA256SUMS_FILE
