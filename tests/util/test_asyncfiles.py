...
from agent_proving_ground._util.asyncfiles import (
    AsyncFilesystem,
    _current_async_fs,
    _RetiredClient,
...
async def test_write_file_streaming_s3_retries_stale_signature_from_start(
    ...
    monkeypatch.setattr("agent_proving_ground._util.asyncfiles.anyio.sleep", no_sleep)
...
def test_run_coroutine_no_loop_uses_configured_backend(
    ...
    """run_coroutine() with no running loop honours APG_ASYNC_BACKEND=trio."""
    import sniffio

    monkeypatch.setenv("APG_ASYNC_BACKEND", "trio")
