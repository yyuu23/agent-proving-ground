"""Tests for `agent_proving_ground.util.notify` and the Apprise plumbing."""
...
from agent_proving_ground.util import notify
from agent_proving_ground.util._notify import (
    active_apprise,
    apprise_scope,
    build_apprise,
)
...
async def test_notify_defaults_title_outside_sample_scope() -> None:
    """Outside an active sample, default title falls back to `AgentProvingGround Agent`."""
    fake = MagicMock()
    fake.notify = MagicMock(return_value=True)
    ...
    kwargs = fake.notify.call_args.kwargs
    assert kwargs.get("title") == "AgentProvingGround Agent"
...
async def test_notify_default_title_uses_sample_context(
    ...
    from agent_proving_ground.log import _samples as samples_module
    ...
    assert kwargs.get("title") == "AgentProvingGround Agent: my_task"
...
def test_build_apprise_true_without_env_var_raises(
    ...
    """notification=True with APG_EVAL_NOTIFICATION unset raises a clear error."""
    monkeypatch.delenv("APG_EVAL_NOTIFICATION", raising=False)

    with pytest.raises(PrerequisiteError) as exc_info:
        build_apprise(True)

    msg = str(exc_info.value.message)
    assert "APG_EVAL_NOTIFICATION" in msg
...
def test_build_apprise_true_with_empty_env_var_raises(
    ...
    monkeypatch.setenv("APG_EVAL_NOTIFICATION", "   ")
...
def test_build_apprise_missing_import_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("APG_EVAL_NOTIFICATION", "slack://...")
...
def test_build_apprise_true_env_var_single_url(
    ...
    monkeypatch.setenv("APG_EVAL_NOTIFICATION", "slack://tok/Bot/#chan")
...
def test_build_apprise_true_env_var_comma_separated(
    ...
    monkeypatch.setenv(
        "APG_EVAL_NOTIFICATION", "slack://abc, macosx:// , discord://xyz"
    )
    fake_module = MagicMock()
    fake_instance = MagicMock()
    fake_module.Apprise = MagicMock(return_value=fake_instance)
...
def test_build_apprise_true_env_var_pointing_at_file(
    ...
    monkeypatch.setenv("APG_EVAL_NOTIFICATION", str(cfg_path))
...
async def test_notify_bounded_when_backend_hangs(
    ...
    from agent_proving_ground.util import _notify
...
def test_eval_config_notification_true_does_not_leak_url(
    ...
    Even with `APG_EVAL_NOTIFICATION` set to a Slack URL, the
    ...
    monkeypatch.setenv("APG_EVAL_NOTIFICATION", secret_url)
