...
from agent_proving_ground._util.constants import HTTP
...
def _ensure_log_propagation() -> Iterator[None]:
    """Counter init_logger() setting propagate=False on agent_proving_ground logger."""
    lgr = logging.getLogger("agent_proving_ground")
...
class TestSampleContextPrefix:
    def test_with_active_sample(self) -> None:
        ...
        with patch("agent_proving_ground.log._samples.sample_active", return_value=mock):
            assert sample_context_prefix() == "[Abc12xY mmlu/42/1 openai/gpt-4o] "

    def test_no_active_sample(self) -> None:
        with patch("agent_proving_ground.log._samples.sample_active", return_value=None):
...
class TestLogModelRetry:
    def test_includes_context_and_error(self, caplog: pytest.LogCaptureFixture) -> None:
        from agent_proving_ground.model._model import log_model_retry
        ...
        with (
            ...
            patch("agent_proving_ground.log._samples.sample_active", return_value=mock_sample),
        ):
