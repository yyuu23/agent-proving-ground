...
async def test_local_sandbox_scopes_server_dir_per_instance() -> None:
    ...
    try:
        ...
        with patch("agent_proving_ground.util._sandbox.local.subprocess", run):
        ...
        assert run.await_args_list[0].kwargs["env"] == {
            "APG_SANDBOX_TOOLS_DIR": expected_a
        }
        assert run.await_args_list[1].kwargs["env"] == {
            "APG_SANDBOX_TOOLS_DIR": expected_b
        }
    ...
    try:
        ...
        with patch("agent_proving_ground.util._sandbox.local.subprocess", run):
            await sandbox.exec(
                ...
                env={"APG_SANDBOX_TOOLS_DIR": "/tmp/shared-sandbox-tools"},
            )

        call = run.await_args
        assert call is not None
        assert call.kwargs["env"] == {
            "APG_SANDBOX_TOOLS_DIR": str(
...
async def test_local_sandbox_does_not_inject_server_dir_into_user_commands(
    ...
    monkeypatch.delenv("APG_SANDBOX_TOOLS_DIR", raising=False)
    sandbox = LocalSandboxEnvironment()
    try:
        result = await sandbox.exec(
            ["sh", "-c", 'printf %s "${APG_SANDBOX_TOOLS_DIR-unset}"']
        )
...
async def test_sample_cleanup_removes_directory_when_server_stop_fails(
    ...
    with patch("agent_proving_ground.util._sandbox.local.logger") as logger:
        await LocalSandboxEnvironment.sample_cleanup(
            task_name="test",
            config=None,
            environments={"default": sandbox},
