...
from agent_proving_ground.util._sandbox.limits import (
...
def test_set_and_reset_limits(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("APG_SANDBOX_MAX_READ_FILE_SIZE", "1048576")
    monkeypatch.setenv("APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE", "2097152")
...
def test_set_limits_partial(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("APG_SANDBOX_MAX_READ_FILE_SIZE", "5242880")
    monkeypatch.delenv("APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE", raising=False)
    tokens = set_sandbox_limits()
...
def test_set_limits_no_env_vars(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("APG_SANDBOX_MAX_READ_FILE_SIZE", raising=False)
    monkeypatch.delenv("APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE", raising=False)
    tokens = set_sandbox_limits()
    assert tokens == []
    assert SandboxEnvironmentLimits.MAX_READ_FILE_SIZE == 100 * 1024**2
...
def test_verify_read_file_size_over_custom_limit(
    ...
    monkeypatch.setenv("APG_SANDBOX_MAX_READ_FILE_SIZE", "5")
    tokens = set_sandbox_limits()
    f = tmp_path / "big.txt"
