...
from agent_proving_ground._util.logger import LogHandler, logger_display_format
...
def log_record(
    ...
    return LogRecord(
        name="agent_proving_ground.test",
        level=INFO,
        pathname=__file__,
        lineno=12,
        msg=message,
...
def test_logger_display_format_defaults_to_rich(
    ...
    monkeypatch.delenv("APG_PY_LOGGER_FORMAT", raising=False)
...
def test_logger_display_format_rejects_invalid(
    ...
    monkeypatch.setenv("APG_PY_LOGGER_FORMAT", "xml")
...
def test_plain_logger_format_writes_single_line(
    ...
    monkeypatch.setenv("APG_TRACE_LEVEL", "info")
    stream = io.StringIO()
    handler = LogHandler(
...
def test_json_logger_format_writes_one_json_record(
    ...
    assert payload["logger"] == "agent_proving_ground.test"
