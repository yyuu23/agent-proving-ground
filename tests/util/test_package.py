...
def test_agent_proving_ground_package():
    assert get_installed_package_name(agent_proving_ground.eval) == "agent_proving_ground"
...
def test_get_distribution_for_object_namespace_package():
    ...
    with (
        patch(
            "agent_proving_ground._util.package.importlib.util.find_spec", return_value=fake_spec
        ),
        patch(
            "agent_proving_ground._util.package.Distribution.from_name",
            side_effect=fake_from_name,
        ),
        patch(
            "agent_proving_ground._util.package.packages_distributions",
            return_value={"ns_pkg": ["dist-a", "dist-b"]},
        ),
        patch(
            "agent_proving_ground._util.package.os.path.realpath", side_effect=os.path.realpath
        ),
    ):
        result = get_distribution_for_object(obj)
...
def test_get_distribution_for_object_fast_path_trusts_name_when_files_unknown():
    ...
    with (
        ...
        patch("agent_proving_ground._util.package.Distribution.from_name", return_value=dist),
    ):
        result = get_distribution_for_object(obj)
