...
from agent_proving_ground._util.background import (
    background_task_group,
    set_background_task_group,
)
from agent_proving_ground.util._throttle import throttle
...
class TestThrottleSyncFallback:
    ...
    def test_first_call_fires_immediately(self) -> None:
        clock = FakeClock()
        with (
            patch("agent_proving_ground.util._throttle.time") as mock_time,
            patch("agent_proving_ground.util._throttle.current_async_backend", return_value=None),
        ):
...
class TestThrottleAsync:
    ...
    async def test_first_call_fires_immediately(self) -> None:
        ...
        async def run(tg):
            with patch("agent_proving_ground.util._throttle.time") as mock_time:
                mock_time.time = clock.time
                fn, calls = _make_recorder(1.0, clock)
                fn("a")
                assert len(calls) == 1
    ...
    async def test_deferred_fires_trailing_event(self) -> None:
        ...
        async def run(tg):
            with (
                patch("agent_proving_ground.util._throttle.time") as mock_time,
                patch("agent_proving_ground.util._throttle.anyio") as mock_anyio,
            ):
                mock_time.time = clock.time
...
class TestThrottleAsyncNoTaskGroup:
    ...
    async def test_trio_no_task_group_falls_back_to_sync(self) -> None:
        ...
        with (
            patch("agent_proving_ground.util._throttle.time") as mock_time,
            patch(
                "agent_proving_ground.util._throttle.current_async_backend",
                return_value="trio",
            ),
            patch("agent_proving_ground.util._throttle.background_task_group", return_value=None),
        ):
            mock_time.time = clock.time
            fn, calls = _make_recorder(1.0, clock)
            fn("first")
