...
class TestToolViewAsStr:
    def test_substituted_content(self) -> None:
        from agent_proving_ground.analysis._dataframe.events.extract import tool_view_as_str
...
class TestTranscriptToolCallEscaping:
    def test_markup_in_title_no_error(self) -> None:
        from agent_proving_ground.tool._tool_transcript import transcript_tool_call
...
class TestRenderToolApprovalEscaping:
    @patch("agent_proving_ground.approval._human.util.display_type", return_value="full")
    def test_markup_in_title_no_error(self, _mock_display: object) -> None:
        from agent_proving_ground.approval._human.util import render_tool_approval
