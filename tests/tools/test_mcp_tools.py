...
def _make_simulated_mcp_client(
    ...
    *,
    ...
    from agent_proving_ground.tool._mcp._compat import jsonrpc_message, jsonrpc_message_root
...
async def test_mcp_tool_call_timeout_becomes_tool_error() -> None:
    ...
    from agent_proving_ground.tool._mcp._compat import jsonrpc_message
    from agent_proving_ground.tool._mcp._local import MCPServerLocalSession
...
def _patch_sandbox_module(monkeypatch, exec_model_request_impl):
    ...
    from agent_proving_ground.tool._mcp import _sandbox as sandbox_module
...
async def test_sandbox_writer_synthesizes_jsonrpc_error_for_non_timeout(monkeypatch):
    ...
    from agent_proving_ground.tool._mcp._compat import jsonrpc_message_root
...
async def test_mcp_server_sse():
    from agent_proving_ground.tool import mcp_server_sse
...
async def test_mcp_sampling_fn():
    ...
    from agent_proving_ground.tool._mcp.sampling import sampling_fn

    with environ_var("APG_EVAL_MODEL", "openai/gpt-4o"):
