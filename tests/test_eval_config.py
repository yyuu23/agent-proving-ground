...
from agent_proving_ground.log._file import list_eval_logs, read_eval_log
...
def run_eval_cli(args: list[str], env: dict[str, str | None] | None = None) -> Result:
    """Invoke `apg eval` in-process via CliRunner.
    ...
    same click command `apg eval` dispatches to, so CLI option parsing and
    config resolution are exercised identically.

    A value of `None` in `env` unsets that variable for the invocation (used to
    clear `APG_EVAL_*` overrides that would otherwise leak in from the
...
def run_eval_retry_cli(
    ...
    """Invoke `apg eval-retry` in-process via CliRunner.
...
def run_eval_set_cli(
    ...
    """Invoke `apg eval-set` in-process via CliRunner.
...
def test_eval_model_spec_env_var_yields_to_explicit_model():
    """An APG_EVAL_MODEL_SPEC must not break an explicit --model.
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = run_eval_cli(
            ...
            env={"APG_EVAL_MODEL_SPEC": MODEL_SPECS[1]},
        )
        assert_cli_success(result)
...
def test_eval_model_spec_cli_allows_model_env_var():
    """APG_EVAL_MODEL is an ambient default, so --model-spec replaces it."""
    with tempfile.TemporaryDirectory() as temp_dir:
        log_dir = Path(temp_dir) / "logs"
        result = run_eval_cli(
            [
            ...
            env={"APG_EVAL_MODEL": "mockllm/from_env"},
        )
        assert_cli_success(result)
        assert_model_spec_temperatures(log_dir)
...
def test_eval_run_config_cli_env_var_overridden_by_run_config():
    """APG_EVAL_MODEL must not override a model set in --run-config."""
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        log_dir = temp_path / "logs"
        run_config = temp_path / "run.yaml"
...
""".strip()
        ...
        result = run_eval_cli(
            ...
            env={"APG_EVAL_MODEL": "anthropic/claude-sonnet-4-6"},
        )
        assert_cli_success(result)
        log = read_eval_log(list_eval_logs(log_dir.as_posix())[0])
        assert log.eval.model == "mockllm/model"
...
def test_eval_run_config_cli_env_var_overridden_by_explicit_flag():
    """An explicit --model flag must beat both APG_EVAL_MODEL and --run-config."""
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        log_dir = temp_path / "logs"
        run_config = temp_path / "run.yaml"
...
def test_eval_run_config_cli_env_var_used_when_run_config_missing_field():
    """APG_EVAL_MODEL still applies when --run-config doesn't set `model`."""
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_path = Path(temp_dir)
        log_dir = temp_path / "logs"
        run_config = temp_path / "run.yaml"
...
""".strip()
        ...
        # APG_EVAL_MODEL from the repo .env and mask the env value
...
def test_eval_cli_score_on_error_default_when_unset():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = run_eval_cli(
            ...
            env={
                "APG_EVAL_SCORE_ON_ERROR": None,
                "APG_EVAL_CONTINUE_ON_FAIL": None,
            },
        )
        assert_cli_success(result)
        log = read_eval_log(list_eval_logs(log_dir.as_posix())[0])
...
def test_eval_retry_cli_preserves_error_flags_from_log():
    """`apg eval-retry` with the flags absent must inherit them from the log.
...
def eval_config_failing_error_flags_task() -> Task:
    ...
    so `apg eval-retry` can be checked for preserving them.
    """
    return Task(
