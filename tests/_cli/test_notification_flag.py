...
that opt-out, exporting `APG_EVAL_NOTIFICATION` — the same env
...
enable (config-file path) or plain `apg eval` to crash (URL
...
def test_env_var_alone_does_not_set_notification() -> None:
    ...
    `APG_EVAL_NOTIFICATION` to this option. That collides with
    ...
    """
    assert _parsed([], env={"APG_EVAL_NOTIFICATION": "slack://hook"}) is None
    assert _parsed([], env={"APG_EVAL_NOTIFICATION": "/tmp/cfg.yml"}) is None
...
def test_env_var_with_bare_flag_still_returns_true() -> None:
    ...
    The CLI shouldn't itself consume `APG_EVAL_NOTIFICATION` —
    it just translates `--notification` to `True` and lets the
    runtime read the env var at apprise-build time.
    """
    assert (
        _parsed(["--notification"], env={"APG_EVAL_NOTIFICATION": "slack://hook"})
        is True
    )
