...
from agent_proving_ground.util._checkpoint import TokenInterval
from agent_proving_ground.util._checkpoint.config import merge_checkpoint_configs
...
def test_real_option_uses_default_sentinel(command: click.Command) -> None:
    ...
    the actual `apg eval` / `apg eval retry` options.
    """
...
def test_env_var_passes_through() -> None:
    assert _parsed([], env={"APG_EVAL_CHECKPOINT": "token:500k"}) == "token:500k"
