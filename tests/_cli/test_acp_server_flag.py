...
def test_env_var_integer() -> None:
    """`APG_EVAL_ACP_SERVER=8000` env var → int 8000."""
    assert _parsed([], env={"APG_EVAL_ACP_SERVER": "8000"}) == 8000


def test_env_var_true() -> None:
    """`APG_EVAL_ACP_SERVER=true` env var → True."""
    assert _parsed([], env={"APG_EVAL_ACP_SERVER": "true"}) is True
...
def test_retry_env_var_false_returns_false() -> None:
    """Env var `APG_EVAL_ACP_SERVER=false` → False (explicit disable)."""
    assert _retry_parsed([], env={"APG_EVAL_ACP_SERVER": "false"}) is False
