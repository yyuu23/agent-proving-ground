...
from agent_proving_ground._cli.util import ctl_server_flag_callback
...
def test_env_var_keep() -> None:
    """`APG_EVAL_CTL_SERVER=keep` env var → "keep"."""
    assert _parsed([], env={"APG_EVAL_CTL_SERVER": "keep"}) == "keep"


def test_env_var_false() -> None:
    """`APG_EVAL_CTL_SERVER=false` env var → False (CI-wide suppression)."""
    assert _parsed([], env={"APG_EVAL_CTL_SERVER": "false"}) is False
