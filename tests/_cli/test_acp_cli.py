"""CLI smoke tests for ``apg acp`` (Phase 13).
...
from agent_proving_ground._cli.acp import acp_command
...
def mock_stdio_streams(monkeypatch):
    ...
    monkeypatch.setattr("agent_proving_ground._cli.acp.stdio_streams", _mock)
...
def short_data_dir(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent_proving_ground._util.process.pid_alive",
        lambda pid: pid > 0,
    )
    try:
        yield dirpath
...
def test_bare_command_invokes_tui_runner(short_data_dir: Path, monkeypatch) -> None:
    """``apg acp`` (no flag) dispatches to ``_tui.run_tui``.
    ...
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.tui.run_tui",
        _fake_run_tui,
    )
...
def test_tui_partial_triple_accepted(short_data_dir: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agent_proving_ground.agent._acp.tui.run_tui", _fake_run_tui)
    runner = CliRunner()
