...
from agent_proving_ground.agent import deepagent, subagent
from agent_proving_ground.agent._deepagent.agent_tool import (
    AgentFuture,
    BackgroundRegistry,
...
from agent_proving_ground.agent._deepagent.deepagent import (
...
from agent_proving_ground.util import message_limit
...
def _build_submit_subagent(name: str, answer: str):
    ...
    from agent_proving_ground.agent._deepagent.subagent import subagent as subagent_factory
...
class TestAgentToolSchema:
    def test_background_enabled_includes_param(self) -> None:
        from agent_proving_ground.agent._deepagent.agent_tool import agent_tool
        from agent_proving_ground.tool._tool_info import parse_tool_info
...
class TestDispatchBackground:
    ...
    async def test_cap_check_blocks_at_max(self) -> None:
        ...
        from agent_proving_ground.agent._deepagent.agent_tool import _dispatch_background
        from agent_proving_ground.agent._deepagent.subagent import subagent as subagent_factory
...
class TestLifecycleToolsSurfaced:
    ...
    def _tool_names(self, da_kwargs: dict) -> set[str]:
        from agent_proving_ground._util.registry import is_registry_object, registry_info
...
class TestFormatStatusUnit:
    ...
    async def test_running_block_truncates_last_message(self) -> None:
        ...
        from agent_proving_ground.agent._deepagent.lifecycle_tools import _format_future_status
...
class TestLifecycleViewers:
    ...
    def test_status_viewer_title(self) -> None:
        from agent_proving_ground.agent._deepagent.lifecycle_tools import _status_viewer
    ...
    def test_wait_viewer_title(self) -> None:
        from agent_proving_ground.agent._deepagent.lifecycle_tools import _wait_viewer
        from agent_proving_ground.tool._tool_call import ToolCall
    ...
    def test_list_viewer_title_filtered(self) -> None:
        from agent_proving_ground.agent._deepagent.lifecycle_tools import _list_viewer
        from agent_proving_ground.tool._tool_call import ToolCall
...
class TestUsedBackgroundTool:
    ...
    def test_lifecycle_tool_detected(self) -> None:
        from agent_proving_ground.agent._deepagent.lifecycle_tools import _used_background_tool
...
class TestBackgroundReminderMessage:
    ...
    async def test_running_is_passive_no_collect_line(self) -> None:
        from agent_proving_ground.agent._deepagent.lifecycle_tools import (
...
class TestNoteSeenTerminal:
    ...
    def test_records_only_terminal(self) -> None:
        from agent_proving_ground.agent._deepagent.lifecycle_tools import _note_seen_terminal
...
class TestEagerCompletionOnContinue:
    ...
    async def test_pushes_on_completion_once(self) -> None:
        from agent_proving_ground.agent._deepagent.lifecycle_tools import background_on_continue
    ...
    async def test_disabled_constant_no_push(self, monkeypatch) -> None:
        from agent_proving_ground.agent._deepagent import lifecycle_tools
...
class TestNestedBackgroundDisabled:
    ...
    def test_nested_agent_tool_omits_background_param(self) -> None:
        from agent_proving_ground.agent._deepagent.agent_tool import _resolve_tools
        from agent_proving_ground.tool._tool_info import parse_tool_info
