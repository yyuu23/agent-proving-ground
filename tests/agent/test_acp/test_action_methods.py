...
from agent_proving_ground.event import SpanBeginEvent
...
def _handler(
    *,
    ...
    """
    from agent_proving_ground.agent._acp.connection import Bound
...
def patch_active_samples(monkeypatch):
    """Patch ``agent_proving_ground.log._samples.active_samples`` to a controlled list.
    ...
    def _patch(*samples: Any) -> None:
        monkeypatch.setattr(
            "agent_proving_ground.log._samples.active_samples", lambda: list(samples)
        )
...
async def test_cancel_tool_call_pending_without_cancel_fn_returns_false(
    ...
    (``src/agent_proving_ground/event/_tool.py:123``). The handler must read
...
def _register_via_picker_and_samples(monkeypatch, samples: list[Any]) -> None:
    ...
    monkeypatch.setattr(picker, "active_samples", lambda: list(samples))
    monkeypatch.setattr("agent_proving_ground.log._samples.active_samples", lambda: list(samples))
