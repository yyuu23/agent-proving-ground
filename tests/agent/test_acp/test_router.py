...
from agent_proving_ground.agent._acp.event_mapping import _AcpEventRouter, _tool_call_status
from agent_proving_ground.agent._acp.transport_live import LiveAcpTransport
from agent_proving_ground.agent._agent import AgentState
from agent_proving_ground.agent._as_tool import as_tool
from agent_proving_ground.event import (
    CompactionEvent,
    Event,
    InfoEvent,
    InterruptEvent,
...
from agent_proving_ground.event._logger import LoggingMessage
from agent_proving_ground.event._model import ModelEvent
from agent_proving_ground.log._samples import _sample_active as samples_var
from agent_proving_ground.log._transcript import Transcript, _transcript
...
from agent_proving_ground.tool._tool import Tool, tool
...
def test_router_exception_does_not_propagate_to_loop(monkeypatch) -> None:
    ...
    token = _transcript.set(tr)
    try:
        from agent_proving_ground.agent._acp import event_mapping
...
# ``content`` array carrying any markdown produced by AgentProvingGround's tool
...
def test_tool_kind_mapping_for_built_in_tools() -> None:
    """Built-in AgentProvingGround tool names map to the appropriate ACP ToolKind.
    ...
    editor to execute commands locally, incompatible with AgentProvingGround's
    sandbox model. See the module-level comment on
    ``_TOOL_KIND_BY_NAME``.
    """
    from agent_proving_ground.agent._acp.tool_content import _tool_kind_for
...
def test_tool_call_start_forwards_view_content_as_markdown() -> None:
    ...
    AgentProvingGround viewer already produced (e.g. ```\bash\nls -la\n```).
    """
    from acp.schema import ContentToolCallContent, TextContentBlock

    from agent_proving_ground.tool._tool_call import ToolCallContent
...
def test_descriptive_title_per_known_tool() -> None:
    ...
    """
    from agent_proving_ground.agent._acp.tool_content import _descriptive_title
...
def test_shell_tool_result_fence_survives_truncation() -> None:
    ...
    """
    from agent_proving_ground.agent._acp.tool_content import _RESULT_CONTENT_MAX_BYTES
...
def test_replay_completed_tool_carries_result_in_first_sight_start() -> None:
    ...
    """
    from agent_proving_ground.agent._acp.event_mapping import replay_transcript
...
def test_is_shell_execution_tool_membership() -> None:
    ...
    from agent_proving_ground.agent._acp.tool_content import _is_shell_execution_tool
...
def test_operator_cancel_keeps_cancel_marker_through_tool_error() -> None:
    ...
    so renderers (TUI + apg view) and ACP clients surface the row
...
def test_operator_cancel_keeps_cancel_marker_through_model_natural_completion() -> None:
    ...
    """
    from agent_proving_ground.event._model import OPERATOR_CANCEL_ERROR
...
async def test_sub_agent_notifications_filtered_by_default() -> None:
    ...
    sample_tok = samples_var.set(sample)
    try:
        from agent_proving_ground.agent._agent import agent as agent_decorator
    ...
    try:
        ...
        def inner() -> Any:
            async def execute(state: AgentState) -> AgentState:
                ...
                from agent_proving_ground.log._transcript import transcript as tr_fn
