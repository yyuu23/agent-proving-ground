...
from agent_proving_ground.agent._acp import picker
from agent_proving_ground.agent._acp.connection import (
    Bound,
    ConnectionHandler,
)
from agent_proving_ground.agent._acp.inspect_ext import INTERACTIVE_META_KEY, PICKER_META_KEY
from agent_proving_ground.agent._acp.picker import PickerTarget, SampleListing
from agent_proving_ground.agent._acp.server import acp_server
from agent_proving_ground.agent._acp.session_router import Forwarders
...
def short_data_dir(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.discovery.inspect_data_dir",
        _stub_data_dir,
    )
    try:
        yield dirpath
...
async def test_initialize_returns_capabilities(short_data_dir: Path) -> None:
    ...
    async with acp_server(eval_id="evt-init", transport=True) as server:
        ...
        try:
            ...
            assert result["agentInfo"]["name"] == "agent-proving-ground"
        finally:
            await client.close()
...
async def test_load_session_stops_old_forwarders_before_changing_binding(
    ...
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.connection.list_picker_targets",
...
async def test_load_session_observe_only_records_non_interactive(
    ...
    handler.connection = MagicMock()
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.connection.list_picker_targets", lambda: []
    )
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.connection.list_all_samples",
        lambda: [
            SampleListing(
                session_id="obs-uuid",
                task="t",
...
async def test_forwarders_uses_immutable_wire_session_id(
    ...
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.session_router.ReplayTranscriptor",
        _CapturingTranscriptor,
    )
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.session_router._filter_subagent_events",
...
async def test_run_replay_interleaves_raw_and_semantic_in_transcript_order() -> None:
    ...
    agent_msg].
    """
    from agent_proving_ground.agent._acp.inspect_ext import (
        APG_EVENT_METHOD,
        RAW_EVENTS_GLOB,
        RawEventForwarder,
    )
...
async def test_picker_selection_holds_bind_lock_across_setup(
    ...
    from agent_proving_ground.agent._acp.connection import PickerMode
...
async def test_picker_redisplay_bails_when_concurrent_bind_landed(
    ...
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.connection.resolve_selection",
        _racing_resolve,
    )
...
# ---------------------------------------------------------------------------
# inspect/list_sessions — direct enumeration for AgentProvingGround-aware clients
...
# consumed by the AgentProvingGround TUI to surface samples whose agent hasn't
...
# skipping the picker. AgentProvingGround-aware clients (the TUI, editors
