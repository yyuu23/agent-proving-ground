...
from agent_proving_ground.agent._acp.inspect_ext import REPLAY_META_KEY
from agent_proving_ground.agent._acp.server import acp_server
from agent_proving_ground.agent._acp.session_router import REPLAY_MAX_EVENTS
...
def register_target(monkeypatch):
    ...
    `agent_proving_ground.log._samples` at call time, so we patch both to keep
    them consistent.
    """
    ...
    def _register(*targets: Any) -> None:
        ...
        monkeypatch.setattr(
            "agent_proving_ground.log._samples.active_samples",
            lambda: list(targets),
        )
...
async def test_session_prompt_marks_connection_as_active_driver(
    ...
    async with acp_server(eval_id="evt-mark-active", transport=True) as server:
        ...
        try:
            ...
            from agent_proving_ground.agent._acp.connection import ConnectionHandler
...
async def test_server_stop_with_open_client_logs_no_handler_shutdown_errors(
    ...
    """
    from agent_proving_ground.agent._acp import server as server_module
...
async def test_start_forwarders_returns_early_when_session_exits_during_title_send(
    ...
    # exited mid-await.
    from agent_proving_ground.agent._acp import connection as connection_mod
