...
The shim is at ``src/agent_proving_ground/approval/_human/acp.py``; the
...
from agent_proving_ground.approval._approval import ApprovalDecision
from agent_proving_ground.approval._human.acp import (
...
from agent_proving_ground.tool._tool_call import ToolCall, ToolCallContent, ToolCallView
...
def test_options_from_choices_includes_escalate_and_modify_as_best_effort() -> None:
    """Unmappable AgentProvingGround choices still round-trip via optionId.
...
async def test_all_clients_raise_then_waits_for_attach() -> None:
    ...
    # Simulate a fresh ``apg acp`` attaching: swap the chain and
...
def _bound_handler(wire: str, target: str):
    ...
    from agent_proving_ground.agent._acp.connection import Bound, ConnectionHandler
...
async def test_request_permission_raises_when_not_bound() -> None:
    ...
    from agent_proving_ground.agent._acp.connection import ConnectionHandler, Unbound
...
def acp_server_running(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "agent_proving_ground.agent._acp.server.acp_server_accepting_clients",
        lambda: True,
    )
...
async def test_entry_returns_none_when_no_active_sample(
    ...
    monkeypatch.setattr("agent_proving_ground.log._samples.sample_active", lambda: None)
    result = await request_human_approval_via_acp(
        message="please confirm",
        call=_make_call(),
        view=_make_view(),
...
async def test_entry_parks_when_live_with_no_clients(
    ...
    could open ``apg acp``. Under exclusive routing the entry
...
async def test_entry_marks_pending_interaction_during_park(
    ...
    session._attachable_override = True
    sample.acp_transport = session
    monkeypatch.setattr("agent_proving_ground.log._samples.sample_active", lambda: sample)
...
async def test_approval_over_real_socket_round_trip(
    ...
    sample.sample.id = "s"
    sample.epoch = 0
    monkeypatch.setattr("agent_proving_ground.log._samples.active_samples", lambda: [sample])
    monkeypatch.setattr("agent_proving_ground.agent._acp.picker.active_samples", lambda: [sample])
