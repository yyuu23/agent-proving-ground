"""End-to-end smoke test for a generic ACP client (no AgentProvingGround extensions).
...
``_meta`` namespace, or AgentProvingGround's plan-rendering / raw-events opt-ins.

The AgentProvingGround TUI uses ``inspect/list_sessions`` for multi-eval
...
this test should fail before any AgentProvingGround-aware client notices.
"""
        ...
        for k, v in payload.items():
            if isinstance(k, str) and (
                ...
                if k in {
                    ...
                    # instead; the flag just lets AgentProvingGround-aware clients
...
async def test_generic_acp_client_full_picker_flow(
    ...
    """A non-AgentProvingGround ACP client can drive the server end-to-end.
...
async def test_generic_client_sees_clean_error_when_bound_session_ends(
    ...
    The server pushes ``inspect/session_ended`` to AgentProvingGround-aware
    clients when the bound react loop exits mid-eval. Non-AgentProvingGround
    ...
    async with acp_server(eval_id="evt-std-ended", transport=True) as server:
        ...
        try:
            ...
            # inspect/session_ended notification on an AgentProvingGround-aware
