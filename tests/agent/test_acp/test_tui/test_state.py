...
def test_consecutive_empty_pending_signals_collapse_as_retry() -> None:
    ...
    assert len(state.items) == 1
    group = state.items[0]
    from agent_proving_ground.agent._acp.tui.state import MessageGroup
...
def test_turn_cap_drops_oldest_assistant_groups_past_limit() -> None:
    ...
    """
    from agent_proving_ground.agent._acp.tui.state import _MAX_ASSISTANT_TURNS
...
def test_mark_complete_drops_empty_pending_message_group() -> None:
    ...
    during generation).
    """
    from agent_proving_ground.agent._acp.inspect_ext import MODEL_EVENT_PENDING_META_KEY
