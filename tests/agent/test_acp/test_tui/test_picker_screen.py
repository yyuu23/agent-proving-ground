...
from agent_proving_ground.agent._acp.tui.app import ApgAcpApp
from agent_proving_ground.agent._acp.tui.client import SessionRow
from agent_proving_ground.agent._acp.tui.picker_screen import (
    PickerScreen,
    _display_task,
    _row_matches,
    _sort_rows,
...
async def test_picker_empty_state_no_server(
    ...
    app = ApgAcpApp(eval_id=None, server=None, client=client)
    async with app.run_test() as pilot:
        ...
        assert "No running AgentProvingGround evals" in heading_text
        assert "apg eval <task> --acp-server" in hint_md
...
async def test_picker_empty_state_with_server() -> None:
    ...
    client = make_fake_client([])
    app = ApgAcpApp(eval_id=None, server="/tmp/explicit.sock", client=client)
    async with app.run_test() as pilot:
        await pilot.pause()
        screen = app.screen
        assert isinstance(screen, PickerScreen)
        ...
        # The bootstrap "apg eval" hint is suppressed because the
        # user has explicitly pointed at a server.
        assert "apg eval" not in hint_md
...
async def test_picker_eval_id_filter_narrows_rows(
    ...
    client = make_fake_client(sample_rows)
    app = ApgAcpApp(eval_id="eval-bbb", server=None, client=client)
    async with app.run_test() as pilot:
        await pilot.pause()
    ...
    async with app.run_test() as pilot:
        ...
        for _ in range(20):
            ...
            if app.screen is not picker:
                break
        from agent_proving_ground.agent._acp.tui.session_screen import SessionScreen
...
def test_row_key_includes_task_for_non_acp_collisions() -> None:
    ...
    """
    from agent_proving_ground.agent._acp.tui.picker_screen import _row_key
...
def test_format_tokens_zero_renders_dash() -> None:
    ...
    from agent_proving_ground.agent._acp.tui.widgets._formatting import format_tokens
...
def test_running_cell_is_right_justified() -> None:
    ...
    from agent_proving_ground.agent._acp.tui.picker_screen import _running_cell
...
def test_pending_cell_approval_is_green() -> None:
    ...
    read as lime.
    """
    from agent_proving_ground.agent._acp.tui.picker_screen import _pending_cell
...
async def test_picker_empty_state_includes_intervention_link_remote() -> None:
    ...
    client = make_fake_client([])
    app = ApgAcpApp(eval_id=None, server="/tmp/x.sock", client=client)
    async with app.run_test() as pilot:
        await pilot.pause()
        screen = app.screen
        assert isinstance(screen, PickerScreen)
...
async def test_app_title_local_when_no_server_flag() -> None:
    """``apg acp`` (no --server) titles the app ``apg acp · local``."""
    client = make_fake_client([])
    app = ApgAcpApp(eval_id=None, server=None, client=client)
    async with app.run_test() as pilot:
        await pilot.pause()
        assert app.title == "apg acp · local"
...
async def test_app_title_includes_server_address() -> None:
    ...
    client = make_fake_client([])
    app = ApgAcpApp(eval_id=None, server="127.0.0.1:4545", client=client)
    async with app.run_test() as pilot:
        await pilot.pause()
        assert app.title == "apg acp · 127.0.0.1:4545"
...
async def test_picker_header_shows_local_target() -> None:
    """In-TUI picker header reads ``apg acp · local`` when no --server."""
    from agent_proving_ground.agent._acp.tui.widgets import AppHeaderWidget
