...
from agent_proving_ground.agent._acp.tui.client import (
    CLIENT_CAPABILITIES,
    attach_session,
    enumerate_sessions,
)
from agent_proving_ground.agent._acp.tui.state import SessionState
...
def short_data_dir(monkeypatch):
    ...
    monkeypatch.setattr("agent_proving_ground.agent._acp.discovery.inspect_data_dir", _stub)
    try:
        yield dirpath
    finally:
...
compaction / info drive the AgentProvingGround-native event chips; span_begin /
...
async def test_enumerate_session_initialize_sets_plan_rendering(
    ...
    import agent_proving_ground.agent._acp.connection as connection_mod
    from agent_proving_ground.agent._acp.inspect_ext import detect_capabilities
...
async def test_permission_handler_returns_allowed_outcome_for_chosen_option() -> None:
    ...
    from agent_proving_ground.agent._acp.tui.client import _make_permission_handler
