...
def trace_action(
    ...
    `HTTP` and `INFO`). List and read trace logs with `apg trace list`
    and related commands (see `apg trace --help` for details).
...
def trace_message(
    ...
    read trace logs with `apg trace list` and related commands
    (see `apg trace --help` for details).
