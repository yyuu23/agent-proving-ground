...
def init_dotenv() -> None:
    ...
    if dotenv_file:
        # is there an APG_LOG_DIR currently in the environment? (we will give it preference)
        environment_log_dir = os.environ.get(APG_LOG_DIR_VAR, None)
        dotenv_log_dir = dotenv_values(dotenv_file).get(APG_LOG_DIR_VAR, None)
        ...
        if inspect_log_dir:
            os.environ[APG_LOG_DIR_VAR] = inspect_log_dir
