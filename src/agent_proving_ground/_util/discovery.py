...
AgentProvingGround's ACP server and control-channel server each advertise
...
from agent_proving_ground._util import process as _process
...
def _resolve_pid_alive(pid_alive_fn: PidAliveFn | None) -> PidAliveFn:
    ...
    patches ``agent_proving_ground._util.process.pid_alive`` actually affects the
...
def write_discovery_file(dir_path: Path, pid: int, payload: dict[str, Any]) -> Path:
    ...
    - **Atomic publish.** Written to a same-PID temp in ``dir_path`` (so the
      ...
      path, so a concurrent enumerator (``apg ctl task list``) sees the complete
...
def list_alive_discovery_entries(
    ...
    :func:`agent_proving_ground._util.process.pid_alive`.
    """
