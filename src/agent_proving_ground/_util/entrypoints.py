from importlib.metadata import entry_points
from logging import getLogger

logger = getLogger(__name__)


def ensure_entry_points(package: str | None = None) -> None:
    # have we already loaded all entry points?

    global _agent_proving_ground_eps_loaded_all
    if _agent_proving_ground_eps_loaded_all:
        return None

    # have we already loaded entry points for this package?
    if package in _agent_proving_ground_eps_loaded:
        return None

    # enumerate entry points
    eps = entry_points(group="agent_proving_ground")
    for ep in eps:
        try:
            # if there is a package filter then condition on that
            if package is not None:
                if ep.name and (ep.name == package):
                    ep.load()
                    _agent_proving_ground_eps_loaded.append(package)

            # if there is no package filter then load unconditionally
            # and mark us as fully loaded
            else:
                ep.load()
                _agent_proving_ground_eps_loaded_all = True

        except Exception as ex:
            logger.warning(
                f"Unexpected exception loading entrypoints from '{ep.value}': {ex}"
            )
    # if we didn't find any entry points, mark us as fully loaded
    if package is not None and package not in _agent_proving_ground_eps_loaded:
        _agent_proving_ground_eps_loaded.append(package)
    elif len(eps) == 0:
        _agent_proving_ground_eps_loaded_all = True


def clear_entry_points_state() -> None:
    global _agent_proving_ground_eps_loaded_all
    _agent_proving_ground_eps_loaded_all = False
    _agent_proving_ground_eps_loaded.clear()


# inspect extension entry points
_agent_proving_ground_eps_loaded: list[str] = []
_agent_proving_ground_eps_loaded_all: bool = False
