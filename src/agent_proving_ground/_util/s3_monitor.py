...
Enabled by setting ``APG_S3_MONITOR=1`` (or ``true``/``yes``). When
...
_ENABLED_ENV = "APG_S3_MONITOR"


def s3_monitor_enabled() -> bool:
    """True if the APG_S3_MONITOR env var is set to a truthy value."""
...
async def monitor_s3_traffic(
    *, interval: float = 1.0, label: str = ""
    ...
    """No-op unless APG_S3_MONITOR is set.
