...
def execute_vscode_commands(commands: VSCodeCommand | list[VSCodeCommand]) -> None:
    ...
    if command_dir is None:
        raise NotImplementedError(
            "Not running in VS Code session or have older version of AgentProvingGround extension"
        )
...
def vscode_workspace_id() -> str | None:
    return os.environ.get("APG_WORKSPACE_ID", None)


def vscode_extension_version() -> Version | None:
    version = os.environ.get("APG_VSCODE_EXT_VERSION", None)
    if version is not None:
        try:
            return Version.parse(version)
        except Exception:
            logger.warning(f"Invalid AgentProvingGround vscode extension version: {version}")
            return None
    else:
        return None
