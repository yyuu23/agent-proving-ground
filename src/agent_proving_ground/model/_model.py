...
if TYPE_CHECKING:
    ...
    from agent_proving_ground.tool import ToolInfo
...
from agent_proving_ground._util.error import PrerequisiteError, exception_message
from agent_proving_ground._util.http import status_code_of
...
from agent_proving_ground._util.retry import report_http_retry
from agent_proving_ground._util.rich import format_traceback
from agent_proving_ground._util.trace import trace_action
from agent_proving_ground._util.working import report_sample_waiting_time, sample_working_time
from agent_proving_ground.model._generate_overrides import generate_config_override
from agent_proving_ground.model._retry import model_retry_config
from agent_proving_ground.tool import Tool, ToolChoice, ToolFunction, ToolInfo
from agent_proving_ground.tool._mcp._remote import is_mcp_server_tool
from agent_proving_ground.tool._tool import ToolSource
from agent_proving_ground.tool._tool_call import ToolCallModelInputHints
from agent_proving_ground.tool._tool_def import ToolDef
from agent_proving_ground.util import concurrency
...
class ModelAPI(abc.ABC):
    ...
    def _apply_api_key_overrides(self) -> None:
        from agent_proving_ground.hooks._hooks import has_api_key_override, override_api_key
    ...
    def model_family(self) -> str:
        ...
        identifier.
        """
        from agent_proving_ground.model._model_info import _get_model_info_direct
...
async def ensure_model_controller(model: "Model", config: GenerateConfig) -> None:
    ...
    """
    from agent_proving_ground.model._providers.none import NoModel
...
class Model:
    ...
    async def count_tokens(
        ...
        from agent_proving_ground.log._samples import cleared_retry_wait
        from agent_proving_ground.util._concurrency import (
    ...
    async def _generate(
        ...
        from agent_proving_ground.log._refusal import report_refusal
        ...
        async def generate() -> tuple[ModelOutput, BaseModel]:
            ...
            if cache_policy:
                ...
                if isinstance(existing, ModelOutput):
                    ...
                    from agent_proving_ground.util._concurrency import _request_was_cache_hit
            ...
            # (resolved per attempt so a live `apg ctl config` override
            ...
            with trace_action(logger, "Model", f"generate ({str(self)})"):
                ...
                try:
                    ...
                    from agent_proving_ground.agent._channel import null_execution_observer
        ...
        from agent_proving_ground.log._samples import set_active_sample_total_turns
    ...
    def should_retry(self, ex: BaseException) -> bool:
        if isinstance(ex, Exception):
            ...
            # See https://github.com/meridianlabs-ai/agent_proving_ground/issues/177
            ...
            from agent_proving_ground.hooks._hooks import has_api_key_override
    ...
    def verify_model_apis(self) -> None:
        if (
            os.getenv("APG_DISABLE_MODEL_API", None) is not None
            and ModelName(self).api != "mockllm"
        ):
            raise RuntimeError("Model APIs disabled by APG_DISABLE_MODEL_API")
    ...
    def _record_model_interaction(
        ...
        def complete(
            ...
            # renderers (TUI + apg view) discriminate on it to
            # suppress display.
    ...
    **model_args: Any,
    ...
    Args:
       model: Model specification.
          ...
          then the model referred to by `APG_EVAL_MODEL`).
    ...
    """
    from agent_proving_ground.hooks._startup import init_hooks
    ...
    if model is None:
        ...
        model = os.getenv("APG_EVAL_MODEL", None)
...
def resolve_models(
    ...
    # resolve NotGiven to current APG_EVAL_MODEL
...
async def log_model_retry(model_name: str, retry_state: RetryCallState) -> None:
    from agent_proving_ground._util.retry import (
        retry_error_summary,
    ...
    from agent_proving_ground.hooks._hooks import emit_model_retry
...
def set_total_messages(input: str | list[ChatMessage]) -> None:
    from agent_proving_ground.log._samples import set_active_sample_total_messages
...
def record_sample_model_fallback(output: ModelOutput) -> None:
    ...
    from agent_proving_ground.log._samples import set_active_sample_fallback_models
