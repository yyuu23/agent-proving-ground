...
def model_retry_config(
    ...
    # `apg ctl config` max_retries/timeout overrides (see the stop
    ...
    async def on_before_sleep(rs: RetryCallState) -> None:
        ...
        if report_retry_wait:
            from agent_proving_ground.log._samples import report_active_sample_retry_wait
    ...
    # The stop condition reads the live `apg ctl config` overrides on
...
def batch_admin_retry_config(
    ...
    Encodes the batcher opt-out from the live `apg ctl config` retry
