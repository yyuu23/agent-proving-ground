...
from agent_proving_ground._util.constants import DEFAULT_MAX_TOKENS
...
from agent_proving_ground.model._openai import chat_choices_from_openai, model_output_from_openai
from agent_proving_ground.tool import ToolChoice, ToolInfo
from agent_proving_ground.tool._tool_choice import ToolFunction
from agent_proving_ground.util._json import JSON_SCHEMA_EXTENDED_FIELDS, json_schema_dump
...
# (total_max_attempts=1), retrying these is left to AgentProvingGround's own retry loop.
...
class SagemakerAPI(ModelAPI):
    ...
    async def _generate_completion(
        ...
        # Parse and store logprobs in the proper AgentProvingGround format
        if logprobs_data is not None:
            from agent_proving_ground.model._model_output import Logprob, Logprobs, TopLogprob
        ...
        if prompt_logprobs_data is not None:
            from agent_proving_ground.model._openai import parse_vllm_prompt_logprobs_raw
