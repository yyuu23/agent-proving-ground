...
from agent_proving_ground._util.images import file_as_data
from agent_proving_ground._util.kvstore import inspect_kvstore
...
from agent_proving_ground.model._generate_config import has_image_output, normalized_batch_config
...
from agent_proving_ground.model._providers._google_batch import GoogleBatcher, batch_request_dict
...
from agent_proving_ground.model._providers._google_computer_use import (
...
class GoogleGenAIAPI(ModelAPI):
    ...
    async def _stream_generate_content(
        ...
        to handle the conversion to AgentProvingGround format.
...
async def content_part(client: Client, content: ApgContent | str) -> Part:
...
async def extract_system_message_as_parts(
    ...
    for message in messages:
        if message.role == "system":
            ...
            elif isinstance(content, list):  # list[ApgContent]
                system_parts.extend(
...
def server_tool_use_from_tool_call(
    ...
    """Convert a Gemini server-side tool call into AgentProvingGround content."""
