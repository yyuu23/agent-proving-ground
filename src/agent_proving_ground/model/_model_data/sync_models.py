...
agent_proving_ground imports): the sync workflow runs it without installing the package
(see .github/workflows/sync_model_data.yml).

Usage:
    python -m agent_proving_ground.model._model_data.sync_models [--source SOURCE]
...
def _normalize_for_lookup(name: str) -> str:
    ...
    Mirror of agent_proving_ground.model._model_info._normalize_for_lookup, duplicated
    because this script must run with no agent_proving_ground install (see the module
...
#   {source.api_key_env}=your_key python -m agent_proving_ground.model._model_data.sync_models --source {source.name}
