...
``max_retries`` — the ``apg ctl config`` retune surface for riding out
...
``agent_proving_ground._eval.evalset``) precisely because they don't affect model
...
:func:`agent_proving_ground.model._retry.model_retry_config` reads them on every
