...
from agent_proving_ground._util.rich import rich_traceback
from agent_proving_ground.log import EvalStats
from agent_proving_ground.log._log import EvalScore
...
# Extra args appended to the suggested `apg eval-retry` command shown
...
def task_interrupted(profile: TaskProfile, samples_completed: int) -> RenderableType:
    ...
    if samples_completed > 0:
        ...
        if task_can_retry(profile):
            ...
            retry_cmd = f"apg eval-retry {log_location}"
            if retry_extras:
