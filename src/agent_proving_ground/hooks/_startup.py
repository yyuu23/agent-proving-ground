...
from agent_proving_ground.hooks._legacy import init_legacy_hooks
...
def init_hooks() -> None:
    ...
    if len(messages) > 0:
        ...
        print(
            f"[blue][bold]agent_proving_ground v{version}[/bold][/blue]\n"
...
def _load_registry_hooks() -> list[Hooks]:
    ...
    from agent_proving_ground.hooks._hooks import get_all_hooks
...
def _verify_all_required_hooks(installed: list[Hooks]) -> None:
    ...
    Required hooks are configured via the `APG_REQUIRED_HOOKS` environment variable.
    If any required hooks are missing, a `PrerequisiteError` is raised.

    Set the `APG_REQUIRED_HOOKS` environment variable to a comma-separated list of
    required hook names e.g. `APG_REQUIRED_HOOKS=package/hooks_1,package/hooks_2`.
    """
    required_hooks_env_var = os.environ.get("APG_REQUIRED_HOOKS", "")
    ...
    if missing_names:
        raise PrerequisiteError(
            ...
            f"APG_REQUIRED_HOOKS is set to '{required_hooks_env_var}'.\n"
