...
from agent_proving_ground._util.file import absolute_file_path, dirname, exists
from agent_proving_ground._util.json import to_json_safe
from agent_proving_ground.log._log import EvalSample, EvalSpec

# Key used in `ScanSpec.metadata` to track agent_proving_ground-side scanner config
...
_APG_CONFIG_HASH_KEY = "__inspect_scan_config_hash__"
...
def verify_scout_prerequisites() -> None:
    ...
    try:
        ...
        from agent_proving_ground._util.error import pip_dependency_error

        raise pip_dependency_error("Scanner support", ["inspect-scout"])

    from agent_proving_ground._util.version import verify_required_version
...
class ScannerConfig(BaseModel):
    ...
    def from_file(cls, path: str) -> "ScannerConfig":
        ...
        Returns:
            ...
            `eval_set(scanner=...)`.
        """
        from agent_proving_ground._util.config import read_config_object, resolve_args
        from agent_proving_ground._util.error import PrerequisiteError
        from agent_proving_ground._util.file import file as open_fs_file
        from agent_proving_ground._util.file import filesystem
        from agent_proving_ground._util.path import pretty_path
...
async def scan_init(
    ...
    *,
    ...
    from agent_proving_ground._eval.task.scan_display import reset_state, set_active
    ...
    # seed agent_proving_ground-side config hash into metadata so a future
    ...
    metadata[_APG_CONFIG_HASH_KEY] = _scan_config_hash(scanner)
    spec = ScanSpec(
        scan_id=scan_id,
...
async def scan_eval_sample(
    ...
    *,
    ...
    from agent_proving_ground._eval.task.scan_display import push_results
    from agent_proving_ground._util.error import PrerequisiteError
...
async def resume_scan_previous_sample(
    ...
    *,
    ...
    `scan_eval_sample`.
    """
    from agent_proving_ground._eval.task.scan_display import mark_completed
...
def _write_snapshot_to_scan_spec(scan_dir: Any, snapshot: Any) -> None:
    ...
    from agent_proving_ground._util.file import file
...
def _verify_scanner_config_unchanged(
    *,
    ...
    prior_hash = (prior.metadata or {}).get(_APG_CONFIG_HASH_KEY)
...
async def _apply_label_updates(recorder: Any, scanner: "Scanners | None") -> None:
    ...
    Preserves the agent_proving_ground-managed config-hash key in metadata.
    ...
    new_metadata[_APG_CONFIG_HASH_KEY] = _scan_config_hash(scanner)
...
def _scan_config_hash(scanner: "Scanners | None") -> str:
    """Hash agent_proving_ground-side scanner config that isn't in `ScannerSpec`.
...
def _filter_row(eval_sample: EvalSample, eval_spec: EvalSpec | None) -> dict[str, Any]:
    ...
    extractor via agent_proving_ground's `import_record`. Two passes — eval-level
    ...
    from agent_proving_ground.analysis._dataframe.evals.columns import EvalColumn
    from agent_proving_ground.analysis._dataframe.record import import_record
    from agent_proving_ground.analysis._dataframe.samples.columns import SampleColumn
