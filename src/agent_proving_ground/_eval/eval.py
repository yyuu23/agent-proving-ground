...
from agent_proving_ground._control.eval_state import reset_run_registries
from agent_proving_ground._control.pause import dispatch_model_name, note_dispatch_models
from agent_proving_ground._control.server import (
    control_server,
    keep_alive_intent,
    request_keep_alive,
    reset_keep_alive,
...
from agent_proving_ground._util.notgiven import NOT_GIVEN, NotGiven
from agent_proving_ground.agent._acp.server import acp_server as _acp_server
from agent_proving_ground.agent._agent import Agent, is_agent
from agent_proving_ground.agent._as_solver import as_solver
from agent_proving_ground.model._model_config import model_roles_config_to_model_roles
from agent_proving_ground.model._model_data.model_data import ModelCost
from agent_proving_ground.model._model_info import set_model_cost
from agent_proving_ground.model._util import resolve_model_costs, resolve_model_roles
from agent_proving_ground.util._anyio import inner_exception
...
from agent_proving_ground._cli.util import parse_cli_args
from agent_proving_ground._display.core.active import active_display as active_task_display
from agent_proving_ground._display.core.active import display as task_display
from agent_proving_ground._eval.task.scan import Scanners, scan_context
from agent_proving_ground._util.asyncfiles import with_async_fs
...
from agent_proving_ground._util.log_context import set_run_shape
from agent_proving_ground._util.logger import warn_once
from agent_proving_ground._util.platform import platform_init
from agent_proving_ground._util.registry import registry_lookup, registry_package_name
from agent_proving_ground.approval._apply import init_tool_approval
from agent_proving_ground.approval._policy import (
    ApprovalPolicy,
    ApprovalPolicyConfig,
    approval_policies_from_config,
    config_from_approval_policies,
)
from agent_proving_ground.log import EvalConfig, EvalLog, EvalLogInfo
from agent_proving_ground.log._file import read_eval_log_async
from agent_proving_ground.log._recorders import create_recorder_for_format
from agent_proving_ground.log._recorders.buffer import cleanup_sample_buffers
from agent_proving_ground.model import (
    GenerateConfig,
    GenerateConfigArgs,
    Model,
)
from agent_proving_ground.model._model import (
    get_model,
    init_active_model,
    init_model_roles,
    resolve_models,
)
from agent_proving_ground.scorer._reducer import reducer_log_names
from agent_proving_ground.solver._chain import chain
from agent_proving_ground.solver._solver import Solver, SolverSpec
from agent_proving_ground.util import SandboxEnvironmentType
from agent_proving_ground.util._checkpoint import CheckpointConfig, normalize_checkpoint
from agent_proving_ground.util._concurrency import AdaptiveConcurrency
from agent_proving_ground.util._display import (
    DisplayType,
    display_type,
    display_type_initialized,
    init_display_type,
)
from agent_proving_ground.util._limit import TokenLimit, token_limit_fields
from agent_proving_ground.util._notify import build_apprise, init_apprise
...
def eval(
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    Args:
        ...
        model: Model(s) for evaluation. If not specified use the value of the APG_EVAL_MODEL
        ...
        ctl_server: Control-channel server for this eval process.
            ...
            clients can still query its state — exit via `apg ctl process release`
            (or `POST /release`).
        ...
        notification: Enable out-of-band notifications when a human-in-the-loop
            ...
            send via the URL(s) in the `APG_EVAL_NOTIFICATION` environment
        ...
        log_realtime: Log events in realtime (enables live viewing of samples in apg view). Defaults to True.
    ...
    if scanner is not None and eval_set_id is None:
        from agent_proving_ground._eval.task.scan import print_scan_status

        resolved_log_dir = absolute_file_path(
            log_dir if log_dir else os.environ.get("APG_LOG_DIR", "./logs")
        )
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    Args:
        ...
        log_buffer: Number of samples to buffer before writing log file.
            If not specified, an appropriate default for the format and filesystem is
            chosen (10 for most all cases, 100 for JSON logs on remote filesystems).
        log_shared: Indicate that the log directory is shared, which results in additional syncing of realtime log data for AgentProvingGround View.
...
async def _eval_async_inner(
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    from agent_proving_ground.hooks._hooks import emit_run_end, emit_run_start
    ...
    try:
        ...
        log_dir = log_dir if log_dir else os.environ.get("APG_LOG_DIR", "./logs")
        ...
        if log_model_api is None:
            log_model_api_env = os.environ.get("APG_EVAL_LOG_MODEL_API")
        ...
        # `apg ctl` CLI clients, TUIs, and agents. Bind failures are
        ...
        async with (
            ...
            # servers are still up so `apg ctl` can read state and
            ...
            if eval_set_id is None and keep_alive_intent() and _ctl_server is not None:
                ...
                rich.get_console().print(
                    ...
                    "or run `apg ctl process release` to let it exit.",
                    markup=False,
                    highlight=False,
                )
...
async def eval_retry_async(
    ...
    Args:
        ...
        log_shared: Indicate that the log directory is shared, which results in
            additional syncing of realtime log data for AgentProvingGround View.
        score: Score output (defaults to True)
        score_display: Show scoring metrics in realtime (defaults to True)
        ctl_server: Control-channel server for this eval process.
            `True` or `None` (default) binds the default AF_UNIX socket;
    ...
    for i, eval_log in enumerate(retry_eval_logs):
        if eval_log.status == "started" and eval_log.location:
            from agent_proving_ground.log._recover import (
    ...
    for eval_log in retry_eval_logs:
        ...
        if log_refusals is None:
            log_refusals_env = os.environ.get("APG_EVAL_LOG_REFUSALS")
...
def eval_init(
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    if len(model_args) == 0:
        env_model_args = os.environ.get("APG_EVAL_MODEL_ARGS", None)
        if env_model_args:
