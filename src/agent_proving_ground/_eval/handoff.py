...
Right after ``apg eval`` is launched, ``apg ctl task list``
...
The current listeners are ``apg eval --json`` and ``inspect
...
This module also hosts the launch-time ``apg ctl`` pointer (see
...
class LaunchHandoff(NamedTuple):
    ...
    pid: int
    """Process id hosting the run (the ``apg ctl process`` selector)."""
...
CTL_POINTER = "Monitor from another shell: apg ctl task list   (apg ctl --help)"
...
def set_ctl_pointer_armed(armed: bool) -> None:
    """Arm (or disarm) the launch-time ``apg ctl`` pointer for this process.
...
def display_admits_ctl_pointer() -> bool:
    ...
    driving ``apg eval`` through a shell tool — is always non-TTY,
    resolves to ``RichDisplay``, and must never be suppressed.
    """
    from agent_proving_ground._display.core.active import active_display
