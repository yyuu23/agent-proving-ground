from anyio.abc import TaskGroup

from agent_proving_ground._control.pause import reset_task_pause_gates
from agent_proving_ground._util.background import set_background_task_group
from agent_proving_ground._util.dotenv import init_dotenv
from agent_proving_ground._util.logger import init_logger
from agent_proving_ground.approval._apply import have_tool_approval, init_tool_approval
from agent_proving_ground.approval._human.manager import init_human_approval_manager
from agent_proving_ground.approval._policy import ApprovalPolicy
from agent_proving_ground.log._refusal import init_refusal_tracking
from agent_proving_ground.log._samples import init_active_samples
from agent_proving_ground.model import GenerateConfig, Model
from agent_proving_ground.model._model import (
    init_active_model,
    init_model_roles,
    init_model_usage,
    init_role_usage,
)
from agent_proving_ground.util._concurrency import init_concurrency
from agent_proving_ground.util._input.manager import init_human_question_manager
from agent_proving_ground.util._subprocess import init_max_subprocesses


def init_runtime_context(
    max_subprocesses: int | None = None,
) -> None:
    init_dotenv()
    init_concurrency()
    # task pause gates are task-id keyed like the sample-semaphore registry
    # init_concurrency just cleared, and reset at the same boundary
    reset_task_pause_gates()
    init_max_subprocesses(max_subprocesses)


def init_eval_context(
    log_level: str | None,
    log_level_transcript: str | None,
    log_refusals: bool | None,
    max_subprocesses: int | None = None,
    task_group: TaskGroup | None = None,
) -> None:
    init_runtime_context(max_subprocesses)
    init_logger(log_level, log_level_transcript)
    init_refusal_tracking(log_refusals)
    init_active_samples()
    init_human_approval_manager()
    init_human_question_manager()
    set_background_task_group(task_group)


def init_model_context(
    model: Model,
    model_roles: dict[str, Model] | None = None,
    config: GenerateConfig = GenerateConfig(),
) -> None:
    init_active_model(model, config)
    init_model_roles(model_roles or {})
    init_model_usage()
    init_role_usage()


def init_task_context(
    model: Model,
    model_roles: dict[str, Model] | None = None,
    config: GenerateConfig = GenerateConfig(),
    approval: list[ApprovalPolicy] | None = None,
) -> None:
    init_model_context(model, model_roles, config)
    if not have_tool_approval():
        init_tool_approval(approval)
