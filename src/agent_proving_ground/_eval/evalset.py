...
from agent_proving_ground._display import display as display_manager
from agent_proving_ground._display.core.panel import set_eval_set_id_display
...
from agent_proving_ground._eval.task.log import plan_to_eval_plan
from agent_proving_ground._eval.task.run import eval_plan_agent_name, resolve_plan
from agent_proving_ground._eval.task.scan import Scanners, scan_already_clean
from agent_proving_ground._util._async import run_coroutine
from agent_proving_ground._util.azure import call_with_azure_auth_fallback
from agent_proving_ground._util.error import PrerequisiteError
from agent_proving_ground._util.file import (
    FileSystem,
    basename,
    file,
    filesystem,
...
from agent_proving_ground.approval._policy import ApprovalPolicy, ApprovalPolicyConfig
...
from agent_proving_ground.scorer._reducer import reducer_log_name
from agent_proving_ground.solver._chain import chain
from agent_proving_ground.solver._solver import Solver, SolverSpec
from agent_proving_ground.util import DisplayType, SandboxEnvironmentType
from agent_proving_ground.util._checkpoint import CheckpointConfig
...
if TYPE_CHECKING:
    from agent_proving_ground._control.eval_state import DeferredStatsProvider
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    Args:
        ...
        ctl_server: Control-channel server for this eval-set process.
            ...
            clients (the `apg ctl` CLI, scripted agents, TUIs) can still
            query state and read results — exit via `apg ctl process release`
    ...
    """
    from agent_proving_ground.hooks._hooks import emit_eval_set_end, emit_eval_set_start
    ...
    from agent_proving_ground.util._concurrency import adaptive_active
    ...
    # after any keep-alive park — so they stay visible in `apg ctl task list`
...
def _register_reused_logs(success_logs: list[Log]) -> None:
    ...
    explicit registration here, ``apg ctl task list`` would show zero
    ...
    """
    from agent_proving_ground._util.dateutil import datetime_from_iso_format_safe
...
def _deferred_sample_stats(log_entry: Log, total: int) -> "DeferredStatsProvider":
    ...
    async def provider() -> DeferredSampleStats:
        from agent_proving_ground.log._file import read_eval_log_sample_summaries_async
...
async def _keep_alive_park(eval_set_id: str, log_dir: str) -> None:
    ...
    async with control_server(run_id=eval_set_id) as ctl_server:
        if ctl_server is None:
            ...
            # `apg ctl process release`), so don't linger.
            return
        ...
        rich.get_console().print(
            ...
            "`apg ctl process release` to let it exit.",
            markup=False,
            highlight=False,
        )
