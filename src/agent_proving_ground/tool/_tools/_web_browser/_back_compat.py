...
from agent_proving_ground.tool import ToolError, ToolResult
from agent_proving_ground.util import SandboxEnvironment, StoreModel, sandbox_with, store_as
from agent_proving_ground.util._sandbox.docker.internal import (
    APG_WEB_BROWSER_IMAGE_DOCKERHUB_DEPRECATED,
)
...
async def _web_browser_sandbox() -> SandboxEnvironment:
    ...
    if sb:
        ...
        msg = dedent(f"""
                The web browser service was not found in any of the sandboxes for this sample. Please add the web browser service to your configuration. For example, the following Docker compose file uses the {APG_WEB_BROWSER_IMAGE_DOCKERHUB_DEPRECATED} image as its default sandbox:

                services:
                  default:
                    image: "{APG_WEB_BROWSER_IMAGE_DOCKERHUB_DEPRECATED}"
                    init: true
