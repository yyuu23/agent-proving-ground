...
the apg_sandbox_tools package. It is typically launched by build_within_container.py
...
- Repository is mounted at /agent_proving_ground via Docker volume mount
...
3. Install apg_sandbox_tools package in container environment
...
2. Repository mounted at /agent_proving_ground, this script executed inside container
3. Source copied to /tmp/apg_sandbox_tools-copy for safe building
...
6. Final tar artifact placed at /agent_proving_ground/src/agent_proving_ground/binaries/<filename>
...
if TYPE_CHECKING:
    from agent_proving_ground.tool._sandbox_tools_utils._build_bundled_executable import (
...
ENTRY_POINT = "src/apg_sandbox_tools/src/apg_sandbox_tools/_cli/main.py"
...
def main() -> None:
    ...
    output_dir = Path("/agent_proving_ground/src/agent_proving_ground/binaries")
...
def _prepare_build_environment() -> None:
    ...
    1. Copy /agent_proving_ground/src/apg_sandbox_tools to /tmp/apg_sandbox_tools-copy
    2. Change to the copy directory
    3. Run pip install . to install the package
    """
    # Container paths (matching build_executable.py)
    repo_dir = Path("/agent_proving_ground")
    source_dir = repo_dir / "src" / "apg_sandbox_tools"
    copy_dir = Path("/tmp/apg_sandbox_tools-copy")
