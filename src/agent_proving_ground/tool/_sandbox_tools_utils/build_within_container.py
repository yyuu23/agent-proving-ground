#!/usr/bin/env python3
"""
Docker-based executable build orchestrator for apg_sandbox_tools.
...
3. Mounts repository at /agent_proving_ground and executes build_executable.py inside container
4. build_executable.py copies source, installs package, and creates executable
5. Final executables placed in src/agent_proving_ground/binaries/ for runtime injection
...
# 2. agent_proving_ground runtime - called when executables missing, package installed
...
# fallback paths.
if TYPE_CHECKING:
    from agent_proving_ground.tool._sandbox_tools_utils._build_config import (
        SandboxToolsArch,
...
def run_docker_container(
    ...
    cmd = [
        ...
        platform,
        "-v",
        f"{repo_root}:/agent_proving_ground:rw",
        "-w",
        "/agent_proving_ground",
        image_name,
        "python3",
        "./src/agent_proving_ground/tool/_sandbox_tools_utils/build_executable.py",
        filename,
    ]
...
def main() -> None:
    ...
    try:
        ...
        print("Build completed. Executable available in src/agent_proving_ground/binaries/")
