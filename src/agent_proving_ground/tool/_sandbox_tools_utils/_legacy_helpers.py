...
apg_tool_support system, which temporarily handles web browser functionality
while the main apg_sandbox_tools system handles all other tools (bash_session,
text_editor, MCP).
...
from agent_proving_ground._util._json_rpc import exec_scalar_request
from agent_proving_ground._util.error import PrerequisiteError
from agent_proving_ground.tool._sandbox_tools_utils._error_mapper import (
    SandboxToolsErrorMapper,
)
from agent_proving_ground.util._sandbox._json_rpc_transport import SandboxJSONRPCTransport
from agent_proving_ground.util._sandbox.context import sandbox_with
from agent_proving_ground.util._sandbox.environment import SandboxEnvironment

LEGACY_SANDBOX_CLI = "inspect-tool-support"
_APG_TOOL_SUPPORT_IMAGE_DOCKERHUB = "aisiuk/inspect-tool-support"
...
async def legacy_tool_support_sandbox(
    ...
    raise PrerequisiteError(
        dedent(f"""
            ...
            For example, the following Docker compose file uses the {_APG_TOOL_SUPPORT_IMAGE_DOCKERHUB} reference image as its default sandbox:

            services:
              default:
                image: "{_APG_TOOL_SUPPORT_IMAGE_DOCKERHUB}"
                init: true

            Alternatively, you can include the service into your own Dockerfile:

            ENV PATH="$PATH:/opt/apg_tool_support/bin"
            RUN python -m venv /opt/apg_tool_support && \\
                /opt/apg_tool_support/bin/pip install inspect-tool-support && \\
                /opt/apg_tool_support/bin/inspect-tool-support post-install
            """).strip()
    )
