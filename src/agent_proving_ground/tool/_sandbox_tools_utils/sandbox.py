...
from agent_proving_ground._util.package import get_package_direct_url
from agent_proving_ground._util.trace import trace_message
from agent_proving_ground.util import input_screen
from agent_proving_ground.util._concurrency import concurrency
from agent_proving_ground.util._sandbox._cli import SANDBOX_CLI, SANDBOX_TOOLS_DIR
from agent_proving_ground.util._sandbox.context import (
    SandboxInjectable,
    sandbox_file_detector,
    sandbox_with_injection,
)
from agent_proving_ground.util._sandbox.environment import SandboxEnvironment
from agent_proving_ground.util._sandbox.recon import Architecture, detect_sandbox_os
...
"""Represents the state of the agent-proving-ground installation.
...
def _uncompressed_tar_bytes(name: str, gz_bytes: bytes) -> bytes:
    ...
    """
    binaries_path = Path(agent_proving_ground.__file__).parent / "binaries"
...
async def _open_executable(executable: str) -> AsyncIterator[BinaryIO]:
    ...
    with warnings.catch_warnings():
        ...
        with resources.path("agent_proving_ground.binaries", executable) as executable_path:
...
def _prompt_user_action(
    ...
    if response != "y":
        build_cmd = (
            "python src/agent_proving_ground/tool/_sandbox_tools_utils/build_within_container.py "
        ...
        try:
            ...
            if install_state == "pypi":
                if musl:
                    ...
                    msg = f"Tool support executable {executable_name} is missing from the PyPI package installation. This indicates a problem with the package. Please reinstall agent_proving_ground."
...
_INSTALL_STATE_OVERRIDE_ENV = "APG_SANDBOX_TOOLS_INSTALL_STATE"
...
def _get_install_state() -> InstallState:
    """Detect the state of the agent-proving-ground installation."""
    if (override := _install_state_override()) is not None:
        return override

    if (direct_url := get_package_direct_url("agent-proving-ground")) is None:
        return "pypi"
...
def _check_main_divergence(url: str) -> Literal["clean", "edited"]:
    ...
    try:
        ...
        pathspecs_to_check = [
            ...
            [
                "src/apg_sandbox_tools",
                ":(exclude)src/apg_sandbox_tools/tests",
                ":(exclude)src/apg_sandbox_tools/design",
                ":(exclude,glob)src/apg_sandbox_tools/**/*.md",
            ],
        ]
