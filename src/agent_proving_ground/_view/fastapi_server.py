...
from agent_proving_ground._display.core.active import display
from agent_proving_ground._eval.evalset import EvalSet, read_eval_set_info
from agent_proving_ground._util.asyncfiles import AsyncFilesystem, bind_async_filesystem
from agent_proving_ground._util.constants import DEFAULT_SERVER_HOST, DEFAULT_VIEW_PORT
from agent_proving_ground._util.error import WriteConflictError
from agent_proving_ground._util.file import filesystem
from agent_proving_ground._util.local_server import get_machine_ip
from agent_proving_ground.log import EvalLog
from agent_proving_ground.log._edit import LogUpdate
from agent_proving_ground.log._file import read_eval_log_headers_async
from agent_proving_ground.log._recorders.buffer import sample_buffer
...
VIEW_REQUEST_HEADER = "X-AgentProvingGround-View-Request"
...
class ApgJsonResponse(JSONResponse):
...
def view_server_app(
    ...
    @app.get("/log-files", response_class=ApgJsonResponse)
    ...
    @app.get(
        "/logs", response_model=LogListingResponse, response_class=ApgJsonResponse
    )
    async def api_logs(
        request: Request,
        log_dir: str | None = Query(None, alias="log_dir"),
    ...
    @app.get(
        "/eval-set",
        response_class=ApgJsonResponse,
    ...
    @app.get(
        "/pending-samples", response_model=Samples, response_class=ApgJsonResponse
    )
...
class _ApgStaticFiles(StaticFiles):
...
def standalone_view_app(
    *,
    ...
    app.mount(
        "/",
        _ApgStaticFiles(directory=resolved_dist_dir.as_posix(), html=True),
        name="static",
    )
...
def view_server(
    ...
    # run app
    display().print(f"AgentProvingGround View: {log_dir}")
