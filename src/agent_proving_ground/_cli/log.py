...
from agent_proving_ground._cli.common import CommonOptions, common_options, process_common_options
from agent_proving_ground._cli.util import int_or_bool_flag_callback
from agent_proving_ground._util.constants import PKG_PATH
from agent_proving_ground.log import EvalStatus, list_eval_logs
from agent_proving_ground.log._convert import convert_eval_logs
...
def log_command() -> None:
    ...
    AgentProvingGround supports two log formats: 'eval' which is a compact, high performance binary format and 'json' which represents logs as JSON.

    The default format is 'eval'. You can change this by setting the APG_LOG_FORMAT environment variable or using the --log-format command line option.

    The 'log' commands enable you to read AgentProvingGround logs uniformly as JSON no matter their physical storage format, and also enable you to read only the headers (everything but the samples) from log files, which is useful for very large logs.
...
def convert_chunked_command(
    ...
    from agent_proving_ground.log._recorders.chunked import convert_eval_logs_to_chunked
...
def export_config_command(log_file: str, output: str | None, fmt: str) -> None:
    ...
    to 'apg eval --run-config' to reproduce the run.
    ...
    from agent_proving_ground.log._config import eval_log_to_run_config_dict
...
def recover_command(
    ...
    **common: Unpack[CommonOptions],
    ...
    if list_mode:
        ...
        from agent_proving_ground.log._recover import RecoveryNotAvailable, RecoveryStats
        ...
        if stats.failed_count > 0:
            ...
            print(f"  apg eval-retry {output_path}")
...
_TS_MONO_APG_COMMON = PKG_PATH / "_view" / "ts-mono" / "packages" / "inspect-common"


def view_type_resource(file: str) -> str:
    _require_submodule()
    resource = _TS_MONO_APG_COMMON / "src" / "types" / file
    with open(resource, "r", encoding="utf-8") as f:
        return f.read()
