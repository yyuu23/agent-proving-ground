"""Detached launch (``--detach``) for ``apg eval`` / ``eval-set`` / ``eval-retry``.
...
exits 0 — leaving the eval running with `apg ctl` as the live
...
``apg ctl`` immediately (see ``design/control-channel.md`` → "The
...
daemon is its reattach surface): AgentProvingGround already has a reattach surface —
...
the AgentProvingGround data dir — reported as ``output_file`` in the re-emitted
launch record.
...
park waiting for an ``apg ctl process release`` from a consumer that
...
from agent_proving_ground._util.appdirs import inspect_data_dir
...
DETACH_HELP = (
    ...
    "with `apg ctl task list` and cancel with `apg ctl task cancel`. "
    ...
    "process alive (and queryable via `apg ctl`) after the eval finishes, "
    "until `apg ctl process release`."
)
...
def exec_detached(ctl_server: bool | str | None) -> NoReturn:
    ...
    if ctl_server is False:
        raise PrerequisiteError(
            ...
            "goes through `apg ctl`): remove --ctl-server=false or drop "
            "--detach."
        )

    output_file = _allocate_output_file()
    argv = [sys.executable, "-m", "agent_proving_ground._cli.main", *_child_args(sys.argv[1:])]
    ...
    # SIGTERM (e.g. `timeout … apg eval --detach`, CI cancellation) must
...
def _child_args(args: list[str]) -> list[str]:
    ...
    enough when detach came from ``APG_EVAL_DETACH``: scrubbing the
...
def _allocate_output_file() -> Path:
    """Allocate the detached run's output file under the AgentProvingGround data dir."""
