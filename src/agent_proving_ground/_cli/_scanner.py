...
from agent_proving_ground._util.registry import registry_lookup
...
def serialize_scanner_cli_args(
    ...
    *,
    ...
    Used to append the user's scanner setup onto the `apg eval-retry`
...
def _resolve_base_scanner(
    ...
    if file_part.endswith(".py") and Path(file_part).exists():
        from agent_proving_ground._eval.task.scan import verify_scout_prerequisites
