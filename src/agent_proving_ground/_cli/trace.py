...
from agent_proving_ground._util.trace import (
    ActionTraceRecord,
    TraceRecord,
    inspect_trace_dir,
    list_trace_files,
...
def trace_command() -> None:
    ...
    AgentProvingGround includes a TRACE log-level which is right below the HTTP and INFO log levels (so not written to the console by default). However, TRACE logs are always recorded to a separate file, and the last 10 TRACE logs are preserved. The 'trace' command provides ways to list and read these traces.
...
def anomalies_options(
    ...
    Single home for the options of `apg trace anomalies` and
    `apg ctl process anomalies` so the two entry points cannot drift as
...
def anomaly_buckets_json(
    ...
    Shared by `apg trace anomalies` and `apg ctl process anomalies`
...
def trace_anomalies(traces: list[TraceRecord]) -> TraceAnomalies:
    ...
    Shared by the human and JSON renderings of `apg trace anomalies` and
    `apg ctl process anomalies` so every entry point derives the same
...
def filter_traces(traces: list[TraceRecord], filter: str | None) -> list[TraceRecord]:
    ...
    One home for the `--filter` semantics of the `apg trace` commands and
    `apg ctl process anomalies`.
    """
    if filter:
        filter = filter.lower()
