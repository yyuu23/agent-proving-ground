"""`apg ctl` — control-channel CLI subcommands.

The ``ctl`` group hosts the commands that operate on a *running* AgentProvingGround
...
- ``process`` — the running AgentProvingGround process itself: ``list`` (implied by the
...
from agent_proving_ground._cli.trace import (
    TraceAnomalies,
    anomalies_options,
...
from agent_proving_ground._control.cancel import TaskCancelAction
from agent_proving_ground._control.discovery import (
    DiscoveredControlServer,
    discovery_dir,
    list_discovered_servers,
)
from agent_proving_ground._control.state import (
    DEFAULT_SAMPLE_LIST_LIMIT,
    SAMPLE_STATUSES,
    effective_sample_limit,
    parse_status_filter,
)
from agent_proving_ground._util._async import configured_async_backend, tg_collect
from agent_proving_ground._util.name_match import match_name_prefix
from agent_proving_ground._util.process import pid_alive
from agent_proving_ground._util.trace import inspect_trace_dir, read_trace_file

if TYPE_CHECKING:
    # TYPE_CHECKING to keep the CLI import-light: `agent_proving_ground.log._samples`
    # pulls in a chunk of the core package this thin HTTP client never needs.
    from agent_proving_ground.log._samples import SampleCancelAction
...
# `CONTROL_API_VERSION` from `agent_proving_ground._control` that its inspect embedded
...
# comment in `agent_proving_ground._control`). The nonzero entries predate strict
...
class _IntOrClearType(_IntOrClearBase):
    ...
    def convert(
        ...
        from agent_proving_ground.model._generate_overrides import (
    ...
    for name, (_, opt) in given.items():
        if name not in verb_params:
            ctx.fail(
                ...
                f"instead: `apg ctl {group.name} list {opt} ...`."
            )
...
def ctl_command() -> None:
    ...
    implied by the bare noun (`apg ctl task` ≡ `apg ctl task list`).
    ...
    A process exits when its eval finishes; launch with `apg eval
    --ctl-server=keep` to keep it inspectable here until you run
    `apg ctl process release`.

    To launch an eval in the background — one that outlives your
    terminal and is driven entirely from here — use `apg eval
    --detach` (see `apg eval --help`).
    """
    return None
...
def _echo_no_running_evals() -> None:
    ...
    click.echo(
        ...
        "Start an eval with `apg eval <task>` — add `--ctl-server=keep` "
...
def _anomalies_pointer(pid: int | None = None) -> str:
    """The stall-site escalation pointer at `apg ctl process anomalies`.
    ...
    if pid is not None:
        return (
            f"`apg ctl process anomalies {pid}` shows the process's "
            "in-flight actions"
        )
    return (
        "`apg ctl process anomalies` shows each running process's in-flight actions"
    )
...
def _deprecation_note(old: str, new: str) -> None:
    ...
    click.echo(
        f"note: `apg ctl {old}` is now `apg ctl {new}` — this hidden "
...
task_group.hint = lambda token: (
    ...
    "`apg ctl task list` (or the bare `apg ctl task`)."
)
...
def task_list_command(as_json: bool) -> None:
    """List running tasks across all live AgentProvingGround processes.
    ...
    Example: apg ctl task list --json
    """
...
def task_log_flush_command(task: str | None, as_json: bool, terse: bool | None) -> None:
    ...
    Tune the buffering policy itself with `apg ctl config --log-buffer`
...
def task_cancel_command(
    ...
    single sample, use `apg ctl sample cancel`. TASK (a task-id prefix
...
def task_pause_command(
    ...
    their time limits — until `apg ctl task resume`. Non-destructive,
    ...
    dispatch), use `apg ctl process pause`. TASK (a task-id prefix or
...
def task_resume_command(
    ...
    """Resume a paused task (the inverse of `apg ctl task pause`).
    ...
    `apg ctl process pause` stays held until `apg ctl process
...
sample_group.hint = lambda token: (
    f"No such command '{token}'. `apg ctl sample` is now a command "
    f"group. To list a task's samples: `apg ctl sample list {token}`; "
    f"for one sample's detail (the old `apg ctl sample`): "
    f"`apg ctl sample show {token} SAMPLE_ID [EPOCH]`."
)
...
def sample_list_command(
    ...
    Example: apg ctl sample list my-task --json
    """
    _run_sample_list(
        task,
        active_since,
...
def sample_show_command(
    ...
    messages (and `--traceback` for tracebacks). Use `apg ctl sample
...
def sample_events_command(
    ...
    Example: apg ctl sample events my-task sample-1 --tail 20
    """
...
@click.option(
    ...
    help=(
        ...
        "log write — the retune side of `apg ctl task log-flush` (lower "
...
def config_command(
    ...
    Any `apg eval` launch flag that can be retuned while the eval runs
    ...
    `--log-shared` are the retune side of `apg ctl task log-flush`: they
    ...
    Example: apg ctl config --max-connections 20 --dry-run
    """
    _run_config(
        task,
...
def process_group(ctx: click.Context, /, **mirrored: Any) -> None:
    """Operate on running AgentProvingGround processes (bare `process` lists them).
...
process_group.hint = lambda token: (
    ...
    "`apg ctl process list` (or the bare `apg ctl process`); to park "
    f"or release one: `apg ctl process keep {token}` / "
    f"`apg ctl process release {token}`; for one's in-flight actions: "
    f"`apg ctl process anomalies {token}`."
)
...
def process_list_command(as_json: bool) -> None:
    """List running AgentProvingGround processes (pids, keep-alive, hosted tasks).
...
def process_keep_command(pid: int | None, as_json: bool) -> None:
    ...
    here — until `apg ctl process release` or Ctrl+C. The runtime
...
def process_pause_command(pid: int | None, dry_run: bool, as_json: bool) -> None:
    ...
    alive — watch `apg ctl task list` for `quiesced` (paused with
    nothing in flight), after which completed work is flushed and the
    process can be killed cleanly if needed. Resume with `apg ctl
...
def process_resume_command(pid: int | None, dry_run: bool, as_json: bool) -> None:
    ...
    `apg ctl task pause`) are deliberately left in place. Note the
...
def process_anomalies_command(
    ...
    analysis is shared with `apg trace anomalies`, which reads any trace
    file by path.
    """
...
def model_group() -> None:
    ...
    MODEL is the exact model name as `apg ctl task list` shows it
...
model_group.hint = lambda token: (
    ...
    f"`apg ctl model pause {token}` / `apg ctl model resume {token}`."
)
...
def model_pause_command(
    ...
    not-yet-started tasks — which `apg ctl task pause` cannot reach.
    ...
    exact name shown by `apg ctl task list`; an unknown name is an
    error. PID is required when several processes run. Idempotent;
    resume with `apg ctl model resume`.
    """
...
def model_resume_command(
    ...
    """Resume a paused model (the inverse of `apg ctl model pause`).
...
def tasks_alias(as_json: bool) -> None:
    """Deprecated alias for `apg ctl task list`."""
...
def samples_alias(
    ...
    """Deprecated alias for `apg ctl sample list`."""
...
def errors_alias(task: str | None, content: bool, as_json: bool) -> None:
    """Deprecated alias for `apg ctl sample errors`."""
...
def events_alias(
    ...
    """Deprecated alias for `apg ctl sample events` (--since is --cursor)."""
...
def keep_alias(pid_arg: int | None, pid: int | None, as_json: bool) -> None:
    """Deprecated alias for `apg ctl process keep`."""
...
def release_alias(pid_arg: int | None, pid: int | None, as_json: bool) -> None:
    """Deprecated alias for `apg ctl process release`."""
...
def flush_alias(task: str | None, as_json: bool, terse: bool | None) -> None:
    """Deprecated alias for `apg ctl task log-flush`."""
...
def buffer_alias(
    ...
    """Deprecated alias for `apg ctl config --log-buffer / --log-shared`."""
...
def limits_alias(
    ...
    """Deprecated alias for `apg ctl config`."""
...
def _validate_cursor(cursor: str | None) -> None:
    ...
    if cursor is None:
        return
    from agent_proving_ground._control.events import decode_cursor
...
def _still_held_note(held: list[str]) -> str:
    ...
    latches = []
    if "process" in held:
        latches.append("the process is paused (`apg ctl process resume`)")
    if "model" in held:
        latches.append("its model is paused (`apg ctl model resume`)")
...
def _terse_held_suffix(held: list[str]) -> str:
    ...
    commands = " / ".join(f"`apg ctl {latch} resume`" for latch in latches)
        ...
        if verb == "pause":
            ...
            if terse_mode:
                ...
                click.echo(
                    ...
                    "attempts will start. Resume with `apg ctl task resume`."
                )
        elif terse_mode:
        ...
        if verb == "pause":
            if dry_run:
                ...
                click.echo(
                    ...
                    "tasks will start. Watch `apg ctl task list` for "
                    "quiesced; resume with `apg ctl process resume`."
                )
        elif dry_run:
    ...
    *,
    ...
    result = _request_json(
        ...
        not_found=(
            ...
            "model name as shown by `apg ctl task list`)."
        ),
...
def _run_process_anomalies(
    ...
    # stamp-after-read is the consistent choice — matching `apg trace
    ...
    if not sections:
        if servers:
            ...
            click.echo(
                ...
                "exited process's trace file post-mortem (`apg trace "
...
def _resolve_target_server(pid: int | None) -> DiscoveredControlServer:
    ...
    _fail(
        ...
        "Pass a PID to disambiguate (see `apg ctl process`).",
    )
...
# https://github.com/meridianlabs-ai/agent_proving_ground/issues/14). A perfectly
...
def _print_keep_alive_footer(summaries: list[dict[str, Any]]) -> None:
    ...
    ``apg ctl process keep``, which turns it on for a running process.
    """
    ...
    if all(flags):
        ...
        click.echo("keep-alive: off  ·  set with `apg ctl process keep`")
    else:
        on = sum(flags)
...
def _print_errored_samples_footer(summaries: list[dict[str, Any]]) -> None:
    ...
    if errored > 0:
        ...
        click.echo(f"{errored} {noun} errored — see `apg ctl sample errors`")
