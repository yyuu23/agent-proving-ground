...
from agent_proving_ground import Epochs, eval, eval_retry
from agent_proving_ground._eval.evalset import eval_set
from agent_proving_ground._eval.handoff import (
    LaunchHandoff,
    set_ctl_pointer_armed,
    set_launch_handoff_listener,
)
from agent_proving_ground._util.config import resolve_args
...
from agent_proving_ground._util.error import PrerequisiteError, SilentException
from agent_proving_ground._util.file import filesystem
from agent_proving_ground._util.samples import parse_sample_id, parse_samples_limit
from agent_proving_ground.log._file import log_file_info
from agent_proving_ground.log._log import EvalConfig, EvalLog
from agent_proving_ground.model import GenerateConfig, GenerateConfigArgs, Model, get_model
from agent_proving_ground.model._cache import CachePolicy
from agent_proving_ground.model._generate_config import (  # noqa: F811
    BatchConfig,
    ImageOutput,
    OutputModality,
    ResponseSchema,
...
from agent_proving_ground.scorer._reducer import create_reducers
from agent_proving_ground.solver._solver import SolverSpec
from agent_proving_ground.util import AdaptiveConcurrency
from agent_proving_ground.util._checkpoint.parse_cli import parse_checkpoint
from agent_proving_ground.util._limit import TokenLimit
...
class EvalCommand(SectionedCommand):
    """`apg eval` / `apg eval-set` command with grouped help output."""
...
NO_LOG_REALTIME_HELP = (
    "Do not log events in realtime (affects live viewing of samples in apg view)"
)
...
NO_SCORE_HELP = (
    "Do not score model output (use the apg score command to score output later)"
)
...
CHECKPOINT_HELP = "Periodically checkpoint sample state so the eval can be resumed via `apg eval retry`. Specify --checkpoint for the default (every 500k tokens), --checkpoint=token:N{k,m,b} / time:N{s,m,h,d} / turn:N / manual for a shorthand trigger, or pass a YAML/JSON file path for a full CheckpointConfig."
...
def scanner_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        multiple=True,
        type=str,
        envvar="APG_EVAL_SCAN_MODEL_ROLE",
        help=(
    ...
    @click.option(
        ...
        type=str,
        envvar="APG_EVAL_SCAN_GENERATE_CONFIG",
...
def eval_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        multiple=True,
        type=str,
        envvar="APG_EVAL_SOLVER_ARGS",
        help="One or more solver arguments (e.g. -S arg=value)",
    )
    @click.option(
        "--solver-config",
        type=str,
        envvar="APG_EVAL_SOLVER_CONFIG",
    ...
    @click.option(
        ...
        # would otherwise auto-bind ``APG_EVAL_NOTIFICATION`` as
        ...
        # ``apg eval`` runs when the env var holds a URL string
        ...
        help=(
            ...
            "`APG_EVAL_NOTIFICATION` environment variable (a single "
    ...
    @click.option(
        ...
        envvar="APG_EVAL_NO_SANDBOX_CLEANUP",
    )
    @click.option(
        "--checkpoint",
        is_flag=False,
    ...
    @click.option(
        ...
        help=(
            ...
            "clients (e.g. the `apg acp` command). Bare flag enables a "
    ...
    @click.option(
        ...
        help=(
            ...
            "on an AF_UNIX socket — the endpoint the `apg ctl` CLI, "
            ...
            "the process exits when `apg ctl process release` is run (or POST "
    ...
    @click.option(
        ...
        envvar="APG_EVAL_EPOCHS_REDUCER",
    )
    @click.option(
        "--no-epochs-reducer",
        type=bool,
        ...
        envvar="APG_EVAL_NO_EPOCHS_REDUCER",
    )
    @click.option(
        "--max-connections",
        type=int,
        help=MAX_CONNECTIONS_HELP,
        envvar="APG_EVAL_MAX_CONNECTIONS",
    )
    @click.option(
        ...
        envvar="APG_EVAL_ADAPTIVE_CONNECTIONS",
    )
    @click.option(
        "--max-retries",
        type=int,
        help=MAX_RETRIES_HELP,
        envvar="APG_EVAL_MAX_RETRIES",
    )
    @click.option(
        "--timeout", type=int, help=TIMEOUT_HELP, envvar="APG_EVAL_TIMEOUT"
    )
    @click.option(
        "--attempt-timeout",
        type=int,
        help=ATTEMPT_TIMEOUT_HELP,
        envvar="APG_EVAL_ATTEMPT_TIMEOUT",
    )
    @click.option(
        "--max-samples",
        type=int,
        help=MAX_SAMPLES_HELP,
        envvar="APG_EVAL_MAX_SAMPLES",
    )
    @click.option(
        "--max-dataset-memory",
        type=click.IntRange(min=0),
        help="Maximum MB of dataset sample data to hold in memory per task. When exceeded, samples are paged to disk.",
        envvar="APG_EVAL_MAX_DATASET_MEMORY",
    )
    @click.option(
        "--max-tasks", type=int, help=MAX_TASKS_HELP, envvar="APG_EVAL_MAX_TASKS"
    )
    @click.option(
        "--max-subprocesses",
        type=int,
        help=MAX_SUBPROCESSES_HELP,
        envvar="APG_EVAL_MAX_SUBPROCESSES",
    )
    @click.option(
        "--max-sandboxes",
        type=int,
        help=MAX_SANDBOXES_HELP,
        envvar="APG_EVAL_MAX_SANDBOXES",
    )
    @click.option(
        "--message-limit",
        type=int,
        help="Limit on total messages used for each sample.",
        envvar="APG_EVAL_MESSAGE_LIMIT",
    )
    @click.option(
        "--token-limit",
        type=str,
    ...
    @click.option(
        ...
        envvar="APG_EVAL_NO_FAIL_ON_ERROR",
    )
    @click.option(
        "--continue-on-fail",
        type=bool,
        ...
        envvar="APG_EVAL_CONTINUE_ON_FAIL",
    )
    @click.option(
        "--retry-on-error",
        is_flag=False,
        ...
        envvar="APG_EVAL_RETRY_ON_ERROR",
    )
    @click.option(
        "--score-on-error",
        type=bool,
        ...
        envvar="APG_EVAL_SCORE_ON_ERROR",
    )
    @click.option(
        "--no-log-samples",
        type=bool,
        ...
        envvar="APG_EVAL_NO_LOG_SAMPLES",
    )
    @click.option(
        "--no-log-realtime",
        type=bool,
        ...
        envvar="APG_EVAL_NO_LOG_REALTIME",
    )
    @click.option(
    ...
    @click.option(
        ...
        envvar="APG_EVAL_LOG_REFUSALS",
    )
    @click.option(
        "--log-buffer", type=int, help=LOG_BUFFER_HELP, envvar="APG_EVAL_LOG_BUFFER"
    )
    @click.option(
        "--log-shared",
        is_flag=False,
        ...
        envvar=["APG_LOG_SHARED", "APG_EVAL_LOG_SHARED"],
    )
    @click.option(
        "--no-score",
        type=bool,
    ...
    @click.option(
        ...
        envvar="APG_EVAL_SCORE_DISPLAY",
    )
    @click.option(
        "--generate-config",
        type=str,
        envvar="APG_EVAL_GENERATE_CONFIG",
    ...
    @click.option(
        ...
        envvar="APG_EVAL_MAX_TOKENS",
    )
    @click.option(
        "--system-message",
        type=str,
        help="Override the default system message.",
        envvar="APG_EVAL_SYSTEM_MESSAGE",
    )
    @click.option(
        "--best-of",
        type=int,
    ...
    @click.option(
        ...
        envvar="APG_EVAL_FREQUENCY_PENALTY",
    )
    @click.option(
        "--presence-penalty",
        type=float,
    ...
    @click.option(
        ...
        envvar="APG_EVAL_PARALLEL_TOOL_CALLS",
    )
    @click.option(
        ...
        envvar="APG_EVAL_INTERNAL_TOOLS",
    )
    @click.option(
        "--max-tool-output",
        type=int,
        help="Maximum size of tool output (in bytes). Defaults to 16 * 1024.",
        envvar="APG_EVAL_MAX_TOOL_OUTPUT",
    )
    @click.option(
        "--cache-prompt",
    ...
    @click.option(
        ...
        envvar="APG_EVAL_FALLBACK_MODELS",
    )
    @click.option(
        "--verbosity",
    ...
    @click.option(
        ...
        envvar="APG_EVAL_REASONING_EFFORT",
    )
    @click.option(
        "--reasoning-mode",
    ...
    @click.option(
        ...
        envvar="APG_EVAL_REASONING_SUMMARY",
    )
    @click.option(
        "--reasoning-history",
    ...
    @click.option(
        ...
        envvar="APG_EVAL_MODALITIES",
    )
    @click.option(
        "--log-format",
        type=click.Choice(["eval", "json"], case_sensitive=False),
        envvar=["APG_LOG_FORMAT", "APG_EVAL_LOG_FORMAT"],
    ...
    @click.option(
        ...
        envvar="APG_LOG_LEVEL_TRANSCRIPT",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    envvar="APG_EVAL_JSON",
    help="Emit machine-readable launch output as JSON lines on stdout (implies --display none): a 'launch' record printed once the control-channel server is bound — reporting run_id, pid, log_dir, and the control socket path ('control' is null when the server is disabled or failed to bind, so its presence guarantees `apg ctl` is usable) — and a 'done' record with each task's log location and status when the eval finishes. To launch in the background instead, use --detach (which implies --json).",
)
@click.option(
    "--detach/--no-detach",
    type=bool,
...
def eval_command(ctx: click.Context, /, **params: Any) -> None:
    ...
    Monitor a running eval from another shell with `apg ctl`
    (see `apg ctl --help`).
    """
    with (
        _armed_ctl_pointer(),
        ...
        # When --run-config is used, env-sourced CLI values (APG_EVAL_*)
        ...
        # unset. Common-options env vars (APG_LOG_LEVEL, APG_LOG_DIR,
        ...
        if params.get("run_config"):
            ...
            for param in ctx.command.params:
                ...
                if not (isinstance(envvar, str) and envvar.startswith("APG_EVAL_")):
                    continue
...
def _armed_ctl_pointer() -> Iterator[None]:
    """Arm the launch-time ``apg ctl`` pointer for this CLI invocation.
...
def _eval_command_impl(
    ...
    **common: Unpack[CommonOptions],
    ...
    # promote it back to "rich" or --trace (/APG_EVAL_TRACE) would
...
@click.option(
    ...
    envvar="APG_EVAL_RETRY_IMMEDIATE",
)
@click.option(
    "--retry-wait",
    type=int,
...
@click.option(
    ...
    envvar="APG_EVAL_RETRY_CONNECTIONS",
)
@click.option(
    "--no-retry-cleanup",
    type=bool,
    ...
    envvar="APG_EVAL_NO_RETRY_CLEANUP",
)
@click.option(
    "--bundle-dir",
    type=str,
...
def resolve_model_spec(
    ...
    - An environment spec against a typed option: the option wins and this
      returns None. An `APG_EVAL_MODEL_SPEC` in a `.env` file must not
...
def eval_exec(
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    if run_config and is_eval_set:
        raise PrerequisiteError("--run-config is only supported by apg eval.")
    ...
    from agent_proving_ground._display.core.results import set_retry_args_suffix
...
def _eval_exec_json(
    ...
    - ``launch`` — printed by the launch-handoff listener the moment the
      ...
      has read this line can trust ``apg ctl`` immediately — an empty
      task list means "no tasks registered yet", never "no server".
    - ``done`` — printed after ``eval()`` returns, with each task's log
      location and status (the live → ``apg log`` handoff). A run
...
@click.option(
    ...
    default=False,
    envvar="APG_EVAL_JSON",
    help="Emit machine-readable launch output as JSON lines on stdout (implies --display none): a 'launch' record printed once the control-channel server is bound — reporting run_id, pid, log_dir, and the control socket path ('control' is null when the server is disabled or failed to bind, so its presence guarantees `apg ctl` is usable) — and a 'done' record with each retried task's log location and status when the retry finishes. Each retried log file runs as its own eval, so a multi-file retry emits one 'launch' record per file (sequentially — each supersedes the previous), and the 'done' record carries the last launch's run_id. To launch in the background instead, use --detach (which implies --json and hands off on the first launch record).",
)
@click.option(
    "--detach/--no-detach",
    type=bool,
...
@click.option(
    "--max-samples", type=int, help=MAX_SAMPLES_HELP, envvar="APG_EVAL_MAX_SAMPLES"
)
@click.option(
    "--max-tasks", type=int, help=MAX_TASKS_HELP, envvar="APG_EVAL_MAX_TASKS"
)
...
@click.option(
    ...
    help=(
        ...
        "external clients (the `apg ctl` CLI, scripted agents) can still "
        "query its state. Run `apg ctl process release` to release. "
        "Observe the run from another shell with `apg ctl task list`."
    ),
    envvar="APG_EVAL_CTL_SERVER",
)
@click.option(
...
@click.option(
    "--max-retries", type=int, help=MAX_RETRIES_HELP, envvar="APG_EVAL_MAX_RETRIES"
)
@click.option("--timeout", type=int, help=TIMEOUT_HELP, envvar="APG_EVAL_TIMEOUT")
@click.option(
    "--attempt-timeout",
    type=int,
    help=ATTEMPT_TIMEOUT_HELP,
