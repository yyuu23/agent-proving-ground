...
from agent_proving_ground._util.dotenv import init_cli_env
...
def common_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        "--debug", is_flag=True, envvar="APG_DEBUG", help="Wait to attach debugger"
    )
    @click.option(
        "--debug-port",
        default=5678,
        envvar="APG_DEBUG_PORT",
...
def process_common_options(options: CommonOptions) -> None:
    ...
    if options.get("traceback_locals", False):
        os.environ["APG_TRACEBACK_LOCALS"] = "1"
    ...
    if options["debug"]:
        ...
        # stdout (e.g. the NDJSON stream of `apg eval --json`)
