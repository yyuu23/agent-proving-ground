...
- ``--stdio`` runs a transparent stdio↔socket bridge for editors
  (Zed etc.) that spawn ``apg acp --stdio`` as a subprocess and
  talk newline-delimited JSON-RPC to a running eval's ACP server.
- Without ``--stdio``, launches the AgentProvingGround-native Textual TUI
  client (Phase 15).
...
from agent_proving_ground.agent._acp.discovery import (
    DiscoveredEval,
    TargetAddress,
    TargetResolutionError,
    resolve_target,
)
from agent_proving_ground.agent._acp.stdio import TripleResolutionError, bridge_stdio
...
def acp_command(
    ...
    """Connect to a running AgentProvingGround eval over the Agent Client Protocol."""
    ...
    try:
        if stdio:
            ...
            from agent_proving_ground.agent._acp.tui import run_tui
...
async def _run_stdio_bridge(
    *,
    ...
    if task_id is not None and sample_id is not None and epoch is not None:
        ...
        try:
            from agent_proving_ground.agent._acp.stdio import preflight_resolve_triple
...
def _log_pick_notice(
    ...
    print(
        f"apg acp: attached to {target.eval_id} (most recent of "
