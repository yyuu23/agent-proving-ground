...
(e.g. Zed connected via ``apg acp --stdio``), the
...
if TYPE_CHECKING:
    ...
    from agent_proving_ground.agent._acp.transport import ApproverClient
...
class _ApprovalRoutingSession(Protocol):
    ...
    :class:`~agent_proving_ground.agent._acp.transport.AcpTransport`) so tests
...
# ACP's kinds are binary allow/deny variants and AgentProvingGround's
...
def _build_request(
    *,
    ...
    """Construct the ACP request body from AgentProvingGround's approval inputs.
    ...
    :func:`agent_proving_ground.agent._acp.tool_content.descriptive_title` and
    ...
    # ``agent_proving_ground.agent._acp.tool_content`` → ``agent_proving_ground.log._transcript``
