...
from agent_proving_ground.analysis._prepare.operation import Operation
from agent_proving_ground.model import get_model_info
from agent_proving_ground.model._model_data.model_data import ModelInfo
...
def model_info(
    ...
    AgentProvingGround includes built in support for many models (based upon the `model` string in the dataframe). If you are using models for which AgentProvingGround does not include model metadata, you may include your own model metadata via the `model_info` argument.

    Args:
        model_info: Additional model info for models not supported directly by AgentProvingGround's internal database.
    """
    import pandas as pd
