...
if TYPE_CHECKING:
    ...
    from agent_proving_ground.tool._tools._skill import Skill

import anyio
from shortuuid import uuid as shortuuid

from agent_proving_ground.agent._agent import Agent, AgentState
from agent_proving_ground.agent._react import react
from agent_proving_ground.agent._run import run
from agent_proving_ground.agent._types import (
...
from agent_proving_ground.tool._tool import Tool, ToolError, ToolSource, tool
...
def background_registry(registry: BackgroundRegistry) -> Iterator[BackgroundRegistry]:
    ...
    set/reset idiom in ``src/agent_proving_ground/util/_span.py``.
    """
...
def _dispatch_background(
    ...
    the cap.
    """
    from agent_proving_ground.util._background import background
...
async def _run_background(
    ...
    Inline replication of ``run()``'s body (``src/agent_proving_ground/agent/_run.py``
    ...
    from agent_proving_ground.util._limit import LimitExceededError, apply_limits
...
def _apply_subagent_type_enum(tool: Tool, subagents: list[Subagent]) -> None:
    ...
    from agent_proving_ground.tool._tool_description import (
        ToolDescription,
...
def _find_memory_tool(
    ...
    from agent_proving_ground._util.registry import is_registry_object, registry_unqualified_name
...
def _resolve_tools(
    ...
    if sa.memory and not _has_memory_tool(tools):
        from agent_proving_ground.tool._tools._memory import memory
    ...
    if merged_skills:
        from agent_proving_ground.tool._tools._skill import skill as skill_fn
...
def _tool_is_parallel_safe(t: Tool | ToolDef | ToolSource) -> bool:
    from agent_proving_ground._util.registry import is_registry_object
    from agent_proving_ground.tool._tool_def import tool_def_fields
...
def _default_readonly_tools() -> list[Tool | ToolDef | ToolSource]:
    ...
    if sandbox_environments_context_var.get(None) is None:
        return []
    from agent_proving_ground.tool._tools._grep import grep
    from agent_proving_ground.tool._tools._list_files import list_files
    from agent_proving_ground.tool._tools._read_file import read_file
