r"""AgentProvingGround-specific deviations from standard ACP.
...
(b) a payload field carried in ``_meta`` under the ``inspect.*``
    namespace,
(c) behavior gated on AgentProvingGround-aware client capability flags.
...
Producer-side grep audit::
    ...
    git grep -nE 'inspect[./]' src/agent_proving_ground/agent/_acp/
...
from agent_proving_ground.agent._acp._guards import acp_guard, acp_send_guard
...
if TYPE_CHECKING:
    ...
    from agent_proving_ground.agent._acp.connection import ConnectionHandler, ConnectionState
    from agent_proving_ground.agent._acp.picker import PickerTarget, SampleListing
    from agent_proving_ground.agent._acp.transport import AcpTransport
    from agent_proving_ground.event._model import ModelEvent
    from agent_proving_ground.model._chat_message import ChatMessageSystem, ChatMessageUser
...
# no first-class message-count field, so the AgentProvingGround TUI reads this key
...
# AgentProvingGround, so there's no pending ``ToolEvent`` for the connection handler to
# cancel — the request would silently no-op. The AgentProvingGround TUI reads this off the
...
APG_EVENT_METHOD = "inspect/event"

# TUI-grade action methods. Always advertised — no capability opt-in.
# Clients that don't know about them simply don't call them.
APG_CANCEL_SAMPLE_METHOD = "inspect/cancel_sample"
APG_CANCEL_TOOL_CALL_METHOD = "inspect/cancel_tool_call"
APG_ATTACH_METHOD = "inspect/attach"
APG_LIST_SESSIONS_METHOD = "inspect/list_sessions"
...
# AgentProvingGround TUI uses this to surface non-ACP samples in the picker
...
APG_LIST_SAMPLES_METHOD = "inspect/list_samples"
...
APG_SESSION_ENDED_METHOD = "inspect/session_ended"
...
# ---------------------------------------------------------------------------
# AgentProvingGround-specific tool knowledge
# ---------------------------------------------------------------------------

# Tool names whose invocations translate to ACP's ``AgentPlanUpdate``
# for plan-capable clients. Hard-coded by tool name (matches AgentProvingGround's
...
class CancelSampleParams(BaseModel):
    """Pydantic param model for :data:`APG_CANCEL_SAMPLE_METHOD`."""
...
class CancelToolCallParams(BaseModel):
    """Pydantic param model for :data:`APG_CANCEL_TOOL_CALL_METHOD`."""
...
class NewSessionParams(BaseModel):
    """Pydantic param model for :data:`APG_ATTACH_METHOD`.

    AgentProvingGround-aware clients (the TUI, editors that already know which
...
class ListSessionsParams(BaseModel):
    """Pydantic param model for :data:`APG_LIST_SESSIONS_METHOD` (no params)."""


class ListSamplesParams(BaseModel):
    """Pydantic param model for :data:`APG_LIST_SAMPLES_METHOD` (no params)."""
...
def register_inspect_routes(router: MessageRouter, handler: ConnectionHandler) -> None:
    ...
    four AgentProvingGround-namespace methods (sample-level cancel, tool-call
    ...
    router.add_route(
        Route(
            method=APG_CANCEL_SAMPLE_METHOD,
    ...
    router.add_route(
        Route(
            method=APG_CANCEL_TOOL_CALL_METHOD,
    ...
    router.add_route(
        Route(
            method=APG_LIST_SESSIONS_METHOD,
    ...
    router.add_route(
        Route(
            method=APG_LIST_SAMPLES_METHOD,
...
def detect_capabilities(
    ...
    """Decide whether this client opts into AgentProvingGround extensions.
...
# only the dict shape here means all AgentProvingGround-specific keys remain in
# one auditable place.
...
def input_message_chunk_meta(
    ...
    # type-check time.
    from agent_proving_ground.model._chat_message import ChatMessageSystem
...
class RawEventForwarder:
    ...
    :data:`APG_EVENT_METHOD` JSON-RPC notification, alongside
    ...
    async def replay_event(self, event: Any) -> None:
        ...
        await self._connection.send_notification(APG_EVENT_METHOD, payload)
...
def sample_listing_meta_dict(listing: SampleListing) -> dict[str, Any]:
    """Canonical camelCase wire shape for an :data:`APG_LIST_SAMPLES_METHOD` entry.
...
class PlanPolicyTransformer:
    ...
    AgentProvingGround's first-party planning tools (:data:`PLAN_TOOL_NAMES`).
...
def build_picker_notification(
    ...
    capability-aware client (e.g. ``apg acp``) can match by
    ``(task, sample_id, epoch)`` without re-parsing the text.

    Both the text format and the ``_meta`` shape are AgentProvingGround-specific
