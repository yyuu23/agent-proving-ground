...
from agent_proving_ground.agent._acp._client_registry import _ClientDriverRegistry
from agent_proving_ground.agent._acp._guards import acp_guard
from agent_proving_ground.agent._acp.transport import (
...
if TYPE_CHECKING:
    from agent_proving_ground.agent._acp.event_mapping import _AcpEventRouter
    from agent_proving_ground.agent._channel import AgentChannel, AgentRef
...
class _InterruptCoordinator:
    ...
    in-proc TUI modal-prompt mode + AgentProvingGround-aware ACP clients both use
...
class _TurnCancelMachinery:
    ...
    :class:`~agent_proving_ground.agent.AgentChannel`; this class only owns the
    ...
    def snapshot_for_cancel(
        ...
        """
        # Deferred import — `agent_proving_ground.event` circularly references the
        # session module via the event union.
        from agent_proving_ground.event._model import (
            LIMIT_CANCEL_ERROR,
...
class LiveAcpTransport:
    ...
    def maybe_bind(self, channel: "AgentChannel", ref: "AgentRef") -> bool:
        ...
        AgentProvingGround-aware ACP clients.
        """
        if self._ref is None:
            ...
            from agent_proving_ground.agent._acp.server import acp_server_accepting_clients
    ...
    def _on_channel_drained(self, items: list[Any]) -> None:
        ...
        ignored.
        """
        from agent_proving_ground.agent._channel import UserMessage as _ChannelUserMessage
    ...
    async def __aenter__(self) -> AcpTransport:
        ...
        (if any) so out-of-task consumers like the in-process AgentProvingGround
    ...
    def submit_user_message(self, msg: ChatMessageUser) -> None:
        ...
        in-proc TUI's modal prompt mode and AgentProvingGround-aware ACP clients
    ...
    def cancel_current_turn(
        ...
        with acp_guard("ACP cancel_current_turn raised; cancel may be partial"):
            ...
            # Deferred import — `agent_proving_ground.event` circularly
            # references this module via the event union.
            from agent_proving_ground.event._interrupt import InterruptEvent
            ...
            if self._ref is not None:
                from agent_proving_ground.agent._channel import Cancel as _ChannelCancel

                self._ref.interrupt(_ChannelCancel(reason=cause))

            # Mark the interrupt as pending and notify subscribers so
            # prompt-mode clients (the in-proc TUI; AgentProvingGround-aware ACP
    ...
    def track_model_event(self, event: ModelEvent) -> Iterator[None]:
        ...
        :func:`agent_proving_ground.log._samples.track_active_model_event` (which
