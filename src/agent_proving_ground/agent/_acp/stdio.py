"""Stdio↔socket bridge for ``apg acp --stdio``.
...
agent_proving_ground's anyio code. Cancellation catches use
...
async def _open_socket(
    ...
    can easily exceed that on the AgentProvingGround transcript firehose.
    """
...
def _rewrite_session_new_to_attach(line: bytes, target_triple: str) -> bytes | None:
    ...
    is dropped because the AgentProvingGround server doesn't host any.
    """
    try:
        msg = json.loads(line)
