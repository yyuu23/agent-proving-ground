...
Schema layer over :mod:`agent_proving_ground._util.discovery` — adds the ACP
...
mechanics live in :mod:`agent_proving_ground._util.process`,
:mod:`agent_proving_ground._util.sockets`, and :mod:`agent_proving_ground._util.discovery`
...
from agent_proving_ground._util.sockets import parse_host_port
...
def list_discovered_evals() -> list[DiscoveredEval]:
    ...
    """
    from agent_proving_ground._util.discovery import list_alive_discovery_entries
...
def resolve_target(
    ...
    if not discovered:
        raise TargetResolutionError(
            ...
            f"Start one with `apg eval <task> --acp-server`."
        )
