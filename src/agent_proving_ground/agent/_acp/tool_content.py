...
The router and the approval shim both need to turn an AgentProvingGround tool
...
from agent_proving_ground._util.url import (
    data_uri_mime_type,
    data_uri_to_base64,
    is_data_uri,
    is_http_url,
...
from agent_proving_ground.tool._tool_call import ToolCallContent, substitute_tool_call_content

# Built-in AgentProvingGround tool name → ACP ``ToolKind`` mapping. The kind
...
#
# IMPORTANT: never map any AgentProvingGround tool to ``"execute"``. ACP's
...
# EDITOR to run shell commands on the user's local machine. AgentProvingGround's
...
def descriptive_title(fn: str, arguments: dict[str, Any] | None) -> str:
    ...
    # File reads — path is the key arg. AgentProvingGround's built-in read_file
...
def content_blocks_from_view(
    ...
    """Build ACP ``ContentToolCallContent`` from an AgentProvingGround tool view.

    AgentProvingGround tools register a custom ``ToolCallViewer`` via the
...
def _image_block(image: ContentImage) -> ContentToolCallContent:
    """Wrap an AgentProvingGround ``ContentImage`` as an ACP ``ImageContentBlock``.

    AgentProvingGround represents the image source as a single ``image`` field
...
# convention. The AgentProvingGround `memory` tool follows the identical shape for its
