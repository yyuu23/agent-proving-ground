...
- :mod:`.transport_live` — :class:`LiveAcpTransport`, the active
  ...
  the agent's :class:`~agent_proving_ground.agent.AgentChannel`:
...
class ElicitationRequest:
    ...
    wire it's one flat object. This AgentProvingGround-side container carries the
...
class AcpTransport(Protocol):
    ...
    :class:`~agent_proving_ground.agent.AgentChannel` (``submit_user_message`` →
    ...
    *producer* on it.
    ...
    def subscribe_transcript_events(
        ...
        Used by the raw-event forwarder to stream AgentProvingGround-native
    ...
    def subscribe_interrupted(self, callback: Callable[[], None]) -> Callable[[], None]:
        ...
        Used by modal-prompt clients (in-proc TUI; AgentProvingGround-aware ACP
    ...
    def maybe_bind(self, channel: "AgentChannel", ref: "AgentRef") -> bool:
        ...
        :class:`agent_proving_ground.agent.AgentChannel` (rebindable per top-level
        :func:`react <agent_proving_ground.agent.react>` invocation in the sample,
    ...
    def is_interactive(self) -> bool:
        ...
        :class:`~agent_proving_ground.agent._channel.AgentChannel` (set via
...
from agent_proving_ground.agent._acp.transport_noop import NoOpAcpTransport  # noqa: E402
