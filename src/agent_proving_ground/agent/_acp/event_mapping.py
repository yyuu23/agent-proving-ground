...
Mapping the AgentProvingGround-native event family onto ACP's ``_meta`` extension
...
from agent_proving_ground.agent._acp.tool_content import (
    _content_for_start,
    _content_for_update,
    _descriptive_title,
    _tool_kind_for,
)
from agent_proving_ground.agent._bridge.util import in_bridge_model_generate
from agent_proving_ground.event import Event, SpanBeginEvent, SpanEndEvent
...
from agent_proving_ground.util._span import AGENT_SPAN_TYPE
...
class SubagentDepthTracker:
    ...
    - :func:`agent_proving_ground.agent._acp.session_router._filter_subagent_events`
...
def _user_message_text(msg: ChatMessageUser | ChatMessageSystem) -> str:
    ...
    image-in-user-message is rare in AgentProvingGround today; full image support
...
def _map_model_event(
    ...
    # so we derive a stable UUIDv5 from the AgentProvingGround ModelEvent uuid
...
# UUIDv5 namespace for deriving message_id from AgentProvingGround ModelEvent uuids.
...
_APG_MESSAGE_ID_NAMESPACE = _uuid_module.UUID(
    "0e22b6ad-7e30-5d7b-9a87-78e3f56f4f93"
)


def _model_event_message_id(model_event_uuid: str) -> str:
    """Map an AgentProvingGround ModelEvent shortuuid to an ACP-compliant UUIDv5 string.

    ACP's message_id field mandates UUID format; AgentProvingGround events use
    ...
    client.
    """
    return str(_uuid_module.uuid5(_APG_MESSAGE_ID_NAMESPACE, model_event_uuid))
...
def _bridge_start_notification(
    ...
    tells the AgentProvingGround TUI not to offer a per-tool cancel that would no-op (the
