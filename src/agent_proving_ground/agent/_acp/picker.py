...
``apg acp`` client reads the structured target list out of the
...
since both are AgentProvingGround-specific extensions on a standard ACP payload.
"""
...
class PickerTarget:
    ...
    :attr:`agent_proving_ground.log._samples.ActiveSample.started`). ``None``
    before the sample's ``start()`` is called. Drives the picker's
    ``running`` column."""

    total_messages: int = 0
    """Running total messages for the sample (from
    :attr:`agent_proving_ground.log._samples.ActiveSample.total_messages`). Drives
    the picker's ``messages`` column; refreshed on rescan."""

    total_tokens: int = 0
    """Running total tokens for the sample (from
    :attr:`agent_proving_ground.log._samples.ActiveSample.total_tokens`). Drives
...
def list_picker_targets() -> list[PickerTarget]:
    ...
    Filters :func:`agent_proving_ground.log._samples.active_samples` to those
...
class SampleListing:
    ...
    ``"question"`` for ``ask_user``. ``None`` otherwise. The AgentProvingGround
...
def list_all_samples() -> list[SampleListing]:
    ...
    Walks :func:`agent_proving_ground.log._samples.active_samples` unfiltered.
