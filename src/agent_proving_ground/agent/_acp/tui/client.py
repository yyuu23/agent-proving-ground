"""ACP client helpers for the ``apg acp`` TUI.
...
from agent_proving_ground.agent._acp._config import ACP_STREAM_BUFFER_LIMIT
...
(plan rendering, raw events). The TUI is AgentProvingGround-aware."""
...
:data:`RAW_EVENTS_META_KEY`. AgentProvingGround-native events (``score``,
    ...
    *,
    ...
    async def _query(eval_id: str, target: TargetAddress) -> list[SessionRow] | None:
        try:
            ...
            print(
                f"apg acp: failed to enumerate {target.describe()}: {exc}",
                file=sys.stderr,
                flush=True,
            )
            return None
...
async def _list_for_target(eval_id: str, target: TargetAddress) -> list[SessionRow]:
    ...
    Uses ``inspect/list_samples`` (the AgentProvingGround-specific superset of
    ...
    try:
        ...
        response: Any = await conn.send_request(APG_LIST_SAMPLES_METHOD, {})
    finally:
        writer.close()
        try:
...
async def attach_session(
    ...
    *,
    ...
    - ``inspect/event`` (notification, opt-in): ``on_inspect_event``
      ...
      render. The payload is a serialized ``agent_proving_ground.event.Event``;
...
async def _open_socket(
    ...
    Mirrors :func:`agent_proving_ground.agent._acp.stdio._open_socket` — same
    ...
    async def _handler(params: Any) -> dict[str, Any]:
        ...
        if mode != "form":
            ...
            raise ValueError(
                ...
                "AgentProvingGround TUI only handles form-mode elicitation"
            )
