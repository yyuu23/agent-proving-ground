"""Picker screen for the ``apg acp`` TUI.
    ...
    def _compose_empty(self) -> ComposeResult:
        ...
        if self._empty_notice is not None:
            with Container(id="empty-state"):
                ...
                yield Markdown(
                    "Re-run `apg acp` without the filter arguments (or with a "
        ...
        if self._server_override is None:
            heading = "No running AgentProvingGround evals found on this machine."
            hint_md = (
                "Run eval or eval-set with the ACP server enabled, "
                "then return here:\n\n"
                "```bash\n"
                "# start server\n"
                "apg eval <task> --acp-server\n"
                "```\n\n"
                "To use ACP with a remote machine, pass a host/port "
                "to `--acp-server`:\n\n"
                "```bash\n"
                "# start server\n"
                'apg eval <task> --acp-server "0.0.0.0:4545"\n\n'
                "# connect to server\n"
                ...
                # address.
                'apg acp --server "198.51.100.0:4545"\n'
                "```\n"
