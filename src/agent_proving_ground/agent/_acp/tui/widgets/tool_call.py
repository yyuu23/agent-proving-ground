...
from agent_proving_ground._util.rich import clean_control_characters
...
class ToolCallWidget(Widget):
    ...
    def _header_text(self) -> str:
        ...
        :func:`agent_proving_ground.agent._acp.tool_content.descriptive_title` —
    ...
    def _extract_plan_entries(self) -> list[dict[str, Any]] | None:
        ...
        path covers the tool-call-as-plan flow AgentProvingGround's react agents
        use today.
        """
...
def _plan_entry_text(entry: dict[str, Any]) -> str:
    """Display text for a plan entry — tries the keys AgentProvingGround tools emit.
...
class _ApprovalContent(Vertical):
    ...
    ``src/agent_proving_ground/approval/_human/panel.py`` — content only,
    no actions.
    """
