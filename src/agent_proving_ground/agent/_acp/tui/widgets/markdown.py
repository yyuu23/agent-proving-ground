...
- :class:`StyledCodeBlock` — a ``rich.markdown.CodeBlock`` subclass
  ...
  the standard AgentProvingGround dark background; fences with no language render
...
The colour theme matches AgentProvingGround's existing transcript renderer
(``transcript_code_theme`` in :mod:`agent_proving_ground._util.transcript`) so
the TUI feels visually continuous with ``apg view`` and the rich
terminal display.
"""
...
from agent_proving_ground._util.transcript import transcript_code_theme

_CODE_BG = "#282c34"
"""Background colour for syntax-highlighted blocks.

Matches the existing ``CustomCodeBlock`` in
:mod:`agent_proving_ground._display.core.rich` so a transcript line you see in
...
class StyledCodeBlock(CodeBlock):
    ...
    AgentProvingGround's tool cards typically emit one labelled fence (the
