...
Two widgets share the same tinted band + ``apg acp`` left-rail
...
def _short_task_name(name: str) -> str:
    ...
    AgentProvingGround task names commonly look like ``inspect_harbor/terminal_bench_2_0``
...
class AppHeaderWidget(Widget):
    """Slim tinted header — ``apg acp`` + transport target.
    ...
    transport they're attached to (``apg acp · local`` vs
    ``apg acp · 127.0.0.1:4545``).
    """
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            yield Static("apg acp", classes="app-title", markup=False)
...
class SessionHeaderWidget(Widget):
    ...
    class BackToPicker(Message):
        """Posted when the operator clicks the ``apg acp`` title.
    ...
    def on_click(self, event: Click) -> None:
        """Treat a click on the ``apg acp`` title as ^S switch sample.
