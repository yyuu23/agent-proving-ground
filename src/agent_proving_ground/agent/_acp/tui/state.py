...
from agent_proving_ground.scorer._metric import CORRECT, INCORRECT, NOANSWER, PARTIAL
from agent_proving_ground.util._span import SCORER_SPAN_TYPE, SCORERS_SPAN_NAME
...
class MessageGroup:
    ...
    When a model event errors and AgentProvingGround retries, each attempt fires
...
class PendingCancel:
    ...
    # ``connection.send_request(APG_CANCEL_SAMPLE_METHOD, …)``.
    connection: Any
    session_id: str
...
"""AgentProvingGround-native transcript events the TUI renders as inline event chips.
...
@dataclass
class EventChip:
    """An inline transcript chip for an AgentProvingGround-native transcript event.
    ...
    kind: EventChipKind
    """Which AgentProvingGround event family this chip represents. Drives the
...
def _value_to_float(value: Any) -> float | None:
    """Emulate :func:`agent_proving_ground.scorer.value_to_float`'s scalar mapping.
...
class SessionState:
    ...
    def __init__(self, *, now: Callable[[], float] = time.monotonic) -> None:
        ...
        # cleared its plan (not produced by either AgentProvingGround planning tool
    ...
    def consume_inspect_event(self, event: dict[str, Any]) -> None:
        ...
        (AgentProvingGround-native event chips), and ``"span_begin"`` /
    ...
    def consume_score_event(self, event: dict[str, Any]) -> None:
        ...
        Score payload shape mirrors ``agent_proving_ground.event.ScoreEvent``
    ...
    # ------------------------------------------------------------------
    # AgentProvingGround-native event chips (sample_limit / error / compaction / info)
    # ------------------------------------------------------------------

    def consume_sample_limit_event(self, event: dict[str, Any]) -> None:
        """Mount an inline event chip for a serialized ``SampleLimitEvent``.

        Wire shape mirrors :class:`agent_proving_ground.event.SampleLimitEvent`:
    ...
    def consume_error_event(self, event: dict[str, Any]) -> None:
        ...
        Wire shape mirrors :class:`agent_proving_ground.event.ErrorEvent`: an
    ...
    def consume_compaction_event(self, event: dict[str, Any]) -> None:
        ...
        Wire shape mirrors :class:`agent_proving_ground.event.CompactionEvent`:
    ...
    def consume_info_event(self, event: dict[str, Any]) -> None:
        ...
        Wire shape mirrors :class:`agent_proving_ground.event.InfoEvent`: an
