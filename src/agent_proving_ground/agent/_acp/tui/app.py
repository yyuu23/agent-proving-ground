"""Top-level Textual app for ``apg acp``.
...
class ApgAcpApp(App[None]):
    """The ``apg acp`` Textual application."""
    ...
    # Bold-blue title styling to match the agent_proving_ground task display
    ...
    def __init__(
        ...
        *,
        ...
        self.title = f"apg acp · {server or 'local'}"
    ...
    async def _enumerate(self) -> list[SessionRow]:
        ...
        try:
            ...
            print(
                f"apg acp: enumeration failed: {exc}",
                file=sys.stderr,
                flush=True,
            )
            return []
