...
from agent_proving_ground.agent._acp.inspect_ext import (
    APG_CANCEL_SAMPLE_METHOD,
    APG_CANCEL_TOOL_CALL_METHOD,
...
from agent_proving_ground.agent._acp.picker import (
    PickerTarget,
    SampleListing,
    list_all_samples,
    list_picker_targets,
...
if TYPE_CHECKING:
    ...
    from agent_proving_ground.log._samples import ActiveSample, SampleCancelAction

logger = getLogger(__name__)

# Version banner included in InitializeResponse. The eval is the
# server in the ACP relationship.
_AGENT_NAME = "agent-proving-ground"
_AGENT_VERSION = "0.10"
...
class ConnectionHandler:
        ...
        match self.state.binding:
            case Bound(
                ...
                if not target.is_interactive:
                    ...
                    # leave the TUI / AgentProvingGround-aware clients showing a
    ...
    async def inspect_list_sessions(self) -> dict[str, Any]:
        """Enumerate attachable sessions for AgentProvingGround-aware clients.
    ...
    async def inspect_list_samples(self) -> dict[str, Any]:
        ...
        ``None``. The AgentProvingGround TUI consumes this so non-ACP samples
    ...
    async def cancel_sample(
        ...
        the sample finishes.
        """
        bound = self._require_bound(session_id, APG_CANCEL_SAMPLE_METHOD)
    ...
    async def cancel_tool_call(
        ...
        bound = self._require_bound(session_id, APG_CANCEL_TOOL_CALL_METHOD)
        sample = _find_active_sample(bound.target_session_id)
        if sample is None:
    ...
    async def _notify_binding(self, target: PickerTarget) -> None:
        ...
        ``inspect.interactive`` flag so AgentProvingGround-aware clients render the
        ...
        # AgentProvingGround-aware clients gate their composer on it.
...
def _find_live_session(session_id: str) -> "AcpTransport | None":
    ...
    Walks :func:`agent_proving_ground.log._samples.active_samples` for a sample
