...
from agent_proving_ground._util.hash import mm3_hash
...
from agent_proving_ground.model._compaction import (
    Compact,
    CompactionStrategy,
)
from agent_proving_ground.model._compaction import (
...
if TYPE_CHECKING:
    # deferred: `_bridge.types` is imported at the top of `agent_proving_ground.agent`, well
    # before `agent_proving_ground.approval` can be initialized (approval -> event -> scorer
...
class AgentBridge:
    ...
    thinking budget, penalties, `n`, logprobs) are dropped; the resolved AgentProvingGround
...
def _condensed_fingerprint(fp: _MessageFingerprint) -> _MessageFingerprint:
    ...
    ``attachment://<mm3-hash-of-text>`` (see `agent_proving_ground.log._condense`);
