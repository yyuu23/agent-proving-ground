...
from agent_proving_ground._util._async import is_callable_coroutine
from agent_proving_ground.agent._agent import Agent, AgentState, agent
...
from agent_proving_ground.model._compaction.types import CompactionStrategy
from agent_proving_ground.model._model import GenerateFilter, ModelEventSink, get_model
from agent_proving_ground.model._model_output import ModelOutput
from agent_proving_ground.model._openai_convert import (
    messages_from_openai,
    messages_to_openai,
)
from agent_proving_ground.model._providers.providers import (
...
from agent_proving_ground.tool._tools._web_search._web_search import (
    WebSearchProviders,
)
...
if TYPE_CHECKING:
    # deferred: importing `agent_proving_ground.approval` at module scope here cycles
    # through approval -> event -> scorer while `agent_proving_ground.agent` is still
    # initializing. Same reason `model/_call_tools.py` defers it.
    from agent_proving_ground.approval._policy import ApprovalPolicy

# Headers blocked from bridge clients (exact match, case-insensitive)
_BLOCKED_BRIDGE_HEADERS = frozenset(
    [
        # AgentProvingGround internal tracking
        "x-irid",
        # Authentication
        "authorization",
        "x-api-key",
        ...
        "anthropic-version",
        # User-Agent would be misleading since AgentProvingGround transforms the request
        "user-agent",
    ]
)
...
async def agent_bridge(
    ...
    *,
    ...
    Provide AgentProvingGround integration for 3rd party agents that use the
    ...
    "inspect/" for non-default models) into the AgentProvingGround model API.
    ...
    Args:
       ...
       web_search: Configuration for mapping model internal
          web_search tools to AgentProvingGround. By default (in-process bridges), will map
       ...
       code_execution: Configuration for mapping model internal
          code_execution tools to AgentProvingGround. By default, will map to the
       ...
       forward_generation_config: Forward client generation parameters (e.g.
          ...
          AgentProvingGround model config and provider defaults govern generation (structural
...
def init_openai_request_patch() -> None:
    ...
    # agent_proving_ground (we only get it transitively via openai >= 3), and this
...
def bridge(
    ...
    """Bridge an external agent into an AgentProvingGround Agent.
    ...
    If you do choose to use the `bridge()` function, these [examples](https://github.com/UKGovernmentBEIS/agent_proving_ground/tree/b4670e798dc8d9ff379d4da4ef469be2468d916f/examples/bridge) demostrate its basic usage.
    :::
