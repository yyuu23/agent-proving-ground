...
from agent_proving_ground.model._model import GenerateFilter, Model, ModelEventSink
from agent_proving_ground.tool._mcp._config import MCPServerConfigHTTP
from agent_proving_ground.tool._mcp._tools_bridge import BridgedToolsSpec
from agent_proving_ground.tool._sandbox_tools_utils.sandbox import sandbox_with_injected_tools
...
from agent_proving_ground.util._checkpoint.checkpointer import Checkpointer
from agent_proving_ground.util._sandbox._cli import SANDBOX_CLI
from agent_proving_ground.util._sandbox.exec_remote import (
    ExecCompleted,
    ExecRemoteProcess,
...
async def sandbox_agent_bridge(
    ...
    *,
    ...
    Provide AgentProvingGround integration for agents running inside sandboxes. Runs
    ...
    runs on port 13131 and routes requests to the current AgentProvingGround model provider.
    ...
    model name "inspect" when calling OpenAI, Anthropic, or Google. Use "inspect/<full-model-name>" to target other AgentProvingGround model providers.
    ...
    Args:
        ...
        web_search: Configuration for mapping model internal web_search tools to
            AgentProvingGround. Withheld by default: a sandboxed agent that names the native
        ...
        code_execution: Configuration for mapping model internal code_execution
            tools to AgentProvingGround. Withheld by default (see `web_search`). Pass `True`
        ...
        bridged_tools: Host-side AgentProvingGround tools to expose to the sandboxed agent
