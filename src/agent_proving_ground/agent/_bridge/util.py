...
from agent_proving_ground._util.url import is_data_uri
from agent_proving_ground.agent._bridge._approval import (
    MAX_CONSECUTIVE_REJECTIONS,
    apply_bridge_tool_approval,
    terminate_for_repeated_rejections,
)
from agent_proving_ground.agent._bridge._errors import BridgePolicyError
from agent_proving_ground.agent._bridge.types import AgentBridge, message_json_hash
from agent_proving_ground.model._chat_message import ChatMessage, ChatMessageUser
from agent_proving_ground.model._generate_config import GenerateConfig, active_generate_config
...
# *how* the underlying model generates. These are the AgentProvingGround model's province
...
def clear_generation_params(config: GenerateConfig) -> None:
    ...
    (the default): the dropped fields then fall back to the resolved AgentProvingGround model
...
def validate_bridge_media(bridge: AgentBridge, messages: Sequence[ChatMessage]) -> None:
    ...
    URL is fetched by the AgentProvingGround process (or by the provider), and anything else
