...
from agent_proving_ground._util.constants import SKIP_TRANSCRIPT_DISPATCH
from agent_proving_ground._util.list import find_last_match
...
from agent_proving_ground.event._store import StoreEvent
...
from agent_proving_ground.util._store import store, store_changes, store_jsonable

if TYPE_CHECKING:
    from agent_proving_ground.log._recorders.buffer.types import TranscriptEventSink
...
def transcript_bounded_enabled() -> bool:
    value = os.environ.get("APG_TRANSCRIPT_BOUNDED")
    if value is None:
        return False
...
def record_interrupt_event(
    *,
    ...
    Internal helper used by AgentProvingGround's cancellation machinery — the ACP
