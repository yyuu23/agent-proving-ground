...
Backs ``GET /evals/<id>/sample/events`` (and ``apg ctl sample events``): a
...
from agent_proving_ground._control.terminal_cache import (
    TerminalSourceCache,
...
async def sample_events(
    ...
    *,
    ...
    from agent_proving_ground.log._transcript import TranscriptHistoryUnavailableError
...
def _running_source(eval_id: str, sample_id: str, epoch: int) -> EventsSource | None:
    ...
    """
    from agent_proving_ground._control.state import find_active_sample
...
async def _resolve_logged_source(
    ...
    :func:`agent_proving_ground._control.state._full_sample` so a just-completed (or
