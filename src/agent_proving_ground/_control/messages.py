...
Backs ``GET /evals/<id>/sample/messages`` (and ``apg ctl sample
...
from agent_proving_ground._control.events import _to_text, _truncate
...
async def _resolve_logged_source(
    ...
    ``EvalSample`` via :func:`agent_proving_ground._control.state._full_sample` — the
