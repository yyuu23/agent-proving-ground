...
- **Task gates** (:func:`pause_task` / :func:`resume_task`) live in a
  ...
  ``agent_proving_ground.util._concurrency`` and reset at the same run boundary, so a
...
# sample-semaphore registry in agent_proving_ground.util._concurrency and reset at the
