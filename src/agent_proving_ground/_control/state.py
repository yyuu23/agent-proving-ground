...
- :func:`agent_proving_ground._control.eval_state.get_eval_states` for ``total`` /
  ``completed`` / ``errored`` counters that survive a sample exiting
  ``active_samples``.
- :func:`agent_proving_ground.log._samples.active_samples` for ``in_flight``
...
from agent_proving_ground._util.error import is_cancellation_message
from agent_proving_ground._util.file import local_path

if TYPE_CHECKING:
    from agent_proving_ground._control.eval_state import EvalState
    from agent_proving_ground.log._samples import ActiveSample
...
# `apg ctl sample list`). The listing is otherwise linear in sample
...
async def current_eval_summaries(started_at: float) -> list[dict[str, Any]]:
    ...
    from agent_proving_ground._control.pause import process_paused
...
async def current_sample_summaries(
    ...
    eval-set triage read behind ``apg ctl sample errors``. Filtering
...
def _add_pending_samples(
    ...
    from agent_proving_ground._control.eval_state import get_eval_state
...
async def sample_error_detail(
    ...
    ``apg ctl sample show`` a single atomic read — without this it would
...
def _build_summary(
    *,
    ...
    from agent_proving_ground._control.pause import task_dispatched_count, task_pause_sources
