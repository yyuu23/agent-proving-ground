...
Shaped like :mod:`agent_proving_ground._control.cancel` and runs on the eval's own
...
_FANOUT_DRAINED = (
    ...
    "drained) — re-run failures with `apg eval-retry` (or "
    "re-invoke `apg eval-set`)"
)
...
async def requeue_sample(
    ...
    from agent_proving_ground._control.state import _full_sample, find_active_sample
    ...
    if prior.error is None:
        return _reject(
            ...
            "(use score invalidation and `apg eval-retry` for post-hoc "
            "re-runs)"
        )
...
def _task_level_reject(state: "EvalState") -> RequeueRejected | None:
    ...
    if state.completed_at is not None:
        ...
        return _reject(
            ...
            "eval-retry` (or re-invoke `apg eval-set`)"
        )
    pending_cancel = (
