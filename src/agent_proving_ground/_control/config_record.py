...
The glue between the config appliers (:mod:`agent_proving_ground._control.limits`) and
the log schema (:mod:`agent_proving_ground.log._config_update`): when a ``PATCH``
config directive actually changes something, the applied changes are grouped
into provenance-carrying :class:`~agent_proving_ground.log._config_update.ConfigUpdate`
...
if TYPE_CHECKING:
    ...
    from agent_proving_ground.log._config_update import ConfigUpdate, ConfigValueChange
...
async def record_config_changes(
    *,
    ...
    """
    from agent_proving_ground._control.eval_state import get_eval_states, latest_eval_for_task
