...
exit. Used by both standalone ``apg eval`` and ``inspect
...
from agent_proving_ground._control.buffer import flush_task_samples
from agent_proving_ground._control.cancel import (
    TaskCancelAction,
    cancel_sample,
    cancel_task,
)
from agent_proving_ground._control.discovery import default_socket_path, discovery_dir
from agent_proving_ground._control.events import DEFAULT_PAGE_LIMIT, sample_events
from agent_proving_ground._control.limits import (
...
from agent_proving_ground._util.discovery import (
    prepare_discovery_dir,
    write_discovery_file,
)
from agent_proving_ground._util.error import PrerequisiteError
from agent_proving_ground._util.sockets import (
    lock_socket_file,
    peer_uid,
    prepare_socket_path,
)
...
def resolve_ctl_server(value: bool | str | None) -> CtlServerConfig:
    ...
    - ``"keep"`` — control server on, and the process parks after the eval
      finishes (until ``apg ctl process release`` / ``POST /release``).

    The CLI spellings (``"true"`` / ``"yes"`` / ``"1"``, ``"false"`` /
    ``"no"`` / ``"0"``, case-insensitive) are accepted too, so programmatic
    callers can forward a flag or ``APG_EVAL_CTL_SERVER`` env value
...
# runtime by ``POST /keep`` (``apg ctl process keep`` -> on) and ``POST /release``
# (``apg ctl process release`` -> off). Last-write-wins rather than "release is
...
class ControlServer:
    ...
    def _build_app(self) -> Any:
        ...
        # `agent_proving_ground._cli.ctl`); a handler 404 without the key would
        ...
        # one row keyed by task_id) — the wire behind `apg ctl task list`
        ...
        async def list_tasks() -> list[dict[str, Any]]:
            ...
            # the launch flag) so `apg ctl task list` can report it.
        ...
        async def sample_cancel(
            ...
            # Function-local: a module-level `agent_proving_ground.log` import from
            # this module is circular (`agent_proving_ground` -> `_eval.eval` ->
            # `_control.server`).
        ...
        async def sample_requeue(
            ...
            from agent_proving_ground._control.requeue import requeue_sample
