...
``apg ctl task list`` through the run (and any keep-alive park).
...
if TYPE_CHECKING:
    ...
    from agent_proving_ground._eval.task.scheduler import SampleRequeue
...
class BufferConfig(NamedTuple):
    """A running eval's sample-buffer parameters (see ``apg ctl config``).
...
class EvalState:
    ...
    :func:`agent_proving_ground.log._samples.active_samples` at read time so
    ...
    :mod:`agent_proving_ground._control.cancel`). ``None`` for reused/synthetic evals
    ...
    :mod:`agent_proving_ground._control.requeue` and ``design/ctl/sample-requeue.md``).
    ...
    reachable), but not for external ones — e.g. ``apg score
...
def clear_all_eval_states() -> None:
    ...
    """
    from agent_proving_ground._control.terminal_cache import clear_terminal_source_caches
