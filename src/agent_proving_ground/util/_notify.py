...
NOTIFICATION_ENV_VAR = "APG_EVAL_NOTIFICATION"
...
def build_apprise(config: bool | str | None) -> "apprise.Apprise | None":
    ...
    Args:
        config:
            - `True`: read URL(s) from the `APG_EVAL_NOTIFICATION` env
...
async def notify(message: str, title: str | None = None) -> None:
    ...
    sample context: title becomes `AgentProvingGround Agent: <task>` and the body starts
    with a `sample: <sample_id>/<epoch>` line followed by the message.
    Outside an active sample, the title is just `AgentProvingGround Agent` and the body
    ...
    Args:
        ...
        title: Optional title. Pass `None` to use the default `AgentProvingGround Agent`
...
def _default_title_and_body(message: str) -> tuple[str, str]:
    ...
    if active is None:
        return "AgentProvingGround Agent", message
    sid = active.sample.id if active.sample.id is not None else "?"
    return f"AgentProvingGround Agent: {active.task}", f"sample: {sid}/{active.epoch}\n\n{message}"
