...
from agent_proving_ground._util._async import coro_print_exceptions
    ...
    if not _cleanup_completed.get():
        ...
        if len(shutdown_projects) > 0:
            if cleanup:
                ...
                print(
                    "\n"
                    "Cleanup all containers  : [blue]apg sandbox cleanup docker[/blue]\n"
                    "Cleanup single container: [blue]apg sandbox cleanup docker <container-id>[/blue]",
                    "\n",
                )
