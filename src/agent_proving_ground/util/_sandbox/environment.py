...
class SandboxEnvironment(abc.ABC):
    ...
    async def exec(
        ...
        By default, each output stream (stdout and stderr) is limited to 10 MiB. You can override this by setting the `APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE` environment variable (specified in bytes).
    ...
    async def read_file(self, file: str, text: bool = True) -> Union[str | bytes]:
        ...
        By default, file size is limited to 100 MiB. You may change this by setting the `APG_SANDBOX_MAX_READ_FILE_SIZE` environment variable (specified in bytes). If exceeded, an `OutputLimitExceededError` will be raised.
    ...
    async def exec_remote(
        ...
        *,
        ...
        """
        from agent_proving_ground.tool._sandbox_tools_utils.sandbox import (
    ...
    async def cli_cleanup(cls, id: str | None) -> None:
        """Handle a cleanup invoked from the CLI (e.g. apg sandbox cleanup).
...
def deserialize_sandbox_specific_config(
    ...
    if is_docker_compatible_fn():
        ...
        from agent_proving_ground.util._sandbox.compose import ComposeConfig
