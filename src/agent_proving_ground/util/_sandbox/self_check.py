...
from agent_proving_ground.util._sandbox.limits import override_max_read_file_size

# If you're wondering these tests are not using pytest fixtures,
# see the discussion https://github.com/UKGovernmentBEIS/agent_proving_ground/pull/347
...
async def test_write_text_file_is_directory(
    ...
    await sandbox_env.write_file(
        "/tmp/agent_proving_ground_test_write_text_file_is_directory/file", "unused content"
    )
    with Raises(IsADirectoryError) as e_info:
        await sandbox_env.write_file(
            "/tmp/agent_proving_ground_test_write_text_file_is_directory",
    ...
    await sandbox_env.exec(
        ["rm", "-rf", "/tmp/agent_proving_ground_test_write_text_file_is_directory"]
    )
...
async def test_write_binary_file_is_directory(
    ...
    # ensure /tmp/directory exists
    await sandbox_env.write_file(
        "/tmp/agent_proving_ground_test_write_binary_file_is_directory/file", "unused content"
    )
    with Raises(IsADirectoryError) as e_info:
        await sandbox_env.write_file(
            "/tmp/agent_proving_ground_test_write_binary_file_is_directory",
            b"\xc3\x28",
        )
    assert e_info is not None, "IsADirectoryError should be raised"
    assert "directory" in str(e_info.value), (
    ...
    await sandbox_env.exec(
        ["rm", "-rf", "/tmp/agent_proving_ground_test_write_binary_file_is_directory"]
    )
...
async def test_exec_as_user(sandbox_env: SandboxEnvironment) -> None:
    username = "agent-proving-ground-test-exec-as-user"
