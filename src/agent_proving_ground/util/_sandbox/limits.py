...
def set_sandbox_limits() -> list[Token[int]]:
    ...
    Override limits using the `APG_SANDBOX_MAX_READ_FILE_SIZE` and `APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE`.

    Returns tokens that can be passed to `reset_sandbox_limits()` to restore
    previous values.
    """
    tokens: list[Token[int]] = []
    max_read_file_size = os.getenv("APG_SANDBOX_MAX_READ_FILE_SIZE", None)
    if max_read_file_size is not None:
        tokens.append(
            _max_read_file_size_var.set(
                _parse_limit_env_var(
                    "APG_SANDBOX_MAX_READ_FILE_SIZE", max_read_file_size
                )
            )
        )
    max_exec_output_size = os.getenv("APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE", None)
    if max_exec_output_size is not None:
        tokens.append(
            _max_exec_output_size_var.set(
                _parse_limit_env_var(
                    "APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE", max_exec_output_size
                )
            )
        )
    return tokens
