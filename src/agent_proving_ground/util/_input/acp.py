...
(:func:`agent_proving_ground.util._input.builtin._dispatch_builtin`) falls through to
...
:mod:`agent_proving_ground.approval._human.acp` one-for-one: same
...
:class:`~agent_proving_ground.agent._acp.transport.AcpTransport`; this module
just consumes them.
...
from agent_proving_ground.agent._acp.transport import ElicitationRequest

from ._types import InputRequest, InputResult

if TYPE_CHECKING:
    from agent_proving_ground.agent._acp.transport import ElicitationClient
...
class _ElicitationRoutingSession(Protocol):
    ...
    :mod:`agent_proving_ground.approval._human.acp`.
    """
...
async def _request_from_driver_with_fallback(
    ...
    Mirrors :func:`agent_proving_ground.approval._human.acp._request_from_driver_with_fallback`
