...
from agent_proving_ground.util._notify import notify

from . import builtin
from ._types import InputRequest, InputResult

if TYPE_CHECKING:
    from agent_proving_ground.event._input import InputField
...
def _record_input_event(request: InputRequest, result: InputResult) -> None:
    # Deferred imports: pulling `agent_proving_ground.event` at module-load time triggers
    # a circular import (event -> scorer -> log -> condense -> event).
    from agent_proving_ground.event._input import InputEvent, InputField
