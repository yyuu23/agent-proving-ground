...
1. `span()` (`src/agent_proving_ground/util/_span.py`) is an `@asynccontextmanager`. Its
...
(`BaseEvent.model_post_init`, `src/agent_proving_ground/event/_base.py:48`) — from the
...
- Cross-task rotation: enter `span_session()` in task A; spawn task B (after
  ...
  attach `caplog.handler` to the `agent_proving_ground.util._span` logger, module has
