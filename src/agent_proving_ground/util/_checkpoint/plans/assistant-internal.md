...
- Layering OK: `util/_checkpoint` already imports `agent_proving_ground.model._chat_message` (`host_context.py`, `hydrate.py`); it must never import `_eval` → aggregator moves out of `run.py`.
...
### 1. New `src/agent_proving_ground/model/_assistant_internal.py`
