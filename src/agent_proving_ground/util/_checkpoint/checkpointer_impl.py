...
of ``agent_proving_ground`` that ultimately reach ``solver._task_state`` and
``dataset.Sample``, so this module must *not* be imported during
initial agent_proving_ground package load — only at sample-run time, via the
...
from agent_proving_ground._util.file import write_text_atomic
...
from agent_proving_ground.model._assistant_internal import (
...
from agent_proving_ground.util._checkpoint.report import ResumeReport, resolve_resume_report
from agent_proving_ground.util._restic import (
    ResticBackupSummary,
    list_changed_files,
    run_backup,
)
from agent_proving_ground.util._sandbox.context import sandbox
from agent_proving_ground.util._span import SpanRotationScope
...
class _EnteredCheckpointer:
    ...
    async def _fire_once(
        ...
        *,
        ...
        # observable live (`apg trace anomalies` Running Actions) and a
