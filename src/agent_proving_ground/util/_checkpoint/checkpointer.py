...
from ``agent_proving_ground.util._checkpoint/__init__.py`` and ``agent_proving_ground.util``
without triggering the heavy ``log``/``solver``/``model`` chain that
the active-session implementation in :mod:`.checkpointer_impl` pulls
in (that chain loops back to ``agent_proving_ground.dataset.Sample`` and breaks
...
class Checkpointer(Protocol):
    ...
    def restored(self) -> ResumeReport | None:
        ...
        AgentProvingGround does not surface it to the model; delivering it is the agent's
...
async def checkpointer() -> AsyncIterator[Checkpointer]:
    ...
    # `agent_proving_ground.log._samples`.
