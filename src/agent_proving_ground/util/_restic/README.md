...
1. Set the new version in `src/agent_proving_ground/util/_restic/version.txt`.
2. Download the official checksums:
   ```bash
   VERSION=$(cat src/agent_proving_ground/util/_restic/version.txt)
   curl -fsSL -o src/agent_proving_ground/util/_restic/SHA256SUMS \
...
3. (Recommended) Verify authenticity before trusting the file:
   ...
   gpg --verify /tmp/restic-SHA256SUMS.asc src/agent_proving_ground/util/_restic/SHA256SUMS
   ```
...
5. Commit both files together:
   ```bash
   git add src/agent_proving_ground/util/_restic/version.txt src/agent_proving_ground/util/_restic/SHA256SUMS
