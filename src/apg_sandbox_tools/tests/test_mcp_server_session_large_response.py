...
The fix is twofold:
  1. The reader's line limit defaults to 256 MiB (overridable via
     APG_MCP_READLINE_LIMIT_BYTES), so realistic large responses just work.
...
import pytest
from apg_sandbox_tools._remote_tools._mcp import (
    mcp_server_session as session_module,
)
from apg_sandbox_tools._remote_tools._mcp.mcp_server_session import (
    MCPServerSession,
)
...
async def test_create_wires_readline_limit_into_subprocess(
    ...
    from apg_sandbox_tools._remote_tools._mcp import mcp_server_session
