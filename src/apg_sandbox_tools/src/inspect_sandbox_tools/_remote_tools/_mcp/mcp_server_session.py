...
from apg_sandbox_tools._util.process_tree import (
...
# APG_MCP_READLINE_LIMIT_BYTES. We size this comfortably above the host-side
...
_READLINE_LIMIT: int = (
    int(os.environ["APG_MCP_READLINE_LIMIT_BYTES"])
    if "APG_MCP_READLINE_LIMIT_BYTES" in os.environ
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # APG_MCP_READLINE_LIMIT_BYTES if needed.
                    raise RuntimeError(
                        "MCP server response line exceeded the read limit of "
                        f"{_READLINE_LIMIT} bytes. Set APG_MCP_READLINE_LIMIT_BYTES "
