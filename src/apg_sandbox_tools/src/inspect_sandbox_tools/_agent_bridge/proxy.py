...
# Mirror of agent_proving_ground.agent._bridge._errors.PROVIDER_ERROR_KEY. The proxy is a
# separate shipped binary that cannot import agent_proving_ground, so the constant is
...
def _handle_model_proxy_error(ex: Exception) -> None:
    ...
    #
    #  - The call to generate (which already benefits from AgentProvingGround's std
    #    model retry behavior). In normal AgentProvingGround agents if generate fails
