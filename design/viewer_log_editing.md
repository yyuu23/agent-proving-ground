...
`agent_proving_ground.log` and lays groundwork for matching UI affordances.
...
- Concurrency control for local-filesystem logs. S3 conditional writes are
  ...
  single-user `apg view` case is the dominant local one and last-writer-wins
  is acceptable there.
...
(`agent_proving_ground.log._edit.LogUpdate`): a discriminated union of `TagsEdit` /
...
Wraps `edit_score` from `agent_proving_ground.log._score`:
...
| File | Change |
|------|--------|
| `src/agent_proving_ground/_view/common.py` | shared `apply_log_edits` (read → `edit_eval_log` → write), ETag plumbing |
| `src/agent_proving_ground/_view/server.py` | aiohttp: `POST /api/log-edit/{log}`, `If-Match` → `WriteConflictError` → 412 |
| `src/agent_proving_ground/_view/fastapi_server.py` | FastAPI: `POST /log-edit/{log:path}`, `AccessPolicy.can_write`, `OnlyDirAccessPolicy.can_write` |
| `src/agent_proving_ground/_view/user_info.py` | new — `GET /api/user-info` returning the git alias (email local-part → user.name → OS login) so the viewer can prefill `provenance.author` |
| `src/agent_proving_ground/log/_edit.py` | fix: no-op filter for `MetadataEdit` tests key presence separately so null-valued additions aren't dropped |
