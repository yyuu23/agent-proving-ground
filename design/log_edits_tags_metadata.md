...
| File | Change |
|------|--------|
| `src/agent_proving_ground/_eval/task/task.py` | Add `tags` parameter to `Task.__init__()` |
| `src/agent_proving_ground/_eval/run.py` | Merge `tags` and `task.tags` (line 219) |
...
```python
# In src/agent_proving_ground/log/_edit.py
...
```python
# In src/agent_proving_ground/log/_log.py, on EvalLog
...
See `EvalLog.recompute_tags_and_metadata()` in [`_log.py`](../src/agent_proving_ground/log/_log.py). It runs automatically on construction/deserialization (via `@model_validator`) and is called explicitly by `edit_eval_log()` after appending a new `LogUpdate`.
...
Add to `agent_proving_ground.log.__init__` exports:
...
| File | Change |
|------|--------|
| `src/agent_proving_ground/log/_log.py` | Add `log_updates` field, `tags` and `metadata` properties to `EvalLog` |
| `src/agent_proving_ground/log/_edit.py` | Add `LogEdit`, `TagsEdit`, `MetadataEdit`, `LogUpdate`, `ProvenanceData`, `edit_eval_log` |
| `src/agent_proving_ground/log/__init__.py` | Export new types and functions |

| `src/agent_proving_ground/_view/www/src/@types/log.d.ts` | Auto-regenerated |
| `src/agent_proving_ground/_view/www/src/app/log-view/tabs/TaskTab.tsx` | Display effective tags/metadata |
| `src/agent_proving_ground/analysis/_dataframe/evals/columns.py` | Use `log.tags` / `log.metadata` for DataFrame columns |
