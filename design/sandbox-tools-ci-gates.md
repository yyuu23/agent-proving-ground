...
2. **Merging injectable source changes that haven't been published.** If `src/apg_sandbox_tools/` changes but `sandbox_tools_version.txt` isn't bumped (and the new `v{N}` isn't uploaded to S3), the next PyPI wheel build will either fail or bundle the wrong binary, and editable installs on `main` will S3-download a binary that doesn't match the source tree. The gates force the bump and verify the S3 upload before the PR can land.

## Fast unit gate

Separate from the slow integration gates below, the **`sandbox-tools-unit`** job runs the package's own fast, self-contained unit tests (`src/apg_sandbox_tools/tests/agent_bridge/`, notably the model proxy) on any change under `src/apg_sandbox_tools/**` — including test-only changes. It installs the package as its own project (`uv pip install "./src/apg_sandbox_tools[dev]"`) so pytest resolves that package's `pyproject.toml` as config (`asyncio_mode=auto`); these tests can't run under the root `test` job, whose anyio conftest blocks `@pytest.mark.asyncio`. It needs no Docker or built binary, so it gives fast feedback on proxy-logic regressions that the slow integration gates (which require a binary + version bump) would not.
