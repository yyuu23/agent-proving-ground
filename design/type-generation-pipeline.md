...
How `agent_proving_ground` Pydantic models become TypeScript types, and every
...
default value — essentially `required = !has_default`. But `agent_proving_ground`
...
`agent_proving_ground` serializes with `exclude_none=True` as standard (`to_json_safe` →
...
contains duplicates of `agent_proving_ground` types (unavoidable from FastAPI transitive
...
identical shapes. Scout's barrel re-exports `agent_proving_ground` types from
