# Deep Agent Harness for AgentProvingGround

## Design

We're planning on adding a `deepagent()` to AgentProvingGround as a peer to `react()`. This would be an alternate top-level agent that bundles the patterns popularized by Claude Code, OpenAI Codex CLI, LangChain deepagents, and Pydantic AI deepagents into a single batteries-included entry point. This RFC describes the proposed design and asks for community feedback before we commit to an API.
...
| **AgentProvingGround (proposed)** | `research`, `plan`, `general`             |
...
``` python
from agent_proving_ground.agent import subagent
...
Subagents support two dispatch modes, corresponding to AgentProvingGround's existing `as_tool()` and `handoff()` primitives:
...
- **Sandbox** — shared (already true per-sample in AgentProvingGround).
...
AgentProvingGround's `span(type="agent")` already renders subagent work as swimlanes in the log viewer. The `agent()` tool dispatch will open a span automatically — no new viewer work needed. Limits (`token_limit`, `message_limit`, `time_limit`, `cost_limit`) apply globally across the parent and all subagent excursions.
...
The subsections below analyze the system prompts shipped by Claude Code, Codex CLI, LangChain deepagents, and Pydantic AI deepagents — what they share, where they diverge, and implications for AgentProvingGround.
...
##### Implications for AgentProvingGround

AgentProvingGround's `deepagent()` prompt must navigate two tensions the coding assistants don't face:

1. **Task-agnostic.** CC and Codex can hard-code git workflows and file-editing conventions because they know they're coding tools. AgentProvingGround agents run evals that might involve coding, research, math, web browsing, or domain-specific tool use. The prompt cannot assume the task domain.

2. **Model-agnostic.** AgentProvingGround must work well with models at very different levels of agentic post-training — from frontier models that have been heavily tuned for agentic tool use to models with minimal agentic post-training that need guidance. The prompt cannot assume a specific level of model capability.
...
Key design principles for AgentProvingGround's system prompt:
...
5. **Don't embed domain knowledge.** CC's git safety rules and Codex's editing constraints are valuable for coding — but they belong in the user's `instructions=` parameter or in tool-level descriptions, not in the base prompt. AgentProvingGround's prompt should teach the *pattern* (plan, delegate, verify) without assuming what the tools do.
...
**AgentProvingGround status:** AgentProvingGround already bounds tool output via `max_tool_output` (default 16 KiB), which prevents runaway tool results from saturating context. `deepagent()` v1 should lean into this existing behavior: read-only filesystem tools should support explicit ranges/limits, `grep()` should be bounded, and truncation messages should make it clear how to retrieve narrower context. AgentProvingGround also has compaction infrastructure (`CompactionEdit`, `CompactionTrim`, `CompactionSummary`, native provider compaction) wired through `react()`. `deepagent()` defaults to `CompactionAuto` (native-first with summary fallback), so compaction is active out of the box. Users can override with a specific strategy or pass `compaction=None` to disable. Durable large-output eviction can be considered later if capped outputs prove too lossy, but it is not a v1 requirement.
...
**AgentProvingGround status:** Shipped. The `agent` tool gained a `background` parameter: `agent(subagent_type, prompt, background=True)` returns an `AGENT-N` handle immediately and runs the child concurrently on the sample task group (`agent_proving_ground.util.background()`), rather than blocking until the child returns.
...
**AgentProvingGround status:** Already supported — `subagent(model=...)` and `research(model=...)` etc. allow per-subagent model selection. Each built-in subagent has different cognitive demands (e.g., `research()` might use a cheaper model, `plan()` a stronger one, `general()` the main agent model), so routing is best configured per-subagent rather than with a blanket default.

#### Limits and budgets

Long-horizon agents need hard resource limits: message counts, tokens, wall-clock time, working time, and cost. LangChain and Pydantic expose these through middleware/capabilities; AgentProvingGround already has them as core task and agent primitives.

**AgentProvingGround status:** Covered. Sample-level limits apply across the parent and all subagent excursions, and `subagent(limits=[...])` can apply additional scoped limits to a child agent. This gives authors both a global budget for the whole sample and per-subagent budgets for expensive or exploratory workers.
...
**AgentProvingGround status:** AgentProvingGround's `react()` loop already handles tool execution errors, but orphaned-call repair is a resilience feature worth considering. If models occasionally produce tool calls that fail silently, the conversation history becomes inconsistent. Lower priority but worth noting for robustness.

#### Checkpointing integration

The separate [AgentProvingGround Checkpointing design](../design/checkpointing.md) proposes mid-sample durability for long-running evals. `deepagent()` is a primary consumer of this capability — deep agent evals are exactly the ones that run for hours or days and need crash recovery.

**AgentProvingGround status:** The checkpointing design is proceeding in parallel. `deepagent()` should accept a `checkpoint=CheckpointConfig(...)` parameter, consistent with `react()`. The integration is straightforward — checkpoints fire at turn boundaries in the agent loop, which `deepagent()` inherits from `react()`.
...
**AgentProvingGround status:** AgentProvingGround does not have a middleware system for agents, and adding one would be a significant new abstraction. However, many of the use cases that middleware serves in other frameworks are already covered by AgentProvingGround's existing primitives: compaction handles context management, spans handle observability, limits handle resource control, and tool-level logic handles approval. We should not add middleware to `deepagent()` — it would add complexity without clear benefit given AgentProvingGround's existing architecture.
...
**AgentProvingGround status:** Less relevant for eval workloads, where the goal is typically autonomous execution. AgentProvingGround's sandbox isolation provides the safety boundary. If needed, users can implement approval logic at the tool level. Not recommended for `deepagent()` defaults.
...
``` python
from agent_proving_ground.agent import subagent
from agent_proving_ground.util import token_limit
...
New exports from `agent_proving_ground.agent`:
...
New tools in `src/agent_proving_ground/tool/_tools/`:
...
Modified files:
- `src/agent_proving_ground/agent/__init__.py` — add new exports.
- `src/agent_proving_ground/tool/_tools/__init__.py` — add new tool exports.
- `src/agent_proving_ground/tool/_tools/_update_plan.py` — deprecate with alias to `todo_write()`.
- `src/agent_proving_ground/tool/_tools/_memory.py` — add `readonly=True` mode that exposes only read operations.

#### 8. Parallel tool execution (shipped)

`execute_tools` (`src/agent_proving_ground/model/_call_tools.py`) partitions a turn's tool calls into ordered stages: consecutive `parallel=True` calls coalesce into one concurrent stage, while each serial call is its own one-element stage that acts as a barrier preserving the model's declared ordering between stateful and stateless calls. This benefits all agents — including `react()` — and enables concurrent subagent dispatch for `deepagent()` (the `agent()` tool is `parallel=True` when its subagents are parallel-safe).
...
To complement execution, the system prompt encourages the model to actually emit independent tool calls together — see `PARALLEL_TOOLS_PROMPT` in `src/agent_proving_ground/agent/_types.py`, shared by `react()`'s `DEFAULT_ASSISTANT_PROMPT`, `deepagent()`'s `CORE_BEHAVIOR`, and the dispatched subagents' assistant prompt.
...
**Files created:**
- `src/agent_proving_ground/agent/_deepagent/__init__.py` — package exports.
- `src/agent_proving_ground/agent/_deepagent/subagent.py` — `Subagent` dataclass (`@dataclass(kw_only=True)`) and `subagent()` factory with input validation.
...
**Files modified:**
- `src/agent_proving_ground/agent/__init__.py` — added `Subagent` and `subagent` to public API.
...
**Files created:**
- `src/agent_proving_ground/tool/_tools/_todo_write.py` — `TodoStep` model (with `content`/`status` fields, status as `Literal` enum) and `todo_write()` tool using standard `@tool` pattern.
- `tests/tools/test_todo_write.py` — basic + mockllm integration tests.

**Files modified:**
- `src/agent_proving_ground/tool/__init__.py` — added `todo_write` export.
...
**Files created:**
- `src/agent_proving_ground/tool/_tools/_read_file.py` — `read_file()` tool using `SandboxEnvironment.read_file()` with line numbers and `offset`/`limit` pagination.
- `src/agent_proving_ground/tool/_tools/_list_files.py` — `list_files()` tool using `find` with optional `depth` control.
- `src/agent_proving_ground/tool/_tools/_grep.py` — `grep()` tool using `grep -rn` with `glob` filter, `fixed_strings`, and `output_mode` ("content" / "files_with_matches" / "count").
- `tests/tools/test_read_file.py`, `test_list_files.py`, `test_grep.py` — constructibility + Docker integration tests.

**Files modified:**
- `src/agent_proving_ground/tool/__init__.py` — added `read_file`, `list_files`, `grep` exports.
- `docs/tools-standard.qmd` — added Read-Only Tools section with examples.
- `docs/reference/agent_proving_ground.tool.qmd` — added reference entries.
...
**Files modified:**
- `src/agent_proving_ground/agent/_deepagent/__init__.py` — added `task_tool` export (internal only).
...
**Files created:**
- `src/agent_proving_ground/agent/_deepagent/research.py` — `research()` factory with task-agnostic prompt, `tools="default"` → `None` (resolved at dispatch time to read-only tools if sandbox available).
- `src/agent_proving_ground/agent/_deepagent/plan.py` — `plan()` factory, same pattern.
- `src/agent_proving_ground/agent/_deepagent/general.py` — `general()` factory, `memory="readwrite"`, inherits parent tools.
...
**Files modified:**
- `src/agent_proving_ground/agent/_deepagent/__init__.py` — added `research`, `plan`, `general` exports.
- `src/agent_proving_ground/agent/__init__.py` — added public exports.
- `docs/reference/agent_proving_ground.agent.qmd` — added Deep Agent section with `subagent`, `Subagent`, `research`, `plan`, `general`.
- `src/agent_proving_ground/agent/_deepagent/task_tool.py` — forked dispatch now uses `timeline_branch()` for proper log viewer rendering; removed `content_only` filtering on forked input to preserve prompt cache and avoid incompatible format errors.
- `src/agent_proving_ground/agent/_deepagent/subagent.py` — removed fork+model validation (documented guidance instead); updated fork docstrings across all factories to recommend same model/family for cache and compatibility.
...
**Files created:**
- `src/agent_proving_ground/agent/_deepagent/prompt.py` — String constants (`CORE_BEHAVIOR`, `MEMORY_INSTRUCTIONS`, `MEMORY_ONLY_INSTRUCTIONS`, `PLAN_ONLY_INSTRUCTIONS`) plus `build_system_prompt()` for layered assembly, `build_subagent_dispatch()` for dynamic subagent listing, and `expand_prompt_placeholders()` for the `prompt=` escape hatch using `str.replace()`.
...
**Files modified:**
- `src/agent_proving_ground/agent/_deepagent/task_tool.py` — `_resolve_tools` now provides sandbox-aware default read-only tools when `sa.tools is None`, injects `memory()`/`memory(readonly=True)` based on `sa.memory`, and passes `compaction=sa.compaction` to child `react()`.
- `src/agent_proving_ground/agent/_deepagent/subagent.py` — added `compaction: CompactionStrategy | None = None` field.
- `src/agent_proving_ground/agent/_deepagent/__init__.py` — added `deepagent` export.
- `src/agent_proving_ground/agent/__init__.py` — added `deepagent` to public API.
- `docs/reference/agent_proving_ground.agent.qmd` — added `### deepagent` to Deep Agent section.
...
- `agent_proving_ground.agent`: `deepagent`, `subagent`, `Subagent`, `research`, `plan`, `general` all exported.
- `agent_proving_ground.tool`: `read_file`, `list_files`, `grep`, `todo_write` all exported. `update_plan` still exported (undocumented backward compat).
- `task_tool` is internal only (not in public API, no naming collision with `@task` decorator — separate registries).
- `docs/reference/agent_proving_ground.agent.qmd`: Deep Agent section with all entries.
- `docs/reference/agent_proving_ground.tool.qmd`: read_file, list_files, grep entries added.
...
- Update agent_proving_ground.agent.qmd with reference sections.
...
**Files renamed:**
- `src/agent_proving_ground/agent/_deepagent/task_tool.py` → `agent_tool.py`
