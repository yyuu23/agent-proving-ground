...
- **Location**: `~/.local/share/agent_proving_ground/samplebuffer/<log_dir_hash>/<filename>.<pid>.db`
...
When invoked with no arguments, `apg log recover` launches a Textual TUI that:
...
```
apg log recover <log_file> [--output <path>] [--no-cleanup]
```
...
```
apg log recover --list [--log-dir <path>] [--json]
```
...
1. Call `timeline_build(events)` (from `agent_proving_ground.event`) to build a hierarchical timeline from the flat event list — this discovers the "main" trajectory
...
`eval_set()` (`src/agent_proving_ground/_eval/evalset.py`) orchestrates multiple task/model combinations with built-in retry, progress tracking, and resumability. It's the recommended way to run complex evaluations.
...
**Buffer DB cleanup:** Automatic recovery passes `cleanup=False`, so the buffer DB is preserved for post-mortem debugging. The user can later run `apg log recover <started_log> --output <elsewhere>` to produce a recovered log for investigation. The 3-day TTL handles eventual cleanup of the buffer DB.
...
`eval_retry` (`src/agent_proving_ground/_eval/eval.py:808`) re-runs an evaluation, reusing completed samples from a previous log and re-running only failed/cancelled/invalidated ones. The key mechanism is `eval_log_sample_source()` (`run.py:1354-1422`):
...
```bash
apg log recover crashed.eval           # → crashed-recovered.eval (status="error")
apg eval-retry crashed-recovered.eval  # → new log re-running cancelled/failed samples
```
...
**Benefits:**
- Users don't need to know about `apg log recover` — retry "just works" on crashed logs
- Maximizes sample reuse: buffer DB samples that completed before the crash are recovered and reused, not re-run
- Single command: `apg eval-retry crashed.eval` handles everything
...
Created `src/agent_proving_ground/log/_recover/` package with:
...
`db_dir` is kept internal to `read_buffer_recovery_data()` for testing only — not exposed in the public API. Functions exported at `agent_proving_ground.log` level.
...
### Step 6: `apg log recover` CLI (direct and list modes) (done: `4deac0eef`)

Added `recover` subcommand to the `log` Click group in `src/agent_proving_ground/_cli/log.py`:
- **Direct mode**: `apg log recover <file> [--output <path>] [--no-cleanup]` — recovers a single log, prints sample count and output path
- **List mode**: `apg log recover --list [--json]` — lists recoverable logs in a rich table with columns: file, task, total samples, flushed, completed, in-progress counts. `--json` outputs JSON instead.
...
**Automatic recovery in `eval_retry_async`** (`src/agent_proving_ground/_eval/eval.py`):
...
**Post-recovery retry suggestion** (`src/agent_proving_ground/_cli/log.py`):
- After `apg log recover`, counts failed/cancelled samples and prints:
  ```
...
**Sync/async refactor** (`src/agent_proving_ground/log/_recover/_api.py`):
...
**Eval set integration** (`src/agent_proving_ground/_eval/evalset.py:as_previous_tasks`):
...
- Users can investigate crashes via `apg log list --status started` → `apg log recover <file> --output <elsewhere>`
