...
Allow callers of `eval_set` (and `eval`) to attach one or more `inspect_scout` scanners that run against each sample's transcript as the sample completes. Scanner results are persisted into a scout-format scan directory under the eval log dir, not embedded in the agent_proving_ground `EvalLog`.

A central design constraint: **the on-disk scan directory should match scout's standalone-`scout scan` layout exactly.** A scan dir produced by an agent_proving_ground eval_set must be indistinguishable from one produced by `scout scan`, so scout's tooling — `scan_resume`, `scan_complete`, the viewer, parquet readers — works unchanged. The eval_set-specific behavior is layered on top: lifecycle is driven by the eval_set call boundaries instead of scout's own CLI, and the per-sample dispatch loop runs alongside the eval rather than reading transcripts back from logs.

`inspect_scout` remains an **optional** dependency. Importing agent_proving_ground must not require inspect_scout. Only callers that pass a non-`None` `scanner` cause scout to be imported.
...
`apg eval`, `apg eval-set`, and `apg eval-retry` accept the same scanner flags (formats mirror `scout scan`):
...
The shared `scanner_options` click decorator (in `agent_proving_ground/_cli/eval.py`) defines this option set once; `eval_options` (used by `eval`/`eval-set`) and `eval-retry` apply it. All three commands use the `EvalCommand` (a `SectionedCommand`) so the options appear under a "Scanner Options:" section in `--help`.
...
3. the `NoModel` sentinel (`agent_proving_ground.model._providers.none.NoModel`) — its `.generate(...)` raises `PrerequisiteError`.
...
The state of the scan dir on disk is what scout's tooling reads, and it's where the agent_proving_ground/scout consistency invariant lives.
...
- `scout scan-resume <scan_dir>` works on an agent_proving_ground-produced scan dir to retry failed scans.
- `scout scan-complete <scan_dir>` works to materialize a buffer-only scan that crashed before finalize.
- The scout viewer reads agent_proving_ground-produced scan dirs without modification.

The only agent_proving_ground-specific behavior is *who triggers* `scan_init` / `record` / `sync` and *when* — eval_set's per-sample call boundaries instead of scout's CLI lifecycle. The data shape is unchanged.
...
2. **Build a one-row table** using scout's `TranscriptColumns` schema. `_filter_row` calls agent_proving_ground's `import_record` twice — once with a synthesized `EvalLog` for eval-level columns (`model`, `task_set`, `eval_metadata`, …), once with the `EvalSample` for sample-level columns (`task_id`, `total_tokens`, `sample_metadata`, …). JSON columns are JSON-encoded by scout's column `value` callbacks so `json_extract` works.
...
The tab hosts scout's standard scan panel — the same `scan_panel(spec, summary, progress)` renderable scout's standalone `scout scan` command shows. agent_proving_ground populates it via a push-based state module (`_eval/task/scan_display.py`):
...
3. `RecorderBuffer` now shares an in-memory `Summary` + `anyio.Lock` per buffer dir across every buffer instance in the process (`_buffer_states` cache). Concurrent ephemeral writers (agent_proving_ground's per-sample dispatch) coordinate through the lock without re-reading `_summary.json` on each record; `cleanup_buffer_dir` invalidates the cache.
...
### 4. Standalone retry — `eval_retry` / `apg eval-retry`
...
| `eval_retry(scanner=...)` / `apg eval-retry --scanner …` | Reuses original scan dir via `scan_id = eval_set_id or run_id`. Verifies scanner config unchanged; new log per retry; rows from all logs preserved (no orphan cleanup of historical logs) |
| Sample's scan errored (parquet `scan_error` populated) | Treated as intentional, not retried by agent_proving_ground. `scout scan-resume` is the path for retrying captured scan errors |
...
- Embedding scanner results into the agent_proving_ground `EvalLog`.
- A unified `apg view` rendering of scan results alongside eval logs.
