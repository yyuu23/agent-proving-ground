...
> **Status: phases 1–2 implemented** (schema + read path + control-channel wiring, and the `--reason`/`--author`/`persisted` surface polish; viewer rendering shipped in ts-mono — effective-value fold with a "changed" affordance in the config card, updates on the timeline). Phases 3–4 (per-sample limit knobs, sample-start stamping) remain future work; retry adoption of retuned values is decided against absent customer demand (open question 1). Design for [meridianlabs-ai/agent_proving_ground#82](https://github.com/meridianlabs-ai/agent_proving_ground/issues/82). Builds on the shipped `apg ctl config` surface described in [control-channel.md](control-channel.md) (phase 3).

## Problem

`apg ctl config` retunes a running eval's launch configuration mid-flight — concurrency caps, log-buffer parameters, the retry-loop overrides, and (planned) the per-sample time / token / message limits. Every one of these changes is applied **purely in memory**: concurrency knobs resize live limiter objects (`src/agent_proving_ground/_control/limits.py`), the retry knobs land in a process-global override dict (`src/agent_proving_ground/model/_generate_overrides.py`), and the buffer knobs reconfigure the live recorder. All of it is deliberately reset at the outermost run boundary (`reset_run_registries()` in `src/agent_proving_ground/_control/eval_state.py`) and none of it is written anywhere durable.
...
- **The retry knobs are consulted at point-of-use, not merged into any `GenerateConfig`** (`_generate_overrides.py`), and they're deliberately excluded from eval-set task identity (`_GENERATE_CONFIG_FIELDS_TO_EXCLUDE` in `src/agent_proving_ground/_eval/evalset.py`).
...
The log has no mechanism for this. The relevant facts (see `src/agent_proving_ground/log/_log.py` and `src/agent_proving_ground/log/_recorders/`):
...
5. **Readable header-only.** Consumers (viewer, `apg log`, agents) must see the updates without reading samples.
...
A new module (natural home: alongside `_edit.py`, e.g. `src/agent_proving_ground/log/_config_update.py`):
...
"""Mid-run configuration changes applied via the control channel (apg ctl)."""
```
...
**D. A sidecar file next to the log** (e.g. `<log>.updates.json`). Rejected: logs are copied, moved, and served (S3, `apg view` bundles) as single files; a sidecar detaches in every one of those flows. The `.eval` format exists precisely so run artifacts travel as one zip.
...
4. **Consumer policy follow-ups**: sample-start limit stamping ([meridianlabs-ai/agent_proving_ground#223](https://github.com/meridianlabs-ai/agent_proving_ground/issues/223)). (Retry adoption of retuned values is decided against absent customer demand — see the `eval_retry` bullet and open question 1.)
