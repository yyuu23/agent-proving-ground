...
by a control-channel incident (meridianlabs-ai/agent_proving_ground#118). Audited at
`ef5fcba34` (2026-07-21), covering `src/agent_proving_ground/_control/` (`server.py`,
...
(meridianlabs-ai/agent_proving_ground#116; see `_BufferedSample` in
...
`EvalState.log_sample_summaries` memo — meridianlabs-ai/agent_proving_ground#154);
...
2. **Every terminal read of a `.json`-format log re-parses and/or
   ...
   meridianlabs-ai/agent_proving_ground#116 fixed the *live* `.json` path
...
3. **Finished/reused evals re-read summaries from the log on every listing
   ...
   **Fixed** (meridianlabs-ai/agent_proving_ground#154):
...
- **Pile-up guard.** `uvicorn limit_concurrency` rejects (503) rather than
  ...
  [meridianlabs-ai/agent_proving_ground#225](https://github.com/meridianlabs-ai/agent_proving_ground/issues/225).
...
- **Disconnect check.** An `await request.is_disconnected()` before
  ...
  [meridianlabs-ai/agent_proving_ground#226](https://github.com/meridianlabs-ai/agent_proving_ground/issues/226).
