# Making `apg ctl` discoverable to agents (without a skill teaching it)

Design for [meridianlabs-ai/agent_proving_ground#128](https://github.com/meridianlabs-ai/agent_proving_ground/issues/128).
...
`apg eval` holds the exact object `ctl` operates on, the control server is
default-on (`resolve_ctl_server` at `src/agent_proving_ground/_eval/eval.py:966`), and
...
`src/agent_proving_ground/_eval/eval.py:1132`; eval-set:
`src/agent_proving_ground/_eval/evalset.py:942`), which fire only when keep-alive was
requested (`--ctl-server=keep` or a runtime `apg ctl process keep` /
...
| 1b | `apg eval --json` startup output with a `control` block | **Shipped** — nothing to do |
| 1c | Observe command in `--ctl-server` flag help | **Open** (help names `apg ctl` and the release command but no observe command) — designed below |
...
**1b — `apg eval --json` (and beyond).** Shipped as the "launch handoff"
slice described in control-channel.md ("The launch handoff is load-bearing"):
`--json` on `eval` / `eval-set` / `eval-retry` (`src/agent_proving_ground/_cli/eval.py:933`,
...
is advertised in `apg ctl --help`
(`src/agent_proving_ground/_cli/ctl.py:286`). The issue's `suggestion` field was not
...
`apg ctl process keep` when keep-alive is off (`ctl.py:_print_keep_alive_footer`),
...
```
Monitor from another shell: apg ctl task list   (apg ctl --help)
```

**Where.** The bind site in `_eval/eval.py`, immediately after the
`control_server(...)` context manager yields and alongside
`emit_launch_handoff` (`src/agent_proving_ground/_eval/eval.py:994-1010`) — a site
that knows the server actually bound, before any task work begins.
Not the task startup panel (`src/agent_proving_ground/_display/core/panel.py:109`'s
...
is the full-screen textual app (`src/agent_proving_ground/_display/core/active.py:24-34`),
...
keep-alive park (`_keep_alive_park`, `src/agent_proving_ground/_eval/evalset.py:919`)
...
(`apg ctl task list` shows the reused EvalStates registered by
...
(see the module docstring of `src/agent_proving_ground/_eval/handoff.py`), and it uses
...
suppression, but the primary audience — an agent driving `apg eval`
...
line names `task list` rather than the bare `apg ctl` because a runnable
command that produces useful output on first invocation teaches more than a
help pointer alone; `apg ctl --help` rides along for the surface map.

### 1c. Observe command in `--ctl-server` help

The flag help (`src/agent_proving_ground/_cli/eval.py:444-453`) names the `apg ctl`
CLI and `apg ctl process release` but never the observe command. Append
...
"Evaluate tasks.", `src/agent_proving_ground/_cli/eval.py:952`). Add one line to each of
...
The `ctl` subcommand docstrings (`src/agent_proving_ground/_cli/ctl.py`) explain
...
- `task_list_command`: `Example: apg ctl task list --json`
- `sample_list_command`: `Example: apg ctl sample list my-task --json`
- `sample_events_command`: `Example: apg ctl sample events my-task sample-1 --tail 20`
- `config_command`: `Example: apg ctl config --max-connections 20 --dry-run`
...
```
N samples errored — see `apg ctl sample errors`
```
...
while `apg ctl sample errors` also lists retried samples. So the view may
...
- CHANGELOG: one line, e.g. "`apg eval` now prints a pointer to
  `apg ctl` monitoring at launch, and eval/ctl help text cross-links the
  two surfaces."
