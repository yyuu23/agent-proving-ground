...
Part of a broader effort to make AgentProvingGround a first-class platform for **LLM-agent-driven eval workflows** — agents like Claude Code launching evals, monitoring them, intervening when needed, and reading results back. The control channel is one piece of that surface (live-eval observation and direction); other pieces — an apg agent, agent-friendly `apg eval` launching, `apg log` for reading finished evals, CLI-wide JSON output conventions — are being developed alongside this work and are referenced in "Related work" at the bottom of the doc.

Within that scope, this doc covers the **control plane for live evals and eval-sets**: external processes (LLM agents, scripted watchdogs, TUIs, CLI commands) connecting to a running AgentProvingGround process to **observe** its state and **direct** it (cancel, modify config, drain, requeue, ...).
...
- Decouple the **AgentProvingGround TUI** from the eval process so the TUI can be opened, closed, and reattached without affecting the eval, and so a single TUI can observe multiple evals.
- Keep a **single coherent surface** — one HTTP endpoint per running AgentProvingGround process, one URL scheme, one auth story — that all consumer classes (LLM agents, scripted agents, TUIs, CLI) share.
...
Agent (via Bash):
  apg eval my_task --model gpt-5  --log-dir ./logs/gpt5  --detach   # implies --json
  apg eval my_task --model claude --log-dir ./logs/opus  --detach
  apg ctl task list --json                                              # watch progress
  apg ctl sample list <task> --active-since <ts> --json                 # poll for what changed / stalls
  apg ctl sample cancel <task> <sample-id> <epoch> --dry-run            # check before acting
  apg ctl sample cancel <task> <sample-id> <epoch>
...
Same surface as scenario 1 — either via the CLI (Bash from a shell script) or via the Python client library (`from agent_proving_ground.control import ControlClient`). The Python library is also the implementation substrate for the CLI commands.

### 3. CLI commands to cancel or modify a running eval

Single-purpose human-driven CLI commands, all under the `apg ctl` subcommand and organized into resource-noun groups (see "CLI grouping" and "CLI command hierarchy" below). Phase-1 and phase-2 commands are shipped; the rest land with their phases (the directives → phase 3):

```
# reads + lifecycle (phases 1-2, shipped)
apg ctl task list                       # list running tasks (bare `ctl task` also works)
apg ctl sample list [TASK] [--active-since TS]  # per-sample table (status / retries / score / timing / idle)
apg ctl sample show TASK SID [EPOCH]    # one sample's summary + error history (--traceback for full)
apg ctl sample errors [TASK]            # triage: samples that errored or were retried
apg ctl sample events TASK SID [EPOCH]  # one sample's transcript events (cursored pull)
                                            #   --cursor CURSOR / --tail N / --from-start / --limit N / --type / --full / --since-time / --until
                                            #   (-f / --follow push is phase 4)

apg ctl sample messages TASK SID [EPOCH]  # one sample's current conversation, snapshot
                                              #   --tail N / --all / --full (see "Sample messages read")
apg ctl process list                    # running AgentProvingGround processes (pid / keep-alive / tasks)
apg ctl process keep [PID]              # keep a running process alive after its eval finishes
apg ctl process release [PID]           # release a lingering keep-alive process

# directives + diagnostics (phase 3; first slices shipped)
apg ctl config [TASK] [--max-samples N] [--max-connections N] [--max-sandboxes N]
                   [--max-subprocesses N] [--log-buffer N] [--log-shared S] [--model M] [--dry-run]
                                            # view / retune launch config mid-flight (shipped)
apg ctl task log-flush [TASK]           # write a running eval's buffered samples to the log now (shipped)
apg ctl task cancel TASK [--action cancel|score|error] [--dry-run]  # cancel an eval — abort, or resolve in-flight samples and complete (shipped)
apg ctl sample cancel TASK SID [EPOCH] [--action score|error|cancel] [--dry-run]  # cancel one sample (shipped)
apg ctl task pause TASK [--dry-run]     # stop starting new samples/retries; in-flight finish; reversible (shipped)
apg ctl task resume TASK [--dry-run]    # resume a paused task (shipped)
apg ctl process pause [PID] [--dry-run] # pause the whole run incl. eval-set task dispatch (shipped)
apg ctl process resume [PID] [--dry-run]# resume a paused run (shipped)
apg ctl task add SPEC [--model M] [-T k=v] [--dry-run]  # add a task to a running --ctl-server=keep eval
apg ctl task drain TASK                 # stop accepting new samples; let in-flight finish
apg ctl sample requeue TASK SID EPOCH   # re-add a failed sample to the queue
apg ctl config TASK --time-limit N     # modify per-sample time/token/message limits (planned)
apg ctl process anomalies [PID] [--filter TEXT] [--all]  # in-flight / anomalous actions from the pid's trace file (shipped)
...
#### CLI grouping: `apg ctl`

All live-eval management commands live under a single `apg ctl` subcommand rather than as flat top-level verbs (`inspect tasks`, `inspect cancel`, ...). The choice is deliberate:

- **Conceptual coherence.** Every command in the group operates on a running AgentProvingGround process via the control endpoint; they share infrastructure (discovery, auth, lifecycle) and a mental model. That's a real subsystem, not an arbitrary bag of verbs.
- **Namespace pressure.** AgentProvingGround's top-level CLI is already crowded (`eval`, `eval-set`, `eval-retry`, `view`, `log`, `score`, `cache`, `sandbox`, `acp`). Adding 7–8 new verbs there crowds discovery for everyone; grouping under `ctl` keeps the top level uncluttered.
- **Discoverability.** `apg ctl --help` is the natural entry point for "what can I do with a running eval?" Both humans and agents read it once and have the whole surface.
...
- **No alias.** `inspect control` does *not* exist as a long-form alias; `apg ctl` is the only canonical form. Two names introduce documentation drift, "which is canonical?" ambiguity, and grep friction without paying for themselves.

`inspect tui` stays at the top level rather than under `ctl` — it's an *application* (like `apg view`) that happens to be a control-channel client, not an operation on a running eval.
...
├── process                     # the running AgentProvingGround process itself (PID selector)
...
- **`task`, not `eval`, as the headline noun.** The CLI deliberately targets tasks (task-id prefix / task name — stable across retries; resolved open question #7), and the limits directive already settled on `/tasks/<task-id>/…` for the same reason. An `eval` noun would re-import the attempt-vs-task confusion the selectors were designed to avoid — and `apg ctl eval` reads badly against the top-level `apg eval`. `eval` stays an HTTP-API concept (attempt-scoped resources under `/evals/<id>/…`), resolved client-side as today.
...
- **Teach through the error.** `ctl sample my-task` fails (the bare-noun default never fires past a positional — correct), but click's stock "No such command 'my-task'" reads as "the group doesn't do this". A group-level unknown-command handler answers instead: "No such command 'my-task'. To list a task's samples: `apg ctl sample list my-task`." The analogy that causes the mistake is one the surface itself teaches, so it will recur; the error message is where to correct it.
...
| `ctl set-limit <id> --time N` | planned | fold into `ctl config TASK --time-limit/--token-limit/--message-limit` | all retunable launch flags in one surface, same spellings as `apg eval`; task-scoped knobs, so `TASK` follows the mutation selector rule |
...
For the agent-discoverability goal, the win is *structured* help, not shorter help: `apg ctl --help` shows three groups plus `config`; `apg ctl sample --help` shows six verbs sharing one selector shape. An agent learns the object model from the shape of the tree rather than from reading seventeen one-line summaries and inferring scopes. Verb symmetry means it can predict `sample cancel` after seeing `task cancel`. (Bash-allowlist granularity is unchanged either way — prefix rules like `apg ctl task cancel*` work in both layouts, and read-vs-write doesn't align with nouns in either.)
...
Today `apg eval --display full` runs a Textual TUI in the eval process itself. Closing the TUI requires killing the eval; running headless (`--display plain`) means giving up live state entirely.

The control channel makes the TUI a **client of the eval process**:

```
apg eval --display none ...      # eval runs headless
...
Concentrated in `src/agent_proving_ground/agent/_acp/` and detailed in [`agent-acp.md`](../acp/agent-acp.md):

| Piece | What it does |
|---|---|
| `--acp-server` flag on `apg eval` | Opens a per-eval AF_UNIX socket (or TCP) and writes a discovery file under `<inspect_data_dir>/acp/<pid>.json` |
...
The control channel runs as an **HTTP server embedded directly in the eval process** (FastAPI + uvicorn), exposing read, direct, and event-subscription operations. The eval process binds its own server distinct from `apg view`'s server; the two run independently (see "Alternatives considered" below for why we're not folding them together yet).

### Why HTTP

- **Excellent CLI ergonomics.** `apg ctl task cancel <task>` is one `httpx` call; shell users can hit endpoints directly with `curl`. Pipe composition works (`apg ctl task list --json | jq ... | xargs apg ctl task cancel`). Easy to write small monitoring scripts in any language.
- **No new dependencies.** FastAPI, uvicorn, and starlette are already hard dependencies in `requirements.txt` (used by the `apg view` server). Picking HTTP costs zero framework footprint.
- **Already a pattern in the codebase.** `src/agent_proving_ground/_view/fastapi_server.py` shows the FastAPI-server-in-AgentProvingGround template — including the bits we need: streaming responses, live in-progress reads (`api_pending_samples`, `api_sample_events`), and an OpenAPI generation pipeline (`_view/_openapi.py`). The control channel reuses the same shape.
...
This endpoint is **separate from** both the existing ACP socket (`<inspect_data_dir>/acp/<pid>.sock`) and the existing `apg view` server (which runs as a separate process serving log files). Three endpoints, three concerns:

| Endpoint | Process | Protocol | Purpose |
|---|---|---|---|
| `apg view` server | Standalone `apg view` process | HTTP (FastAPI) | Read historical / live log files from disk |
...
**Path A: CLI subprocess with `--json` output (primary).** Any agent that can run shell commands uses `apg ctl task list --json`, `apg ctl sample list <task> --json`, `apg ctl task cancel <task>`, `apg ctl sample events <task> <sid> --cursor <cursor> --json` directly. Works with Claude Code's Bash tool, with shell scripts, with any subprocess-capable runtime.

Modern LLMs are demonstrably excellent at:
- Reading `apg ctl --help` and discovering subcommands.
- Parsing JSON output and composing follow-up calls.
- Pipe composition (`apg ctl task list --json | jq '.tasks[] | select(.status=="stuck") | .task_id' | xargs -I{} apg ctl task cancel {}`).
...
| Per-tool permissions | Claude Code's Bash allowlist (`apg ctl task cancel*` vs `apg ctl task list`) covers most of the granularity gap. |
| Tool discovery | `apg ctl --help` is enumerable; agents read it natively. |

Costs of building / maintaining an MCP wrapper, on the other hand, are real: extra process to spawn / configure / debug, second surface to keep in lockstep with the CLI, two vocabularies (`inspect_list_evals` vs `apg ctl task list`) for the same operations, failure modes invisible to the user.
...
```python
from agent_proving_ground.control import ControlClient
...
**Network exposure: zero by construction.** The endpoint binds an AF_UNIX socket at `<inspect_data_dir>/control/<pid>.sock`. AF_UNIX is a filesystem object, not a network socket — it has no IP, no port, doesn't traverse any network stack. It is structurally impossible to reach from another machine. It also isn't reachable from inside containers (Docker, AgentProvingGround's sandboxes) unless the user explicitly bind-mounts `inspect_data_dir` into the container — which AgentProvingGround's own sandbox setups do not do.
...
| Root on the same machine | Yes | Filesystem perms can't constrain root. Not in scope — if you don't trust root, no AgentProvingGround setting helps. |
...
The directory and socket modes are applied via `chmod` on every server start (idempotent), so a directory created before the hardening landed gets locked down on the next bind. The discovery JSON is handled differently: it's created owner-only at `open()` time (the mode is passed to `os.open`, capped by the umask) and published with an atomic temp-write-then-rename, so it is never *momentarily* more permissive than 0600 (no post-write `chmod` window) and a concurrent `apg ctl task list` reader never observes a torn / partial-JSON file. Some filesystems ignore Unix permissions (FUSE, certain network mounts); the fallback is benign — everything still lives under `inspect_data_dir`, which is user-scoped, so the loss of defence-in-depth is bounded.

**What this buys us.** With the directory at 0700, an attempted connection from another user's process fails at the directory-traversal step (`EACCES`) before the socket file's own permissions are even consulted. The socket and JSON 0600 modes are belt-and-suspenders: they protect against a misconfigured umask, a future code path that lowers the directory perms, or a user running AgentProvingGround under different identities (sudo etc.) that accidentally widen perms.

**What this does NOT buy us:**

- **Same user, different process.** Filesystem perms can't distinguish "your AgentProvingGround eval" from "an untrusted script you ran as yourself" — both run with your UID. To enforce "only the launching eval can be controlled" would require an application-layer secret (cookie, capability token). Not in scope for v1 — trust model matches every other user-local IPC (D-Bus session bus, X11 socket, ssh-agent).
...
8. **Self-targeting from an LLM-agent inside an eval.** An LLM-driven agent monitoring an eval might itself be running *inside* an AgentProvingGround eval (a meta-solver). Should it be able to control its own parent eval? Probably not by default — worth a "no self-targeting" guard if/when this comes up. (Becomes relevant with the phase-3 write endpoints.)
...
### Process keep-alive: `--ctl-server=keep` on `apg eval` / `apg eval-set`
...
- **Forgotten release.** Agent crashes / loses track of the pid. The process lingers indefinitely. Mitigation: a future idle timeout on the park could auto-shutdown after N minutes. Not in v1; tracked as [meridianlabs-ai/agent_proving_ground#227](https://github.com/meridianlabs-ai/agent_proving_ground/issues/227).
- **Multiple lingering processes.** Several agents each run their own keep-alive eval. `apg ctl process release` with no args errors and lists pids; the agent disambiguates by passing the `PID`.
...
The control endpoint is bound by default whenever `apg eval` runs. Every running eval benefits from being controllable, and the agent-enablement story falls apart if agents or TUIs can't discover evals launched without a special flag (per-launch opt-in defeats the whole "Claude Code uses AgentProvingGround by default" framing).
...
`APG_EVAL_CTL_SERVER` mirrors the flag (same values: `false` / `true` / `keep`), following the `APG_EVAL_ACP_SERVER` precedent. This lets a test runner or CI config globally suppress the surface (`APG_EVAL_CTL_SERVER=false`) without modifying each `apg eval` invocation.
...
- Set `APG_EVAL_CTL_SERVER=false` globally for the test session.
...
- **`--ctl-server`** on `apg eval` / `apg eval-set` — on/off plus the `keep` park value (see Implementation notes).
...
- **CLI:** `apg ctl sample messages TASK SID [EPOCH]` — a new verb in the `sample` noun group. Standard selector conventions apply: `TASK` is required (a `SAMPLE_ID` follows it), and `EPOCH` defaults to 1 on reads with the resolved `{sample_id, epoch}` echoed in the envelope.
...
This phase also carries the **trace-log anomalies affordances** (see "Trace-log anomalies for stall diagnosis") — not a directive, but scheduled here because the diagnosis need is current and the work touches no server code: `--json` on `apg trace anomalies` (and `http`) first (shipped), then `apg ctl process anomalies` (client-side trace-file read; deliberately no HTTP endpoint — shipped, with the dead-pid post-mortem fallback) and the stall-site pointers (remaining).
...
Unlike the concurrency knobs these aren't limiter-shaped — they're read from the per-call `GenerateConfig` inside the generate retry loop. The mechanism is a **live override layer** (`agent_proving_ground.model._generate_overrides`, process-scoped like `max_sandboxes` / `max_connections`, reset at the outermost run boundary alongside the other control-channel registries) consulted at the *point of use* rather than merged into any config object:
...
`POST /tasks` (CLI: `apg ctl task add SPEC [...]`) submits a **task spec** that runs in the target process under the same `run_id`, appearing as a new sibling eval in `task list` / `sample list` / `sample events`. Returns the new `eval_id`.

**A spec, not a `Task`.** The wire is HTTP/JSON; a `Task` carries code (solvers, scorers, dataset). So the directive carries a *spec* — registry name or file path, `-T` task args, model, config / limit overrides — which the running process resolves in-process via the registry, exactly as `apg eval <name>` does at launch. A task the process can't resolve (not importable in its environment) fails with a clear error; `--dry-run` surfaces that before committing.
...
**Scope.** Standalone `apg eval --ctl-server=keep` first; eval-set — which owns its own retry loop and task resolution — is a later increment.
...
`POST /evals/<id>/sample/requeue?sample_id=<sid>&epoch=<n>` (CLI: `apg ctl sample requeue TASK SID [EPOCH] [--dry-run]`) re-adds one errored/cancelled sample to the live run — it goes to the back of the sample queue and re-runs under the task's normal machinery (query-param sample addressing, like `sample` / `sample/events`, since sample ids may contain URL-reserved characters; `epoch` fails closed like `sample cancel`). [`sample-requeue.md`](sample-requeue.md) owns the semantics (the requeueable-states decision table, attempt identity, counter/log reconciliation) and the injectable sample scheduler it introduced into the eval runner.
...
- **CLI ergonomics are poor.** `apg ctl task cancel <task>` needs a JSON-RPC client wrapper; no `curl` story; harder to script ad-hoc.
...
### H2: Extend the existing `apg view` server to handle live state + control

Rather than running a separate HTTP server in the eval process (H1, chosen), one could imagine **the view server** absorbing the control role: `apg view` already reads live in-progress sample state via the sample-buffer files; extending it to also expose control operations would unify "live observation" and "log viewing" at one endpoint.
...
The control channel is one slice of a broader effort to make AgentProvingGround a first-class platform for LLM-agent-driven eval workflows. The pieces below are out of scope for this doc but are being designed / built alongside it; they share the same JSON-first, agent-friendly conventions:

- **Agent-friendly `apg eval`.** Launching an eval as an agent: `--json` output reporting the started run_id / log path / control-channel address; `--detach` so the agent can fire-and-monitor rather than block; structured pre-flight estimates (cost, sample count). The handoff from "launched" to "monitored via control channel" depends on this. The launch-handoff slice of `--json` and `--detach` have both shipped (see "The launch handoff is load-bearing" and "Detached launch" above); the pre-flight estimates remain.
- **Agent-friendly `apg log`.** Reading finished evals as an agent: `apg log summary <file> --json`, `apg log sample <file> <id> --json`, `apg log compare <a> <b> --json`. The control channel handles live evals; `apg log` handles finished ones. The handoff between them is part of the agent workflow.
...
- **Cost / budget visibility.** Agents making scope decisions need to see pre-run estimates and live spend. The control channel exposes live spend (via `EvalState` model usage); pre-run estimates and budget enforcement live in `apg eval`'s launch surface.
...
- **"Using AgentProvingGround from an LLM agent" documentation.** A guide that walks through the full workflow (launch, monitor, manage, inspect, compare, iterate) and points at every relevant CLI command. Lives in `docs/`, not `design/`.
...
Where this doc's design touches one of those surfaces (eg. the `EvalState` model that powers `apg ctl task list`), we describe what the control channel does and reference the broader work for the surrounding context.
