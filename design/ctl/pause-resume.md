...
> **Status: implemented** (v1 quiesce semantics — the gate module lives in `src/agent_proving_ground/_control/pause.py`; Future work below remains open). Companion to [`control-channel.md`](control-channel.md), which owns the control-channel architecture this surface rides on; this doc owns the pause/resume semantics and implementation plan. Originating issue: meridianlabs-ai/agent_proving_ground#90. The model-scoped latch (below) followed from review discussion on the upstream PR: meridianlabs-ai/agent_proving_ground#130.

`apg ctl` can today observe a running eval, retune its config, and cancel it — but there is nothing between "running" and "cancelled". An operator (or watchdog agent) facing a provider incident, a cost overrun, or a suspicious-looking transcript has exactly two levers: throttle (`ctl config`) or kill (`ctl task cancel`). Killing forfeits the run's place in line — resuming means a fresh `eval-set` invocation, a new process, and re-running every sample that wasn't durably complete.
...
- **Provider incident.** The model API starts returning elevated error rates. Today the choices are "let the retry loop grind" (burning the retry budget and money) or cancel. With pause: `apg ctl process pause`, wait out the incident (watching with the read surface, which stays live), `apg ctl process resume`. Nothing is lost; no relaunch. The protection is partial until in-flight work drains: already-running samples keep grinding their own retry loops through the incident — pause stops the queue from feeding more samples into it.
...
- **Pause across a process restart.** Planned machine maintenance. Pause, wait for quiesce (all in-flight samples finished and flushed), kill the process; later, re-invoke `apg eval-set` on the same `log_dir` — the existing run-vs-reuse logic re-runs only what didn't complete. Pause's job here is to make the kill point *clean* (nothing in flight, everything flushed); the resume half already exists. (A spot-instance reclaim's short warning can't wait for quiesce — that is crash recovery's job, `design/recover.md`, or the hard pause below.)
...
```
apg ctl task pause TASK [--dry-run]        # pause one task (sample dispatch + its retries)
apg ctl task resume TASK [--dry-run]       # resume it
apg ctl model pause MODEL [PID] [--dry-run]  # pause one model's dispatch (see "Model-scoped latch")
apg ctl model resume MODEL [PID] [--dry-run] # resume it
apg ctl process pause [PID] [--dry-run]    # pause the whole run: every task + the eval-set task/retry scheduler
apg ctl process resume [PID] [--dry-run]   # resume the whole run
```
...
- `apg eval-set` re-invoked on the same `log_dir` re-runs only tasks without a complete success log (`list_latest_eval_logs` / `log_samples_complete` in `src/agent_proving_ground/_eval/evalset.py`) — cancelled, errored, and crashed (`started`) logs are all retry seeds.
