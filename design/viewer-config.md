...
Reference for `agent_proving_ground.viewer` — the typed Pydantic surface a `Task`
author passes via `task.viewer=...` to control how the AgentProvingGround log
viewer renders that task's log.

```python
from agent_proving_ground import Task
from agent_proving_ground.viewer import ViewerConfig, TaskSamplesView, ...
...
```python
from agent_proving_ground.viewer import (
    MetadataField,
    SampleScoreView,
    SampleScoreViewSort,
    ScannerResultView,
