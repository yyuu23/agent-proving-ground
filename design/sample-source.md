...
`src/agent_proving_ground/_eval/task/sample_source.py` defines a subclassable base class
with no-op defaults:
...
- **Task resolution** — `resolve_dataset_or_source`
  ([task.py](../src/agent_proving_ground/_eval/task/task.py)) returns a `ResolvedDataset`
...
- **The dispatcher** — in [task/run.py](../src/agent_proving_ground/_eval/task/run.py),
...
- **Growing totals** — `total_samples` grows as samples are added, updating the
  ...
  via `record_samples_added` ([eval_state.py](../src/agent_proving_ground/_control/eval_state.py)).
