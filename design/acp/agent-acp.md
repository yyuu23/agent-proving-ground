...
AgentProvingGround eval, observe a specific sample's agent activity, and interact with it
(send messages, interrupt, approve tool calls) via the Agent Client Protocol.

## Goals

- A **generic agent-side facility** that any AgentProvingGround agent can opt into (not just `react()` / `deepagent()`).
- An **eval-level JSON-RPC 2.0 endpoint** (unix socket or TCP) opened by `apg eval --agent-acp` that:
  - enumerates running tasks/samples/epochs,
  - exposes ACP methods routed to a chosen `(task, sample_id, epoch)`.
- An **`apg acp`** stub binary that talks ACP-over-stdio to an editor and proxies into the eval.
...
### AgentProvingGround primitives we'll reuse
...
```
Editor (Zed)                  apg acp                    apg eval --acp-server
+----------+   ACP over   +------------------+   ACP over   +-------------------------+
...
external `apg acp` stub. The middle layer was a single
...
tool calls **must not** leak as separate updates. AgentProvingGround's regular
...
| AgentProvingGround event | ACP update |
|---|---|
...
blocks map straight onto AgentProvingGround's `ContentText` / `ContentImage`. Other
...
- Listens on an AF_UNIX socket at
  ...
  `inspect_data_dir(...)` helper (from `agent_proving_ground._util.appdirs`),
...
- **PID liveness on enumerate.** When `apg acp` enumerates active
...
The picker is exposed *through* the ACP channel so editors with no AgentProvingGround-specific UI still get a usable flow. The path depends on which method the client called:
...
### 4. `apg acp` — TUI + editor stub

`apg acp` is **both** the human-facing ACP client we ship (a Textual
TUI) and the editor-passthrough stub. Mode is selected by a flag:

```
apg acp                  # textual TUI client (default)
apg acp --stdio          # passthrough: ACP-over-stdio ↔ socket
apg acp [--eval-id EID]  # disambiguate when multiple evals are running
```
...
- **Stdio mode**: Zed (or any other ACP editor) is configured with
  `apg acp --stdio` as its agent server command. The stub speaks ACP
...
### 5. In-process ACP for the existing AgentProvingGround TUI
...
This makes ACP the canonical interaction protocol *inside* AgentProvingGround, with
...
When ACP clients are attached and AgentProvingGround's `approval` framework would
...
the normal AgentProvingGround view).
...
- The `AcpSession` is **transport-agnostic**; the existing AgentProvingGround TUI
...
1. ~~**Replay on attach**~~ — *resolved during the Phase 6 (TUI
   ...
   tool results the model consumes are all capped); AgentProvingGround's own
...
Phases are ordered to maximize independent testability and incremental commit value. Phases 1–6 land the **in-process foundation** — each adds a self-contained piece exercisable with unit tests against a mock model. Phase 7 makes the foundation **immediately user-visible** by wiring the existing TUI to it (without `--agent-acp` needing to exist yet). Phases 8–10 add the **socket transport**. Phase 11 ships the **`apg acp` CLI**. Phase 12 extends to **`deepagent()`**. Phases 13–14 add **approval UI** and **token streaming** as standalone enhancements.
...
**Files created:**
- `src/agent_proving_ground/agent/_acp/__init__.py` — package re-exports.
- `src/agent_proving_ground/agent/_acp/session.py` — Protocol, no-op impl, real impl, ContextVar, factory, accessor (all in one module).
- `tests/agent/acp/__init__.py` — test package marker.
- `tests/agent/acp/test_session.py` — 15 unit tests.

**Files modified:**
- `src/agent_proving_ground/agent/__init__.py` — added `AcpSession` and `acp_session` to imports and `__all__`. `current_acp_session` is intentionally *not* surfaced at the top level — agent authors don't need it, only internal code that wants to peek the active session without entering a scope does. Import it via `from agent_proving_ground.agent._acp import current_acp_session` if needed.
...
- `current_acp_session()` — peek accessor returning the ContextVar's value (no-op singleton by default). Exported from `agent_proving_ground.agent._acp` only; not surfaced at the top-level `agent_proving_ground.agent` namespace.
...
Verification: `pytest tests/agent/acp/ -v` (14 passed asyncio); `pytest tests/agent/acp/ --runtrio -v` (14 passed trio); `mypy --exclude tests/test_package src/agent_proving_ground/agent/_acp src/agent_proving_ground/agent/__init__.py tests/agent/acp` clean; `ruff format` / `ruff check --fix` clean; public API import smoke succeeds.
...
**Files created:**
- `src/agent_proving_ground/event/_interrupt.py` — `InterruptEvent` class.
- `tests/log/test_interrupt_event.py` — 12 unit tests.
- `src/agent_proving_ground/_view/ts-mono/packages/inspect-components/src/transcript/InterruptEventView.tsx` — new ts-mono view component.

**Files modified:**
- `src/agent_proving_ground/model/_chat_message.py` — extended `ChatMessageBase.source` from `Literal["input", "generate"] | None` to `Literal["input", "generate", "operator"] | None`.
- `src/agent_proving_ground/event/_event.py` — added `InterruptEvent` to the `Event` union.
- `src/agent_proving_ground/event/__init__.py` — re-exported `InterruptEvent`.
- `src/agent_proving_ground/log/_transcript.py` — added module-level `record_interrupt_event(...)` helper that appends to the current transcript via `transcript()._event(...)`. Kept as a module function rather than a `Transcript` method since callers are internal-only (Phase 3's cancellation machinery) and we don't want to grow the public `Transcript` API for it.
- `src/agent_proving_ground/_view/inspect-openapi.json` — regenerated to include the new event and the extended source Literal.
- `src/agent_proving_ground/_view/ts-mono/packages/inspect-common/src/types/generated.ts` — regenerated TypeScript types.
- `src/agent_proving_ground/_view/ts-mono/packages/inspect-common/src/types/index.ts` — exported `InterruptEvent`.
- `src/agent_proving_ground/_view/ts-mono/packages/inspect-components/src/transcript/types.ts` — added `InterruptEvent` to `EventType` union and `"interrupt"` to `eventTypeValues`.
- `src/agent_proving_ground/_view/ts-mono/packages/inspect-components/src/transcript/icons.ts` — added `interrupt: "bi bi-slash-circle"`.
- `src/agent_proving_ground/_view/ts-mono/packages/inspect-components/src/transcript/TranscriptVirtualList.tsx` — added `case "interrupt":` dispatch to `InterruptEventView`.
...
**Design decisions:**
- Field name: `interrupted_tool_call_id` (not `interrupted_tool_id` as the earlier design sketch had) — disambiguates from "tool name" and matches AgentProvingGround's `ToolCall.id` convention. Design-doc schema sketch updated to match.
- Helper exposed as a module-level `record_interrupt_event(...)` rather than `Transcript.interrupt(...)` — keeps the public `Transcript` API lean since the only callers are internal AgentProvingGround code (Phase 3's cancellation machinery). Implementation just calls `transcript()._event(InterruptEvent(...))`.
...
**Files modified:**
- `src/agent_proving_ground/agent/_acp/session.py` — extended Protocol, `LiveAcpSession`, and `NoOpAcpSession` with the Phase 3 surface; added `TurnCancelled` exception. ~300 → ~545 LoC, still single-file.
- `src/agent_proving_ground/tool/_tool_call.py` — added `"cancelled"` to `ToolCallError.type` Literal.
- `src/agent_proving_ground/_view/inspect-openapi.json` — regenerated to include the extended `ToolCallError.type` enum.
- `src/agent_proving_ground/_view/ts-mono/packages/inspect-common/src/types/generated.ts` and `apps/scout/src/types/generated.ts` — regenerated TS types for the new `"cancelled"` enum value.
...
- `AgentState` and `ModelEvent` typed via `TYPE_CHECKING` forward references to avoid runtime import cycles (`agent_proving_ground.agent._agent` may import from `_acp` in Phase 4; `agent_proving_ground.event._model` pulls in the whole event/scorer/log subsystem).
...
- `get_model(...)` must be called with `memoize=False` in tests — AgentProvingGround memoizes by default, so without it the same mockllm instance (with its custom_outputs already exhausted) gets reused across tests in the same pytest run.
...
**Files modified:**
- `src/agent_proving_ground/util/_span.py` — added `AGENT_SPAN_TYPE = "agent"` constant with a docstring explaining the convention.
- `src/agent_proving_ground/agent/_run.py:93` — replaced literal `type="agent"` with `type=AGENT_SPAN_TYPE`.
- `src/agent_proving_ground/agent/_as_tool.py:67` — same.
- `src/agent_proving_ground/agent/_as_solver.py:71` — same.
- `src/agent_proving_ground/model/_call_tools.py:583` — same (handoff dispatch's inner agent span; the outer `type="handoff"` and `type="tool"` spans are unrelated).

**Files created:**
- `tests/agent/test_acp/test_span_boundary.py` — 7 regression tests.

**What landed:**
- A single `AGENT_SPAN_TYPE` constant in `src/agent_proving_ground/util/_span.py` is the shared name for both producers (the four span call sites that wrap agent invocations) and Phase 6's eventual consumer (the router that counts nesting depth).
...
7. **Grep-style guard**: walks the entire `src/agent_proving_ground` tree for `span(...type="agent"...)` literal usage and fails if any remain — protects against new agent-invocation paths silently skipping the marker.
...
Verification: `pytest tests/agent/test_acp/ tests/log/test_transcript_subscribers.py` (101 passed asyncio); `--runtrio` passes (55 trio variants); `ruff format`/`check` clean; `mypy` clean. No OpenAPI/TS regen needed — Phase 6 is pure consumer code reading existing AgentProvingGround event types and producing acp-package objects.

### Phase 7: AgentProvingGround-native TUI — Interrupt + interject prompt ✅
...
2. **Persisted into the eval log** so `apg eval-retry` brings the same transport back automatically. New `EvalConfig.acp_server: bool | int | str | None = None` field; retry follows the existing `value or eval_log.eval.config.field` override pattern.

The name `--acp-server` (rather than `--acp` or `--agent-acp`) is deliberate: it pairs with the future `apg acp` *client* subcommand. The asymmetric noun makes the role asymmetric obvious — one flag enables the server, the other command runs a client.

**What was built:**

1. **`EvalConfig.acp_server`** in `src/agent_proving_ground/log/_log.py` — Pydantic field, auto-persists, defaults to `None`, backward-compatible with old logs.
2. **`--acp-server` CLI flag** in `src/agent_proving_ground/_cli/eval.py` — added to `eval_options()` (covers `apg eval` + `apg eval-set`) and a separate copy on `apg eval-retry` so users can override the persisted value on retry. Env var: `APG_EVAL_ACP_SERVER`.
3. **`acp_server` parameter threaded through** `eval()` / `eval_async()` / `eval_retry()` / `eval_retry_async()` in `src/agent_proving_ground/_eval/eval.py`. Retry merge uses the same `value if value is not None else eval_log.eval.config.acp_server` pattern as the rest of the replay-able flags.
4. **`AcpServer` class + `acp_server()` async context manager** in `src/agent_proving_ground/agent/_acp/server.py`. Binds the requested transport, writes a per-PID discovery JSON at `<inspect_data_dir>/acp/<pid>.json` (`{pid, eval_id, socket_path, port, started_at}`), accepts connections, wraps each in `acp.connection.Connection` with an empty `acp.router.MessageRouter`, and tears everything down at exit. PID-liveness checked via `os.kill(pid, 0)`; stale discovery files + orphan socket nodes are swept on each `start()`. AF_UNIX everywhere (Win 10+ supports it); older Windows errors with a hint to pass `--acp-server=<port>`.
5. **Lifecycle integration** — the eval runner wraps its eval-loop body in `async with _acp_server(eval_id=run_id, transport=config.acp_server):` (`src/agent_proving_ground/_eval/eval.py`). When `config.acp_server` is falsy the context yields `None` and binds nothing; otherwise the server lives for the eval's duration. **Per-eval lifecycle** (chosen over per-process): each `eval()` call gets its own socket named after its `run_id`. In `eval-set`, each sub-eval rebinds.
...
- Client SDK / `apg acp` CLI (Phase 11).
...
1. **Phase 6 router enhancements** (`src/agent_proving_ground/agent/_acp/event_mapping.py`):
...
2. **`AcpSession` protocol additions** (`src/agent_proving_ground/agent/_acp/session.py`):
...
**Caveats deferred:**
- Full multi-modal prompt translation (image/audio/resource blocks → AgentProvingGround content types). Phase 10 supports `TextContentBlock` fully and emits placeholder text for the others with a one-shot warning. Real multi-modal lands in Phase 13.
...
**Real production bug surfaced + fixed during Phase 11 testing** — `Transcript._process_event` could recurse infinitely when a transcript subscriber raised. The recursion: subscriber raises → `logger.exception("Transcript subscriber raised; continuing")` → `agent_proving_ground`'s `LogHandler` (installed by eval) calls `log_to_transcript` → `transcript()._event(LoggerEvent(...))` → re-enters `_process_event` → subscriber loop fires the same broken subscriber → raises → infinite. In tests this manifested as `test_router_exception_does_not_propagate_to_loop` failing with `RecursionError` whenever it ran AFTER a test that used `eval()` (the deepagent integration tests being the trigger). In production, any consistently-broken subscriber would have hung the agent process. Fixed in `src/agent_proving_ground/log/_transcript.py` by adding a `_notifying_subscribers` re-entry guard: the recursive event still gets appended to `_events` and sent to the log writer, but the subscriber loop is skipped on re-entry, breaking the cycle. The same bug would have affected any subscriber (not just the picker / ACP router) so the fix lives at the transcript layer.
...
### Phase 12: AgentProvingGround-specific ACP extensions: `inspect/*` action methods ✅

Two TUI-grade affordances that don't map onto any generic ACP method live as non-standard JSON-RPC requests under the `inspect/` namespace. The AgentProvingGround TUI has them today; the JSON-RPC surface exposes the same primitives so editor clients can offer them too.
...
**Sample lookup.** A small `_find_active_sample(session_id)` helper walks `agent_proving_ground.log._samples.active_samples()` for the `ActiveSample` whose `acp_session.session_id` matches — sibling pattern to the existing `_find_live_session`. O(samples) per call; fine for expected scale (a handful of in-flight samples).
...
### Phase 13: `apg acp --stdio` (editor bridge) ✅

Shipped the `apg acp` subcommand in its minimum-viable form: a stdio↔socket bridge that lets external editors (Zed etc.) speak ACP to a running eval through the standard "spawn an agent subprocess on stdio" contract those editors already support. Phase 13 is *just* a transport adapter — one bridge process, one eval, line-framed byte forwarding. The TUI client (Phase 15) and approval UI (Phase 14) are independent concerns that layer on top of the same surface.

**Components landed:**

- **`apg acp --stdio`** — bridge mode. Forwards newline-delimited JSON-RPC frames 1:1 between the editor's stdio and the eval's AF_UNIX (or loopback TCP) socket. Bare `apg acp` (no `--stdio`) exits non-zero with a "TUI not yet implemented, use --stdio (Phase 15)" message — Phase 15's PR lifts this gate.
- **Discovery resolution** — `src/agent_proving_ground/agent/_acp/discovery.py`. Public surface: `discovery_dir()`, `pid_alive()`, `parse_host_port()`, `has_unix_sockets()`, `cleanup_stale_discovery_files()` (all moved out of `server.py` so the CLI can import them without server internals), plus the new `TargetAddress`, `DiscoveredEval`, `TargetResolutionError`, `list_discovered_evals()`, and `resolve_target()`. Policy: `--socket` wins (parses path or `host:port`); else `--eval-id` looks up the file; else auto-discover. Auto-discovery picks the most-recently-started alive eval; emits a stderr notice naming the choice + the candidate list when more than one is available, so editor debug panes show what happened.
- **Bridge** — `src/agent_proving_ground/agent/_acp/stdio.py`. Two concurrent forwarders (stdin→socket, socket→stdout) with `asyncio.wait(return_when=FIRST_COMPLETED)` semantics so the bridge exits as soon as either side hits EOF — needed because the sibling forwarder may be blocked on `readline()` from a still-open reader. Line-framed via `StreamReader.readline()`; raw-byte forwarding could split or merge frames at chunk boundaries.
- **CLI** — `src/agent_proving_ground/_cli/acp.py`. Click subcommand registered in `_cli/main.py`. Uses `acp.stdio.stdio_streams()` (cross-platform — POSIX `loop.connect_read_pipe`, Windows thread-fed feeder + custom stdout transport) to get the stdio streams; calls `bridge_stdio()` with them. Rejects `--eval-id` + `--socket` together (mutually exclusive — different intents); translates `ConnectionRefusedError` / `FileNotFoundError` to clean stderr diagnostics + exit 2.
...
- **Multi-eval policy = "pick the newest" (with stderr notice), not "error and require --eval-id".** Editor configs that hard-code `apg acp --stdio` should keep working when the developer happens to spawn a second eval mid-session. Stderr notice naming the chosen eval + the candidate list gives the user a paper trail.
...
**Verification:** `pytest tests/agent/test_acp/ tests/_cli/test_acp_server_flag.py tests/_cli/test_acp_cli.py` → 293 passed (was 264 in Phase 12). `ruff format` / `ruff check` clean. `mypy --exclude tests/test_package src tests` clean (1024 source files). Manual smoke: `apg acp --help` shows the three flags; `apg acp` exits 2 with the Phase-15 message; `apg acp --stdio` against an empty discovery dir exits 2 with the "no running evals" hint.
...
- **Approval shim** (`src/agent_proving_ground/approval/_human/acp.py`). `request_human_approval_via_acp(message, call, view, choices) → Approval | None`. Returns `None` to signal "fall through to in-proc panel/console" when no clients attached or every client in the driver chain raised. Otherwise builds a `RequestPermissionRequest` (reusing `descriptive_title` + `content_blocks_from_view` from `_tool_content` for visual consistency with live tool-call rendering), then calls `_request_from_driver_with_fallback` which iterates the driver chain sequentially: first success wins, transport-level failures on a given client fall through to the next, cancellation propagates to the caller. Wait-forever (no timeout — the human at the editor is the source of truth). The original broadcast/race shipped with Phase 14 was replaced because ACP has no `$/cancelRequest`-equivalent for outbound requests — broadcasting left losing editors with stale permission cards forever.
- **`human_approver` checks ACP first** (`src/agent_proving_ground/approval/_human/approver.py`). One added line: call the shim before falling through to `panel_approval` / `console_approval`. Zero behavior change in the no-client case — the ACP routing is a strict superset.
...
**Verification.** `pytest tests/agent/test_acp/ tests/_cli/test_acp_server_flag.py tests/_cli/test_acp_cli.py tests/approval/` — 382 passed. `ruff format` + `ruff check` clean across all modified source and test files. `mypy --exclude tests/test_package src tests` clean (1026 source files). Manual smoke against Zed via `apg eval <task> --acp-server --approval=human` confirms the permission card renders with the bash command and the user's response advances the eval.
...
AgentProvingGround samples genuinely terminate server-side mid-eval (the react loop exits, scoring runs, sample completes), which doesn't map onto that model. `inspect/session_ended` fills the gap by giving AgentProvingGround-aware clients a positive push signal so their UI can flip to `complete` without waiting for transport EOF (the connection stays open so the client can switch to another sample).

For non-AgentProvingGround clients the discovery path is lazy-on-prompt: any `session/prompt` after the bound session is gone returns `internal_error` with `reason: "bound session no longer active"` and `target_session_id` in the structured payload. That's the standard ACP discovery pattern for this class of state mismatch and editors surface the error to the user. No additional server change makes the standard story better — there's no place in standard ACP to push the signal proactively.

### Audit rule

Every `inspect.*` string literal (method names, `_meta` keys, capability allowlists) lives in `src/agent_proving_ground/agent/_acp/inspect_ext.py`. Other modules import constants from that file rather than synthesizing the strings inline. A producer-side grep audit (`git grep -nE 'inspect[./]' src/agent_proving_ground/agent/_acp/`, filtering docstrings and the audit file itself) catches drift. Deleting `inspect_ext.py` would leave a strict-ACP server behind in principle, which is the test we run mentally when adding new functionality.


## asyncio / anyio boundary

The rest of agent_proving_ground is **anyio-native** so the codebase can run under both asyncio and trio backends (per AGENTS.md's `tg_collect`, `anyio.sleep`, `anyio.Event` conventions). The ACP implementation is an exception: it's intentionally **asyncio-bound** at the `acp`-library boundary. Documenting that here so future maintainers don't try to migrate the wrong things.
...
**What IS anyio-native within ACP.** The in-process pub/sub layer in `session.py` uses `anyio.create_memory_object_stream`, `anyio.Event`, `anyio.CancelScope`, and `anyio.BrokenResourceError` throughout. The session is the natural composition point with the rest of agent_proving_ground's agent framework (which IS anyio); the transport/JSON-RPC layer below it is the asyncio island.
