...
The mechanism is a module-level `acp_server_accepting_clients()` accessor (`src/agent_proving_ground/agent/_acp/server.py`) backed by a `ContextVar` that the `acp_server(...)` context manager flips on for the bound-server lifetime and off when the server stops. Routing shims in `src/agent_proving_ground/util/_input/acp.py` and `src/agent_proving_ground/approval/_human/acp.py` consult the accessor at their public entry; the inner driver-chain loop unconditionally parks on chain exhaustion regardless of attach history. The flag is deliberately separate from "is the current transport a `LiveAcpTransport`": the Live transport is opened per-sample regardless of `--acp-server` (sub-agent reachability needs the in-process pub/sub plumbing), and nested `acp_session()` blocks install a `NoOp` shadow that would miss-route human-in-the-loop traffic. Both shims also resolve the routing target via `sample_active().acp_transport` (the outermost Live, pinned at sample startup) rather than the ContextVar — sub-agent isolation is for *event publishing*, not human channels.

## Concepts and types

```python
# src/agent_proving_ground/util/_input/_types.py
...
### 1. Routing core — `src/agent_proving_ground/util/_input/`

Mirrors `src/agent_proving_ground/approval/_human/`. Files:
...
### 2. ACP transport — `src/agent_proving_ground/agent/_acp/`
...
### 3. The `ask_user` tool — `src/agent_proving_ground/tool/_tools/_ask_user.py`
...
`ToolError` for declined/cancelled is recoverable so the model can adapt. Registered in `src/agent_proving_ground/tool/__init__.py`. The `_normalize_schema_types` shim lowercases `type` field values to handle providers (notably Gemini) that emit uppercase JSON Schema type names.

### 4. The `notify_user` tool — `src/agent_proving_ground/tool/_tools/_notify_user.py`
...
### 5. The `notify()` helper — `src/agent_proving_ground/util/_notify.py`
...
- Inside a sample: title `AgentProvingGround Agent: <task>`, body starts with `sample: <sample_id>/<epoch>` followed by the message.
- Outside a sample: title `AgentProvingGround Agent`, body is the unmodified message.
...
- `src/agent_proving_ground/util/_input/request.py` — fires before `_dispatch_builtin`, so the operator is pinged regardless of which surface collects the answer.
- `src/agent_proving_ground/approval/_human/approver.py` — fires before the ACP/panel/console approval dispatch chain, same rationale.
- `src/agent_proving_ground/tool/_tools/_notify_user.py` — fires when the model invokes the tool.
...
- `notification=True` → read URL(s) from the `APG_EVAL_NOTIFICATION` environment variable. The env-var value can be a single Apprise URL, a comma-separated list of URLs, or a path to an Apprise YAML/text config file. Unset/empty env var raises.
...
This shape closes four leak vectors that URL-string-acceptance would open: source code, shell history, process listings, and `EvalConfig.notification` in serialized eval logs (which stores only `True` or a file path — never URL contents). See `src/agent_proving_ground/util/_notify.py:build_apprise` for the validation.
...
## Public Python API — `src/agent_proving_ground/util/__init__.py`

```python
from agent_proving_ground.util import (
    notify,
    request_input,
    InputOutcome,
    InputRequest,
...
ACP schema types (`ElicitationSchema` and the five property schemas) are **not** re-exported — users import them directly from `acp.schema`. They're all available there (confirmed in the installed 0.10.0 package). Reasons: (1) avoid bloating the public surface of `agent_proving_ground.util`; (2) if upstream renames a field, users see the upstream type directly rather than chasing an AgentProvingGround alias.

The model-callable tools live under `agent_proving_ground.tool`:

```python
from agent_proving_ground.tool import ask_user, notify_user
```
...
- **Manual UI verification:**
  - `apg eval ... --display full` with an agent that calls `ask_user` → "Question" tab appears and renders form inputs.
