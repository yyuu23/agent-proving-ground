...
You are a coding agent with full access to the AgentProvingGround repository (`agent_proving_ground`). This brief specifies one focused refactor. It assumes familiarity with AgentProvingGround's agent layer (`@agent`, `AgentState`, the `react()` agent), the ambient execution-context accessors (`transcript()`, `store()`, the limit context managers), and the existing ACP integration.
...
1. **Introduce the core primitive** in the agent runtime (`agent_proving_ground.agent` or wherever the ambient accessors live): `agent_channel()`, the channel type, an `AgentRef` producer-side handle, and `AgentInterrupted`. Source-agnostic; ambient; **not** on `AgentState`.
...
```
agent_proving_ground.agent (core)
