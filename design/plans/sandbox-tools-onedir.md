...
(`src/agent_proving_ground/util/_sandbox/_json_rpc_transport.py:60`), which spawns a fresh
...
### 0. Version bump — `src/agent_proving_ground/tool/_sandbox_tools_utils/sandbox_tools_version.txt`
...
### 1. Build — `src/agent_proving_ground/tool/_sandbox_tools_utils/_build_bundled_executable.py`
...
### 3. Injection — `src/agent_proving_ground/tool/_sandbox_tools_utils/sandbox.py`
...
### 4. CLI path constant — `src/agent_proving_ground/util/_sandbox/_cli.py`
...
### 5. Runtime CLI — `src/apg_sandbox_tools/.../_cli/main.py`
...
### 6. Runtime daemon — `src/apg_sandbox_tools/.../_cli/server.py`  (verify, likely keep)
...
1. **Build (local, arm64 to match host):**
   `python src/agent_proving_ground/tool/_sandbox_tools_utils/build_within_container.py --arch arm64`
   → confirm `binaries/inspect-sandbox-tools-arm64-v<N>-dev` is a gzipped tar of the
   onedir tree.
2. **Distro check:** `python -m agent_proving_ground.tool._sandbox_tools_utils.validate_distros`
