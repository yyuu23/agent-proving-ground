...
`src/agent_proving_ground/_eval/task/task_source.py` defines a subclassable base class
with no-op defaults:
...
- **Interception** — `eval_async` ([eval.py](../src/agent_proving_ground/_eval/eval.py),
...
- **Notification firing** — in
  [task/run.py](../src/agent_proving_ground/_eval/task/run.py): `task_run_sample` calls
...
- **The batch loop** — [eval.py](../src/agent_proving_ground/_eval/eval.py) (~line 999):
...
([loader.py](../src/agent_proving_ground/_eval/loader.py), ~line 86) guards against it:
...
- **`enqueue_task()`**
  ([task/enqueue.py](../src/agent_proving_ground/_eval/task/enqueue.py)) — the *imperative*
...
- `TaskSource` is exported from `agent_proving_ground` (`__init__.py` `__all__`).
- `@task_source` decorator ([registry.py](../src/agent_proving_ground/_eval/registry.py),
  exported from `agent_proving_ground`) — **mirrors `@task`**. Decorates a function
...
- **Load-by-name** — `resolve_task_source(tasks, task_args)`
  ([loader.py](../src/agent_proving_ground/_eval/loader.py)), called by `eval_async`,
  ...
  anything else, so normal task resolution proceeds. So `apg eval
  rl.py@my_source -T lr=0.1` and `eval("rl.py@my_source")` both drive the live
  path.
- The `Tasks` type alias ([task/tasks.py](../src/agent_proving_ground/_eval/task/tasks.py))
...
([eval.py](../src/agent_proving_ground/_eval/eval.py)) takes a live path instead of the
`run_batches` loop: it calls `eval_run` **once** with the seed and a
`TaskInjection` ([run.py](../src/agent_proving_ground/_eval/run.py)) built from the run's
enqueuer + source:
...
`run_task_retry_attempts` ([run.py](../src/agent_proving_ground/_eval/run.py)) is a single
...
- **`SandboxManager`** ([run.py](../src/agent_proving_ground/_eval/run.py)) — starts
...
- **`Display.update_task_count(n)`** ([display.py](../src/agent_proving_ground/_display/core/display.py))
...
- **`TaskEnqueuer.on_enqueue`** ([enqueue.py](../src/agent_proving_ground/_eval/task/enqueue.py))
...
([evalset.py](../src/agent_proving_ground/_eval/evalset.py)) resolves the task list,
...
([evalset.py](../src/agent_proving_ground/_eval/evalset.py), `def task_identifier`) is
...
3. **eval_set assumes identifier *uniqueness* — sources may violate it.**
   ...
   ([evalset.py](../src/agent_proving_ground/_eval/evalset.py)) hard-errors if two tasks
...
([eval.py](../src/agent_proving_ground/_eval/eval.py)) reconstructs a `Task` /
