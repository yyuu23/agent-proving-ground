...
- Scoring: New `precomputed_scores()` scorer applies scores computed outside of AgentProvingGround (e.g. human ratings) from a JSON or JSON Lines file, matched to samples by id and epoch.
...
- Security: Control channel sample reads (`apg ctl sample events` / `messages` / `show` / `errors`) now return metadata only by default, so monitors that never pass `--content` cannot be prompt-injected by the evaluated agent's output.
...
- Control Channel: `apg ctl` mutations with piped or captured output now print one outcome line each instead of repeating the full task header (`--terse/--no-terse` to override).
- Control Channel: Every `apg ctl` command's `--help` now sketches its `--json` payload's top-level keys.
...
- Control Channel: `apg ctl sample` and other unscoped commands no longer appear to hang on eval sets with many running tasks. (#4789)
- AgentProvingGround View: `bundle_log_dir()` allows `output_dir` starting with `hf/` when `log_dir` is current directory or parent directory.
...
- AgentProvingGround View: Logs inside the configured directory now open on Windows when listings identify them with canonical file URIs. (#4765)
...
- Models: New `--model-spec` option runs several models in one `apg eval` or `apg eval-set` command, each with its own generation config, model args, and base url.
...
- Sandbox Agent Bridge: Agents can no longer make AgentProvingGround fetch a URL or read a host file by putting it in image or document content.
- Control Channel: `apg ctl task list` now reports per-eval `refusals` and `http_retries`.
...
- Control Channel: `apg ctl sample events --full` now pretty-prints the raw events instead of rendering a mostly-empty summary table.
- Control Channel: New `apg ctl sample messages TASK SID [EPOCH]` reads a running (or buffered-but-unlogged) sample's current conversation as a snapshot, with `--tail`/`--all`/`--full`.
- Control Channel: New `apg ctl task pause|resume` and `apg ctl process pause|resume` commands pause a running eval or eval-set (in-flight samples finish; nothing new starts) and resume it in place, with `paused`/`quiesced` reported by `apg ctl task list`.
...
- AgentProvingGround View: New Timeline tab in the log viewer — a time-axis chart of run activity combined with a filterable event history. (#453)
- AgentProvingGround View: Images embedded in message content as markdown render again (safe `data:`/`file:` sources only, gated by the sanitizer and capped to the transcript column width); raw HTML in message content remains escaped as before. Thanks @rasmusfaber. (#505)
- AgentProvingGround View: Codex Multi-Agent V2 sub-agent model events now show the inter-agent handoff as the turn boundary instead of displaying the entire forked parent context as recent messages. (#504)
- AgentProvingGround View: Codex Code Mode `exec` calls render their JavaScript source as a highlighted, expandable input body instead of a truncated single-line header summary; header-only tool blocks no longer sit flush against their bottom edge. (#504)
- AgentProvingGround View: Live sample summaries read from in-progress logs are deduplicated by id and epoch, so requeued samples no longer appear twice. (#476)
- AgentProvingGround View: Fixed a leaked keyup listener in the image lightbox. (#479)
- AgentProvingGround View: Accessibility improvements across viewer components (jsx-a11y recommended ruleset). (#484)
...
- Control Channel: `apg ctl sample list`/`show` now report a running sample's in-flight activity (`generating 7:12`, `bash 0:41`, `retrying in 0:45`), and `sample events` renders pending events, so long model calls and retry backoffs no longer read as silent idle.
- Retry: Samples reused from a prior attempt no longer stay resident in memory awaiting a flush, and are written to the new attempt's log (readable by `apg ctl` and viewers) as soon as the reuse sweep completes.
...
- Control Channel: `apg ctl sample events --tail` (including the default first page) now counts events matching the `--type` filter, so default reads show a useful recent window instead of a near-empty page. (meridianlabs-ai/agent_proving_ground#162)
- Control Channel: Where the CLI already shows a stall — a long-idle running sample in `apg ctl sample list`, or a busy-skipped process — it now points at `apg ctl process anomalies` as the escalation.
- Control Channel: `apg eval` now prints a one-line pointer to `apg ctl` monitoring at launch, eval/ctl help text cross-links the two surfaces, and `apg ctl task list` flags errored samples.
...
- Bugfix: Solvers, tasks, scorers, and metrics that declare `**kwargs` now survive replay from logs (e.g. `eval_retry`, `apg score`), including keyword arguments named `name`. (#4375)
...
- AgentProvingGround View: Improved log parsing performance; added real-payload benchmarks. (#384)
- AgentProvingGround View: Fixed timeline discarding a solver span containing a single agent span child when it also contains other displayed events. (#443)
- AgentProvingGround View: Timeline now requires a tool-calling loop for utility-agent classification and surfaces the hidden event count. (#425)
- AgentProvingGround View: Added connection limit history display to the Stats tab. (#447)
- AgentProvingGround View: Fixed viewer to show the Action tab first for annotated browser actions. (#429)
- AgentProvingGround View: Downloads of large local log files no longer fail, and the view server stays responsive while reading or listing large local logs.
- MCP: Support the mcp 2.0 package (in addition to mcp 1.x), whose breaking API redesign previously made all `mcp_server_*()` tools fail. (meridianlabs-ai/agent_proving_ground#170)
...
- Control Channel: New `apg ctl model pause|resume` commands pause one model's dispatch (including not-yet-started eval-set tasks) while the rest of the run continues.
- Control Channel: `apg ctl task list` now suggests only the resume commands for latches actually holding a paused task, instead of always listing all three.
- Control Channel: Added `apg ctl sample requeue` to re-run one errored/cancelled sample inside the still-running eval.
...
- Control Channel: New `apg ctl task pause|resume` and `apg ctl process pause|resume` commands pause a running eval or eval-set (in-flight samples finish; nothing new starts) and resume it in place, with `paused`/`quiesced` reported by `apg ctl task list`.
- Control Channel: New `apg ctl sample messages TASK SID [EPOCH]` reads a running (or buffered-but-unlogged) sample's current conversation as a snapshot, with `--tail`/`--all`/`--full`.
- Control Channel: New `apg ctl process anomalies [PID]` shows a running process's in-flight and anomalous trace actions by reading its trace file directly, so it works against busy, hung, or even exited processes.
- Control Channel: `apg ctl config` changes are now recorded in each affected eval log (`EvalLog.config_updates`, with author, timestamp, optional `--reason`, and old → new values); `effective_eval_config()`/`effective_generate_config()` fold them over the launch config.
- Control Channel: `apg ctl sample events --full` now pretty-prints the raw events instead of rendering a mostly-empty summary table.
- Control Channel: The control server now starts under fastapi 0.140.7+, which had silently disabled it by removing an internal helper. (meridianlabs-ai/agent_proving_ground#163)
...
- Agent Bridge: Side model calls (e.g. opencode's session title generation) can no longer replace the agent's real conversation in the tracked agent state. (meridianlabs-ai/agent_proving_ground#140)
...
- Tracing: trace-file reads (`apg trace` commands and `apg ctl process anomalies`) now skip truncated, corrupt, or unrecognized records — e.g. from a hard-killed process or a newer inspect version — instead of failing entirely.
- Windows: process liveness checks (used by `apg ctl` discovery and pid targeting) no longer risk sending a console Ctrl+C to the probed process or misreporting live processes as dead.
- AgentProvingGround View: Added transcript event navigation — keyboard-driven turn and agent navigation with a focus view for drilling into agent subtrees (#354).
- AgentProvingGround View: Added connection limit history display to the Stats tab. (#447)
- AgentProvingGround View: Improved log parsing performance; added real-payload benchmarks. (#384)
- AgentProvingGround View: Timeline now requires a tool-calling loop for utility-agent classification and surfaces the hidden event count. (#425)
- AgentProvingGround View: Fixed timeline discarding a solver span containing a single agent span child when it also contains other displayed events. (#443)
- AgentProvingGround View: Fixed viewer to show the Action tab first for annotated browser actions. (#429)
- AgentProvingGround View: Fixed sanitization of state diff HTML, added alt text to images, and cleaned up leaked timers and scroll-to-top behavior (#428).
...
- Control Channel: `apg ctl sample list` no longer recomputes sample summaries on every request, so polling an eval buffering many large samples (e.g. a retry's carried transcripts) can no longer stall the eval process.
...
- Control Channel: `apg ctl sample list` now shows per-sample turn count and, when a token limit is configured, its computed usage and configured ceiling.
- Control Channel: `apg ctl task cancel` and `apg ctl sample cancel` gain an `--action` option (`score`/`error`/`cancel`) selecting how samples are resolved, so a cancelled eval can still complete (replaces `apg ctl sample cancel --error`).
- Control Channel: `apg ctl sample errors` now filters errored/retried samples server-side, so triage polls of large evals no longer build and ship the full sample listing.
- Control Channel: `apg ctl` sample commands given an exact full task id now stop discovery at the process hosting it, skipping busy-retry delays from older sibling processes.
- Control Channel: `apg ctl config --key NAME LIMIT` retunes any named `concurrency()` limit mid-flight, without the registering code opting in; the config view lists the registered keys.
- Control Channel: `apg ctl config` can now retune `--timeout`, `--attempt-timeout` and `--max-retries` mid-flight, reaching even generate calls already retrying (pass `clear` to restore launch config).
- Control Channel: `apg ctl sample list` now caps its listing at the 100 most relevant rows by default (`--limit`/`--all` to adjust, `--status` to filter) and reports a complete status histogram plus a `truncated` flag in its envelope.
- Control Channel: `apg ctl sample events` now shows the data payload of `transcript().info()` events in its default compact output (previously only the source was shown).
- Control Channel: `apg ctl sample events` now accepts `--type all` (shell-safe spelling of `--type '*'`) and `--from-start` to read the full backlog from the first event.
- Control Channel: `apg ctl sample events` gains `--limit N` to cap the events returned per page (combinable with `--from-start`, `--tail`, or a cursor).
- Control Channel: New `--json` flag for `apg eval`, `apg eval-set`, and `apg eval-retry` emits machine-readable launch-handoff and `done` JSON lines on stdout, so agents can use `apg ctl` immediately after launch instead of sleep-and-retrying.
- Control Channel: New `--detach` flag for `apg eval`, `apg eval-set`, and `apg eval-retry` runs the eval in the background — the command prints the launch record and returns once the control endpoint is bound, leaving the eval running detached from the terminal (monitor with `apg ctl task list`).
- CLI: `apg trace anomalies` and `apg trace http` gain a `--json` flag emitting machine-readable output for agents and shell pipelines; the JSON always includes error/timeout buckets (no `--all` needed).
- CLI: `apg trace anomalies` no longer errors when `--filter` matches an action's completion record but not its start record.
...
- Control Channel: `apg ctl sample show` now reads the sample's summary and error detail in one atomic request, so its output can no longer mix data from different attempts.
- Control Channel: `apg ctl sample show` now reports a cancelled sample as `cancelled`/`pending` (matching `sample list`) instead of `error` with the cancellation as its error.
...
- Control Channel: Added `apg ctl task cancel` and `apg ctl sample cancel` to cancel a running task or one running sample (idempotent, with `--dry-run`).
...
- AgentProvingGround View: Reuse one warm async S3 client and connection pool across requests — for both log reads and directory listings — instead of creating one per operation, eliminating the per-request credential/connection cold-start (e.g. `/log-headers` ~3s -> ~0.3s, `/logs` ~1.5s -> ~0.06s).
...
- Control Channel: Reorganized the `apg ctl` CLI into resource-noun groups (`ctl task`, `ctl sample`, `ctl config`, `ctl process`); the old flat spellings remain as hidden deprecated aliases, except `ctl sample` which is now the group (use `ctl sample show`).
- Control Channel: `apg ctl` sample commands now warn and skip an eval that stays busy through the retries instead of failing outright, with stderr caveats (and an honest non-zero exit when no tasks remain visible) wherever the skip could mislead.
- Control Channel: `apg ctl sample show`/`sample events` payload reads and `process keep`/`release` now retry a busy eval with narrated attempts instead of failing after a single short attempt.
- Control Channel: `apg ctl config` now errors when the target process runs an inspect version too old to support a requested knob, instead of silently ignoring it.
- Control Channel: Failed `apg ctl` `--json` invocations now emit a structured `{"error": {kind, exception, message, status}}` envelope on stdout (exit code still non-zero) instead of only stderr prose or a raw traceback; human output is unchanged.
- Control Channel: `max_subprocesses` is now retunable mid-flight via `apg ctl config --max-subprocesses` and the `PATCH /config` endpoints, alongside the other concurrency knobs.
...
- Control Channel: `apg ctl tasks` now includes model and solver columns.
- Control Channel: Added `apg ctl limits` to view or retune a running eval's `max_samples` / `max_sandboxes` / `max_connections` concurrency limits mid-flight (with `--dry-run`).
...
- AgentProvingGround View: Validate Host and browser origins, prevent framing, and require authorization or explicit acknowledgement for non-loopback binds.
...
- Control Channel: Added `apg ctl flush` (write a running eval's buffered samples to the log now, e.g. to make S3-backed results analyzable without waiting) and `apg ctl buffer` (view or change the `--log-buffer` / `--log-shared` sample-buffer params of a running eval).
- Control Channel: `apg ctl tasks` no longer drops eval processes owned by another user, which were previously misreported as dead.
...
- Control Channel: `apg ctl tasks` now pins each eval's reported start to its first sample's start instead of letting it drift forward as early samples finish.
- Control Channel: `apg ctl` reads now use a 15s timeout and retry a busy eval up to 8 times (printing a status on each timeout) before failing with a non-zero exit, instead of silently dropping a momentarily-unresponsive eval from the listing.
...
- Scoring: Support `model` and `model_roles` overrides for re-scoring (`apg score --model` / `--model-role`).
...
- AgentProvingGround View: Require frontend-only headers for mutations and use non-GET routes for log deletion and client messages.
- AgentProvingGround View: Improve MathJax Sanitization
- AgentProvingGround View: Fix stale running status on nav
- AgentProvingGround View: Fix broken commit links for ssh-style GitHub origins
- AgentProvingGround View: Cap oversized tool/text output to prevent resize layerization stalls
- AgentProvingGround View: Fix event panel nav pills never expanding back from picker mode
- AgentProvingGround View: Fix inline MathJax stacking under inherited white-space: pre-wrap
- AgentProvingGround View: guard AgentProvingGround view mutation requests
...
- Control Channel: `apg ctl tasks` now reports keep-alive status (`on` / `off` / `mixed`) and a new `apg ctl keep` command (backed by `POST /keep`) latches keep-alive on a running process so it parks after its eval.
...
- AgentProvingGround View: Improve sample reading performance in viewer by serving `/log-bytes` range requests as plain responses instead of line-iterating a `BytesIO`.
- AgentProvingGround View: Fix log-list grid columns snapping back to default widths while data loads
- AgentProvingGround View: Fix transcript deep links across timelines, approvals, collapsed regions, and lanes; add event label pills.
- AgentProvingGround View:  Improve tool input density
...
- Control Channel: `apg eval` / `apg eval-set` now bind a per-process control server (AF_UNIX, default on) exposing a read surface for the live run. New `apg ctl` commands let another process (CLI, scripts, agents) observe a running eval / eval-set.
...
- AgentProvingGround View: Pending sample buffers now store per-sample segment cursors to avoid repeatedly streaming co-batched segments after a sample reaches its own cursor. In-progress buffers written by this version may not be readable by older AgentProvingGround versions until the eval finishes and the ephemeral buffer is removed.
...
- AgentProvingGround View: Display model fallbacks (samples grid column, `has_fallbacks`/`fallbacks` filter variables, sample header, transcript fallback marker and model-event badge).
- AgentProvingGround View: Fix occasional intermittent hang while attempting to navigate to sample
- AgentProvingGround View: Improve server side tool rendering transcripts and messages
- AgentProvingGround View: Fix stuck completed streaming samples
...
- AgentProvingGround View: Improve model event rendering - add INFO tab (usage + stop reason) and a Stop Reason display
- AgentProvingGround View: Fix: stop VirtualList from fighting deep-link scroll in WebKit
- AgentProvingGround View: Fix: collapse the timeline by default when only main + scoring lanes exist
- AgentProvingGround View: Fix: make collapse-all include model events (symmetry with expand-all)
- AgentProvingGround View: Improved sample message/transcript placeholders + steady streaming
- AgentProvingGround View: Color tool calls in model-event views like the messages tab
- AgentProvingGround View: Improve log list responsiveness during bulk sync
...
- Agent Bridge: By default the bridge no longer forwards client generation-tuning parameters (e.g. `max_tokens`, `temperature`, reasoning effort/tokens) to the resolved AgentProvingGround model, leaving these parameters entirely determined by the evaluation config. Pass `forward_generation_config=True` to `agent_bridge()`/`sandbox_agent_bridge()` to restore previous behavior.
...
- Transcript: Bound resident memory for long-running samples by evicting older events to a history provider (opt-in via the `APG_TRANSCRIPT_BOUNDED` environment variable). `transcript().events` remains a full, compatible view; use `transcript().history` for memory-aware access.
- AgentProvingGround View: New `ViewerConfig` (passed via `Task(viewer=...)`) lets eval authors customize how a task's sample list, score panel, and scanner results render in the log viewer — including sample-list columns, default sort, score labels, and color scales. See [Custom Views](https://inspect.aisi.org.uk/task-views.html).
- Transcript: Revert disabling of buffer history database when running tests.
- AgentProvingGround View: Dark mode, event and message color support.
- AgentProvingGround View: Migrated the sample transcript to a virtualized list for smoother rendering of long transcripts.
- AgentProvingGround View: Collapse same-name nested solver/agent spans in transcripts.
- AgentProvingGround View: Fix truncation of long transcript outlines when scrolling.
- AgentProvingGround View: Switch to dev dependencies for scout and inspect apps.
- AgentProvingGround View: Fixes to live sample following behavior.
- AgentProvingGround View: Properly clear log/sample before paint when viewing log (#284).
- AgentProvingGround View: don't render primitive solvers as sub-agents (#287).
- Bugfix: AgentProvingGround View sample-list columns now expand to fill the available width.
...
- OpenAI: Map AgentProvingGround `todo_write` tool to native OpenAI/Codex `update_plan` tool.
...
- AgentProvingGround View: Improvements to Codex CLI agent rendering.
...
- AgentProvingGround View: Add UI for editing log tags and metadata.
...
- Reasoning: `--reasoning-effort` now works uniformly across Claude 3.7–4.5 and Gemini 2.5. AgentProvingGround automatically bridges effort to a `budget_tokens` / `thinking_budget` value using a fixed table (minimal=2048, low=4096, medium=10000, high=16000, xhigh/max=32000). Previously these models silently ignored `reasoning_effort`.
...
- Scanning: Compatibility with AgentProvingGround Scout 0.4.37.
- Async: `run_coroutine()` (and the sync log/analysis helpers built on it) now honour `APG_ASYNC_BACKEND=trio` when called with no running event loop, rather than always using asyncio.
...
- AgentProvingGround View: Replaced the aiohttp server with a FastAPI server. `fastapi` and `uvicorn` are now required dependencies.

## 0.3.223 (18 May 2026)

- Config: Add `apg log export-config` command to export a run config from an existing log file.
- Anthropic: Skip thinking blocks when placing lookback cache_control.
- AsyncFilesystem: Add `get_file()` and `exists()` methods.
- AgentProvingGround View: Fix regression where switching task tabs would reload log, causing latency.
...
- Config: Add `--run-config` option to `apg eval` for single-file run configuration.
- Eval Set: Run [AgentProvingGround Scout](https://meridianlabs-ai.github.io/inspect_scout/) scanners over each task's logs as part of `eval_set` (CLI `--scanner` / `ScannerConfig`). Scans incrementally as logs land, reuses prior results across resumes, and renders progress alongside the existing eval view.
...
- Imports: Delay importing heavier dependencies (e.g. s3fs, boto3, numpy, rich.markdown) for faster imports of `agent_proving_ground` module.
- Logging: `APG_PY_LOGGER_FORMAT` env var (`rich`/`plain`/`json`) for non-TTY-friendly single-line console logs.
...
- AgentProvingGround View: Pending samples fetch directly from S3 with chunked loading
- AgentProvingGround View: Score color scales applied to sample-header chips
- AgentProvingGround View: Outline close icon aligned; rootHeader made sticky
- AgentProvingGround View: Fixed model retry event rendering
- AgentProvingGround View: Fixed message ordering in running-sample Messages tab with cross-poll caching
...
- Bugfix: `apg log recover` — preserve `sample.uuid` for crashed in-progress samples (initial buffer summary now carries `state.uuid`; recovery synthesizes a fallback uuid for legacy buffer rows).
...
- AgentProvingGround View: Fix extraneous console errors
- AgentProvingGround View: In Folder and Task List, fix cmd+click/middle-click to open log in a background tab.
- AgentProvingGround View: Task samples - preserve sort and other state in VS Code.
...
- Add `download()` and `gdrive_download()` helpers for fetching external files with SHA256 verification, caching, and transient-error retry. `gdrive_download()` requires the optional `gdown` dependency, installed via `pip install agent_proving_ground[gdown]`.
...
- AgentProvingGround View: Add score comparator with NaN filtering and regex-escaped property matching (#167)
- AgentProvingGround View: Improve grid sorting with NaN values and fix sample column sizing (#166)
- AgentProvingGround View: Make live-streaming message/call pool processing idempotent by entry id to prevent duplicated messages in API request views (#168)
...
- AgentProvingGround View: Collapse the LogView title bar on scroll
- AgentProvingGround View: Add additional column support when viewing task samples.
- AgentProvingGround View: Add sample uuid column id and filtering.
- AgentProvingGround View: Fix issue causing column sizing and re-ordering to snap back to initial state.
- AgentProvingGround View: Fix copy command including large binary payloads.
...
- Display: Add a "Flow" tab to the full (textual) display, populated by AgentProvingGround Flow via an internal `set_flow_content()` integration point.
- AgentProvingGround View: Fix live transcript streaming so model inputs and calls resolve to the correct messages across buffer segments.
...
- AgentProvingGround View: Display approvals within tool calls and filter auto-approvals from the display.
- AgentProvingGround View: Improve formatting of rejected and long approval messages.
- AgentProvingGround View: Fix phantom "more" toggles on expandable panels by measuring overflow against the correct font size.
- AgentProvingGround View: Keep the expandable panel toggle on screen when content exceeds the viewport height.
- AgentProvingGround View: Remove duplicated borders and corners on root elements of the transcript.
- AgentProvingGround View: Default to showing branches expanded in the timeline.
- AgentProvingGround View: Fix outline, citation, and swimlane navigation to reliably scroll to the targeted event.
- AgentProvingGround View: Add branch-aware citation deep-linking and land target messages just below the sticky bar.
- AgentProvingGround View: Detect additional todo/task tool names for inspect-deepagent.
- AgentProvingGround View: Persist task list filter and sort independently per scope to prevent state leaking between Tasks and Folders.
- AgentProvingGround View: Add Tokens and Duration columns to the samples list.
- AgentProvingGround View: Fix timeline error markers that flagged every model and tool event.
- AgentProvingGround View: Add a fade-out affordance at the bottom of truncated content in expandable panels.
- AgentProvingGround View: Collapse system, user, assistant, and tool chat messages by default with per-role line caps.
- AgentProvingGround View: Improve sample appearance at narrow widths.
- AgentProvingGround View: Improve reliability of viewing running samples.
...
- AgentProvingGround View: Improve wrapping behavior in message and events.
- AgentProvingGround View: Fix issues positioning chat messages within model events.
- AgentProvingGround View: Improve tool call error rendering.
- AgentProvingGround View: Improve model event tool configuration rendering.
...
- AgentProvingGround View: Metadata with more than 5 children will be collapsed by default (for real).
- AgentProvingGround View: Improve column selection with fine-grained metrics selection in Folder and Tasks view.
- AgentProvingGround View: Minor UX improvements.
...
- AgentProvingGround View: Metadata with more than 5 children will be collapsed by default.
...
- AgentProvingGround View: Display results of Scout scanners used as scorers next to transcripts.
- AgentProvingGround View: New columns in task and log view: tags, % completed, sample errors, and error.
- AgentProvingGround View: Fix regression in messages view which causes excessive whitespace between messages.
- AgentProvingGround View: Fix error when attempting to collapse all or expand all events in transcripts.
- AgentProvingGround View: Improvements to expand / collapse behavior in transcripts.
- AgentProvingGround View: Don't show empty entries in messages view when a message is retried.
...
- Eval Logs: New `apg log recover` command for recovering crashed eval logs from the sample buffer database. Recovers both completed (unflushed) and in-progress samples. Automatic recovery is integrated into `eval_set()` and `eval_retry()`.
...
- Sandboxes: `APG_SANDBOX_MAX_READ_FILE_SIZE` and `APG_SANDBOX_MAX_EXEC_OUTPUT_SIZE` environment variables for overriding limits.
...
- AgentProvingGround View: Use FastAPI server when `fastapi` and `uvicorn` packages are available.
- AgentProvingGround View: Transcript viewing improvements for complex transcripts (timeline + other fixes)
- AgentProvingGround View: Introduce new 'Tasks' view of log directory which shows tasks recursively as a flat list.
- AgentProvingGround View: Fix error when viewing the API information for a running Model Event.
...
- AgentProvingGround View: Inline application assets.
...
- AgentProvingGround View: Fix printing for samples with large transcripts or many messages.
- AgentProvingGround View: Fix issues that would cause a running sample display to wait for the task to complete before showing final score.
- AgentProvingGround View: Fix regression that could hide assistant messages with only tool calls.
- AgentProvingGround View: Move log viewer frontend from `src/agent_proving_ground/_view/www/` into `ts-mono/apps/inspect/` monorepo (pnpm + Vite + Jest). Built assets are copied to `src/agent_proving_ground/_view/dist/` via a Vite plugin. No user-facing changes.
- AgentProvingGround View: Built TypeScript code is now minified and committed via git lfs.
...
- AgentProvingGround View: Copy button for log files now copies the absolute path (or S3 URI) rather than the relative serving path.
...
- Logging: APG_SUBPROCESS_REDIRECT_TO_LOGGER env var to pipe subprocess output to logging.
...
- AgentProvingGround View: Improve reliability of scrolling to bottom of long transcripts.
- AgentProvingGround View: Improve viewer loading performance for large samples.
- AgentProvingGround View: Ensure tool output wraps for rendered text.
- AgentProvingGround View: Improve scrolling behavior when moving to next scroll result.
...
- AgentProvingGround View: Fix error that prevented samples from being printed.
- AgentProvingGround View: Always render `<think>` tags that appear in assistant messages.
- AgentProvingGround View: Improve nested list formatting in rendered markdown content.
- AgentProvingGround View: Correct display of nested solver > agent in transcripts.
...
- AgentProvingGround View: Add 'None' preset when choosing transcript events to display.
- AgentProvingGround View: Add middle-click support to open tasks and samples in a new browser tab.
...
- AgentProvingGround Score: Add support for an optional list of metrics when rescoring a log.
- AgentProvingGround View: Embed viewer directly in the log directory instead of a `viewer/` subdirectory, fixing permission issues when serving logs.
- AgentProvingGround View: Fix incorrectly themed sample column header text in VS Code (especially dark themes).
- AgentProvingGround View: Fix issue where expanding one message in a sample chat would expand all messages.
...
- AgentProvingGround View: Fix regression that prevented proper display of running samples.
...
- AgentProvingGround View: Add `apg view embed` command and `--embed-viewer` eval-set option to embed a log viewer into a log directory.
- AgentProvingGround View: Display proper message when transcript events are removed to reduce eval log size.
- AgentProvingGround View: Properly compute nested span parents when rendering the event tree.
...
- AgentProvingGround View: Fix truncation of the bottom of events and messages panels.
- AgentProvingGround View: Improve appearance of model events in transcript.
...
- AgentProvingGround View: Add support for find in log list.
- AgentProvingGround View: Fix regression displaying running samples when switching samples.
...
- AgentProvingGround View: Improve virtualized find in transcript by matching event titles as well as contents.
...
- AgentProvingGround View: Presigned URL support for S3 log files, enabling direct browser-to-S3 byte-range fetches with parallel chunk downloads and a determinate progress bar for large samples.
...
- AgentProvingGround View: Add support for downloading sample JSON.
- AgentProvingGround View: Server returns correct content length for responses.
...
- Bugfix: Fix AgentProvingGround View showing stale sample data when rapidly switching between samples.
...
- AgentProvingGround View: Display all non-undefined edited score values.
...
- AgentProvingGround View: Make samples in task detail sortable, inline epoch filter, show sample status.
...
- Bugfix: Fix regression in live sample display in AgentProvingGround View.
...
- AgentProvingGround View: Fix regression in log viewer navigation in VSCode.
- AgentProvingGround View: Improve transcript display appearance in VSCode.
- AgentProvingGround View: Improve log events display in transcripts.
- Bugfix: Fix off-by-one in `_read_all_summaries` that skipped the last sample summary.
- AgentProvingGround View: Fix stale state read in logsSlice after syncLogs.
...
- AgentProvingGround View: Show live requests and responses for model call events in the transcript.
- AgentProvingGround View: Improve scroll performance when viewing sample transcripts and messages.
...
- AgentProvingGround View: Fix a regression which affected the display of samples within VSCode.
...
- AgentProvingGround View: Add sticky headers for transcript events and show turn number to improve "where am I" UX.
- AgentProvingGround View: Show errors in model call events in the transcript.
- AgentProvingGround View: Show tracebacks in model call events in the transcript.
- AgentProvingGround View: Fix slow performance loading samples caused by unnecessary repeat load calls in SampleDetailView.
- AgentProvingGround View: Fix incorrectly disabled sample navigation when viewing samples with numeric sample ids.
- AgentProvingGround View: Fix issue where a large number of logs could result in laggy or hung viewer in VSCode.
...
- AgentProvingGround View: Improve reliability of code syntax highlighting in messages and events.
- AgentProvingGround View: Support zstd compression of eval log file contents.
- AgentProvingGround View: Fix issue where viewing sample events could result in flashing and scroll oscillation.
- AgentProvingGround View: Render `<think>` tags when included in user messages.
...
- Eval Logs: Support Zstd compression of eval logs for improved performance via `APG_USE_ZSTD` environment variable.
...
- [nnterp](https://inspect.aisi.org.uk/providers.html#nnterp) model provider enabling use of `StandardizedTransformer` models with AgentProvingGround.
...
- AgentProvingGround View: Fix issue where switching from a running to a non-running evaluation could display incorrect metrics in the title region.
- AgentProvingGround View: Fix sample switching when viewing live transcripts.
...
- AgentProvingGround View: Fix "Cannot add property timestamp, object is not extensible" error when viewing live transcripts.
...
- AgentProvingGround View: Timestamps for USER and ASSISTANT transcript of model events, `yyyy-mm-dd hh:mm:ss` format (keep local time zone).
- AgentProvingGround View: Remove events from JSON before parsing if Sample JSON is too large.
...
- AgentProvingGround View: Improve sample summary truncation (use markdown truncation instead of line clamping).
- AgentProvingGround View: Fix issue with typing over selection in 'Find'
- AgentProvingGround View: Fix issues with 'Find' scrolling, keyboard behavior, and restoration of scroll position / panel expansion.
- AgentProvingGround View: Support find using JSON-like syntax.
...
- AgentProvingGround View: Fix regression where viewing samples with identical id/epoch would re-use the previous sample details.
- AgentProvingGround View: Show event timestamp in tooltips in all types of events in transcripts.
- AgentProvingGround View: Show sample invalidation status in sample header.
...
- Sandboxing: Add APG_SANDBOX_SETUP_TIMEOUT env var to override default 300s setup timeout.
...
- Anthropic: Correctly replay reasoning when sourced from AgentProvingGround cache.
...
- AgentProvingGround View: Don't attempt to display events when the events are too large for the browser to deserialize (e.g. 350MB+ of events).
- AgentProvingGround View: Improve rendering of tool output with ANSI codes. Support viewing raw/unrendered ANSI output.
- AgentProvingGround View: Scale ANSI display in messages view to preserve row/column layout without wrapping.
- AgentProvingGround View: Render custom tool view when viewing messages.
- AgentProvingGround View: Fix cmd+click on tasks/samples to open in new tab.
- AgentProvingGround View: Only stream log bytes when requested chunks are large (>50MB)
- AgentProvingGround View: Add Show Retried Logs button when inside an eval set and some logs were retried (both Tasks and Samples are now de-duplicated by default).
- AgentProvingGround View: Improved non-native find for virtualized lists (better CTRL-f)
...
- AgentProvingGround View: Properly display dict scores in sample list.
- AgentProvingGround View: Improve display of Codex `shell_command` tool calls.
- AgentProvingGround View: Improve the display of very wide metrics results in the results dialog.
...
- AgentProvingGround View: Add "robots" meta tag with "noindex,noai,noimageai".
- AgentProvingGround View: Enhance `apg view bundle` to publish `hf/` prefixed targets to Hugging Face Spaces.
- AgentProvingGround View: Improve rendering of Open Router reaasoning blocks.
- AgentProvingGround View: Correct the filter type for `Log File` column of the samples list.
...
- AgentProvingGround View: Display timestamp in sample limit events tooltip.
- AgentProvingGround View: Fix issue where stale log files could be displayed in static deployments of the viewer.
- AgentProvingGround View: Fix issue where switching away from Viewer in VSCode could cause an error when returning to the viewer tab.
- AgentProvingGround View: Tasks filter split as into per-column filters. Option to display scorer columns in Choose Columns popover.
...
- Tasks: Ability to run AgentProvingGround tasks defined within datasets on [Hugging Face Hub](https://inspect.aisi.org.uk/tasks.html#hugging-face).
...
- AgentProvingGround View: Print IP address of remote server when binding to 0.0.0.0.
...
- AgentProvingGround View: Expand dictionary scores into separate scores when viewing samples.
...
- MCP Tools Bridge: Added `BridgedToolsSpec` and `bridged_tools` parameter to `sandbox_agent_bridge()` for exposing host-side AgentProvingGround tools to sandboxed agents via MCP protocol.
...
- AgentProvingGround View: Fix regression where the display of samples with errors would result in unusuably wide sample list view.
- AgentProvingGround View: Properly compute sample list columns for running evaluations that return dictionary scores.
...
- Sandboxes: Added `evals_in_eval` example for running AgentProvingGround evaluations inside other evaluations.
...
- AgentProvingGround View: Improve markdown rendering performance.
- AgentProvingGround View: Reduce use of virtualized display for smaller transcripts and message lists.
- AgentProvingGround View: Add support for copying sample messages (as text).
- AgentProvingGround View: Improved JSON parsing performance & scalability.
...
- Model API: Conversion functions for translating raw model input and output into AgentProvingGround types.
...
- AgentProvingGround View: Truncate display of large sample summary fields to improve performance.
- AgentProvingGround View: Fix regression in displaying S3 log files in VS Code.
...
- AgentProvingGround View: Streaming for log bytes requests in fastapi view server.
...
- AgentProvingGround View: Fixed theming issue with the score selector control in VS Code.
- AgentProvingGround View: Improve display of grouped metrics
- AgentProvingGround View: Don't render markdown images in the sample list input, target, or answer.
- AgentProvingGround View: Add Copy UUID button to sample toolbar.
- AgentProvingGround View: Add new samples view which displays a grid of all samples within a given folder of tasks (recursively).
- AgentProvingGround View: Invalidate client side caches for updates to statically deployed versions of the viewer.
- AgentProvingGround View: Fix flash of error that can occur when viewing folders with no eval-set info in statically deployed versions of the viewer.
...
- AgentProvingGround View: Fix performance regression when loading a log view from an S3 bucket.
- AgentProvingGround View: Fix flash of 'No events' message when loading sample transcripts.
...
- AgentProvingGround View: Display copy button for model events api request and response JSON.
- AgentProvingGround View: Support selecting multiple scorers for display in the sample list.
- AgentProvingGround View: Show multiple scores in sample view.
- AgentProvingGround View: Fix issue where samples would jump around when viewing running evals with high concurrency.
- AgentProvingGround View: Improve log and sample list performance by adding a persistent local cache.
- AgentProvingGround View: Display a message when there are no events to display in a transcript.
...
- AgentProvingGround View: Fix regression that resulted in an error when displaying JSON log files.
...
- AgentProvingGround View: Improve the display of grouped metrics.
...
- AgentProvingGround View: Properly display task error when a task error is present in the log file.
- AgentProvingGround View: Improve display of running eval sets (correct duplicate files and incorrect progress display).
- AgentProvingGround View: Provide option to run `apg view` server using uvicorn / fastapi.
...
- Event API: Created new `agent_proving_ground.event` module with event related tyeps and functions.
...
- AgentProvingGround View: Properly truncate sample input and and target (in sample header) even when it contains large pre-formatted text blocks.
...
- Scoring: Fix regression in `apg score` command (and `inspect_score` function) when scoring log files on S3.
- AgentProvingGround View: Improve display of OpenAI Codex tool calls in transcript and messages.
- AgentProvingGround View: Display eval set progress in the lower right corner of the log list.
- AgentProvingGround View: Make message and event link affordances slightly more discoverable.
- AgentProvingGround View: Preserve query parameters in log viewer URLs when copying links.
- AgentProvingGround View: Fix issue where sometimes the incorrect log is displayed when new logs are added to a log directory while viewing a log.
...
- AgentProvingGround View: Improve rendering for Codex CLI shell tool output.
...
- Sandbox tools: Rewrite `agent-proving-ground` package installation type detection code.
...
- AgentProvingGround View: Fix regression sorting folder and logs in list (folders should sort to the front of the list)
- AgentProvingGround View: Properly reset page when navigating between folders.
- AgentProvingGround View: Always show reasoning blocks (previously we hid them when there was no content, i.e. Responses API store=True).
- AgentProvingGround View: Improve the display of Codex Agent `update_plan` and `shell` tool inputs.
- AgentProvingGround View: Fix flash of error message when initially viewing a log file in VS Code.
- AgentProvingGround View: Properly create tree for transcripts when tasks include async work generating spans and events.
...
- AgentProvingGround View: Display pending tasks in eval sets (tasks that have not yet started running)
- AgentProvingGround View: Fine tune status appearance to improve legibility
- AgentProvingGround View: Fix issue displaying folders with with overlapping path prefixes.
...
- AgentProvingGround View: Add support for cmd + arrow up/down to navigate the samples list.
- AgentProvingGround View: Improve scroll keyboard handling in sample transcript view.
- AgentProvingGround View: Improve scroll keyboard handling in sample messages view.
- AgentProvingGround View: Improve find support within sample list, transcript, and messages.
- AgentProvingGround View: Fix issue that would result in `attachments:/` appearing in content when viewing running samples.
...
- Scoring: `apg score` now supports streaming via the `--stream` argument.
- AgentProvingGround View: Starting the view server with a path to a specific log file will automatically open that log file (if it exists) rather than showing the log list.
- AgentProvingGround View: Improve sample scoring detail layout
- AgentProvingGround View: Reduce h1-h6 heading sizes
- AgentProvingGround View: Fix error that caused 'json too large' message to appear incorrectly for sample JSON.
- AgentProvingGround View: Improve filtering of log files in log list (improve performance and loading progress).
- AgentProvingGround View: Add cmd+F shortcut for filtering log in log list.
- AgentProvingGround View: Fix regression in tool input syntax highlighting.
- AgentProvingGround View: Focus transcript or messages when sample dialog is loaded, allowing use of keyboard shortcuts like cmd + arrow down for scrolling.
- AgentProvingGround View: Focus log list when the log list is shown, allowing use of keyboard shortcuts like cmd + F.
...
- AgentProvingGround View: Add support for filtering sample transcripts by event types. Be default, filter out `sample_init`, `sandbox`, `store`, and `state` events.
- AgentProvingGround View: Add support for displaying raw markdown source when viewing sample data.
- AgentProvingGround View: Remove sample list / title content when sample is displaying (prevents find from matching content behind the sample detail).
- AgentProvingGround View: Custom rendering for TodoWrite tool calls.
...
- Scoring: Refactor `apg score` to call same underlying code as `score()`.
...
- Agent Bridge: AgentProvingGround model roles can now be addressed by bridged agents (e.g. "inspect/red-team").
...
- AgentProvingGround View: Convert samples in the sample list to use simple a tags for navigation. This allows typical user gestures like cmd+click to work correctly.
- AgentProvingGround View: Update document titles when viewing a sample, log, or log dir to better disambiguate tabs or windows. Use reverse pyramid to place details at the head of the title.
- AgentProvingGround View: Increase sample size limit to 100MB (samples larger than that are not browsable in the viewer).
- Tool Support: Converted to a new runtime reconnaissance and injection architecture for `apg_tool_support`.
...
- AgentProvingGround View: Improved handling of scores and messages with large or complex metadata.
- AgentProvingGround View: Web search and other server-side tool calls (e.g. remote MCP) are now shown in the transcript.
- AgentProvingGround View: Properly display scores with list values.
...
- Tool Support: Converted `apg_tool_support` to use a Unix socket rather than a tcp port for intra-container RPC.
...
- Analysis functions are out of beta (`agent_proving_ground.analysis.beta` is deprecated in favor of `agent_proving_ground.analysis`).
...
- AgentProvingGround View: Do not use instance cache for S3FileSystem (eliminates some errors with large eval sets)
...
- AgentProvingGround View: Use MathJax rather than Katex for math rendering.
- AgentProvingGround View: Fix issue with scores 'More...' link not being displayed in some configurations.
- AgentProvingGround View: Fix issue displaying tool calls in transcript in some configurations.
...
- ReAct Agent: Require submit tool to have no errors before you exit the react loop.
- AgentProvingGround View: Use MathJax rather than Katex for math rendering.
- AgentProvingGround View: Supporting linking to events via `uuid` field (or `event_id` in analysis data frames).
- Bugfix: Use the output filesystem when creating directories in `apg log convert`
...
- [TransformerLens](https://inspect.aisi.org.uk/providers.html#transformer-lens) model provider enabling use of `HookedTransformer` models with AgentProvingGround.
...
- Eval Log: Add `--resolve-attachments` option to `apg log dump`.
- Hooks: Provide full `EvalSample` (rather than only the summary) to `on_sample_end()` hook.
- AgentProvingGround View: Compatiblility for sites published to GitHub Pages for `apg view bundle`.
- AgentProvingGround View: The bundle produced for deployment now includes a much more compact manifest, improving support for bundling large numbers of files.
...
- [Hooks](https://inspect.aisi.org.uk/extensions.html#hooks): Generic lifecycle hooks for AgentProvingGround extensions.
...
- AgentProvingGround View: Fix failure to restore VSCode state when switching to/from tabs for some class of log files.
- Bugfix: Conform to breaking changes in `mistralai` package (1.9.1).

## 0.3.111 (29 June 2025)

- AgentProvingGround View: Fix issue with tab switching when running in VS Code.
...
- Fix store_as examples, document agent_proving_ground.scorer.score
...
- Docs: Correct shared documentation snippet that describes Dockerfile customization for AgentProvingGround Tool Support.
- AgentProvingGround View: Properly wrap log configuration values in evaluation header.
- AgentProvingGround View: Support for displaying and navigating directories of evaluation logs.
- AgentProvingGround View: Improved handling of agent handoffs in transcript outline view.
- AgentProvingGround View: Use numerical rather the correct/incorrect UI for scores with 0/1 values.
...
- [sandbox_service()](https://inspect.aisi.org.uk/agent-custom.html#sandbox-service) function for making available methods to a sandbox for calling back into the main AgentProvingGround process.
...
- AgentProvingGround View: Display metadata for Chat Messages.
- AgentProvingGround View: Increase transcript outline font size.
- AgentProvingGround View: Add support for filtering by sample id, sample metadata.
...
- AgentProvingGround View: Add support for displaying citations for web searches in the transcript.
- AgentProvingGround View: Correctly update browser URL when navigation between samples.
...
- AgentProvingGround View: Display sample id & epoch in sample dialog title bar.
- AgentProvingGround View: Don't open sample dialog when simply navigating the sample list.
- AgentProvingGround View: Fix error that could occur when determine transcript outline collapse state.
- AgentProvingGround View: Show the correct sample when opening a sample from a sorted list.
...
- Limit `docker compose` concurrency to 2 * os.cpu_count() by default (override with `APG_DOCKER_CLI_CONCURRENCY`).
...
- AgentProvingGround View: Outline view for transcript which enables high level navigation to solvers, agents, scorers, etc.
- AgentProvingGround View: Fix an issue that prevented the display of the viewer in VSCode when the viewer tab was moved to the background.
- AgentProvingGround View: Don't error when metadata contains null values.

## v0.3.99 (22 May 2025)

- Exported `view()` function for running AgentProvingGround View from Python.
...
- AgentProvingGround View: Improve text zooming and wrapping when rendering sample errors.
- AgentProvingGround View: Preserve log mtime-ordering in the bundle output directory
...
- AgentProvingGround View: Improve sample score column layout, markdown render explanation.
...
- AgentProvingGround View: Improve the performance and memory efficiency of the viewer when viewing large samples with long, complex transcripts.
- AgentProvingGround View: Improve the performance of the viewer when viewing large, complex sample or task metadata.
- AgentProvingGround View: Live display of subtask, tool and other child events when viewing a running evaluation.
- AgentProvingGround View: Transcript rendering improvements including less complex overall layout, more collapsible entities, and improved rendering of sandbox events, tool calls, and other events.
- AgentProvingGround View: Message rendering improvement including coloring user messages, reducing layout complexity, and other minor improvements.
- AgentProvingGround View: Render metadata for samples and tasks as an interactive tree.
- AgentProvingGround View: When deployed via `apg view bundle`, support linking to individual transcript events or messages.
- AgentProvingGround View: Reduce the maximum size of the header (before it is collapsed) when evals have large numbers of metrics.
...
- AgentProvingGround View: Properly sort samples by score even when there are samples with errors.
- AgentProvingGround View: Allow filtering of samples by score when evals are running.
...
- AgentProvingGround View: Properly display 'more' options when content is collapsed.
- AgentProvingGround View: Fix issue that prevented filtering of sample list when viewing a running evaluation.
- AgentProvingGround View: Fix selection of specific metrics within scorers when a scorer produces more than one metric.
...
- [Event tree](https://inspect.aisi.org.uk/reference/agent_proving_ground.log.html#event-tree) functions for organising transcript events into a tree of spans.
- `apg log convert` now always fully re-writes log files even of the same format (so that e.g. sample summaries always exist in the converted logs).
...
- AgentProvingGround View: Split info tab into task, models, and info for improved layout.
...
- AgentProvingGround View: Support for viewing logs in Google Cloud Storage (gc://).
- AgentProvingGround View: Improved display of reasoning blocks.
- AgentProvingGround View: Improved display and layout of transcript and events.
- AgentProvingGround View: Improved Tool input and output display.
- AgentProvingGround View: Improved display of sample input, target, answer, and scoring information (improve column width behavior).
- AgentProvingGround View: Add support for linking to logs, specific log tabs, individual samples, and sample tabs within samples.
- AgentProvingGround View: Collapse sample init view by default.
- AgentProvingGround: Properly store and restore NaN values when viewing logs in VSCode.
...
- AgentProvingGround View: Collapse user messages after 15 lines by default.
- AgentProvingGround View: Improved spacing between transcript events.
...
- `APG_EVAL_LOG_FILE_PATTERN` environment variable for setting the eval log file pattern.
...
- AgentProvingGround View: Resolve sample attachments before including in realtime event stream.
...
- AgentProvingGround View: [Live updates](https://inspect.aisi.org.uk/log-viewer.html#live-view) to running evaluation logs.
- [Agent](https://inspect.aisi.org.uk/agents.html) protocol and [agent_proving_ground.agent](https://inspect.aisi.org.uk/reference/agent_proving_ground.agent.html) module with new system for creating, composing, and executing agents.
...
- Model API: New [execute_tools()](https://inspect.aisi.org.uk/reference/agent_proving_ground.model.html#execute_tools) function (replaces deprecated `call_tools()` function) which handles agent handoffs that occur during tool calling.
...
- AgentProvingGround View: Fallback to content range request if initial HEAD request fails.
- AgentProvingGround View: Improve error message when view bundles are server from incompatible servers.
- AgentProvingGround View: Render messages in `user` and `assistant` solver events.
- AgentProvingGround View: Improved support for display of nested arrays.
- AgentProvingGround View: Improved rendering of complex scores and metrics.
- AgentProvingGround View: Properly handle filtering of dictionary scores.
- AgentProvingGround View: Render math in model input and output using katex.
- AgentProvingGround View: Improve sample score rendering (single scoring tab with scores rendered in a table).
- AgentProvingGround View: Improve sample count display in sample list footer.
- AgentProvingGround View: Properly refresh running evals when restoring from being backgrounded.
...
- AgentProvingGround View: Correctly parse NaN values (use JSON5 for all JSON parsing)
...
- AgentProvingGround View: Properly wrap tool input for non-code inputs like `think`.
...
- AgentProvingGround View: Fix error sorting numeric or categorical score results.
- AgentProvingGround View: Properly wrap model API call text in the transcript.
- Bugfix: Only initialise display in eval_set if it wasn't initialised from the CLI
- Bugfix: Set the global log level based on the specified AgentProvingGround log level.
...
- AgentProvingGround View: Fix issue generating diffs for nested arrays.
- AgentProvingGround View: Fix layout issue with sample error display in sample detail summary.
- AgentProvingGround View: Better support large eval files (in excess of 4GB).
- AgentProvingGround View: Correctly display 'None' when passed in tool calls.
- AgentProvingGround View: Fix 'Access Denied' error when using `apg view` and viewing the log in a browser.
...
- New `apg trace http` command which will show all HTTP requests for a run.
- More consistent use of `max_retries` and `timeout` configuration options. These options now exclusively control AgentProvingGround's outer retry handler; model providers use their default behaviour for the inner request, which is typically 2-4 retries and a service-appropriate timeout.
...
- Logging: AgentProvingGround no longer sets the global log level nor does it allow its own messages to propagate to the global handler (eliminating the possibility of duplicate display). This should improve compatibility with applications that have their own custom logging configured.
...
- Web Browser: [Various improvements](https://github.com/UKGovernmentBEIS/agent_proving_ground/pull/1314) to performance and robustness along with several bug fixes.
...
- AgentProvingGround View: Improve rendering of JSON within logging events.
- AgentProvingGround View: Improve virtualized rendering of Sample List, Sample Transcript, and Sample Messages.
- Task Display: Let plugins display counters ('rich' and 'full' display modes only).
- AgentProvingGround View: Fix layout issues with human agent terminal session playback.
- AgentProvingGround View: Improve tool input / output appearance when rendered in VSCode.
- AgentProvingGround View: Display reasoning tokens in model usage for the samples and for the complete eval.
- AgentProvingGround View: Improve model api request / response output when rendered in VSCode.
- AgentProvingGround View: Improve rendering of some tool calls in the transcript.
...
- Remove root logging handlers upon AgentProvingGround logger initialisation (as they result in lots of log spam if left installed).
...
- AgentProvingGround View: Fix display of score dictionaries containing boolean values
...
- AgentProvingGround View: Improve the display of subtasks with no inputs or events.
- AgentProvingGround View: Fix transcript display of phantom subtask or other phantom events.
- AgentProvingGround View: Fix formatting issues in sample error display
...
- AgentProvingGround View: Improve scorer display formatting.
- Bugfix: AgentProvingGround view now correctly renders arrays with embedded `null` values.
- Bugfix: AgentProvingGround view now correctly handles scorers with no metrics.
...
- Improvements to [scoring workflow](https://inspect.aisi.org.uk/scorers.html#sec-scorer-workflow) (`apg score` command and `score()` function).
...
- AgentProvingGround View: Convert codebase from JS/Preact to Typescript/React
...
- Computer: Moved out of beta (i.e. from `agent_proving_ground.tool.beta` into `agent_proving_ground.tool`).
- Sandboxes: Docker now uses `tee` for write_file operations.
- AgentProvingGround View: Handle Zip64 zip files (for log files greater than 4GB)
...
- Various improvements for [reasoning models](https://github.com/UKGovernmentBEIS/agent_proving_ground/pull/1229) including extracting reasoning content from assistant messages.
...
- [Agent Bridge](https://inspect.aisi.org.uk/agent-bridge.html) for integrating external agent frameworks with AgentProvingGround.
- [Goodfire](https://inspect.aisi.org.uk/models.html#goodfire) model provider.
- Add `@wraps` to functions wrapped by AgentProvingGround decorators to preserve type information.
...
- AgentProvingGround View: allow filtering samples by compound expressions including multiple scorers. (thanks @andrei-apollo)
- AgentProvingGround View: improve rendering performance and stability for the viewer when viewing very large eval logs or samples with a large number of steps.
...
- AgentProvingGround View: never truncate tool result images and display at default width of 800px.
- AgentProvingGround View: display tool error messages in transcript when tool errors occur.
- AgentProvingGround View: display any completed samples even if the task fails because of an error
- AgentProvingGround View: don't display the 'input' column heading if there isn't an input
...
- AgentProvingGround View: Various improvements to appearance of tool calls in transcript.
...
- AgentProvingGround View: Correct display of sample ID for single sample tasks.
...
- AgentProvingGround View: Fix issue with incorrectly displayed custom tool views.
...
- AgentProvingGround View: Prevent circular reference error when rendering complex tool input.
- AgentProvingGround View: Resolve display issue with sorting by sample then epoch.

## v0.3.48 (01 December 2024)

- [Realtime display](https://github.com/UKGovernmentBEIS/agent_proving_ground/pull/865) of sample transcripts (including ability to cancel running samples).
...
- `EvalLog` now includes a [location](https://github.com/UKGovernmentBEIS/agent_proving_ground/pull/872) property indicating where it was read from.
- Use [tool views](https://inspect.aisi.org.uk/approval.html#tool-views) when rendering tool calls in AgentProvingGround View.
- Consistent behavior for `max_samples` across sandbox and non-sandbox evals (both now apply `max_samples` per task, formerly evals with sandboxes applied `max_samples` globally).
- Log files now properly deal with scores that produce Nan. (fixes [#834](https://github.com/UKGovernmentBEIS/agent_proving_ground/issues/834))
...
- Improve path handling when using `apg log convert` to convert a single log file.
...
- Move `APG_DISABLE_MODEL_API` into `generate()` (as opposed to `get_model()`)
...
- Write custom tool call views into transcript for use by AgentProvingGround View.
...
- Add `APG_DISABLE_MODEL_API` environment variable for disabling all Model APIs save for mockllm.
...
- Support APG_EVAL_MODEL_ARGS environment variable for calls to `eval()`.
...
- AgentProvingGround view bundle: support for bundling directories with nested subdirectories.
...
- Fix memory leak in AgentProvingGround View worker-based JSON parsing.
- Add `fail_on_error` option for `eval_retry()` and `apg eval-retry`.
...
- Restore support for `--port` and `--host` on AgentProvingGround View.
...
- Display total samples in AgentProvingGround View
...
- Various improvements to layout and display for AgentProvingGround View transcript.

## v0.3.34 (30 September 2024)

- Support for `max_tokens` on OpenAI o1 models (map to `max_completion_tokens`).
- Fix regression of log and debug options on `apg view`
- Improved focus management for AgentProvingGround View
...
- Option to disable ANSI terminal output with `--no-ansi` or `APG_NO_ANSI`
...
- Make subtasks collapsible in AgentProvingGround View.
...
- Fix error when running `apg info log-types`
...
- Ability to [publish](https://inspect.aisi.org.uk/log-viewer.html#sec-publishing) a static standalone AgentProvingGround View website for a log directory.
...
- Enable customisation of model generation cache dir via `APG_CACHE_DIR` environment variable.
...
- Rename `ToolEnvironment` to `SandboxEnvironment` and `tool_environment()` to `sandbox()` (moving the renamed types from `agent_proving_ground.tool` to `agent_proving_ground.util`). Existing symbols will continue to work but will print deprecation errors.
- Moved the `bash()`, `python()`, and `web_search()` functions from `agent_proving_ground.solver` to `agent_proving_ground.tool`.  Existing symbols will continue to work but will print deprecation errors.
...
- Handle duplicate tool call ids in AgentProvingGround View.
- Handle sorting sample ids of different types in AgentProvingGround View.
...
- Reduce foreground task contention for AgentProvingGround View history loading.
- Ability to host standalone version of AgentProvingGround View to view single log files.
...
- Improved handling of very large (> 100MB) log files in AgentProvingGround View.
...
- Relocate tool related types into a new top-level `agent_proving_ground.tool` module (previous imports still work fow now, but result in a runtime deprecation warning).
- Decouple tools entirely from solvers and task state (previously they had ways to interact with metadata, removing this coupling will enable tool use in lower level interactions with models). Accordingly, the `call_tools()` function now operates directly on messages rather than task state.
- Support token usage for Google models (AgentProvingGround now requires `google-generativeai` v0.5.3).
...
- Resolve `APG_LOG_DIR` in `.env` file relative to `.env` file parent directory.
...
- New `apg eval-retry` command to retry a log file from a task that ended in error or cancellation.
- New `retryable_eval_logs()` function and `--retryable` option for `apg list logs` to query for tasks not yet completed within a log directory.
- Add `shuffled` property to datasets to determine if they were shuffled.
- Remove unused `extensions` argument from `list_eval_logs()`.

## v0.3.13 (31 May 2024)

- Bugfix: AgentProvingGround view was not reliably updating when new evaluation logs were written.
...
- Improved performance for AgentProvingGround View on very large datasets (virtualized sample list).
...
- Supporting infrastructure in AgentProvingGround CLI for VS Code extension (additional list and info commands).

## v0.3.6 (06 May 2024)

- Show first log file immediately (don't wait for fetching metadata for other logs)
- Add `--version` CLI arg and `apg info version` command for interrogating version and runtime source path.
- Fix: exclude `null` config values in output from `apg info log-file`

## v0.3.5 (04 May 2024)

- Fix issue with logs from S3 buckets in apg view.
...
- `apg view` command for viewing eval log files.
...
- Relative path to `APG_LOG_DIR` in `.env` file is now correctly resolved for execution within subdirectories.
- `apg list tasks` and `list_tasks()` now only parse source files (rather than loading them), ensuring that it is fast even for task files that have non-trivial global initialisation.
- `apg list logs` and `list_eval_logs()` now enumerate log files recursively by default, and only enumerate json files that match log file naming conventions.
- Provide `header_only` option for `read_eval_log()` and `apg info log-file` for bypassing the potentially expensive reading of samples.
- Provide `filter` option for `list_eval_logs()` to filter based on log file header info (i.e. anything but samples).
- Added `__main__.py` entry point for invocation via `python3 -m agent_proving_ground`.
