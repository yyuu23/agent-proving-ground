...
AgentProvingGround's approval mode enables you to create fine-grained policies for approving tool calls made by models. For example, the following are all supported:
...
``` bash
apg eval browser.py --approval human
```
...
``` bash
apg eval browser.py --approval approval.yaml
```
...
from agent_proving_ground.approval import ApprovalPolicy, human_approver, auto_approver
...
from agent_proving_ground.approval import human_approver
...
```python
from agent_proving_ground.approval import approval, ApprovalPolicy, human_approver, auto_approver
...
```python
from agent_proving_ground.model import execute_tools
from agent_proving_ground.approval import ApprovalPolicy, human_approver
...
Agents integrated via the [Agent Bridge](agent-bridge.qmd) run their own tool loop, so AgentProvingGround never executes their tool calls. Approval policies are applied instead to the tool calls in each model response, before that response is handed back to the agent. This covers both `agent_bridge()` and `sandbox_agent_bridge()`, and every API dialect they support.
...
1.  **A rejection discards every tool call in the same response.** Each call gets a result explaining what happened, and the calls that weren't themselves rejected are told which call caused it so the model can re-issue them on their own. This differs from AgentProvingGround-executed tools, where each call is handled independently.
...
Because AgentProvingGround doesn't execute these tool calls, no tool event is recorded for them — the transcript shows the approval decision rather than a tool call with an approval attached. Two things follow from that when reading a log:
...
AgentProvingGround includes two built-an approvers: `human` for interactive approval at the terminal and `auto` for automatically approving or rejecting specific tools. You can also create your own approvers that implement just about any scheme you can imagine.
...
Assuming we have properly [registered our approver](extensions-approvers.qmd#sec-extensions-approvers) as an AgentProvingGround extension, we can then use this it in an approval policy:
