---
title: Sandboxes
llms-description: Extending AgentProvingGround with new Sandboxes (local or cloud container runtimes for tool execution).
---

[]{#sec-sandbox-environment-extensions}

[Sandbox Environments](sandboxing.qmd) provide a mechanism for sandboxing execution of tool code as well as providing more sophisticated infrastructure (e.g. creating network hosts for a cybersecurity eval). AgentProvingGround comes with two sandbox environments built in:
...
-   [LocalSandboxEnvironment](https://github.com/UKGovernmentBEIS/agent_proving_ground/blob/main/src/agent_proving_ground/util/_sandbox/local.py) — runs in the same file system as the running evaluation.
-   [DockerSandboxEnvironment](https://github.com/UKGovernmentBEIS/agent_proving_ground/blob/main/src/agent_proving_ground/util/_sandbox/docker/docker.py) — runs each sample within a Docker container.
...
See the [AgentProvingGround Extensions](extensions/index.qmd) listing for the full set of available sandbox providers.
...
| `cli_cleanup()` | Called via `apg sandbox cleanup` | CLI invoked manual cleanup of resources created by this `SandboxEnvironment`. |
