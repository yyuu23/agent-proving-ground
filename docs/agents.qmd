...
Agents combine planning, memory, and tool usage to pursue more complex, tasks (e.g. a Capture the Flag challenge). AgentProvingGround supports a variety of approaches to agent evaluations, including:

1.  Using AgentProvingGround's built-in [ReAct Agent](react-agent.qmd).

2.  Using the [Deep Agent](deepagent.qmd) for long-horizon tasks with subagent delegation, memory, and planning.

3.  Using software engineering agents like Claude Code and Codex CLI via the [AgentProvingGround SWE](https://meridianlabs-ai.github.io/inspect_swe/) package.
...
AgentProvingGround also includes features suited to more complex agent evaluations including [Checkpointing](checkpointing.qmd) to recover from failures, [Intervention](intervention.qmd) to enable communication with running agents, and [Limits](agent-custom.qmd#agent-limits) to set token, message, and time limits for agent execution.

Below, we'll cover the basic role and function of agents in AgentProvingGround. Subsequent articles provide more details on the ReAct Agent, Deep Agent, custom agents, and multi-agent systems.

## Agent Basics

The AgentProvingGround `Agent` protocol enables the creation of agent components that can be flexibly used in a wide variety of contexts. Agents are similar to solvers, but use a narrower interface that makes them much more versatile. A single agent can be:
...
``` python
from agent_proving_ground.agent import Agent, agent, react
from agent_proving_ground.tool import web_search
...
1.  Agents can be passed as a `Solver` to any AgentProvingGround interface that takes a solver:
...
3.  Agents can be used as a standard tool using the `as_tool()` function:
    ...
    ``` python
    from agent_proving_ground.agent import as_tool
...
4.  Agents can participate in multi-agent systems where the conversation history is shared across agents. Use the `handoff()` function to create a tool that enables handing off the conversation from one agent to another:
    ...
    ``` python
    from agent_proving_ground.agent import handoff
    from agent_proving_ground.solver import use_tools, generate
...
See these additional articles to learn more about creating agent evaluations with AgentProvingGround:
...
-   [Custom Agents](agent-custom.qmd) describes AgentProvingGround APIs available for creating custom agents.

-   [Agent Bridge](agent-bridge.qmd) enables the use of agents from 3rd party frameworks like OpenAI Agents SDK, LangChain, and Pydantic AI with AgentProvingGround.
