...
``` python
from agent_proving_ground.scorer import accuracy, value_to_float
...
2.  The `score` function is declared as `async`. This is so that it can participate in AgentProvingGround's optimised scheduling for expensive model generation calls (this scorer doesn't call a model but others will).
