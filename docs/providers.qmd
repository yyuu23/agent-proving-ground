...
AgentProvingGround has support for a wide variety of language model APIs and can be extended to support arbitrary additional ones. Support for the following providers is built in to AgentProvingGround:
...
apg eval arc.py --model openai/gpt-4o-mini
```
...
By default, AgentProvingGround uses the standard OpenAI Chat Completions API for GPT-4 models and the new [Responses API](https://platform.openai.com/docs/api-reference/responses) for GPT-5 and o-series models and the `computer_use_preview` model.

If you want to manually enable or disable the Responses API you can use the `responses_api` model argument. For example:

``` bash
apg eval math.py --model openai/gpt-4o -M responses_api=true
```
...
OpenAI Responses API assistant messages can include a [`phase`](https://developers.openai.com/api/docs/guides/reasoning#phase-parameter) label that distinguishes intermediate commentary from the final answer. AgentProvingGround preserves and replays `phase` values returned by OpenAI. To additionally synthesize missing `phase` values for assistant messages constructed outside the Responses API, use the `responses_phase` model argument:

``` bash
apg eval math.py --model openai/gpt-5.4 -M responses_phase=true
```
...
By default, AgentProvingGround's implementation of the Responses API does not store messages on the server. Reasoning content (which is intended to be opaque to clients) is handled using encrypted payloads (via the "reasoning.encrypted_content" include option). To control this behavior explicitly use the `responses_store` model argument. For example:

``` bash
apg eval math.py --model openai/o4-mini -M responses_store=True
```
...
``` bash
apg eval math.py --model openai/gpt-5.4 \
...
apg eval math.py --model openai/azure/gpt-4o-mini
```
...
Note that if the `AZUREAI_OPENAI_API_VERSION` is not specified, AgentProvingGround will generally default to the latest deployed version, which as of this writing is `2025-03-01-preview`. When using managed identity for authentication, install the `azure-identity` package and leave `AZUREAI_OPENAI_API_KEY` undefined.

### OpenAI on AWS Bedrock {#openai-on-aws-bedrock}

The `openai` provider supports OpenAI models served through [Amazon Bedrock](https://aws.amazon.com/bedrock/). Use the normal `openai` provider with the `bedrock` qualifier, and use a standard OpenAI model identifier (AgentProvingGround automatically adds prefixes and suffixes required by Bedrock). For example:

``` bash
export AWS_BEARER_TOKEN_BEDROCK=your-bedrock-api-key
apg eval arc.py --model openai/bedrock/gpt-5.5
```

You don't need to set a region for the example above — AgentProvingGround defaults to `us-east-2`. The available model ids (e.g. `gpt-5.5`, `gpt-oss-120b`) and the regions they're offered in vary over time and by account; the [`bedrock-mantle` documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/bedrock-mantle.html) lists the supported regions.
...
``` bash
apg eval arc.py --model openai/bedrock/gpt-5.5 -M aws_region=us-east-2
```
...
| `BEDROCK_OPENAI_API_KEY` | Bedrock bearer API key — AgentProvingGround-convention alias (takes precedence if both are set). |
| `BEDROCK_OPENAI_BASE_URL` | Custom endpoint override (optional; the AWS-standard `AWS_BEDROCK_BASE_URL` is also accepted). By default AgentProvingGround targets the region's Mantle endpoint, choosing the model-appropriate path automatically (`/openai/v1` for frontier models like GPT-5.x and Codex, `/v1` for open-weight models like gpt-oss). |
| `AWS_REGION` / `AWS_DEFAULT_REGION` | AWS region (optional; defaults to `us-east-2`). |

The first option is a static bearer key. Generate an [Amazon Bedrock API key](https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys.html) and set it via `AWS_BEARER_TOKEN_BEDROCK` (the name used in the AWS and OpenAI docs; AgentProvingGround also accepts `BEDROCK_OPENAI_API_KEY`).

The second option uses your standard AWS credentials (IAM roles, instance profiles, SSO, AssumeRole, `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`, or a configured `AWS_PROFILE`). If no bearer key is set, AgentProvingGround generates short-lived bearer tokens from these credentials. This requires the `aws-bedrock-token-generator` package:
...
If the package is not installed and no bearer key is available, AgentProvingGround raises an error explaining how to proceed.
...
apg eval arc.py --model anthropic/claude-sonnet-4-0
```
...
Some Anthropic features require that you include a beta identifier in the `betas` field of model requests. AgentProvingGround automatically includes the requisite identifier for beta features it utilizes (e.g. "mcp-client-2025-04-04", "computer-use-2025-01-24", etc.).
...
``` bash
apg eval arc.py --model anthropic/claude-sonnet-4-0 -M betas=context-1m-2025-08-07
```

### Refusal Fallback {#anthropic-refusal-fallback}

::: {.callout-note appearance="simple"}
The model fallback feature described below requires the development version of AgentProvingGround. You can install the development version from GitHub with:

```bash
pip install git+https://github.com/UKGovernmentBEIS/agent_proving_ground
```
:::

Claude 5 classifiers can decline a request, returning a refusal (surfaced by AgentProvingGround as `stop_reason="content_filter"`). Such a request can usually be served by another Claude model. Set the `fallback_models` generate config to retry refused requests on one or more fallback models (tried in order) within the same request, using Anthropic's [server-side fallback](https://platform.claude.com/docs/en/build-with-claude/refusals-and-fallback):

``` bash
apg eval arc.py --model anthropic/claude-fable-5 --fallback-models claude-opus-4-8
```
...
``` bash
apg eval arc.py --model anthropic/claude-sonnet-4-6 -M betas=cache-diagnosis-2026-04-07
```
...
AgentProvingGround enables Anthropic [prompt caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) by default, using the standard 5-minute cache TTL. Use the `cache_ttl` model arg (`-M`) to specify a different TTL. Valid values are "5m" (the default) and "1h":

``` bash
apg eval arc.py --model anthropic/claude-sonnet-4-6 -M cache_ttl=1h
```
...
``` bash
apg eval arc.py --model anthropic/claude-sonnet-4-0 -M streaming=true
```
...
apg eval arc.py --model anthropic/bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0
```
...
apg eval ctf.py --model anthropic/vertex/claude-3-5-sonnet-v2@20241022
```
...
apg eval math.py --model anthropic/azure/Claude-4-0-Sonnet-2411
```
...
apg eval arc.py --model google/gemini-2.5-pro
```

For the `google` provider, custom model args (`-M`) are forwarded to the `genai.Client` function.
Google GenAI requests use a default SDK transport timeout of 1 hour when `timeout` is not configured; setting `timeout` applies the same value to each Google SDK request attempt and to AgentProvingGround's overall retry budget.
...
apg eval task.py --model google/your-model \
...
explicit API key, then `GOOGLE_API_KEY`, is used. AgentProvingGround refreshes the token as needed before each
...
apg eval ctf.py --model google/vertex/gemini-2.0-flash
```
...
By default, AgentProvingGround sets all four categories to `none` (enabling all content). You can override these defaults by using the `safety_settings` model argument. For example:
...
``` bash
apg eval eval.py -M "safety_settings={'hate_speech': 'low_and_above'}"
```
...
``` bash
apg eval arc.py --model google/gemini-2.5-pro -M streaming=true
```
...
apg eval arc.py --model mistral/mistral-large-latest
```
...
``` bash
apg eval arc.py --model mistral/mistral-large-latest -M conversation_api=false
```
...
apg eval math.py --model mistral/azure/Mistral-Large-2411
```
...
apg eval arc.py --model deepseek/deepseek-v4-pro
```
...
apg eval arc.py --model moonshot/kimi-k3
```
...
apg eval arc.py --model grok/grok-3-mini
```
...
``` bash
apg eval arc.py --model grok/grok-3-mini -M streaming=true
```

The `grok` provider also supports a `disable_retry` model argument that disables internal GRPC retries. For example:

``` bash
apg eval arc.py --model grok/grok-3-mini -M disable_retry=true
```
...
``` bash
apg eval arc.py --model grok/grok-4.6 -M service_tier=priority
```
...
export AWS_DEFAULT_REGION=us-east-1
apg eval bedrock/meta.llama2-70b-chat-v1
```
...
export AWS_ACCESS_KEY_ID=access-key-id
export AWS_SECRET_ACCESS_KEY=secret-access-key
apg eval arc.py --model sagemaker/my-endpoint-name \
...
``` bash
apg eval arc.py --model sagemaker/my-endpoint \
  -M region_name=us-west-2 \
  -M read_timeout=300 \
  -M stream=true
```
...
``` bash
apg eval arc.py --model sagemaker/my-cpt-endpoint \
  -M region_name=us-west-2 \
...
from agent_proving_ground.scorer import perplexity
...
``` bash
apg eval perplexity_eval.py --model sagemaker/my-endpoint \
  -M region_name=us-west-2
```
...
$ apg eval math.py --model azureai/Llama-3.3-70B-Instruct
```
...
``` bash
apg eval ctf.py -M emulate_tools=true
```
...
apg eval arc.py --model together/MiniMaxAI/MiniMax-M2.7
```
...
``` bash
apg eval arc.py --model together/MiniMaxAI/MiniMax-M2.7 -M stream=true
```
...
apg eval arc.py --model groq/llama-3.1-70b-versatile
```
...
apg eval arc.py --model fireworks/accounts/fireworks/models/kimi-k3
```
...
apg eval arc.py --model sambanova/DeepSeek-V1-0324
```
...
apg eval arc.py --model cloudflare/@cf/meta/llama-3.1-70b-instruct
```
...
apg eval arc.py --model perplexity/sonar
```
...
apg eval arc.py --model hf/openai-community/gpt2
```
...
``` bash
$ apg eval arc.py --model hf/openai-community/gpt2 -M device=cuda:0
```
...
For Hugging Face models, AgentProvingGround will use a tokenizer chat template when available. 
...
``` bash
apg eval gsm8k.py --model hf/Qwen/Qwen3-1.7B-Base \
...
``` bash
apg eval gsm8k.py --model hf/Qwen/Qwen3-1.7B-Base -M use_chat_template=false
```
...
``` bash
$ apg eval arc.py --model hf/openai-community/gpt2 -M hidden_states=true
```
...
``` bash
$ apg eval arc.py --model hf/openai-community/gpt2 -M do_sample=false
```

### Trust Remote Code

Some Hugging Face models ship custom Python code in their repositories that the `transformers` library will execute on load when `trust_remote_code=True`. Because executing remote code is a security risk (the model author can run arbitrary code in your evaluation process), AgentProvingGround defaults `trust_remote_code` to `False` and will not forward `trust_remote_code` from generic `model_args`. To opt in for a specific model you trust, pass it explicitly:

``` bash
apg eval arc.py --model hf/some-org/custom-arch-model -M trust_remote_code=true
```
...
``` bash
apg eval arc.py --model hf/mistralai/Ministral-3-8B-Instruct-2512 -M auto_model_class=AutoModelForImageTextToText
```
...
``` bash
$ apg eval arc.py --model hf/local -M model_path=./my-model
```
...
``` bash
pip install vllm
apg eval arc.py --model vllm/openai-community/gpt2
```
...
| Variable | Description |
|----------------------------|--------------------------------------------|
| `VLLM_BASE_URL` | Base URL for requests (optional, defaults to the server started by AgentProvingGround) |
...
Note: AgentProvingGround reuses a single server entry per base model name, so two `vllm/<same-model>` instances pointed at different URLs will collapse to the first URL. This caveat does not apply to the typical solver-vs-judge setup since the two models are different.
...
``` bash
$ apg eval arc.py --model vllm/meta-llama/Meta-Llama-3-8B-Instruct -M device='0,1,2,3'
```
...
``` bash
$ apg eval arc.py --model vllm/local -M model_path=./my-model
```
...
``` bash
apg eval arc.py --model vllm/meta-llama/Llama-3-8B:myorg/my-lora-adapter
```
...
``` bash
apg eval task.py --model vllm/meta-llama/Llama-3-8B:my-adapter -M max_lora_rank=128
```
...
apg eval arc.py --model vllm/meta-llama/Llama-3-8B:my-adapter
```

**Option 2: Enable dynamic loading**

Start the server with `VLLM_ALLOW_RUNTIME_LORA_UPDATING=True` to let AgentProvingGround load adapters dynamically:
...
Note: When AgentProvingGround starts the vLLM server itself, it automatically sets `VLLM_ALLOW_RUNTIME_LORA_UPDATING=True`.
...
``` bash
apg eval gsm8k.py --model vllm/Qwen/Qwen3-1.7B-Base -M use_chat_template=false
```

::: {.callout-note}
`use_chat_template` only takes effect when AgentProvingGround starts the vLLM server. When connecting to an existing server via `VLLM_BASE_URL`, set `--chat-template` when starting the server instead.
:::
...
``` bash
apg eval task.py --model vllm-completions/EleutherAI/pythia-70m
```
...
``` bash
apg eval perplexity_eval.py --model vllm/meta-llama/Meta-Llama-3-8B \
  --prompt-logprobs 1
```
...
Rather than letting AgentProvingGround start and stop a vLLM server every time you run an evaluation (which can take several minutes for large models), you can instead start the server manually and then connect to it. To do this, set the model base URL to point to the vLLM server and the API key to the server's API key. For example:

``` bash
$ export VLLM_BASE_URL=http://localhost:8080/v1
$ export VLLM_API_KEY=<your-server-api-key>
$ apg eval arc.py --model vllm/meta-llama/Meta-Llama-3-8B-Instruct
```

or

``` bash
$ apg eval arc.py --model vllm/meta-llama/Meta-Llama-3-8B-Instruct --model-base-url http://localhost:8080/v1 -M api_key=<your-server-api-key>
```
...
apg eval arc.py --model sglang/meta-llama/Meta-Llama-3-8B-Instruct
```
...
| Variable | Description |
|----------------------------|--------------------------------------------|
| `SGLANG_BASE_URL` | Base URL for requests (optional, defaults to the server started by AgentProvingGround) |
...
SGLang is a fast and efficient language model server that supports a variety of model architectures and configurations. Its usage in AgentProvingGround is almost identical to the [vLLM provider](#vllm). You can either let AgentProvingGround start and stop the server for you, or start the server manually and then connect to it:
...
``` bash
$ apg eval arc.py --model sglang/meta-llama/Meta-Llama-3-8B-Instruct --model-base-url http://localhost:8080/v1 -M api_key=<your-server-api-key>
```
...
The [nnterp](https://ndif-team.github.io/nnterp/index.html) provider enables you to use `StandardizedTransformer` models with AgentProvingGround. To use the nnterp provider, install the `nnterp` package:
...
``` bash
apg eval arc.py --model nnterp/openai-community/gpt2
```
...
The [TransformerLens](https://github.com/neelnanda-io/TransformerLens) provider allows you to use `HookedTransformer` models with AgentProvingGround.
...
Unlike other providers, TransformerLens requires you to first load a `HookedTransformer` model instance and then pass it to AgentProvingGround. This is because TransformerLens models expose special hooks for accessing and manipulating internal activations that need to be set up before use in the inspect framework.
...
``` bash
pip install openai
apg eval arc.py --model ollama/llama3.1
```

Note that you should be sure that Ollama is running on your system before using it with AgentProvingGround.
...
``` bash
pip install openai
apg eval arc.py --model llama-cpp-python/llama3
```

Note that you should be sure that the [llama-cpp-python server](https://llama-cpp-python.readthedocs.io/en/latest/server/) is running on your system before using it with AgentProvingGround.
...
If your model provider makes an OpenAI API compatible endpoint available, you can use it with AgentProvingGround via the `openai-api` provider, which uses the following model naming convention:

```         
openai-api/<provider-name>/<model-name>
```

AgentProvingGround will read environment variables corresponding to the api key and base url of your provider using the following convention (note that the provider name is capitalized):
...
apg eval arc.py --model openai-api/deepseek/deepseek-v4-flash
```
...
``` bash
$ apg eval arc.py --model openai-api/<provider>/<model> -M responses_api=true
```
...
``` bash
apg eval ctf.py --model openai-api/<provider>/<model> -M emulate_tools=true
```
...
By default, AgentProvingGround sets `"strict": true` on tool function schemas for the `openai-api` provider. This preserves compatibility with providers that require strict tool schemas. You can override this using the `strict_tools` model arg:

``` bash
apg eval arc.py --model openai-api/<provider>/<model> -M strict_tools=false
```
...
``` bash
$ apg eval arc.py --model openai-api/<provider>/<model> -M stream=true
```
...
``` bash
apg eval task.py --model openai-api-completions/<provider>/<model>
```
...
apg eval arc.py --model openrouter/gryphe/mythomax-l2-13b
```
...
apg eval arc.py --model hf-inference-providers/openai/gpt-oss-120b
```
...
pip install openai
export HF_TOKEN=your-huggingface-token
apg eval arc.py --model hf-inference-providers/openai/gpt-oss-120b:cerebras
```
...
``` bash
apg eval arc.py --model hf-inference-providers/openai/gpt-oss-120b -M stream=false
```
