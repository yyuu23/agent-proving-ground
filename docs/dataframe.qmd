...
AgentProvingGround eval logs have a hierarchical structure which is well suited to flexibly capturing all the elements of an evaluation. However, when analysing or visualising log data you will often want to transform logs into a [dataframe](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html). The **agent_proving_ground.analysis** module includes a variety of functions for extracting [Pandas](https://pandas.pydata.org/) dataframes from logs, including:
...
::: callout-note
#### AgentProvingGround Viz

[AgentProvingGround Viz](https://meridianlabs-ai.github.io/inspect_viz/) is a data visualization framework built to work with the AgentProvingGround data frame functions described below. After you've explored the basics of data frames you may also want to check out AgentProvingGround Viz.
:::
...
from agent_proving_ground.analysis import evals_df
evals_df("logs")   
```
...
For example, if you have used the [`apg view bundle`](log-viewer.qmd#sec-publishing) command to publish logs to a website, you can use the `log_viewer()` operation to map log file paths to their published URLs:
...
AgentProvingGround includes built in support for many models (based upon the `model` string in the dataframe). If you are using models for which AgentProvingGround does not include model metadata, you may include your own model metadata (see the `model_info()` reference for additional details).
...
Add a "log_viewer" column to an eval data frame by mapping log file paths to remote URLs. Pass mappings from the local log directory (or S3 bucket) to the URL where the logs have been publishing using [`apg view bundle`](https://inspect.aisi.org.uk/log-viewer.html#sec-publishing). For example:
...
The `turn_count` column is the number of turns (top-level model generations) used by the sample, and `token_limit_usage` is the metered value of the sample's token limit, respecting the limit's type (`None` when no token limit was configured). Both are `None` for logs written by older versions of AgentProvingGround.
...
::: {.callout-note appearance="simple"}
Sample summaries were enhanced in version 0.3.93 (May 1, 2025) to include the `metadata`, `model_usage`, `total_time`, `working_time`, and `retries` fields. If you need to read any of these values you can update older logs with the new fields by round-tripping them through `apg log convert`. For example:

``` bash
$ apg log convert ./logs --to eval --output-dir ./logs-amended
```
:::
...
from agent_proving_ground.analysis import EvalColumn
