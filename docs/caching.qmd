...
There are two types of caching available: AgentProvingGround local caching and provider level caching. We'll first describe local caching (which works for all models) then cover [provider caching](#sec-provider-caching) which currently works only for Anthropic models.
...
```bash
apg eval arc.py --cache     # 7 day cache (default)
apg eval arc.py --cache 1D  # 1 day cache
apg eval arc.py --cache 4W  # 4 week cache
```
...
``` bash
$ apg eval ctf.py --model openai/gpt-4-turbo-2024-04-09
```
...
Use the `apg cache` command the view the current contents of the cache, prune expired entries, or clear entries entirely. For example:
...
$ apg cache clear
$ apg cache clear --model openai/gpt-4-turbo-2024-04-09

# prune expired entries from the cache
$ apg cache list --pruneable
$ apg cache prune
$ apg cache prune --model openai/gpt-4-turbo-2024-04-09
```

See `apg cache --help` for further details on management commands.

### Cache Directory

By default the model generation cache is stored in the system default location for user cache files (e.g. `XDG_CACHE_HOME` on Linux). You can override this and specify a different directory for cache files using the `APG_CACHE_DIR` environment variable. For example:

``` bash
$ export APG_CACHE_DIR=/tmp/inspect-cache
```
...
``` bash
apg eval ctf.py --cache-prompt=false # force caching off
```
...
Providers will typically provide various means of customising the scope of cache usage. The AgentProvingGround `cache-prompt` option will by default attempt to make maximum use of provider caches (in the Anthropic implementation system messages, tool definitions, and all messages up to the last user message are included in the cache).
