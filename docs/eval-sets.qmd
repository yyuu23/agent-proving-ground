...
Most of the examples in the documentation run a single evaluation task by either passing a script name to `apg eval` or by calling the `eval()` function directly. While this is a good workflow for developing single evaluations, you'll often want to run several evaluations together as a *set*. This might be for the purpose of exploring hyperparameters, evaluating on multiple models at one time, or running a full benchmark suite.

The `apg eval-set` command and `eval_set()` function and provide several facilities for running sets of evaluations, including:
...
Run a set of evaluations using the `apg eval-set` command or `eval_set()` function. For example:

``` bash
$ apg eval-set mmlu.py mathematics.py \
...
Eval sets often run for a long time: to run one in the background, detached from your terminal, launch it with `--detach` and monitor it with `apg ctl` (see [Detached Launch](control-channel.qmd#detached-launch)).
...
Notice that we create our tasks from a function decorated with `@task`. Doing this is a critical requirement because it enables AgentProvingGround to capture the arguments to `create_task()` and use that to distinguish the two tasks (in turn used to pair tasks to log files for retries).
...
Eval sets handle running, retrying, and tracking a set of tasks. As that work grows (many tasks across many models with varying parameters, then reviewing and promoting the results), you may want more structure than ad-hoc scripts provide. [AgentProvingGround Flow](https://meridianlabs-ai.github.io/inspect_flow/) is a companion package for running and managing evaluations at scale, built on top of eval sets. It provides:
...
See the [AgentProvingGround Flow documentation](https://meridianlabs-ai.github.io/inspect_flow/) to learn more.
...
Once all of the tasks in an eval set are complete, re-running `apg eval-set` or `eval_set()` on the same log directory will be a no-op as there is no more work to do. At this point you can use the `list_eval_logs()` function to collect up logs for analysis:
...
We can run both of these tasks with the following command (note for this and the remainder of examples we'll assume that you have let an `APG_EVAL_MODEL` environment variable so you don't need to pass the `--model` argument explicitly):

``` bash
$ apg eval-set ctf.py --log-dir logs-run-42
```
...
``` bash
$ apg eval-set security --log-dir logs-security-02-09-24
```

Note that some of the files in this directory don't contain evals (e.g. `import.py` and `analyze.py`). These files are not read or executed by `apg eval-set` (which only executes files that contain `@task` definitions).

If we wanted to run more than one directory we could do so by just passing multiple directory names. For example:

``` bash
$ apg eval-set security persuasion --log-dir logs-suite-42
```
...
Note that the `apg list tasks` command can also be used to enumerate tasks in plain text or JSON (use one or more `-F` options if you want to filter tasks):

``` bash
$ apg list tasks security
$ apg list tasks security --json
$ apg list tasks security --json -F light=true
```

You can feed the results of `apg list tasks` into `apg eval-set` using `xargs` as follows:

``` bash
$ apg list tasks security | xargs \
   apg eval-set --log-dir logs-security-42
```

::: {.callout-important appearance="simple"}
One important thing to keep in mind when using attributes to filter tasks is that both `apg list tasks` (and the underlying `list_tasks()` function) do not execute code when scanning for tasks (rather they parse it). This means that if you want to use a task attribute in a filtering expression it needs to be a constant (rather than the result of function call). For example:
