...
Solvers are the heart of AgentProvingGround evaluations and can serve a wide variety of purposes, including:
...
AgentProvingGround has a number of built-in solvers, each of which can be customised in some fashion. Built in solvers can be imported from the `agent_proving_ground.solver` module. Below is a summary of these solvers. There is not (yet) reference documentation on these functions so the best way to learn about how they can be customised, etc. is to use the **Go to Definition** command in your source editor.
...
1.  The function applies the `@solver` decorator—this registers the `Solver` with AgentProvingGround, making it possible to capture its name and parameters for logging, as well as make it callable from a configuration file (e.g. a YAML specification of an eval).

2.  The `solve()` function is declared as `async`. This is so that it can participate in AgentProvingGround's optimised scheduling for expensive model generation calls (this solver doesn't call `generate()` but others will).
...
When creating custom solvers, it's critical that you understand AgentProvingGround's concurrency model. More specifically, if your solver is doing non-trivial work (e.g. calling REST APIs, executing external processes, etc.) please review [Parallelism](parallelism.qmd#sec-parallel-solvers-and-scorers) for a more in depth discussion.
