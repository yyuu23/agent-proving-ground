---
title: Approvers
llms-description: Extending AgentProvingGround with new Approvers (approve, modify, or reject tool calls).
---
...
If you are publishing an approver within a Python package, you should register an `agent_proving_ground` [setuptools entry point](https://setuptools.pypa.io/en/latest/userguide/entry_point.html). This will ensure that inspect loads your extension before it attempts to resolve approvers by name.
...
You can then register your `auto_approver` AgentProvingGround extension (and anything else imported into `_registry.py`) like this in `pyproject.toml`:
