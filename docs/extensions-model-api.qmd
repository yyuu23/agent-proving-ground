---
title: Model APIs
llms-description: Extending AgentProvingGround with new Model APIs (model hosting services, local inference engines, etc.).
---
...
In addition, there are a number of optional properties and methods you can override to adapt AgentProvingGround's behaviour to your provider (default max tokens and connections, identifying rate limit errors, whether to collapse consecutive messages, etc.)—see [Provider Options](#sec-provider-options) below.

See the implementation of the [built-in model providers](https://github.com/UKGovernmentBEIS/agent_proving_ground/tree/main/src/agent_proving_ground/model/_providers) for additional insight on building a custom provider.
...
``` {.python filename="custom.py"}
from agent_proving_ground.model import ModelCall
...
The `ModelAPI` base class defines a number of properties and methods you can override to adapt AgentProvingGround's behaviour to your provider's requirements. All have sensible defaults, so you only need to override the ones relevant to your provider. The most frequently used are:
...
``` bash
apg eval ctf.py --model custom/my-model
```
