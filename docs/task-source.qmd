...
Task sources require the development version of AgentProvingGround, which you can install from GitHub:
...
::: {.callout-note appearance="simple"}
Task sources are supported by `eval()` / `eval_async()` (and `apg eval`) only. `eval_set`, `eval_retry`, and `score` require a fixed, resumable set of tasks and raise an error if passed one.
:::
...
```python
from agent_proving_ground import Task, TaskSource
...
```python
from agent_proving_ground import TaskSource
...
```python
from agent_proving_ground import TaskSource, task_source
...
```bash
apg eval curriculum.py@curriculum -T target=0.9 --model openai/gpt-4o
```
...
```python
from agent_proving_ground import enqueue_task
...
```bash
apg eval curriculum.py@curriculum --max-tasks 6 --model openai/gpt-4o,openai/gpt-4o-mini
```
