...
In order to distinguish successful generate requests from rate limited and retried requests, AgentProvingGround installs hooks into the HTTP client of various model packages. This is not possible for some models (`azureai`) and in these cases the `working_time` will include any internal retries that the model client performs.
:::
