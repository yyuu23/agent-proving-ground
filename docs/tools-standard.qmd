...
llms-description: AgentProvingGround's built-in tools for code execution, text editing, computer use, web search, and web browsing.
tbl-colwidths: [40,60]
---
...
from agent_proving_ground.solver import generate, system_message, use_tools
from agent_proving_ground.tool import bash_session
...
from agent_proving_ground.scorer import includes
from agent_proving_ground.solver import generate, system_message, use_tools
from agent_proving_ground.tool import bash_session, text_editor
...
``` bash
apg eval computer.py --model anthropic/claude-sonnet-4-6
apg eval computer.py --model openai/gpt-5.4
apg eval computer.py --model google/gemini-3-flash-preview
```
...
Two of the AgentProvingGround examples demonstrate basic computer use:

-   [computer](https://github.com/UKGovernmentBEIS/agent_proving_ground/tree/main/examples/computer/computer.py) — Three simple computing tasks as a minimal demonstration of computer use.

    ``` bash
    apg eval examples/computer
    ```

-   [intervention](https://github.com/UKGovernmentBEIS/agent_proving_ground/tree/main/examples/intervention/intervention.py) — Computer task driven interactively by a human operator.
...
``` bash
apg eval computer.py --approval approval.yaml
```
...
from agent_proving_ground.scorer import match
from agent_proving_ground.solver import generate, use_tools
from agent_proving_ground.tool import bash, python, web_browser
...
from agent_proving_ground.scorer import includes
from agent_proving_ground.agent import react
from agent_proving_ground.tool import bash, skill, todo_write
...
from agent_proving_ground.scorer import includes
from agent_proving_ground.agent import react
from agent_proving_ground.tool import read_file, list_files, grep
...
Note that the `memory()` tool does not require a [Sandbox Environment](sandboxing.qmd)—despite using file-like paths (e.g. `/memories/notes.md`), it stores all data in-memory using AgentProvingGround's sample store.
...
from agent_proving_ground.scorer import includes
from agent_proving_ground.agent import react
from agent_proving_ground.tool import memory
...
from agent_proving_ground.scorer import includes
from agent_proving_ground.solver import generate, system_message, use_tools
from agent_proving_ground.tool import bash_session, text_editor, think
...
The `ask_user()` and `notify_user()` tools let models communicate with a human operator during a sample. They pair with AgentProvingGround's [Agent Intervention](intervention.qmd) features (the `apg acp` client, the in-process task display, and out-of-band notifications).
...
The prompt is dispatched to whichever surface is attached: an ACP client (e.g. `apg acp`) if connected, otherwise the in-process Textual panel or the console. The sample pauses on the call until the operator responds.
