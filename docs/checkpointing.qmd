...
The agent checkpointing feature described below requires the development version of AgentProvingGround. You can install the development version from GitHub with:
...
apg eval ctf.py --checkpoint

# checkpoint using explicit triggers
apg eval ctf.py --checkpoint=turn:3
apg eval ctf.py --checkpoint=time:15m # <1>
apg eval ctf.py --checkpoint=token:100K # <2>

# pass a yaml or json config file
apg eval ctf.py --checkpoint=checkpoint.yaml
```
...
``` bash
apg eval-set arc.py ctf.py --checkpoint # CRASH!
apg eval-set arc.py ctf.py --checkpoint # recover

apg eval arc.py --checkpoint             # CRASH!
apg eval-retry logs/<log-file-name>.eval # recover
```

For incomplete samples, AgentProvingGround locates the checkpoint directory, restores each sample's most recent committed checkpoint, and continues. The following is restored before the agent runs:
...
3.  Events and Store: AgentProvingGround's internal per-sample state is rehydrated (the events appear under a synthesized `prior_run` span in the new `.eval` log).
...
``` python
from agent_proving_ground import eval
from agent_proving_ground.util import CheckpointConfig, TokenInterval
...
from agent_proving_ground.util import CheckpointConfig, TimeInterval
...
from agent_proving_ground.util import CheckpointSampleConfig
...
When more than one of the sample / task / eval layers supplies a configuration, AgentProvingGround merges them **per-field** at sample-run time. Each layer's fields default to `None`; the highest-priority layer with a non-`None` value wins per field. Precedence is **eval \> sample \> task**. Sample beats task because the task defines an agent's standard policy for *all* of its samples and an individual sample specializes it. Eval/CLI is always highest because it's the operator's run-time override.
...
2.  **Capture**: When the trigger fires, AgentProvingGround writes the host-side context files (agent-tracked state, events, and store) and runs a [restic backup](https://restic.net) on the agent scaffold host and each configured sandbox path.

3.  **Record**: AgentProvingGround writes a `ckpt-NNNNN.json` checkpoint file at the per-sample root. A checkpoint is available for resumption only once this file is in place.
...
``` python
from agent_proving_ground.util import checkpointer
...
- Restic binary — On first use, AgentProvingGround fetches a pinned [restic](https://restic.net) binary and caches it. For offline / air-gapped environments, pre-warm the cache with `apg download restic`.