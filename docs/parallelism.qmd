...
AgentProvingGround runs evaluations using a parallel async architecture, eagerly executing many samples in parallel while at the same time ensuring that resources aren't over-saturated by enforcing various limits (e.g. maximum number of concurrent model connections, maximum number of subprocesses, etc.).

There are a progression of concurrency concerns, and while most evaluations can rely on the AgentProvingGround default behaviour, others will benefit from more customisation. Below we'll cover the following:
...
AgentProvingGround uses [asyncio](https://docs.python.org/3/library/asyncio.html) as its async backend by default, but can also be configured to run against [trio](https://trio.readthedocs.io/en/stable/). See the section on [Async Backends](#async-backends) for additional details.
...
If you want to specify multiple models when using the `--model` CLI argument or `APG_EVAL_MODEL` environment variable, just separate the model names with commas. For example:

``` bash
APG_EVAL_MODEL=openai/gpt-4-turbo,google/gemini-2.5-pro
```

## Multiple Tasks {#sec-multiple-tasks}

By default, AgentProvingGround runs a single task at a time. This is because most tasks consist of 10 or more samples, which generally means that sample parallelism is enough to make full use of the `max_connections` defined for the active model. 
...
``` bash
$ apg eval --model openai/gpt-4 --max-dataset-memory 512
```
...
[Sandbox Environments](sandboxing.qmd) (e.g. Docker containers) often allocate resources on a per-sample basis, and also make use of the AgentProvingGround `subprocess()` function for executing commands within the environment.
...
1.  It's critical that connections to other APIs use `async` HTTP APIs (i.e. the `httpx` module rather than the `requests` module). This is because AgentProvingGround's parallelism relies on everything being `async`, so if you make a blocking HTTP call with `requests` it will actually hold up all of the rest of the work in the system!

2.  As with model APIs, rate limits may be in play, so it's important not to over-saturate these connections. Recall that AgentProvingGround runs all samples in parallel so if you have 500 samples and don't do anything to limit concurrency, you will likely end up making hundreds of calls at a time to the API.

Here's some (oversimplified) example code that illustrates how to call a REST API within an AgentProvingGround component. We use the `async` interface of the `httpx` module, and we use AgentProvingGround's `concurrency()` function to limit simultaneous connections to 10:

``` python
import httpx
from agent_proving_ground.util import concurrency
from agent_proving_ground.solver import Generate, TaskState
...
Generally speaking, you should try to make all of the code you write within AgentProvingGround solvers, tools, and scorers as parallel as possible. The main idea is to eagerly post as much work as you can, and then allow the various concurrency gates described above to take care of not overloading remote APIs or local resources. There are two keys to writing parallel code:
...
``` bash
$ apg eval --model openai/gpt-4 --max-subprocesses 4
```
...
AgentProvingGround asynchronous code is written using the [AnyIO](https://anyio.readthedocs.io/en/stable/) library, which is an async backend independent implementation of async primitives (e.g. tasks, synchronization, subprocesses, streams, etc.).

AnyIO in turn supports two backends: Python's built-in [asyncio](https://docs.python.org/3/library/asyncio.html) library as well as the [Trio](https://trio.readthedocs.io/en/stable/) async framework. By default, AgentProvingGround uses asyncio and is compatible with user code that uses native asyncio functions.

### Using Trio

To configure AgentProvingGround to use Trio, set the `APG_ASYNC_BACKEND` environment variable:

```bash
export APG_ASYNC_BACKEND=trio
apg eval math.py
```

Note that there are some features of AgentProvingGround that do not yet work when using Trio, including:

1. Full screen task display uses the [textual](https://textual.textualize.io/) framework, which currently works only with asyncio. AgentProvingGround will automatically switch to "rich" task display (which is less interactive) when using Trio.
...
4. The `--acp-server` option (which exposes a running eval over the Agent Client Protocol so external editors can attach) depends on the asyncio-only `acp` library. AgentProvingGround raises a clear startup error if `--acp-server` is specified while the Trio backend is configured. Evals that don't use `--acp-server` are unaffected and run normally under Trio.

### Portable Async

If you are writing async code in your AgentProvingGround solvers, tools, scorers, or extensions, you should whenever possible use the [AnyIO](https://anyio.readthedocs.io/en/stable/) library rather than asyncio. If you do this, your AgentProvingGround code will work correctly no matter what async backend is in use.
