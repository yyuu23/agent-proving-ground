...
AgentProvingGround includes a runtime tracing tool that can be used to diagnose issues that aren't readily observable in eval logs and error messages. Trace logs are written in JSON Lines format and by default include log records from level `TRACE` and up (including `HTTP` and `INFO`).
...
Action logging enables you to observe execution times, errors, and commands that hang and cause evaluation tasks to not terminate. The [`apg trace anomalies`](#anomalies) command enables you to easily scan trace logs for these conditions.

## Usage

Trace logging does not need to be explicitly enabled—logs for the last 10 top level evaluations (i.e. CLI commands or scripts that calls eval functions) are preserved and written to a data directory dedicated to trace logs. You can list the last 10 trace logs with the `apg trace list` command:

``` bash
apg trace list # --json for JSON output
```

Trace logs are written using [JSON Lines](https://jsonlines.org/) format and are gzip compressed, so reading them requires some special handing. The `apg trace dump` command encapsulates this and gives you a normal JSON array with the contents of the trace log (note that trace log filenames include the ID of the process that created them):

``` bash
apg trace dump trace-86396.log.gz
```
...
``` bash
apg trace dump trace-86396.log.gz --filter model
```
...
``` bash
apg trace anomalies trace-86396.log.gz
```

### Errors and Timeouts

By default, the `apg trace anomalies` command prints only currently running or cancelled actions (as these are what is required to diagnose an evaluation that doesn't complete). You can optionally also display actions that ended with errors or timeouts by passing the `--all` flag:

``` bash
apg trace anomalies --all
```
...
As with the `apg trace dump` command, you can apply a filter when listing anomalies. For example:

```bash
apg trace anomalies --filter model
```

## HTTP Requests

You can view all of the HTTP requests for the current (or most recent) evaluation run using the `apg trace http` command. For example:
...
```bash
apg trace http --failed --filter bedrock
```
...
``` python
from agent_proving_ground.util import trace_action
...
Your custom trace actions will be reported alongside the standard traced actions in `apg trace anomalies`, `apg trace dump`, etc.
...
``` python
from agent_proving_ground.util import trace_message
