...
from agent_proving_ground.model import ChatMessageSystem, get_model
...
from agent_proving_ground.agent import AgentState, agent
...
In some cases agents will want to retain state across multiple invocations, or even share state with other agents or tools. This can be accomplished in AgentProvingGround using the `Store`, which provides a sample-scoped scratchpad for arbitrary values.
...
from agent_proving_ground.agent import Agent, agent
...
``` python
from agent_proving_ground.agent import Agent, AgentState, agent
from agent_proving_ground.model import ChatMessageSystem, Model
...
This information is provided within the AgentProvingGround log viewer in the **Transcript** tab (which sits alongside the Messages, Scoring, and Metadata tabs in the per-sample display).
...
``` python
from agent_proving_ground.util import span
...
2.  The AgentProvingGround log viewer will create a visual delineation for the span, which will make it easier to see the flow of activity within the transcript.
...
Note that `collect()` is similar to [`asyncio.gather()`](https://docs.python.org/3/library/asyncio-task.html#asyncio.gather), but also works when [Trio](https://trio.readthedocs.io/en/stable/) is the AgentProvingGround async backend.

The AgentProvingGround `collect()` function also automatically includes each task in a `span()`, which ensures that its events are grouped together in the transcript.
...
Sandbox services make available a set of methods to a sandbox for calling back into the main AgentProvingGround process. For example, the [Human Agent](human-agent.qmd) uses a sandbox service to enable the human agent to start, stop, score, and submit tasks.
...
```python
from agent_proving_ground.util import background, sandbox_service
