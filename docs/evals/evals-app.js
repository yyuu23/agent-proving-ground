...
function detailPage(eval_) {
  ...
  const optionsText = [
    `apg eval ${eval_.code} --model anthropic/claude-opus-4-6`,
    `apg eval ${eval_.code} --model google/gemini-3.1-pro-preview`,
    `apg eval ${eval_.code} --limit 10`,
    `apg eval ${eval_.code} --temperature 0.5`,
    `apg eval ${eval_.code} --reasoning-effort medium`,
  ].join('\n');
  ...
  return h('div', { class: 'container pb-3', style: { maxWidth: '800px' } },
    ...
    h('div', { class: 'py-4' },
      h('section', {},
        h('p', {},
          ...
          h('a', { href: eval_.url || sourceUrl, target: '_blank', rel: 'noopener' },
            eval_.source === 'harbor' ? `Harbor Registry: ${eval_.name}` : `AgentProvingGround Evals: ${eval_.name}`),
          '.',
        ),
      ),
      ...
      h('section', {},
        ...
        }, 'View all options in the AgentProvingGround docs \u2197'),
      ),
    ),
  );
}
