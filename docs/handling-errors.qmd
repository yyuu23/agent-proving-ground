...
Failed samples are *not scored* and a warning indicating that some samples failed is both printed in the terminal and shown in AgentProvingGround View when this occurs.
...
While `fail_on_error` is typically specified at the `Task` level, you can also override the task setting when calling `eval()` or `apg eval` from the CLI. For example:
...
``` bash
apg eval ctf.py --retry-on-error    # retry 1 time
apg eval ctf.py --retry-on-error=3  # retry up to 3 times
```
...
``` bash
apg eval ctf.py --score-on-error
```
...
``` bash
apg log recover --list
```

To recover a specific log:

``` bash
apg log recover path/to/crashed.eval
```
...
``` bash
apg log recover path/to/crashed.eval --overwrite
```
...
To re-run the 5 failed/cancelled samples:
  apg eval-retry path/to/crashed-recovered.eval
```
...
When using `eval_set()` or `eval_retry()`, crash recovery is performed automatically. If a log with status `"started"` is encountered during retry, AgentProvingGround will opportunistically attempt to recover unflushed samples from the buffer database before re-running the evaluation. This maximizes sample reuse---completed samples recovered from the buffer are not re-run.
...
apg log list --status started

# Recover a crashed log for investigation (write outside the eval set directory)
apg log recover path/to/started.eval --output ~/recovered/started-recovered.eval
```
...
``` python
from agent_proving_ground.log import recover_eval_log, recoverable_eval_logs
