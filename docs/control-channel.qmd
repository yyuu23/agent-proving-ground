...
llms-description: Launch evals from scripts and agents (apg eval --json launch handoff, and --detach to run an eval in the background, detached from the terminal), then observe and direct them from another process with the apg ctl commands - live task and sample status, error and retry detail, transcript events, conversation snapshots, stall diagnosis from trace logs, task and sample cancellation, sample requeue, pause and resume, launch-config retuning, log flushing, and keep-alive.
---

## Overview

Every `apg eval` or `apg eval-set` process binds a local control endpoint that exposes the live state of the run. The `apg ctl` commands connect to it from another terminal, so you can check on a long-running eval — progress, stalled samples, errors, transcript activity — and direct it — cancel a stalled sample or a whole task, retune concurrency limits and log buffering — without parsing log files.

Commands are grouped by resource noun (`task`, `sample`, `model`, `process`), plus a top-level `config` command:

| Command | Description |
|----|----|
| `apg ctl task list` | List running tasks across all live AgentProvingGround processes. |
| `apg ctl task log-flush` | Write buffered completed samples to the log now. |
| `apg ctl task cancel` | Cancel a running task. |
| `apg ctl task pause` | Pause a running task (stop starting new work). |
| `apg ctl task resume` | Resume a paused task. |
| `apg ctl sample list` | List samples (running, completed, and pending). |
| `apg ctl sample errors` | List samples that errored or were retried. |
| `apg ctl sample show` | Show one sample's summary and error history. |
| `apg ctl sample events` | Read one sample's transcript events. |
| `apg ctl sample messages` | Read one sample's current conversation. |
| `apg ctl sample cancel` | Cancel one running sample. |
| `apg ctl sample requeue` | Re-run one errored or cancelled sample inside the live run. |
| `apg ctl config` | View or retune launch configuration mid-flight. |
| `apg ctl model pause` | Pause one model's dispatch across the run. |
| `apg ctl model resume` | Resume a paused model. |
| `apg ctl process list` | List running AgentProvingGround processes. |
| `apg ctl process anomalies` | Show in-flight and anomalous actions from a process's trace log. |
| `apg ctl process keep` | Make a process stay alive after its eval finishes. |
| `apg ctl process release` | Let a keep-alive process exit. |
| `apg ctl process pause` | Pause a whole running eval or eval-set. |
| `apg ctl process resume` | Resume a paused eval or eval-set. |

: {tbl-colwidths=[35,65]}

A bare noun implies `list`: `apg ctl task` ≡ `apg ctl task list`, and likewise for `sample` and `process`. All commands accept `--json` for structured output, which makes them straightforward to use from scripts and from coding agents like Claude Code. The `list` / `show` / `errors` / `events` / `messages` / `anomalies` commands are read-only. The others direct the run deliberately: `config` retunes launch parameters (never interrupting in-flight work), `task log-flush` forces a log write that would happen anyway, `process keep`/`release` only affect what happens after the eval finishes, `task pause`/`resume`, `model pause`/`resume`, and `process pause`/`resume` stop and restart dispatch reversibly without touching in-flight work, `task cancel` / `sample cancel` interrupt work explicitly, and `sample requeue` re-runs an errored or cancelled sample (all three idempotently, with `--dry-run` support).

The endpoint is a Unix domain socket under the current user's AgentProvingGround data directory. It is not reachable over the network or by other users on the same machine, and it requires no configuration.

::: {.callout-note}
Earlier releases exposed these operations as flat verbs (`apg ctl tasks`, `samples`, `sample`, `errors`, `events`, `limits`, `flush`, `buffer`, `keep`, `release`). Those spellings still work as hidden, deprecated aliases (each prints a pointer to the new spelling on stderr) and will be removed in a future release — except `sample`, whose name now belongs to the command group: use `apg ctl sample show` for what `apg ctl sample` did.
:::

## Launch Handoff

Right after launching an eval, an empty `apg ctl task list` is ambiguous: the control endpoint may simply not be bound yet. A script or agent that launches an eval and then drives it with `apg ctl` should launch with `--json`:

```bash
apg eval ctf.py --json
```
...
The detached process's stdout and stderr go to a file under the AgentProvingGround data directory, reported as `output_file` in the launch record. The process exits on its own when the eval finishes, and that file's last line is the completion signal — after the handoff, the eval's terminal and results are read entirely through the surfaces this page documents:

1. **Monitor** the running eval with `apg ctl task list --json` (and drill down with `apg ctl sample list` / `errors` / `events`).
2. **Intervene** if needed: `apg ctl sample cancel`, `apg ctl task cancel`, `apg ctl config`.
3. **Detect completion**: when the eval finishes the process exits (dropping out of the `apg ctl` listings), leaving a `done` record — overall success plus each task's `status` and `log_location` — as the last line of `output_file`.
...
AgentProvingGround never deletes these output files: one accumulates per detached run under the data directory until you remove it. Since the file is the completion signal, remove it only after its `done` record has been read.

To instead keep the process alive after the eval finishes — its state still queryable via `apg ctl` until you `apg ctl process release` it — pass `--ctl-server=keep` explicitly (or latch it onto an already-running detached eval with `apg ctl process keep`). The `done` record is then written only when the process is released. Prefer the default exit-when-done unless whatever will issue the release is certain to outlive the eval: a long eval routinely outlives the shell or agent session that launched it, and an unreleased parked process lingers indefinitely.

`--detach` works the same on `apg eval-set` and `apg eval-retry` (a multi-file retry hands off on its first `launch` record; later files' records go to the output file). Because a detached eval must be observable and cancellable while running, combining `--detach` with `--ctl-server=false` is an error.
...
`apg eval` in the foreground or under nohup/tmux. Instead:

1. Launch with `apg eval <task> --detach`. It prints a JSON
   ...
   its `launch` record.
2. Poll `apg ctl task list --json` to watch progress. When the
...
3. If samples stall or error, inspect with `apg ctl sample
   list` / `apg ctl sample errors`, and cancel with
   `apg ctl sample cancel` / `apg ctl task cancel`.
4. Read results from each task's `log_location` (reported in both
   the `done` record and `apg ctl task list`).
```

## Listing Tasks

`apg ctl task` lists the tasks of every running eval on the machine:
...
`apg ctl sample list` lists samples with their live status:

```bash
$ apg ctl sample list gpqa
...
The `turns` column counts top-level model generations (blank when unknown, e.g. for samples logged by older versions of AgentProvingGround). When any listed sample has a token limit configured, `limit usage` and `limit total` columns are also shown: the metered value for that limit — respecting its type (`all`, `output`, or a formula) — against the configured ceiling. The `--json` rows carry these as `turn_count`, `token_limit_usage`, `token_limit_total`, and `token_limit_type`.
...
`apg ctl sample errors` is a triage view of the samples that errored or were retried (across all running tasks when `TASK` is omitted):

```bash
$ apg ctl sample errors gpqa --content
...
`apg ctl sample show` drills into one sample: its status, timing, token usage, in-flight activity, and score, plus its full error history — including errors from prior attempts (both task-level retries and sample-level `retry_on_error`). Pass `--content` for the error messages, or `--traceback` (which implies `--content`) for full tracebacks:

```bash
$ apg ctl sample show gpqa 9 --traceback
```

## Transcript Events

`apg ctl sample events` reads a running sample's transcript — the sequence of model calls, tool calls, errors, and scores it has produced so far:

```bash
$ apg ctl sample events gpqa 17 --content
time      event  summary
--------  -----  -------------------------------------------------
...
`apg ctl sample messages` reads one sample's current conversation — its message list as it stands right now:

```bash
$ apg ctl sample messages gpqa 17 --tail 3 --content
#   role       content
--  ---------  --------------------------------------------------
...
Unlike `sample events`, this is a snapshot, not a stream: solver and agent code can rewrite the message list (for example, compaction replaces a prefix with a summary), so there is no resume cursor. Each call returns the conversation as it looks at that moment — by default a recent tail (the last 20 messages), with each row carrying its absolute index so a tailed view lines up with the full one. To watch a sample incrementally, poll and compare the reported total `count` (the cheap staleness signal), or use `apg ctl sample events` for event-grain resumable reads.
...
`apg ctl sample requeue` re-runs one errored or cancelled sample inside the still-running eval — the recovery move when a sample failed for a transient reason (a provider incident, a flaky sandbox) and you'd rather not wait for the whole run to finish and `apg eval-retry` it. The sample goes to the back of the sample queue and re-runs under the task's normal machinery: its prior errors ride along as retry history, a checkpointed sample resumes from its checkpoint, and the run's final log and counters reflect the fresh outcome (the superseded attempt does not count toward a fail-on-error threshold).

```bash
$ apg ctl sample requeue gpqa 17
```
...
```bash
$ apg ctl sample requeue gpqa 17 3
```
...
Requeuing a sample that completed *successfully* is rejected — re-running or re-scoring a success is out of scope (use score invalidation and `apg eval-retry` for post-hoc re-runs). Also rejected: a task that has finished or is between attempts (re-run failures with `apg eval-retry`, or let the queued task retry handle them), and a task with a cancel in flight. `--dry-run` reports what would be re-run — the prior error, the attempt number, and whether a checkpoint resume is available — without changing anything, and reports the rejections above too, so an agent can probe safely.
...
```bash
$ apg ctl sample errors gpqa --json | jq -r '.samples[] | "\(.sample_id) \(.epoch)"' |
    while read -r s e; do apg ctl sample requeue gpqa "$s" "$e"; done | tee requeue.log
...
$ apg ctl config gpqa --max-connections 20
```
...
Completed samples are buffered and written to the (possibly remote, e.g. S3) log in batches (see `--log-buffer` above). `apg ctl task log-flush` writes any buffered samples to the log immediately, so they become readable and analyzable without waiting for the buffer to fill. It is safe to repeat — a flush with nothing pending writes nothing.

```bash
$ apg ctl task log-flush gpqa
```

## Processes and Keep Alive

`apg ctl process` lists the running AgentProvingGround processes (their pids, keep-alive status, and hosted tasks). The pid is the selector `process keep` / `process release` take; with a single running process it can be omitted.
...
```bash
apg eval ctf.py --ctl-server=keep
```

With `keep`, the process stays running after the eval finishes — its state remains queryable via `apg ctl` and its logs are fully written — until you release it:

```bash
apg ctl process release
```

You can also latch keep-alive onto an eval that is *already running* (launched without `--ctl-server=keep`) with `apg ctl process keep`.

If more than one process is parked, `release` lists their pids and you disambiguate by passing one (`apg ctl process release <pid>`). Release also works ahead of time: issued while the eval is still running, it means "exit when done" — the process skips the park and exits as soon as the eval finishes (it never cancels in-flight work). `keep` and `release` are last-write-wins, so a `keep` issued after a `release` (while the eval is still running) restores the park. From Python, pass `ctl_server="keep"` to `eval()` or `eval_set()`. For eval sets, keep-alive requires `retry_immediate=True` (the default).
...
```bash
apg eval ctf.py --ctl-server=false
```

The `APG_EVAL_CTL_SERVER` environment variable mirrors the option (for example, set `APG_EVAL_CTL_SERVER=false` to disable it across a CI job). If the server fails to bind (for example, on a read-only filesystem) the eval logs a warning and runs normally without it — eval results never depend on the control channel.
