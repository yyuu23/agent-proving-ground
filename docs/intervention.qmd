...
Agent intervention uses the [Agent Client Protocol](https://agentclientprotocol.com), a standard for interactively controlling running agents. To enable ACP for an eval, pass the `--acp-server` option to `apg eval`:

```bash
apg eval terminal_bench_2 --acp-server
```

Then, in a separate terminal, run `apg acp`:
...
Interrupts and operator messages are recorded in the AgentProvingGround log:
...
`apg acp` defaults to local evals. For remote evals, bind a TCP loopback port on the eval host and forward it over SSH---the ACP server has no built-in authentication, so the port should not be exposed directly:

```bash
# eval listening for ACP connections on a loopback port
apg eval terminal_bench_2 --acp-server 4545
...
While the ACP client described above is the preferred way to manage agent intervention, all of the intervention features also work with the standard AgentProvingGround full task display. Each call to `eval()` establishes which client it will use for intervention using the `--acp-server` option:
...
``` python
from agent_proving_ground.agent import agent, react
from agent_proving_ground.tool import ask_user, bash, text_editor
...
Notifications fire whenever a human-in-the-loop interaction is posted (the `ask_user()` tool and tool-approval prompts). AgentProvingGround uses [Apprise](https://appriseit.com) to route notifications, which you can install with:
...
Notifications are opt-in per eval. Notification URLs usually contain secrets, so neither the Python API nor the CLI accepts URL strings directly. Set them in the `APG_EVAL_NOTIFICATION` environment variable, or pass a path to an Apprise config file. This keeps URLs out of source code, shell history, and eval logs.

Set the environment variable in your `.env` file:

``` bash
# single URL
APG_EVAL_NOTIFICATION="slack://TokenA/TokenB/TokenC/#agent-help"

# multiple URLs (comma-separated)
APG_EVAL_NOTIFICATION="macosx://,slack://TokenA/TokenB/TokenC/#agent-help"

# or point at an Apprise config file
APG_EVAL_NOTIFICATION="/path/to/apprise.yml"
```

Then enable notifications for the eval. Using the CLI:

``` bash
apg eval ctf_challenge --notification
```
...
``` bash
apg eval ctf_challenge --notification /path/to/apprise.yml
```
...
``` python
from agent_proving_ground.agent import agent, react
from agent_proving_ground.tool import ask_user, notify_user, bash, text_editor
...
from agent_proving_ground.model import execute_tools, get_model
...
Any client that speaks the [Agent Client Protocol](https://agentclientprotocol.com) can attach to a running eval. Custom clients speak ACP directly over the eval's socket. AgentProvingGround implements the full standard ACP surface plus a handful of extensions; a client that uses only standard methods works without modification.

The standard surface is documented at [agentclientprotocol.com](https://agentclientprotocol.com). The methods AgentProvingGround expects:
...
AgentProvingGround-aware clients can opt into richer behavior by declaring capabilities at `initialize` and calling extension methods. Extensions are namespaced `inspect/*` (methods) or `inspect.*` (metadata keys); a standard ACP client ignores them.
