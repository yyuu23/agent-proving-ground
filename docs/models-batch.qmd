...
apg eval arc.py --model openai/gpt-4o --batch

# Specify a batch size (e.g. 1000 requests per batch)
apg eval arc.py --model openai/gpt-4o --batch 1000

# Pass a YAML or JSON config file with batch configuration
apg eval arc.py --model openai/gpt-4o --batch batch.yml
```
...
``` python
from agent_proving_ground.model import BatchConfig
eval(
...
4.  **Status Monitoring**: AgentProvingGround periodically checks batch completion status.
