...
This article documents both basic and advanced use of AgentProvingGround tasks, which are the fundamental unit of integration for datasets, solvers, and scorers. The following topics are explored:
...
from agent_proving_ground.solver import chain_of_thought, generate
...
``` bash
apg eval security.py --task-config=config.yaml
```
...
from agent_proving_ground.scorer import includes
from agent_proving_ground.solver import chain, generate, solver, use_tools
from agent_proving_ground.tool import bash, python
...
apg eval ctf.py --solver=ctf_agent

# run with a different number of attempts
apg eval ctf.py --solver=ctf_agent -S attempts=5
```
...
Every CLI flag can be set as an environment variable using the `APG_EVAL_` prefix (with hyphens converted to underscores). Set these in the shell, or place them in a `.env` file that AgentProvingGround reads automatically from the current directory (searching parent directories if not found). Use this layer for project or session defaults you want applied across runs without specifying them each time:

``` {.makefile filename=".env"}
APG_EVAL_MODEL=anthropic/claude-sonnet-4-5
APG_EVAL_TEMPERATURE=0.0
APG_EVAL_MAX_CONNECTIONS=20
APG_EVAL_MAX_RETRIES=5
```
...
Parameters passed to `eval()` or on the `apg eval` command line take highest precedence, and apply to all tasks being evaluated in the call.
...
``` bash
apg eval inspect_evals/simpleqa \
    --model anthropic/claude-sonnet-4-5 \
    --temperature 0.0 \
    --max-tokens 4096 \
    --epochs 5 \
...
``` bash
apg eval my_task.py --temperature 0.0 --max-tokens 4096
```

Or from a YAML/JSON file using `--generate-config`:

``` bash
apg eval my_task.py --generate-config config.yaml
```
...
``` bash
apg eval --run-config run.yaml --temperature 0.9
```
...
To generate a run config from an existing eval log, use `apg log export-config`, which writes the realised configuration as `--run-config`-compatible YAML:
...
``` bash
apg eval my_task.py -T scorer=original
```
...
You can re-score an existing log file with a different scorer using `apg score`. The `--scorer` flag accepts a name (any function decorated with `@scorer`, see [Custom Scorers](custom-scorers.qmd)) or a `file.py@name` reference:

``` bash
# scorer registered via @scorer decorator
apg score log_file.eval --scorer my_scorer

# scorer defined in a file
apg score log_file.eval --scorer scorers.py@custom_scorer
```
:::
...
Tasks in packages can be *registered* such that users can easily refer to them by name from the CLI. For example, the [AgentProvingGround Evals](https://github.com/UKGovernmentBEIS/agent_proving_ground) package includes a suite of tasks that can be run as follows:

``` bash
apg eval inspect_evals/gaia
apg eval inspect_evals/swe_bench
```
...
The `_registry.py` file serves as a place to import things that you want registered with AgentProvingGround. For example:
...
``` toml
[project.entry-points.agent_proving_ground]
...
``` toml
[tool.poetry.plugins.agent_proving_ground]
evals = "evals._registry"
```
:::
...
Datasets hosted on Hugging Face Hub can include an `eval.yaml` file that provides AgentProvingGround task definitions. For example, the [OpenEvals/aime_24](https://huggingface.co/datasets/OpenEvals/aime_24) dataset can be evaluated with:

``` bash
apg eval hf/OpenEvals/aime_24 --model openai/gpt-5
```
...
```bash
apg eval hf/OpenEvals/MuSR@df154a5 --model openai/gpt-5
```
