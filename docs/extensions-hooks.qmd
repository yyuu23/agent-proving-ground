---
title: Hooks
llms-description: Extending AgentProvingGround with Hooks (run code during AgentProvingGround lifecycle events for logging and monitoring frameworks).
---

Hooks enable you to run arbitrary code during certain events of AgentProvingGround's lifecycle, for example when runs, tasks or samples start and end.
...
from agent_proving_ground.hooks import Hooks, RunEnd, RunStart, SampleEnd, hooks
...
For a more complete example of creating hooks see the [wandb_weave.py](https://github.com/UKGovernmentBEIS/agent_proving_ground/blob/main/examples/hooks/wandb_weave.py), [mlflow_tracking.py](https://github.com/UKGovernmentBEIS/agent_proving_ground/blob/main/examples/hooks/mlflow_tracking.py), and [mlflow_tracing.py](https://github.com/UKGovernmentBEIS/agent_proving_ground/blob/main/examples/hooks/mlflow_tracing.py) examples.
...
Each event method is `async`, returns `None`, and receives a single data object carrying the details of the event. Treat that data as read-only: the event class itself is frozen, but the models nested inside it are not, and some of them are the objects AgentProvingGround goes on to write to the log. Implement only the methods you need. The events below are grouped by lifecycle level, from the outermost scope (an entire `eval_set()`) down to individual model calls. All of the data types are importable from `agent_proving_ground.hooks`; see the [`agent_proving_ground.hooks`](reference/agent_proving_ground.hooks.qmd) reference for their full field definitions.
...
| `on_model_usage` | `ModelUsageData` | When a model call completes *without* hitting AgentProvingGround's local cache (`data.usage`, `data.call_duration`, `data.retries`). |
| `on_model_cache_usage` | `ModelCacheUsageData` | When a model call is satisfied by AgentProvingGround's local cache (`data.usage`). |
...
Consequently `self` is shared by every eval set, run, task, sample, and epoch, and calls for concurrent samples interleave at `await` points. Key per-sample state by `data.sample_id` and remove it in `on_sample_end`, and don't create event-loop-bound resources (async clients, `anyio` primitives) in `__init__` --- create them in `on_run_start`. See the [`Hooks`](reference/agent_proving_ground.hooks.qmd) reference for the full details.

## Registration

Packages that provide hooks should register an `agent_proving_ground` [setuptools entry point](https://setuptools.pypa.io/en/latest/userguide/entry_point.html). This will ensure that inspect loads the extension at startup.
...
You can then register your `wandb_hooks` AgentProvingGround extension (and anything else imported into `_registry.py`) like this in `pyproject.toml`:
...
Once you've done this, your hook will be enabled for AgentProvingGround users that have this package installed.
...
Another thing you might want to do is *ensure* that all users in a given environment are running with a particular set of hooks enabled. To do this, define the `APG_REQUIRED_HOOKS` environment variable, listing all of the hooks that are required:

``` bash
APG_REQUIRED_HOOKS=w&b_hooks
```
...
-   Use some custom model API authentication mechanism in conjunction with a custom reverse proxy for the model API to avoid AgentProvingGround ever having access to real API keys

``` python
from agent_proving_ground.hooks import hooks, Hooks, ApiKeyOverride
