...
- Submodule: `src/agent_proving_ground/_view/ts-mono` → https://github.com/meridianlabs-ai/ts-mono (branch `main`)
- Regeneration command: `python src/agent_proving_ground/_view/schema.py` (from repo root, venv activated; requires `pnpm install` done in the submodule)
- Generated artifacts:
  - `src/agent_proving_ground/_view/inspect-openapi.json` (this repo)
...
- CI jobs (`.github/workflows/log_viewer.yml`):
  ...
  - `dist-validation` — the checked-in viewer bundle `src/agent_proving_ground/_view/dist` must match `pnpm --filter @meridianlabs/log-viewer build` run at the pinned submodule commit
...
- Sibling consumer: `inspect_scout` (also embeds ts-mono; its `apps/scout/src/types/generated.ts` duplicates the shared types). Its regen script `scripts/export_openapi_schema.py` regenerates its `openapi.json` from the installed agent_proving_ground and chains scout's `types:generate`.
