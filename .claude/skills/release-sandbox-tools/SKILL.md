...
description: Build, validate, and publish a new version of the inspect-sandbox-tools injectable executables. Use when code under src/apg_sandbox_tools/ changed and a new injectable version must ship, or when asked to build/validate/upload sandbox tools binaries.
---
...
`src/apg_sandbox_tools/design/RELEASING.md`.
...
- The version must be bumped. It's a plain integer in
  `src/agent_proving_ground/tool/_sandbox_tools_utils/sandbox_tools_version.txt` and must be
  greater than main's:

  ```sh
  git show origin/main:src/agent_proving_ground/tool/_sandbox_tools_utils/sandbox_tools_version.txt
  ```
...
```sh
.venv/bin/python src/agent_proving_ground/tool/_sandbox_tools_utils/build_within_container.py --all --dev=false
```
...
`src/agent_proving_ground/binaries/`:
...
```sh
.venv/bin/python -m agent_proving_ground.tool._sandbox_tools_utils.validate_distros
```
...
>
> `! .venv/bin/python src/agent_proving_ground/tool/_sandbox_tools_utils/upload_to_s3.py {V}`
...
PyPI bundling is not part of this process — the `agent_proving_ground` release script
